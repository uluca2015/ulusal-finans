...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            LAST_SYNC_OK  = True
            save_config()
            save_data()  # Mevcut veriyi GitHub'a da yaz
            print(f"  Bağımsız moda geçildi. Önceki dosya: {onceki_dosya}")
            self.send_json({'ok': True, 'count': len(DATA), 'msg': f'{len(DATA)} kayıt sistemde saklandı, Excel bağlantısı kesildi'})

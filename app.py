...
def banka_kart_kullanim_guncelle():
    ...
    try:
        # Tür eşleştirme: gider türü → banka kart alanı
        tur_map = {
        ...
        # Her banka kartı için kullanımı sıfırla sonra hesapla
        for kart in BANKA_KART_DATA:
            banka_adi = (kart.get('banka') or '').upper().strip()
            firma = kart.get('firma', '')
            # Bu kartın türleri sıfırla
            for alan in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
                kart[f'kullanim_{alan}'] = 0.0
            # Gider tablosundan bu bankaya ait ödenmemiş kayıtları topla
            for rec in DATA:
                ...
                kart[f'kullanim_{alan}'] = round(kart.get(f'kullanim_{alan}', 0) + tutar, 2)
        save_banka_kart()
    except Exception as e:
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX17-KK2")
...
<div class="view" id="vw-kk" style="min-height:0">
  ...
  <div class="ov" id="kk-harcama-mod" style="display:none">
    <div class="mod" style="max-width:580px;width:96%">
      <div class="mtit"><span id="kk-harcama-baslik">Harcama Ekle</span><button class="mclose" onclick="kkHarcamaKapat()">✕</button></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:16px">
        <div><label class="flb">Tarih *</label><input class="fi2" type="date" id="kk-h-tarih"></div>
        <div><label class="flb">Firma / Ödeme Yeri *</label><input class="fi2" id="kk-h-firma" placeholder="Market, Restoran..."></div>
        <div><label class="flb">Tutar (₺) *</label><input class="fi2" type="number" id="kk-h-tutar" placeholder="0.00" step="0.01"></div>
        <div><label class="flb">Kategori</label>
      ...
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="kkHarcamaKaydet()" style="flex:1">💾 Kaydet</button>
...
function kkListeRender(q=''){
  ...
  list.innerHTML=filtered.map(k=>{
    ...
    const kull=parseFloat(k.kullanilan||0);
    const kalan=parseFloat(k.kalan||0);
    ...
    const topHarcama=harcamalar.reduce((s,h)=>s+parseFloat(h.tutar||0),0);
      ...
      <div style="border-top:1px solid var(--b1)">
        ...
        <div style="overflow-x:auto;max-height:320px;overflow-y:auto">
          <table style="width:100%;border-collapse:collapse;font-size:12px">
            ...
            <tbody>
              ${harcamalar.map((h,i)=>`
              <tr style="border-bottom:1px solid var(--b1);${i%2===0?'background:var(--s2)':'background:var(--s3)'}">
                ...
                <td style="padding:7px 10px;font-weight:600;color:var(--t1)">${h.firma||'—'}</td>
                <td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:var(--am);font-weight:700">₺${fmt(h.tutar)}</td>
                <td style="padding:7px 10px;color:var(--t3)">${h.kategori||'—'}</td>
        ...
        <div style="padding:10px 16px;background:var(--s3);display:flex;gap:24px;flex-wrap:wrap;font-size:12px;border-top:1px solid var(--b1)">
          <span>📊 Dönem harcaması: <strong style="color:var(--am);font-family:var(--mono)">₺${fmt(topHarcama)}</strong></span>
...
function kkHarcamaEkleAc(kkId){
  ...
  document.getElementById('kk-harcama-baslik').textContent='Harcama Ekle';
...
function kkHarcamaDuzenle(kkId,harcamaId){
  ...
  document.getElementById('kk-harcama-baslik').textContent='Harcama Düzenle';
  const f=(fid,val)=>{const el=document.getElementById(fid);if(el)el.value=val||'';};
...
async function kkHarcamaKaydet(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){toast('Kaydedildi ✓','s');kkHarcamaKapat();renderKK();}
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata: '+e.message,'e');}
}

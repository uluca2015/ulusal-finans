...
def save_users(users):
    # Diske yaz (blocking - hızlı)
    try:
        os.makedirs(os.path.dirname(USERS_FILE), exist_ok=True)
        with open(USERS_FILE, 'w', encoding='utf-8') as f:
            json.dump(users, f, ensure_ascii=False, indent=2)
        print(f"  save_users: {len(users)} kullanici diske yazildi → {USERS_FILE}")
    except Exception as e:
        print(f"  save_users DISK HATASI: {e}")
        raise  # Önemli: bu hatayı yukarı ilet
    # GitHub'a arka planda yedekle (non-blocking - timeout riski yok)
    _users_snap = dict(users)  # thread-safe snapshot
    def _github_sync():
        try:
            if GITHUB_TOKEN:
                ...
                print(f"  save_users: GitHub yedegi tamamlandi")
        except Exception as e:
            print(f"  save_users github hatasi (kritik degil): {e}")

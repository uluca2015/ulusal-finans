...
SESSIONS   = {}
SESSION_TTL = 1800  # 30 dakika inaktivite
...
<nav class="sbar">
  ...
  <div class="nav"    id="nv-banka-kart" onclick="showView('banka-kart')">🏦 Banka Kartları</div>
      ...
      <div style="background:var(--s2);border-radius:10px;padding:14px;border:1px solid var(--b2)">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
          <div><label class="flb">Banka Adı *</label>
            <select class="fi2" id="bk-banka" onchange="bankaKartBankaSecildi()">
...
async function api(url,opts={}){
  ...
  if(resp.status===401 && url !== '/api/login'){
    // Oturum süresi doldu - login sayfasına yönlendir
...
function bankaKartKapat(){ document.getElementById('bk-mod').style.display='none'; _bkEditId=null; }

async function bankaKartKaydet(){
  const gv=id=>{const el=document.getElementById(id);return el?el.value.trim():'';};
  const gn=id=>parseFloat(document.getElementById(id)?.value)||0;
  if(!gv('bk-banka')){ toast('Banka adı zorunlu','e'); return; }
  // Hesapları topla
  const hesaplar=[];
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){ toast('Kaydedildi','s'); bankaKartKapat(); renderBankaKart(); }
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){ toast('Hata: '+e.message,'e'); }
}

...
def banka_kart_kullanim_guncelle():
    ...
    try:
        ...
        for kk in KK_DATA:
            ...
            # Ödenmiş harcamaları düş
            odemeler = sum(float(h.get('tutar',0)) for h in harcamalar if h.get('odendi'))
            kullanim = sum(float(h.get('tutar',0)) for h in harcamalar if not h.get('odendi'))
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX18-KK3")
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/kk/liste':
            ...
            for kk in KK_DATA:
                harcamalar = kk.get('harcamalar', [])
                kullanilan = round(sum(float(h.get('tutar',0)) for h in harcamalar), 2)
                limit = float(kk.get('limit', 0) or 0)
        ...
        if path == '/':
            ...
            for kk in KK_DATA:
                harcamalar = kk.get('harcamalar', [])
                kullanilan_kk = round(sum(float(h.get('tutar',0)) for h in harcamalar), 2)
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);margin-bottom:16px;overflow:hidden">
      ...
      <div style="border-top:1px solid var(--b1)">
        ...
        <div style="padding:10px 16px;background:var(--s3);display:flex;gap:24px;flex-wrap:wrap;font-size:12px;border-top:1px solid var(--b1)">
          ...
          ${topOdeme>0?`<span>💰 Ödeme: <strong style="color:var(--g);font-family:var(--mono)">-₺${fmt(topOdeme)}</strong></span>`:''}

...
def _get_secret():
    # Sabit secret - GITHUB_TOKEN veya sabit değer
    return (GITHUB_TOKEN or 'ulusal-finans-secret-2024').encode('utf-8')
...
def _taksitli_odeme_senkronize(kredi_id):
    ...
    if is_doner:
        # Döner krediler: tek satır, limit=ana_para, kullanılan=kullanim alanı
        kullanim = float(kredi.get('kullanim') or 0)
        if kullanim <= 0: return  # kullanım yoksa ödeme tablosuna ekleme
        vade_str = kredi.get('vade_bitis','')
        try:
            dt = datetime.strptime(vade_str, '%Y-%m-%d') if '-' in vade_str else datetime.strptime(vade_str, '%d.%m.%Y')
        ...
        DATA.append({
            ...
            'yil':        yil,
            'durum':      'ÖDENMEDİ',
            'anapara':    round(kullanim, 2),
            'faiz':       0.0,
            'bsmv':       0.0,
            'tutar':      round(kullanim, 2),
            'aciklama':   f"{tur} kullanım — Limit: {kredi.get('ana_para',0):,.0f} ₺",
  ...
  <div class="ov" id="taksitli-mod" style="display:none">
    <div class="mod" style="max-width:700px;width:96%;max-height:92vh;overflow-y:auto">
      ...
      <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div id="tk-alan-kullanim-tarihi"><label class="flb">Kullandırım Tarihi</label>
          <input class="fi2" type="date" id="tk-kullanim-tarihi" placeholder="Kredinin bankadan alındığı tarih"></div>

    ...
    <div class="mod" style="max-width:700px;width:96%;max-height:92vh;overflow-y:auto">
      ...
      <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div><label class="flb">Banka *</label>
          <input class="fi2" id="tk-banka" placeholder="Yapıkredi, TEB..."></div>
                ...
                return `<tr style="border-bottom:1px solid var(--b1);${od?'opacity:.65':''}">
                  ...
                  <td style="padding:6px 10px;text-align:center">
                    <span class="bdg ${od?'bgi':'bri'}" onclick="taksitDurumToggle('${k.id}',${t.no},'${od?'ÖDENMEDİ':'ÖDENDİ'}')"
                      style="cursor:pointer;user-select:none" title="Tıkla: durum değiştir">${od?'ÖDENDİ':'ÖDENMEDİ'}</span>
                  </td>
                </tr>`;
              }).join('')}
            </tbody>
...
function taksitliYeniAc(){
  ...
  document.getElementById('taksitli-mod-baslik').textContent='Yeni Taksitli Kredi';
  ['tk-firma','tk-banka','tk-kredi-no','tk-tur','tk-ana-para','tk-faiz','tk-vade','tk-baslangic','tk-aciklama'].forEach(id=>{
    const el=document.getElementById(id);
    if(el){if(id==='tk-firma')el.value='ULUSAL';else if(id==='tk-tur')el.value='TAKSİTLİ KREDİ';else if(id==='tk-bsmv')el.value='5';else el.value='';}
  });
...
function taksitliDuzenle(id){
  ...
  s('tk-firma',k.firma);s('tk-banka',k.banka);s('tk-kredi-no',k.kredi_no);
  ...
  // İlk taksit tarihi
  if(k.taksitler&&k.taksitler.length){
    try{
...
async function taksitliKaydet(){
  ...
  const tur = gv('tk-tur');
  const donerTurler = ['ROTATİF','KMH','VİNOV','TEMİNAT MEKTUBU'];
  const isDoner = donerTurler.includes(tur) && tur!=='VİNOV';

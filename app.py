...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX21-KK6")
...
function kkOzetBar(){
  ...
  // Server'dan gelen kullanilan/kalan değerleri kullan (backend doğru hesaplıyor)
  // Fallback: harcamalar listesinden hesapla
  let topLim=0, topKull=0, topKalan=0;
  _kkData.forEach(k=>{
    const limit=parseFloat(k.limit||0);
    // Server değeri varsa kullan, yoksa harcamalardan hesapla
    let kull, kalan;
    if(k.kullanilan!==undefined && k.kalan!==undefined){
      kull  = parseFloat(k.kullanilan||0);
      kalan = parseFloat(k.kalan||0);
    } else {
      const harcs=(k.harcamalar||[]);
      kull  = harcs.filter(h=>!h.odendi&&h.tur!=='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
      kalan = Math.max(0,limit-kull);
    }
    topLim  +=limit;
    topKull +=kull;
  ...
  const kulPct  = topLim>0?Math.round(topKull/topLim*100):0;
  const barRenk = kulPct>80?'#ef4444':kulPct>50?'#f59e0b':'#10b981';

  document.getElementById('kk-ozet-bar').innerHTML=`
    ...
    <div class="met" style="border-left:3px solid var(--acc)">
      ...
      <div class="mvl" style="color:var(--acc)">₺${fmt(topLim)}</div>
    </div>
    <div class="met" style="border-left:3px solid ${barRenk}">
      <div class="mlb">Kullanılan <span style="font-size:10px;color:var(--t3)">(${kulPct}%)</span></div>
      <div class="mvl" style="color:var(--am)">₺${fmt(topKull)}</div>
      <div style="margin-top:5px;height:4px;background:var(--b1);border-radius:2px;overflow:hidden">
        <div style="height:100%;width:${kulPct}%;background:${barRenk};border-radius:2px"></div>
      </div>
    </div>
    <div class="met" style="border-left:3px solid #10b981">
      ...
      <div class="mvl" style="color:#10b981">₺${fmt(topKalan)}</div>

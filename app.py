...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX22-KK7")
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            # Kaynak 3: KK_DATA — kredi kartı kalan limitleri
            for kk in KK_DATA:
                harcamalar = kk.get('harcamalar', [])
                # Sadece gerçek harcamalar (ödeme kayıtları dahil edilmez)
                kullanilan_kk = round(sum(
                    float(h.get('tutar', 0)) for h in harcamalar
                    if not h.get('odendi') and h.get('tur') != 'odeme'
                ), 2)
                limit_kk = float(kk.get('limit', 0) or 0)
                bos_kk = round(limit_kk - kullanilan_kk, 2)
                if bos_kk > 0:
                    gecikme_faiz = float(kk.get('gecikme_faiz', 0) or 0)
                    kullanilabilir.append({
                        'id':          f"kk_{kk.get('id','')}",
                        'banka':       kk.get('banka',''),
                        'tur':         'Kredi Kartı Limiti',
                        'firma':       kk.get('firma',''),
                        'limit':       limit_kk,
                        'kullanilan':  kullanilan_kk,
                        'bos':         bos_kk,
                        'faiz':        round(gecikme_faiz/12, 4) if gecikme_faiz > 1 else gecikme_faiz,
                        'faiz_yillik': gecikme_faiz,
                        'aciklama':    f"Kredi Kartı — {kk.get('kart_no','')[-4:] if kk.get('kart_no') else ''}",
                        'kaynak':      'kk',
                    })
...
async function hesaplaOneri(){
  ...
  if(d.oneriler&&d.oneriler.length>0){
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:8px;margin-bottom:10px">
      ...
      ${d.oneriler.map(o=>{
        ...
        const kaynak = o.kaynak==='banka_kart' ? '🏦 Banka Kartı' : o.kaynak==='kk' ? '💳 Kredi Kartı' : '📋 Kredi Listesi';
        const renkBorder = o.kaynak==='banka_kart' ? 'var(--g)' : o.kaynak==='kk' ? 'var(--acc2)' : 'var(--acc)';
        return `
  ...
  if(tumLimitler.length>0){
    const kkLimitler = tumLimitler.filter(k=>k.kaynak==='kk');
    const kkTopKalan = kkLimitler.reduce((s,k)=>s+k.bos,0);
    ktDiv.innerHTML=`
      <div style="font-size:12px;font-weight:600;color:var(--t2);margin-bottom:7px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        ...
        <span style="font-weight:400;color:var(--t3);font-size:11px">🏦 Banka Kartları &nbsp;|&nbsp; 📋 Kredi Listesi &nbsp;|&nbsp; 💳 Kredi Kartları</span>
        ${kkTopKalan>0?`<span style="font-size:11px;background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.3);border-radius:5px;padding:2px 8px;color:var(--acc2)">💳 KK Kalan Limit: ₺${fmtN(kkTopKalan)}</span>`:''}
      </div>
      <div class="tc">
          ...
          <tbody>
            ${tumLimitler.map(k=>{
              ...
              return `<tr style="${isKK?'background:rgba(99,102,241,.06)':''}">
                ...
                <td class="num" style="color:${isKK?'var(--acc2)':'var(--acc)'};font-family:var(--mono);font-weight:700">${fmtN(k.bos)}</td>

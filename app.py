...
def _taksitli_odeme_senkronize(kredi_id):
    ...
    if is_doner:
        ...
        if tur == 'ROTATİF' and kullanim > 0 and vade_dt and faiz_yillik > 0:
            ...
            while dt_iter <= vade_dt:
                if dt_iter.month in ceyreksonu_aylar and dt_iter > bas_dt:
                    ...
                    taksit_no += 1
                dt_iter = datetime(dt_iter.year + (dt_iter.month // 12),
                                   dt_iter.month % 12 + 1, 1)
            # Vade sonunda ana para
            if vade_dt:
                vade_tarih_str = vade_dt.strftime('%d.%m.%Y')
                _tarih_kayit(vade_tarih_str, kullanim, 0, 0, kullanim,
                             f'ROTATİF vade sonu — ana para iadesi', taksit_no)

        elif tur == 'KMH' and kullanim > 0 and faiz_yillik > 0:
            ...
            if faiz_aylik > 0:
                taksit_tut = kullanim * (faiz_aylik*(1+faiz_aylik)**vade_ay) / ((1+faiz_aylik)**vade_ay-1)
            else:
                taksit_tut = kullanim / vade_ay
            kalan_ana = kullanim
            ...
            _tarih_kayit(tarih_str2, kullanim, 0, 0, kullanim,
                         f"{tur} kullanım — Limit: {ana_para:,.0f} ₺", 0)
    else:

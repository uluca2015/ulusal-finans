...
def github_yedekten_yukle():
    ...
    for yol, tur in dosya_map.items():
        try:
            ...
            if   tur=='gider':     DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(DATA_FILE, DATA)
            elif tur=='gelir':     GELIR_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(GELIR_FILE, GELIR_DATA)
            elif tur=='banka':     BANKA_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(BANKA_FILE, BANKA_DATA)
            elif tur=='kredi':     KREDI_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(KREDI_FILE, KREDI_DATA)
            elif tur=='cari':      CARI_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(CARI_FILE, CARI_DATA)
            elif tur=='notlar':    NOTLAR[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(NOTLAR_FILE, NOTLAR)
            elif tur=='ipotek':    IPOTEK_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(IPOTEK_FILE, IPOTEK_DATA)
            elif tur=='banka_kart':BANKA_KART_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(BANKA_KART_FILE, BANKA_KART_DATA)
            elif tur=='hatirlatma':HATIRLATMALAR[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(HATIRLATMA_FILE if 'HATIRLATMA_FILE' in dir() else os.path.join(APP_DIR,'nakit_akis_hatirlatmalar.json'), HATIRLATMALAR)
            elif tur=='docs_meta': DOCS_META[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(DOCS_META_FILE, DOCS_META)
            elif tur=='otel':      OTEL_DATA[:]=veri if isinstance(veri,list) else []; _dosyaya_kaydet_json(os.path.join(APP_DIR,'nakit_akis_otel.json'), OTEL_DATA)
            yuklu += 1
        except Exception as _e:
            print(f"  {tur} yuklenemedi: {_e}")
...
def main():
    ...
    if GITHUB_TOKEN:
        def _restore_bg():
            import time as _t
            _t.sleep(2)  # Sunucu önce başlasın
            print("  GitHub'dan veriler yukleniyor (arka plan)...")

...
async function api(url,opts={}){
  ...
  if(resp.status===401 && url !== '/api/login'){
    // Sadece login sayfasına git, hemen yönlendirme
    console.warn('401 alındı:', url);
    // toast göster ama otomatik yönlendirme yapma
    // Kullanıcı manuel çıkış yapmadıkça sayfada kalsın
  }
...
async function profilKaydet(){
  ...
  try{
    const r=await fetch('/api/users/me_update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ad})});
    const d=await r.json();
...
async function loadKullanicilar(){
  ...
  try{
    const r=await fetch('/api/users/list',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const d=await r.json();
...
async function kullaniciSifreDlg(uname){
  ...
  try{
    const r=await fetch('/api/users/change_password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:uname,new_password:yeni})});
    const d=await r.json();
...
async function izinKaydet(){
  ...
  try{
    const r=await fetch('/api/users/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:_izinDlgUser, izinler:secili})});
    const d=await r.json();
    if(d.ok){

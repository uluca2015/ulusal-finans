...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX16-KK")
...
function kkListeRender(q=''){
  ...
  if(!filtered.length){
    list.innerHTML='<div style="color:var(--t3);text-align:center;padding:40px;font-size:13px">Henüz kredi kartı eklenmedi.<br><small>Yukarıdaki "+ Yeni Kart" ile başlayın.</small></div>';
    return;
  }
  list.innerHTML=filtered.map(k=>{
    ...
    const renk=pct>80?'var(--r)':pct>50?'var(--am)':'var(--g)';
    const harcamalar=k.harcamalar||[];
    const acik=_kkAktifKartId===k.id;
    return `
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);margin-bottom:12px;overflow:hidden">
      <!-- Kart başlığı -->
      <div style="padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer" onclick="kkToggle('${k.id}')">
        <div style="width:44px;height:28px;background:linear-gradient(135deg,#1e3a5f,#2563eb);border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">💳</div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:14px">${k.banka||'—'} <span style="font-size:11px;color:var(--t3);font-family:var(--mono)">${k.kart_no?'**** '+k.kart_no.slice(-4):''}</span></div>
          <div style="font-size:11px;color:var(--t3);margin-top:2px">
            ${k.firma||''} · Kesim: ${k.kesim_gun||'—'}. · Ödeme: ${k.odeme_gun||'—'}. · Gecikme Faizi: %${parseFloat(k.gecikme_faiz||0).toFixed(2)}/yıl
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:var(--mono);font-size:13px;color:${renk};font-weight:700">₺${fmt(kalan)} kalan</div>
          <div style="font-size:11px;color:var(--t3)">₺${fmt(kull)} / ₺${fmt(limit)}</div>
          <div style="margin-top:4px;height:4px;width:120px;background:var(--b1);border-radius:2px;overflow:hidden">
            <div style="height:100%;width:${pct}%;background:${renk};border-radius:2px;transition:.3s"></div>
          </div>
        </div>
        <div style="display:flex;gap:6px;margin-left:8px">
          <button onclick="event.stopPropagation();kkDuzenle('${k.id}')" class="btn bgh" style="padding:4px 8px;font-size:12px" title="Düzenle">✏️</button>
          <button onclick="event.stopPropagation();kkSil('${k.id}')" class="btn bgh" style="padding:4px 8px;font-size:12px;color:var(--r)" title="Sil">🗑</button>
        </div>
      </div>
      <!-- Harcama paneli (toggle) -->
      ${acik?`
      <div style="border-top:1px solid var(--b1);padding:14px 16px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
          <span style="font-size:12px;font-weight:600;color:var(--acc)">📋 Harcama Kayıtları (${harcamalar.length} adet)</span>
          <button class="btn bp bsm" onclick="kkHarcamaEkleAc('${k.id}')">+ Harcama Ekle</button>
        </div>
        ${harcamalar.length?`
        <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="background:var(--s3)">
            <th style="padding:6px 8px;text-align:left;color:var(--t3);font-weight:600">S.No</th>
            <th style="padding:6px 8px;text-align:left;color:var(--t3);font-weight:600">Tarih</th>
            <th style="padding:6px 8px;text-align:left;color:var(--t3);font-weight:600">Firma / Yer</th>
            <th style="padding:6px 8px;text-align:right;color:var(--t3);font-weight:600">Tutar</th>
            <th style="padding:6px 8px;text-align:left;color:var(--t3);font-weight:600">Kategori</th>
            <th style="padding:6px 8px;text-align:left;color:var(--t3);font-weight:600">Açıklama</th>
            <th style="padding:6px 8px;text-align:center;color:var(--t3);font-weight:600">İşlem</th>
          </tr></thead>
          <tbody>
          ${[...harcamalar].sort((a,b)=>(b.tarih||'').localeCompare(a.tarih||'')).map((h,i)=>`
            <tr style="border-bottom:1px solid var(--b1);${i%2===0?'':'background:var(--s3)'}">
              <td style="padding:6px 8px;color:var(--t3)">${i+1}</td>
              <td style="padding:6px 8px;font-family:var(--mono)">${h.tarih||'—'}</td>
              <td style="padding:6px 8px;font-weight:500">${h.firma||'—'}</td>
              <td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--am);font-weight:600">₺${fmt(h.tutar)}</td>
              <td style="padding:6px 8px;color:var(--t3)">${h.kategori||'—'}</td>
              <td style="padding:6px 8px;color:var(--t2);font-size:11px">${h.aciklama||''}</td>
              <td style="padding:6px 8px;text-align:center;white-space:nowrap">
                <button onclick="kkHarcamaDuzenle('${k.id}','${h.id}')" class="btn bgh" style="padding:2px 7px;font-size:11px">✏️</button>
                <button onclick="kkHarcamaSil('${k.id}','${h.id}')" class="btn bgh" style="padding:2px 7px;font-size:11px;color:var(--r)">🗑</button>
              </td>
            </tr>`).join('')}
          </tbody>
        </table>
        </div>
        <div style="margin-top:10px;padding:8px 12px;background:var(--s3);border-radius:6px;display:flex;gap:20px;font-size:12px">
          <span style="color:var(--t3)">Toplam harcama:</span>
          <span style="font-family:var(--mono);color:var(--am);font-weight:700">₺${fmt(harcamalar.reduce((s,h)=>s+parseFloat(h.tutar||0),0))}</span>
          <span style="color:var(--t3)">Kalan limit:</span>
          <span style="font-family:var(--mono);color:var(--g);font-weight:700">₺${fmt(kalan)}</span>
        </div>`:'<div style="color:var(--t3);font-size:12px;padding:12px 0;text-align:center">Henüz harcama eklenmedi</div>'}
      </div>`:''}
    </div>`;
  }).join('');
}
...
function kkToggle(id){
  _kkAktifKartId=(_kkAktifKartId===id)?null:id;
  kkListeRender(document.getElementById('kk-arama')?.value||'');
}

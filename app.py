...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX25-BULK")
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def _do_POST_inner(self):
        ...
        elif self.path == '/api/gider/excel_sablon':
            # Gider tablosu için Excel şablonu indir
            try:
                import openpyxl
                from openpyxl.styles import Font, PatternFill, Alignment
                wb = openpyxl.Workbook()
                ...
                notlar = [
                    ('GRUP FİRMASI', 'ULUSAL, ABC, BRG, BRK veya başka bir firma adı'),
                    ('ÖDEME TÜRÜ', 'TAKSİTLİ KREDİ, ROTATİF, KMH, KREDİ KARTI, TEMİNAT MEKTUBU, DİĞER'),
                    ('ÖDEME YAPILACAK YER', 'Banka adı veya ödeme yeri (ZİRAAT BANKASI, GARANTİ BBVA vb.)'),
                    ('KREDİ/ÇEK NO', 'Kredi referans numarası, çek numarası vb. (isteğe bağlı)'),
                    ('VADE - ÖDEME TARİHİ', 'GG.AA.YYYY formatında (örn: 15.04.2026)'),
                    ('ÖDEME AYI', 'Türkçe ay adı (Ocak, Şubat, Mart ... Aralık)'),
                    ('DURUMU', 'ÖDENDİ veya ÖDENMEDİ'),
                    ('ÖDEME ANAPARA', 'Sayısal tutar (TL)'),
                    ('ÖDEME FAİZ', 'Sayısal tutar (TL)'),
                    ('ÖDEME BSMV', 'Sayısal tutar (TL)'),
                    ('ÖDEME TUTARI', 'Anapara + Faiz + BSMV toplamı'),
                    ('AÇIKLAMA', 'İsteğe bağlı açıklama'),
                ]

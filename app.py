...
function initApp(){
  try {
    ...
    xhrs.open('GET', '/api/stats', false); // false = senkron
    xhrs.send();
    var s = JSON.parse(xhrs.responseText);

    // /api/data çek
    var xhrd = new XMLHttpRequest();
    xhrd.open('GET', '/api/data', false);
    xhrd.send();
    var d = JSON.parse(xhrd.responseText);

    allData=d.records||[]; filtered=[...allData]; doSort(); renderTbl();

    var fm=function(n){n=n||0;return'₺'+n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});};
    ...
    document.getElementById('db-sub').textContent=(s.total_count||0)>0
      ...
      : 'Veri yok — sol menüden Kaynak Dosya seçin';
    populateFilters(s.firmas||[],s.turler||[],s.aylar||[],s.yillar||[]);

    // Kaynak durum
    loadSourceStatus();

    // URL'de sayfa parametresi varsa git
    ...
    if(sub) sub.textContent='Hata: '+e.message;
    ['ch-mo','ch-pie','ch-st'].forEach(function(id){
      var el=document.getElementById(id);
      if(el) el.innerHTML='<div style="color:#f87171;font-size:11px;padding:20px;text-align:center">Yüklenemedi</div>';
    });
  }
}
...
def main():
    ...
    # GitHub yedeginden yukle (local veri yoksa)
    if GITHUB_TOKEN and len(DATA) == 0:
        print("  Local veri yok, GitHub yedeginden yukleniyor...")
        github_yedekten_yukle()
    elif GITHUB_TOKEN and len(DATA) > 0:
        print(f"  Local veri mevcut: {len(DATA)} kayit, GitHub yedegi atlanıyor")

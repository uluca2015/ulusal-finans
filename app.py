...
async function bkIpotekYazdir(bkId){
  ...
  <title>İpotek Listesi - ${b.banka}</title>
  <style>
    ...
    h3{font-size:13px;color:#374151;margin-bottom:16px}
    .ozet{display:flex;gap:16px;margin-bottom:16px}
    .met{background:#f3f4f6;border-radius:8px;padding:10px 16px;min-width:140px}
    ...
    .fazla{color:#059669;font-weight:600}
    .eksik{color:#dc2626;font-weight:600}
    .num{text-align:right;font-family:monospace}
    @media print{body{margin:8px}h2{font-size:14px}}
  </style></head><body>
  <h2>🏦 ${b.banka}${b.sube?' — '+b.sube:''} İpotek Listesi</h2>
  <h3>${b.firma||''} · ${new Date().toLocaleDateString('tr-TR')}</h3>
  <div class="ozet">
    ...
    <div class="met" style="border:2px solid ${fark>=0?'#059669':'#dc2626'}">
      <div class="met-lbl">${fark>=0?'EKSPERTİZ FAZLASI':'EKSPERTİZ EKSİĞİ'}</div>
      <div class="met-val" style="color:${fark>=0?'#059669':'#dc2626'}">${fmtl(Math.abs(fark))}</div>
    </div>
  </div>
  <table>
    <thead><tr>
      <th>S.No</th><th>Sahibi</th><th>İl/İlçe</th><th>Adres</th><th>Nitelik</th>
      <th class="num">İpotek ₺</th><th class="num">Ekspertiz ₺</th><th>Fark</th>
      <th>İpotek Tarihi</th><th>DASK Vade</th>
    </tr></thead>
    <tbody>
    ${ipotekler.map((ip,i)=>{
      const fk=((ip.exp_deger||0)-(ip.tutar||0));
      const fkCls=fk>=0?'fazla':'eksik';
      const fkTxt=fk>=0?'+'+fmtl(fk):'-'+fmtl(Math.abs(fk));
      return \`<tr>
        <td>\${i+1}</td>
    ...
    <tfoot><tr style="background:#1e3a5f;color:#fff;font-weight:700">
      <td colspan="5" style="padding:7px 10px">TOPLAM (${ipotekler.length} taşınmaz)</td>
      <td class="num" style="padding:7px 10px">${fmtl(totIpt)}</td>
      <td class="num" style="padding:7px 10px">${fmtl(totExp)}</td>
      <td class="${fark>=0?'fazla':'eksik'}" style="padding:7px 10px">${fark>=0?'+':'-'}${fmtl(Math.abs(fark))}</td>
      <td colspan="2"></td>
    </tr></tfoot>
  </table>
  ...
  else toast('Popup engelleyici! Tarayıcı ayarlarını kontrol edin.','e');
}

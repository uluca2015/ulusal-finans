...
async function kullaniciEkle(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){
      if(msg){msg.textContent='✓ Eklendi';setTimeout(()=>msg.textContent='',3000);}
      document.getElementById('yeni-uname').value='';
      document.getElementById('yeni-pw').value='';
      document.getElementById('yeni-ad').value='';
      loadKullanicilar();
      toast('Kullanıcı eklendi','s');
    } else { if(msg) msg.textContent='Hata: '+(d.msg||''); toast('Hata: '+(d.msg||''),'e'); }
  }catch(e){toast('Hata','e');}
}

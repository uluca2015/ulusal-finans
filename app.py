...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX14")
    ...
    def _do_POST_inner(self):
        ...
        if self.path == '/api/ipotek/kaydet':
            ...
            try:
                ...
                # Banka değişmişse banka kartı ipotek listelerini senkronize et
                if yeni_banka != eski_banka or eski is None:
...
<div class="view" id="vw-finans">
  ...
  <div class="src-card" style="margin:0;background:rgba(79,156,249,.04);border-color:rgba(79,156,249,.2)">
    ...
    <div style="margin-bottom:14px">
      ...
      <!-- Filtre satırı 1: Yıl + Ay (çoklu listbox) -->
      ...
      <!-- Filtre satırı 2: Tarih aralığı + Butonlar -->
      ...
      <div style="font-size:10px;color:var(--t3);margin-top:7px;padding:6px 10px;background:var(--s2);border-radius:6px">
...
async function ipotekKaydet(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){
      toast('Kaydedildi ✓ Banka kartı bağlantısı güncellendi','s');
      ipotekKapat();
      renderIpotek();
...
async function ipotekSil(id){
  ...
  try{
    ...
    if(d.ok){
      toast('Silindi','i');
      renderIpotek();
      // Banka kart verisini güncelle (bağlantı temizlendi)
      try{const rb=await api('/api/banka_kart',{credentials:'include'});const db=await rb.json();_bkData=db.records||[];}catch(e){}
    } else toast('Hata','e');

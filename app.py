...
function _tkDoldurBankaDd(secili){
  ...
  if(!sel) return;
  // Banka kartlarından benzersiz banka isimlerini al
  const bankalar = [...new Set((_bkData||[]).map(k=>k.banka).filter(Boolean))].sort();
  sel.innerHTML = '<option value="">— Seçiniz —</option>';
  bankalar.forEach(b=>{
    const o = document.createElement('option');
  ...
  // Eğer seçili değer listede yoksa manuel giriş seçeneği ekle
...
function taksitliDuzenle(id){
  ...
  // Banka dropdown'ı doldur, sonra değer seç

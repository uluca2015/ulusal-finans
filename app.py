...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-23-FIX7")
...
function bkListeRender(){
  ...
  const fm=n=>n?(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:0}):'—';
  ...
  const satirlar = allIpotekler.length
    ? allIpotekler.map(ip=>`
        <label style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--s2);border-radius:8px;margin-bottom:6px;cursor:pointer;border:1px solid ${mevcutIds.includes(String(ip.id))?'var(--acc)':'transparent'}">
          ...
          <span style="font-family:var(--mono);font-size:12px;color:var(--am);white-space:nowrap">₺${(ip.tutar||0).toLocaleString('tr-TR')}</span>
        </label>`)
      .join('')

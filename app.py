...
SESSIONS   = {}  # Kara liste (çıkış yapılan tokenlar)
...
def github_yedekten_yukle():
    ...
    for yol, tur in dosya_map.items():
        ...
        elif tur=='banka_kart': BANKA_KART_DATA[:]=veri if isinstance(veri,list) else []; save_banka_kart()
        elif tur=='docs_meta':
            DOCS_META[:]=veri if isinstance(veri,list) else []
            with open(DOCS_META_FILE,'w',encoding='utf-8') as _f: json.dump(DOCS_META,_f,ensure_ascii=False,indent=2)
        elif tur=='hatirlatma': HATIRLATMALAR[:]=veri if isinstance(veri,list) else []; save_hatirlatmalar()
                    ...
                    if dosya_info.get('type') == 'file':
                        ...
                        if not os.path.exists(local_path):
                            try:
                                ...
                                print(f"  Belge yuklenemedi: {dosya_adi}: {_de}")
                print(f"  Belgeler klasoru geri yuklendi")

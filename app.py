...
def load_users():
    ...
    # Lokal dosyada sadece admin varsa veya dosya yoksa GitHub'dan kontrol et
    if len(users) <= 1 and GITHUB_TOKEN:
        try:
            github_users = github_dosya_oku(f'{BACKUP_PREFIX}nakit_akis_users.json')
            if github_users and isinstance(github_users, dict) and len(github_users) > len(users):
                print(f"  load_users: GitHub'dan {len(github_users)} kullanici yuklendi (lokalde {len(users)} vardi)")
                users = github_users
                _dosyaya_kaydet_json(USERS_FILE, users)
        except Exception as _lue:
            print(f"  load_users GitHub kontrol hatasi: {_lue}")

    if users:
        return users

    # Hiçbir yerde bulunamazsa default admin oluştur
...
def save_users(users):
    ...
    # 2. GitHub'a ANINDA yedekle (blocking - process ölmeden önce tamamlanır)
    if GITHUB_TOKEN:
        try:
            icerik = json.dumps(users, ensure_ascii=False, indent=2)
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-23-FIX9")
...
async function initApp(){
  ...
  try {
    ...
    setTimeout(async function(){
      try{
        const rm=await fetch('/api/me'); const dm=await rm.json();
        if(dm.ok){
          _meData=dm;

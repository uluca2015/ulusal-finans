...
_bk_github_timer = None

def save_banka_kart():
    global _bk_github_timer
    # 1. Diske yaz (hızlı - blocking OK)
    try:
        _snap = json.dumps(BANKA_KART_DATA, ensure_ascii=False, indent=2)
        with open(BANKA_KART_FILE, 'w', encoding='utf-8') as f:
            f.write(_snap)
        print(f"  save_banka_kart disk OK: {len(BANKA_KART_DATA)} kayit")
    except Exception as e:
        print(f"  save_banka_kart DISK HATASI: {e}")
        _snap = json.dumps(BANKA_KART_DATA, ensure_ascii=False, indent=2)
    # 2. GitHub'a debounced background thread ile yaz
    # (Hızlı ardarda kayıtlarda tek seferlik GitHub yazması)
    if GITHUB_TOKEN:
        if _bk_github_timer and _bk_github_timer.is_alive():
            pass  # Zaten bekleyen bir thread var, disk yeterli
        else:
            def _gh_write(snap=_snap):
                try:
                    ok = github_dosya_yukle(
                        f'{BACKUP_PREFIX}nakit_akis_banka_kart.json', snap,
                        f'Banka karti - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
                    )
                    print(f"  save_banka_kart GitHub: {'OK' if ok else 'BASARISIZ'}")
                except Exception as e:
                    print(f"  save_banka_kart GitHub HATA: {e}")
            _bk_github_timer = threading.Thread(target=_gh_write, daemon=False)
            _bk_github_timer.start()
...
def save_ipotek():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_ipotek.json', _d,
...
def save_settings(s):
    ...
    threading.Thread(target=_github_sync, daemon=False).start()
...
def banka_kart_kullanim_guncelle():
    """Gider tablosundaki kredi kayıtlarından banka kartı kullanımlarını hesapla.
    NOT: Manuel girilen kullanim_ değerleri korunur, sadece gider tablosunda
    eşleşen kayıt varsa o değer güncellenir.
    """
    try:
        tur_map = {
        ...
        for kart in BANKA_KART_DATA:
            ...
            # Gider tablosundan bu bankaya ait eşleşen kayıtları bul
            gider_hesap = {}  # alan -> toplam tutar
            for rec in DATA:
                ...
                gider_hesap[alan] = round(gider_hesap.get(alan, 0) + tutar, 2)
            # Sadece gider tablosunda eşleşme bulunan alanları güncelle
            # Manuel girilen değerlere DOKUNMA
            if gider_hesap:
                for alan, tutar in gider_hesap.items():
                    kart[f'kullanim_{alan}'] = tutar
                print(f"  {kart.get('banka','?')}: gider tablosundan {len(gider_hesap)} alan guncellendi")
        # Sadece diske yaz (startup'ta GitHub'a yazma - kullanıcı kaydettiğinde yazılır)
        try:
            with open(BANKA_KART_FILE, 'w', encoding='utf-8') as f:
                json.dump(BANKA_KART_DATA, f, ensure_ascii=False, indent=2)
        except Exception as _we:
            print(f"  banka_kart disk yazma: {_we}")
    except Exception as e:
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-23-FIX11")
...
_data_github_timer = None

def save_data():
    global _data_github_timer
    try:
        _snap = json.dumps(DATA, ensure_ascii=False, indent=2)
        ...
        _snap = json.dumps(DATA, ensure_ascii=False, indent=2)
    if GITHUB_TOKEN and not _RESTORING:
        if not (_data_github_timer and _data_github_timer.is_alive()):
            def _gh(snap=_snap):
                try:
                    github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_data.json', snap,
                        f'save_data - {datetime.now().strftime("%d.%m.%Y %H:%M")}')
                except Exception as e:
                    print(f"save_data GitHub hatasi: {e}")
            _data_github_timer = threading.Thread(target=_gh, daemon=False)
            _data_github_timer.start()
...
def save_gelir():
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_gelir.json', _d,
...
def save_banka():
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_banka.json', _d,
...
_kredi_github_timer = None

def save_kredi():
    global _kredi_github_timer
    try:
        _snap = json.dumps(KREDI_DATA, ensure_ascii=False, indent=2)
        ...
        _snap = json.dumps(KREDI_DATA, ensure_ascii=False, indent=2)
    if GITHUB_TOKEN and not _RESTORING:
        if not (_kredi_github_timer and _kredi_github_timer.is_alive()):
            def _gh(snap=_snap):
                try:
                    github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_kredi.json', snap,
                        f'save_kredi - {datetime.now().strftime("%d.%m.%Y %H:%M")}')
                except: pass
            _kredi_github_timer = threading.Thread(target=_gh, daemon=False)
            _kredi_github_timer.start()
...
def save_cari():
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_cari.json', _d,
...
def save_notlar():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_notlar.json', _d,
...
def save_otel_data():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_otel.json', _d,
...
def save_hatirlatmalar():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_hatirlatma.json', _d,
...
def reload_from_source(force=False):
    ...
    try:
        ...
        if count > 0:
            ...
            # Mevcut DATA'daki durum değişikliklerini koru
            # (sistemde ÖDENDİ işaretlenen kayıtlar Excel reload'da sıfırlanmasın)
            if DATA:
                # Mevcut durum bilgilerini anahtar olarak sakla
                # Anahtar: firma + tarih + tutar (kayıt kimliği gibi)
                durum_map = {}
                for mevcut in DATA:
                    anahtar = (
                        str(mevcut.get('firma','')).strip().upper(),
                        str(mevcut.get('tarih','')).strip(),
                        str(round(float(mevcut.get('tutar',0)),2)),
                        str(mevcut.get('tur','')).strip().upper(),
                    )
                    if mevcut.get('durum') == 'ÖDENDİ':
                        durum_map[anahtar] = 'ÖDENDİ'

                # Yeni kayıtlara mevcut durum bilgisini uygula
                for rec in records:
                    anahtar = (
                        str(rec.get('firma','')).strip().upper(),
                        str(rec.get('tarih','')).strip(),
                        str(round(float(rec.get('tutar',0)),2)),
                        str(rec.get('tur','')).strip().upper(),
                    )
                    if anahtar in durum_map:
                        rec['durum'] = 'ÖDENDİ'

                korunan = sum(1 for r in records if r.get('durum')=='ÖDENDİ')
                print(f"  Durum korundu: {korunan} ÖDENDİ kaydi aktarildi")
...
def bagimsiz_moda_gec():
    """Excel kaynak bağlantısını kes, mevcut veriyi sisteme al."""
    global SOURCE_FILE, SOURCE_SHEET, AUTO_RELOAD, LAST_SYNC_MSG, LAST_SYNC_OK
    onceki = SOURCE_FILE or ''
    SOURCE_FILE   = None
    SOURCE_SHEET  = None
    AUTO_RELOAD   = False
    LAST_SYNC_MSG = 'Bağımsız mod — Excel bağlantısı kesildi'
    LAST_SYNC_OK  = True
    save_config()
    save_data()
    print(f"  Bağımsız moda geçildi. Önceki: {onceki}, {len(DATA)} kayıt korundu")
    return {'ok': True, 'count': len(DATA),
            'msg': f'{len(DATA)} kayıt sistemde saklandı, Excel bağlantısı kesildi'}
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        global DOCS_META, DOCS_DIR, SOURCE_FILE, SOURCE_SHEET, AUTO_RELOAD, LAST_SYNC_MSG, LAST_SYNC_OK
        ...
        if path == '/api/banka_kart/ipotek_excel':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            bk_id = qs.get('id',[''])[0]
            kart = next((k for k in BANKA_KART_DATA if str(k.get('id'))==bk_id), None)
            if not kart: self.send_json({'ok':False,'msg':'Bulunamadı'}); return
            try:
                import openpyxl
                from openpyxl.styles import Font, PatternFill, Alignment, PatternFill
                wb = openpyxl.Workbook(); ws = wb.active
                ws.title = f"İpotek-{kart.get('banka','')[:20]}"
                # Başlık
                ws.merge_cells('A1:J1')
                ws['A1'] = f"{kart.get('banka','')} İpotek Listesi"
                ws['A1'].font = Font(bold=True, size=14, color='FFFFFF')
                ws['A1'].fill = PatternFill("solid", fgColor="1E3A5F")
                ws['A1'].alignment = Alignment(horizontal='center')
                ws.row_dimensions[1].height = 28
                # Başlıklar
                hdrs = ['S.No','Sahibi','İl','İlçe','Adres','Nitelik','İpotek Tutarı (₺)',
                        'Ekspertiz Değeri (₺)','İpotek Tarihi','DASK Vade']
                for ci, h in enumerate(hdrs, 1):
                    c = ws.cell(2, ci, h)
                    c.font = Font(bold=True, color='FFFFFF', size=10)
                    c.fill = PatternFill("solid", fgColor="2D4A6B")
                    c.alignment = Alignment(horizontal='center')
                # Veri
                ids = [str(x) for x in (kart.get('ipotekler') or [])]
                kayitlar = [r for r in IPOTEK_DATA if str(r.get('id')) in ids]
                for ri, ip in enumerate(kayitlar, 3):
                    row = [ri-2, ip.get('sahip',''), ip.get('il',''), ip.get('ilce',''),
                           ip.get('adres',''), ip.get('nitelik',''),
                           float(ip.get('tutar') or 0), float(ip.get('exp_deger') or 0),
                           ip.get('ipotek_tarih',''), ip.get('dask_vade','')]
                    for ci, v in enumerate(row, 1):
                        c = ws.cell(ri, ci, v)
                        if ci in (7,8): c.number_format = '#,##0.00'
                # Toplam satırı
                r_tot = len(kayitlar) + 3
                ws.cell(r_tot, 1, 'TOPLAM').font = Font(bold=True)
                ws.cell(r_tot, 7, sum(float(ip.get('tutar') or 0) for ip in kayitlar)).number_format='#,##0.00'
                ws.cell(r_tot, 8, sum(float(ip.get('exp_deger') or 0) for ip in kayitlar)).number_format='#,##0.00'
                # Kolon genişlikleri
                for i,w in enumerate([5,18,10,10,25,15,18,18,14,12], 1):
                    ws.column_dimensions[openpyxl.utils.get_column_letter(i)].width = w
                import io as _io
                buf = _io.BytesIO(); wb.save(buf); buf.seek(0)
                fname = f"ipotek_{kart.get('banka','').replace(' ','_')}.xlsx"
                self.send_file(buf.read(), fname, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return

        if path == '/api/banka_kart/ipotek_pdf':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            bk_id = qs.get('id',[''])[0]
            kart = next((k for k in BANKA_KART_DATA if str(k.get('id'))==bk_id), None)
            ...
            try:
                ids = [str(x) for x in (kart.get('ipotekler') or [])]
                kayitlar = [r for r in IPOTEK_DATA if str(r.get('id')) in ids]
                def fm(n): return f"{float(n or 0):,.2f}".replace(',','X').replace('.',',').replace('X','.')
                tot_ipt = sum(float(r.get('tutar') or 0) for r in kayitlar)
                tot_exp = sum(float(r.get('exp_deger') or 0) for r in kayitlar)
                fark = tot_exp - tot_ipt
                satirlar = ''
                for i, ip in enumerate(kayitlar):
                    fk = float(ip.get('exp_deger') or 0) - float(ip.get('tutar') or 0)
                    fk_cls = 'color:#059669' if fk >= 0 else 'color:#dc2626'
                    satirlar += f'''<tr style="background:{'#f9fafb' if i%2 else '#fff'}">
                      <td>{i+1}</td><td><b>{ip.get('sahip','—')}</b></td>
                      <td>{ip.get('il','')}{' / '+ip.get('ilce','') if ip.get('ilce') else ''}</td>
                      <td>{ip.get('adres','—')}</td><td>{ip.get('nitelik','—')}</td>
                      <td style="text-align:right;font-family:monospace">₺{fm(ip.get('tutar'))}</td>
                      <td style="text-align:right;font-family:monospace">₺{fm(ip.get('exp_deger'))}</td>
                      <td style="{fk_cls}">{"+" if fk>=0 else "-"}₺{fm(abs(fk))}</td>
                      <td>{ip.get('ipotek_tarih','—')}</td><td>{ip.get('dask_vade','—')}</td>
                    </tr>'''
                html = f'''<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8">
                <title>İpotek Listesi - {kart.get('banka','')}</title>
                <style>
                  body{{font-family:Arial,sans-serif;font-size:11px;margin:15px;color:#1a1a1a}}
                  h2{{font-size:15px;color:#1e3a5f;margin-bottom:3px}}
                  h3{{font-size:11px;color:#6b7280;margin-bottom:12px}}
                  .ozet{{display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap}}
                  .met{{background:#f3f4f6;border-radius:6px;padding:8px 14px}}
                  .met-lbl{{font-size:9px;color:#6b7280;font-weight:700}}
                  .met-val{{font-size:13px;font-weight:700;color:#1e3a5f}}
                  table{{width:100%;border-collapse:collapse;font-size:10px}}
                  th{{background:#1e3a5f;color:#fff;padding:6px 8px;text-align:left}}
                  td{{padding:5px 8px;border-bottom:1px solid #e5e7eb}}
                  @page{{margin:15mm}}
                </style></head><body>
                <h2>🏦 {kart.get('banka','')} İpotek Listesi</h2>
                <h3>{kart.get('firma','')} · {datetime.now().strftime('%d.%m.%Y')}</h3>
                <div class="ozet">
                  <div class="met"><div class="met-lbl">TOPLAM EKSPERTİZ</div><div class="met-val">₺{fm(tot_exp)}</div></div>
                  <div class="met"><div class="met-lbl">TOPLAM İPOTEK</div><div class="met-val">₺{fm(tot_ipt)}</div></div>
                  <div class="met" style="border:2px solid {'#059669' if fark>=0 else '#dc2626'}">
                    <div class="met-lbl">{'EKSPERTİZ FAZLASI' if fark>=0 else 'EKSPERTİZ EKSİĞİ'}</div>
                    <div class="met-val" style="color:{'#059669' if fark>=0 else '#dc2626'}">₺{fm(abs(fark))}</div>
                  </div>
                </div>
                <table>
                  <thead><tr><th>No</th><th>Sahibi</th><th>İl/İlçe</th><th>Adres</th><th>Nitelik</th>
                    <th>İpotek ₺</th><th>Ekspertiz ₺</th><th>Fark</th><th>İp. Tarihi</th><th>DASK</th></tr></thead>
                  <tbody>{satirlar}</tbody>
                  <tfoot><tr style="background:#1e3a5f;color:#fff;font-weight:700">
                    <td colspan="5" style="padding:6px 8px">TOPLAM ({len(kayitlar)} taşınmaz)</td>
                    <td style="text-align:right;padding:6px 8px;font-family:monospace">₺{fm(tot_ipt)}</td>
                    <td style="text-align:right;padding:6px 8px;font-family:monospace">₺{fm(tot_exp)}</td>
                    <td style="padding:6px 8px;color:{'#86efac' if fark>=0 else '#fca5a5'}">{"+" if fark>=0 else "-"}₺{fm(abs(fark))}</td>
                    <td colspan="2"></td>
                  </tr></tfoot>
                </table></body></html>'''
                self.send_file(html.encode('utf-8'),
                    f"ipotek_{kart.get('banka','').replace(' ','_')}.pdf",
        ...
        if path == '/':
        ...
        elif path == '/api/source/bagimsiz':

            # Excel kaynak bağlantısını kes, mevcut veriyi sisteme al
            _sonuc = bagimsiz_moda_gec()
            self.send_json(_sonuc)
            ...
            # Belirtilen ay/yıl/tarih aralığı için finansman ihtiyacını hesapla
            # Çoklu seçim: ay=Ocak&ay=Şubat&ay=Mart veya tek ay=Ocak
            aylar  = qs.get('ay',  [])   # list
            yillar = qs.get('yil', [])   # list
            bas    = qs.get('bas', [''])[0]  # YYYY-MM-DD
            bit    = qs.get('bit', [''])[0]  # YYYY-MM-DD

            def rec_filtre(r):
                tarih = r.get('tarih', '')  # DD.MM.YYYY
                r_yil = tarih[6:10] if len(tarih) >= 10 else ''
                r_ay  = r.get('ay', '')
                # Yıl filtresi
                if yillar and r_yil not in yillar: return False
                # Ay filtresi
                if aylar and r_ay not in aylar: return False
                # Tarih aralığı filtresi
                if bas or bit:
                    try:
                        from datetime import datetime as _dt
                        gun, ay2, yil2 = tarih.split('.')
                        r_date = _dt(int(yil2), int(ay2), int(gun))
                        if bas:
                            bas_d = _dt.strptime(bas, '%Y-%m-%d')
                            if r_date < bas_d: return False
                        if bit:
                            bit_d = _dt.strptime(bit, '%Y-%m-%d')
                            if r_date > bit_d: return False
                    except: pass
                return True
            ...
            # O dönem gider toplamı
            gider_bekl = sum(
                r.get('tutar',0) for r in DATA
                if r.get('durum') != 'ÖDENDİ' and rec_filtre(r)
            )

            # O dönem gelir toplamı (tahsil edilmemiş)
            gelir_bekl = sum(
                r.get('tutar',0) for r in GELIR_DATA
                if r.get('durum') != 'TAHSİL EDİLDİ' and rec_filtre(r)
            )
            ...
            # Kaynak 1: KREDI_DATA (finansman yönetiminde manuel girilen)
            kullanilabilir = []
            for k in KREDI_DATA:
            ...
            # Kaynak 2: BANKA_KART_DATA (banka kartları modülünden)
            TUR_ADI = {
            ...
            # Tüm gider verilerini JSON olarak indir (durum bilgileri dahil)
            data_json = json.dumps(DATA, ensure_ascii=False, indent=2).encode('utf-8')
            fname = f"nakit_akis_gider_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
            self.send_file(data_json, fname, 'application/json; charset=utf-8')
...
/* FİNANSMAN AY CHECKBOXları */
.fn-ay-chip{display:flex;align-items:center;gap:5px;background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:4px 8px;cursor:pointer;font-size:12px;color:var(--t2);transition:all .12s;user-select:none}
.fn-ay-chip:hover{border-color:var(--acc);color:var(--t1)}
.fn-ay-chip input[type=checkbox]{accent-color:var(--acc);width:13px;height:13px;cursor:pointer;flex-shrink:0}
.fn-ay-chip:has(input:checked){background:rgba(79,156,249,.15);border-color:var(--acc);color:var(--acc);font-weight:600}
...
<div class="view" id="vw-tbl">
  ...
  <div class="tc">
    ...
    <table>
      ...
      <tfoot id="tbl-footer" style="position:sticky;bottom:0;z-index:2">
        <tr style="background:var(--s2);border-top:2px solid var(--acc);border-bottom:1px solid var(--b1)">
          <td colspan="4" style="padding:9px 12px">
            <span id="tf-filtre-info" style="font-family:var(--mono);font-size:11px;color:var(--acc)"></span>
            <span id="tf-count" style="font-family:var(--mono);font-size:11px;color:var(--t3);margin-left:10px"></span>
          </td>
          <td colspan="2" style="padding:9px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:2px">ANAPARA</div>
            <div id="tf-anapara" style="font-family:var(--mono);font-size:12px;color:var(--t2)"></div>
          </td>
          <td style="padding:9px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:2px">FAİZ</div>
            <div id="tf-faiz" style="font-family:var(--mono);font-size:12px;color:var(--t2)"></div>
          </td>
          <td class="num" style="padding:9px 12px;background:var(--s3)">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:2px">TOPLAM TUTAR</div>
            <div id="tf-tutar" style="font-family:var(--mono);font-size:14px;font-weight:700;color:var(--t1)"></div>
          </td>
          <td></td>
        </tr>
        <tr style="background:var(--s1);border-top:1px solid var(--b1)">
          <td colspan="4" style="padding:6px 12px;font-size:11px;color:var(--t3);font-family:var(--mono)">
            📊 Filtre Özeti
          </td>
          <td colspan="2" style="padding:6px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:1px">ÖDENDİ</div>
            <div id="tf-odendi" style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--g)"></div>
          </td>
          <td style="padding:6px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:1px">BEKLEYEN</div>
            <div id="tf-bekleyen" style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--r)"></div>
          </td>
          <td class="num" style="padding:6px 12px;background:var(--s3)">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:1px">ÖDEME ORANI</div>
            <div id="tf-oran" style="font-family:var(--mono);font-size:12px;color:var(--acc)"></div>
          </td>
          <td></td>
        </tr>
      </tfoot>
...
<div class="view" id="vw-src">
  ...
  <!-- BAĞIMSIZ MOD KARTI -->
  <div class="src-card" style="background:rgba(52,211,153,.05);border-color:rgba(52,211,153,.25)">
    <h3 style="color:var(--g)">🔓 Bağımsız Mod — Excel'den Kopar</h3>
    <div style="font-size:12px;color:var(--t2);line-height:1.8;margin-bottom:14px">
      Mevcut verileri sisteme alır ve Excel kaynak bağlantısını tamamen keser.<br>
      Bundan sonra tüm kayıtlar <strong style="color:var(--t1)">sistem üzerinden</strong> yönetilir —
      giriş/çıkış yapılsa da veriler kaybolmaz.<br>
      <span style="color:var(--am)">⚠ Bu işlem geri alınamaz. Önce Excel verilerinizin sistemde yüklü olduğundan emin olun.</span>
    </div>
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
      <button class="btn" style="background:var(--g);color:#fff;border:none;padding:9px 18px;border-radius:var(--rad);cursor:pointer;font-weight:600;font-size:13px"
        onclick="bagimsizModaGec()">
        ✂ Excel Bağlantısını Kes &amp; Verileri Sisteme Al
      </button>
      <span id="bagimsiz-msg" style="font-size:12px;color:var(--g)"></span>
    </div>
  </div>



  <!-- SİSTEM VERİSİ DIŞA AKTAR -->
  <div class="src-card" style="background:rgba(79,156,249,.04);border-color:rgba(79,156,249,.15)">
    <h3 style="color:var(--acc)">💾 Sistem Verisini Yedekle</h3>
    <div style="font-size:12px;color:var(--t2);margin-bottom:12px">
      Sistemdeki tüm gider kayıtlarını Excel olarak indirin. Durum bilgileri (ÖDENDİ/ÖDENMEDİ) de dahildir.
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <a href="/api/export/excel" class="btn bp bsm" style="text-decoration:none">⬇ Excel İndir</a>
      <a href="/api/export/json" class="btn bo bsm" style="text-decoration:none">⬇ JSON İndir</a>
    </div>
  </div>
...
<div class="view" id="vw-gelir">
  ...
  <div class="tc">
    ...
    <table>
      ...
      <tfoot id="gelir-footer">
        <tr style="background:var(--s2);border-top:2px solid var(--g)">
          <td colspan="4" style="padding:9px 12px">
            <span id="gtf-count" style="font-family:var(--mono);font-size:11px;color:var(--t3)"></span>
          </td>
          <td colspan="1" style="padding:9px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:2px">TAHSİL EDİLDİ</div>
            <div id="gtf-tahsil" style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--g)"></div>
          </td>
          <td class="num" style="padding:9px 12px;background:var(--s3)">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:2px">TOPLAM GELİR</div>
            <div id="gtf-tutar" style="font-family:var(--mono);font-size:14px;font-weight:700;color:var(--t1)"></div>
          </td>
          <td></td>
        </tr>
        <tr style="background:var(--s1);border-top:1px solid var(--b1)">
          <td colspan="4" style="padding:6px 12px;font-size:11px;color:var(--t3);font-family:var(--mono)">📊 Filtre Özeti</td>
          <td style="padding:6px 10px">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:1px">BEKLENİYOR</div>
            <div id="gtf-bekleyen" style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--am)"></div>
          </td>
          <td class="num" style="padding:6px 12px;background:var(--s3)">
            <div style="font-size:9.5px;color:var(--t3);font-family:var(--mono);margin-bottom:1px">TAHSİL ORANI</div>
            <div id="gtf-oran" style="font-family:var(--mono);font-size:12px;color:var(--g)"></div>
          </td>
          <td></td>
        </tr>
      </tfoot>
...
<div class="view" id="vw-finans">
  ...
  <div class="src-card" style="margin:0;background:rgba(79,156,249,.04);border-color:rgba(79,156,249,.2)">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      ...
      <div style="display:flex;gap:6px">
        <button class="btn bp" onclick="hesaplaOneri()" style="padding:8px 20px;font-size:13px">▶ Analiz Et</button>
        <button class="btn bo" onclick="fnFiltreSifirla()" style="padding:8px 14px;font-size:12px">↺ Sıfırla</button>
      </div>
    </div>

    <!-- Filtre Satırı -->
    <div style="background:var(--s3);border:1px solid var(--b1);border-radius:var(--rad);padding:14px 16px;margin-bottom:14px">
      <div style="display:grid;grid-template-columns:1fr 2fr 1fr 1fr;gap:16px;align-items:start">

        <!-- Yıl Seçimi -->
        <div>
          <div style="font-size:10px;font-weight:700;color:var(--t3);font-family:var(--mono);letter-spacing:.06em;margin-bottom:5px">YIL <span style="color:var(--t3);font-weight:400">(Ctrl ile çoklu)</span></div>
          <select id="fn-yil" multiple
            style="width:100%;height:80px;background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);color:var(--t1);font-size:12px;padding:3px 6px;outline:none;font-family:var(--mono)">
          </select>
        </div>

        <!-- Ay Seçimi -->
        <div>
          <div style="font-size:10px;font-weight:700;color:var(--t3);font-family:var(--mono);letter-spacing:.06em;margin-bottom:5px">AY <span style="color:var(--t3);font-weight:400">(Ctrl ile çoklu)</span></div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px">
            <label class="fn-ay-chip"><input type="checkbox" value="Ocak" onchange="fnAyToggle()"> Ocak</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Şubat" onchange="fnAyToggle()"> Şubat</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Mart" onchange="fnAyToggle()"> Mart</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Nisan" onchange="fnAyToggle()"> Nisan</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Mayıs" onchange="fnAyToggle()"> Mayıs</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Haziran" onchange="fnAyToggle()"> Haziran</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Temmuz" onchange="fnAyToggle()"> Temmuz</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Ağustos" onchange="fnAyToggle()"> Ağustos</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Eylül" onchange="fnAyToggle()"> Eylül</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Ekim" onchange="fnAyToggle()"> Ekim</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Kasım" onchange="fnAyToggle()"> Kasım</label>
            <label class="fn-ay-chip"><input type="checkbox" value="Aralık" onchange="fnAyToggle()"> Aralık</label>
          </div>
          <!-- Hidden select for backward compat -->
          <select id="fn-ay" multiple style="display:none"></select>
        </div>

        <!-- Tarih Aralığı -->
        <div>
          <div style="font-size:10px;font-weight:700;color:var(--t3);font-family:var(--mono);letter-spacing:.06em;margin-bottom:5px">BAŞLANGIÇ TARİHİ</div>
          <input type="date" id="fn-bas" class="fi2" style="width:100%;font-size:12px;padding:7px 10px">
        </div>
        <div>
          <div style="font-size:10px;font-weight:700;color:var(--t3);font-family:var(--mono);letter-spacing:.06em;margin-bottom:5px">BİTİŞ TARİHİ</div>
          <input type="date" id="fn-bit" class="fi2" style="width:100%;font-size:12px;padding:7px 10px">
        </div>
...
<div class="ov" id="bk-mod" style="display:none">
  ...
  <div class="mod" style="max-width:860px;width:96%;max-height:92vh;overflow-y:auto">
    ...
    <!-- ══ İPOTEK BÖLÜMÜ ══ -->
    <div style="border-top:1px solid var(--b1);margin:0;padding:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <div style="font-size:12px;font-weight:700;color:var(--am);letter-spacing:.04em">🏠 BAĞLI İPOTEKLER / TAŞINMAZLAR</div>
        <div style="display:flex;gap:6px">
          <button onclick="bkModalIpotekEkle()" style="background:rgba(16,185,129,.15);color:var(--g);border:1px solid rgba(16,185,129,.3);padding:5px 12px;border-radius:var(--rad);cursor:pointer;font-size:11px;font-weight:600">+ Taşınmaz Ekle</button>
          <button onclick="bkModalIpotekExcel()" style="background:rgba(16,185,129,.1);color:var(--g);border:1px solid rgba(16,185,129,.2);padding:5px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px" title="Excel İndir">📊</button>
          <button onclick="bkModalIpotekYazdir()" style="background:rgba(79,156,249,.1);color:var(--acc);border:1px solid rgba(79,156,249,.2);padding:5px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px" title="Yazdır">🖨</button>
        </div>
      </div>
      <!-- İpotek özet -->
      <div id="bk-ipt-ozet" style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap"></div>
      <!-- İpotek listesi -->
      <div id="bk-ipt-liste" style="max-height:220px;overflow-y:auto;border:1px solid var(--b1);border-radius:var(--rad)">
        <div style="color:var(--t3);font-size:12px;padding:14px;text-align:center">Kayıt yükleniyor...</div>
      </div>
    </div>
...
async function bagimsizModaGec(){
  const msg = document.getElementById('bagimsiz-msg');
  if(!confirm(
    'Excel kaynak bağlantısı kesilecek ve mevcut ' + (allData?.length||0) + ' kayıt sisteme alınacak.\n\n' +
    'Bundan sonra veriler Excel\'e bağlı kalmayacak, sistem üzerinden yönetilecek.\n\n' +
    'Devam etmek istiyor musunuz?'
  )) return;

  if(msg){ msg.style.color='var(--t3)'; msg.textContent='⏳ İşleniyor...'; }

  try {
    const r = await api('/api/source/bagimsiz');
    const d = await r.json();
    if(d.ok){
      if(msg){ msg.style.color='var(--g)'; msg.textContent='✓ ' + d.msg; }
      toast('✓ Bağımsız moda geçildi — ' + d.count + ' kayıt sistemde', 's');
      loadSourceStatus();
      // Kaynak dosya alanını temizle
      const pathEl = document.getElementById('src-path');
      if(pathEl) pathEl.value = '';
    } else {
      if(msg){ msg.style.color='var(--r)'; msg.textContent='Hata: '+(d.msg||''); }
      toast('Hata: '+(d.msg||''),'e');
    }
  } catch(e) {
...
function updateTblFooter(){
  const totTutar   = filtered.reduce((s,d)=>s+(parseFloat(d.tutar)||0), 0);
  const totAnapara = filtered.reduce((s,d)=>s+(parseFloat(d.anapara)||0), 0);
  const totFaiz    = filtered.reduce((s,d)=>s+(parseFloat(d.faiz)||0), 0);
  const totOdendi  = filtered.filter(d=>d.durum==='ÖDENDİ').reduce((s,d)=>s+(parseFloat(d.tutar)||0), 0);
  const totBekl    = filtered.filter(d=>d.durum!=='ÖDENDİ').reduce((s,d)=>s+(parseFloat(d.tutar)||0), 0);
  const fmt = n => (parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const el = id => document.getElementById(id);
  if(el('tf-tutar'))    el('tf-tutar').textContent    = '₺ ' + fmt(totTutar);
  if(el('tf-anapara'))  el('tf-anapara').textContent  = totAnapara ? fmt(totAnapara) : '';
  if(el('tf-faiz'))     el('tf-faiz').textContent     = totFaiz    ? fmt(totFaiz)    : '';
  if(el('tf-odendi'))   el('tf-odendi').textContent   = '₺ ' + fmt(totOdendi);
  if(el('tf-bekleyen')) el('tf-bekleyen').textContent = '₺ ' + fmt(totBekl);
  if(el('tf-count'))    el('tf-count').textContent    = filtered.length + ' kayıt';
  // Filtre bilgisi
  const aktifFiltreler = [];
  const firma = document.getElementById('ffr')?.value; if(firma) aktifFiltreler.push(firma);
  const tur   = document.getElementById('ftu')?.value; if(tur)   aktifFiltreler.push(tur);
  const durum = document.getElementById('fdu')?.value; if(durum) aktifFiltreler.push(durum);
  const ay    = document.getElementById('fay')?.value; if(ay)    aktifFiltreler.push(ay);
  const yil   = document.getElementById('fyr')?.value; if(yil)   aktifFiltreler.push(yil);
  if(el('tf-filtre-info')) el('tf-filtre-info').textContent = aktifFiltreler.length ? 'Filtre: ' + aktifFiltreler.join(' · ') : 'Tüm kayıtlar';
  if(el('tf-oran')){
    const oran = totTutar>0 ? Math.round(totOdendi/totTutar*100) : 0;
    el('tf-oran').textContent = oran+'% ödendi';
    el('tf-oran').style.color = oran>=80?'var(--g)':oran>=50?'var(--am)':'var(--r)';
  }
}
...
function renderGelirTbl(){
  ...
  // Gelir alt toplam
  const gTotTutar  = gelirFiltered.reduce((s,r)=>s+(parseFloat(r.tutar)||0),0);
  const gTotTahsil = gelirFiltered.filter(r=>r.durum==='TAHSİL EDİLDİ').reduce((s,r)=>s+(parseFloat(r.tutar)||0),0);
  const gTotBekl   = gelirFiltered.filter(r=>r.durum!=='TAHSİL EDİLDİ').reduce((s,r)=>s+(parseFloat(r.tutar)||0),0);
  const gFmt = n => (parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const gEl = id => document.getElementById(id);
  if(gEl('gtf-tutar'))    gEl('gtf-tutar').textContent    = '₺ ' + gFmt(gTotTutar);
  if(gEl('gtf-tahsil'))   gEl('gtf-tahsil').textContent   = '₺ ' + gFmt(gTotTahsil);
  if(gEl('gtf-bekleyen')) gEl('gtf-bekleyen').textContent = '₺ ' + gFmt(gTotBekl);
  if(gEl('gtf-count'))    gEl('gtf-count').textContent    = gelirFiltered.length + ' kayıt';
  if(gEl('gtf-oran')){
    const gOran = gTotTutar>0 ? Math.round(gTotTahsil/gTotTutar*100) : 0;
    gEl('gtf-oran').textContent = gOran+'% tahsil';
    gEl('gtf-oran').style.color = gOran>=80?'var(--g)':gOran>=50?'var(--am)':'var(--r)';
  }
...
async function renderFinans(){
  ...
  if(fnYil&&d.yillar){
    const secili=Array.from(fnYil.selectedOptions).map(o=>o.value);
    fnYil.innerHTML='';
    d.yillar.forEach(y=>{
      const o=document.createElement('option');
      o.value=y; o.text=y;
      if(secili.includes(y)) o.selected=true;
      fnYil.appendChild(o);
    });
  }
  // Ay listesi HTML'de sabit tanımlı, seçimleri koru
...
function fnAyToggle(){
  // Checkbox'lardan hidden select'e senkronize et
  const ayEl=document.getElementById('fn-ay');
  if(!ayEl) return;
  const checked=Array.from(document.querySelectorAll('.fn-ay-chip input[type=checkbox]:checked')).map(c=>c.value);
  Array.from(ayEl.options).forEach(o=>{ o.selected=checked.includes(o.value); });
}

function fnFiltreSifirla(){
  const yilEl=document.getElementById('fn-yil');
  if(yilEl) Array.from(yilEl.options).forEach(o=>o.selected=false);
  // Checkboxları temizle
  document.querySelectorAll('.fn-ay-chip input[type=checkbox]').forEach(c=>c.checked=false);
  const ayEl=document.getElementById('fn-ay');
  if(ayEl) Array.from(ayEl.options).forEach(o=>o.selected=false);
  const b=document.getElementById('fn-bas'); if(b) b.value='';
  const e=document.getElementById('fn-bit'); if(e) e.value='';
}

async function hesaplaOneri(){

  // Çoklu seçim: seçilen tüm değerleri al
  const yilEl=document.getElementById('fn-yil');
  const ayEl=document.getElementById('fn-ay');
  const yillar=yilEl?Array.from(yilEl.selectedOptions).map(o=>o.value).filter(v=>v):[];
  const aylar=ayEl?Array.from(ayEl.selectedOptions).map(o=>o.value).filter(v=>v):[];
  const bas=document.getElementById('fn-bas')?.value||'';
  const bit=document.getElementById('fn-bit')?.value||'';

  const btn=document.querySelector('#vw-finans button.btn.bp');

  if(btn){btn.textContent='⏳';btn.disabled=true;}

  const params=new URLSearchParams();

  yillar.forEach(y=>params.append('yil',y));
  aylar.forEach(a=>params.append('ay',a));
  ...
  if(tumLimitler.length>0){
    ktDiv.innerHTML=`
      <div style="font-size:12px;font-weight:600;color:var(--t2);margin-bottom:7px">
        Tüm Kullanılabilir Limitler — Düşük Faizden Yükseğe
        <span style="font-weight:400;color:var(--t3);margin-left:8px">(Banka Kartları + Kredi Listesi)</span>
      </div>
      <div class="tc">
        <table style="font-size:12px">
          ...
          <tbody>
            ${tumLimitler.map(k=>{
              ...
              const kaynak = k.kaynak==='banka_kart'
                ? '<span style="color:var(--g);font-size:10px">🏦 Kart</span>'
                : '<span style="color:var(--acc);font-size:10px">📋 Liste</span>';
              return `<tr>
                ...
                <td class="num" style="color:var(--acc);font-family:var(--mono);font-weight:600">${fmtN(k.bos)}</td>
...
function bankaKartEkleAc(){
  ...
  // Yeni kart - ipotek bölümünü temizle
  const iptListe=document.getElementById('bk-ipt-liste');
  const iptOzet=document.getElementById('bk-ipt-ozet');
  if(iptListe) iptListe.innerHTML='<div style="color:var(--t3);font-size:12px;padding:14px;text-align:center">Önce kartı kaydedin, sonra taşınmaz ekleyebilirsiniz.</div>';
  if(iptOzet) iptOzet.innerHTML='';
  window._bkModalEditId = null;
}
...
function bankaKartDuzenle(id){
  ...
  bkHesapla();
  document.getElementById('bk-mod').style.display='flex';
  // İpotek bölümünü yükle
  setTimeout(()=>bkModalIpotekYukle(id), 150);
}

function bankaKartKapat(){ const m=document.getElementById('bk-mod'); m.classList.remove('on'); m.style.display=''; _bkEditId=null; window._bkModalEditId=null; }

// ══ MODAL İPOTEK YÖNETİMİ ══════════════════════════════════════════════════

async function bkModalIpotekYukle(bkId){
  window._bkModalEditId = bkId;
  const liste = document.getElementById('bk-ipt-liste');
  const ozet  = document.getElementById('bk-ipt-ozet');
  if(!liste) return;

  liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px;text-align:center">⏳ Yükleniyor...</div>';
  if(ozet) ozet.innerHTML='';

  const b = _bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px;text-align:center">Kaydedilmemiş kart.</div>'; return; }

  try{
    const r=await api('/api/ipotek'); const d=await r.json();
    const tumIpt = d.records||[];
    const ipIds = (b.ipotekler||[]).map(x=>String(x));
    const bankaIpt = tumIpt.filter(ip=>
      ipIds.includes(String(ip.id)) ||
      (ip.banka && ip.banka.toLowerCase()===b.banka.toLowerCase())
    );

    const fm=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
    const ftl=n=>{const v=parseFloat(n)||0;return v>0?'₺'+fm(v):'₺0,00';};

    // ÖZET KARTLAR
    const totIpt = bankaIpt.reduce((s,ip)=>s+(parseFloat(ip.tutar)||0),0);
    const totExp = bankaIpt.reduce((s,ip)=>s+(parseFloat(ip.exp_deger)||0),0);
    const fark = totExp-totIpt;
    if(ozet) ozet.innerHTML=`
      <div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">TAŞINMAZ</div>
        <div style="color:var(--acc);font-weight:700">${bankaIpt.length} adet</div>
      </div>
      <div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">TOPLAM İPOTEK</div>
        <div style="color:var(--am);font-weight:700;font-family:var(--mono)">${ftl(totIpt)}</div>
      </div>
      <div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">EKSPERTİZ</div>
        <div style="color:var(--acc);font-weight:700;font-family:var(--mono)">${ftl(totExp)}</div>
      </div>
      <div style="background:var(--s2);border:1px solid ${fark>=0?'var(--g)':'var(--r)'};border-radius:var(--rad);padding:6px 12px;font-size:11px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">${fark>=0?'FAZLA ✓':'EKSİK ⚠'}</div>
        <div style="color:${fark>=0?'var(--g)':'var(--r)'};font-weight:700;font-family:var(--mono)">${ftl(Math.abs(fark))}</div>
      </div>`;

    // TABLO
    if(!bankaIpt.length){
      liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:16px;text-align:center">Bu bankaya bağlı taşınmaz yok. "+ Taşınmaz Ekle" ile ekleyebilirsiniz.</div>';
      return;
    }

    let html='<table style="width:100%;border-collapse:collapse;font-size:11px">'
      +'<thead><tr style="background:var(--s2);color:var(--t3);font-size:10px;position:sticky;top:0">'
      +'<th style="padding:6px 8px;text-align:left">Sahibi</th>'
      +'<th style="padding:6px 8px;text-align:left">Konum / Nitelik</th>'
      +'<th style="padding:6px 8px;text-align:right">İpotek ₺</th>'
      +'<th style="padding:6px 8px;text-align:right">Ekspertiz ₺</th>'
      +'<th style="padding:6px 8px;text-align:right">Fark</th>'
      +'<th style="padding:6px 8px;text-align:center">DASK</th>'
      +'<th style="padding:6px 8px;text-align:center;width:70px">İşlem</th>'
      +'</tr></thead><tbody>';

    bankaIpt.forEach(function(ip,i){
      const fk=(parseFloat(ip.exp_deger)||0)-(parseFloat(ip.tutar)||0);
      const fkR=fk>=0?'var(--g)':'var(--r)';
      html+='<tr style="border-bottom:1px solid var(--b1);'+(i%2?'background:rgba(255,255,255,.015)':'')+'">'
        +'<td style="padding:6px 8px;font-weight:600">'+( ip.sahip||'—')+'</td>'
        +'<td style="padding:6px 8px;color:var(--t3);font-size:10px">'+(ip.il||'')+' '+(ip.ilce?'/ '+ip.ilce:'')+' · '+(ip.nitelik||'—')+'</td>'
        +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--am)">'+ftl(ip.tutar)+'</td>'
        +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--acc)">'+ftl(ip.exp_deger)+'</td>'
        +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);font-weight:600;color:'+fkR+'">'+(fk>=0?'+':'')+ftl(Math.abs(fk))+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:10px;color:'+(ip.dask_vade?'var(--t2)':'var(--t3)')+'">'+(ip.dask_vade||'—')+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'
        +'<button onclick="bkModalIpotekDuzenle(''+ip.id+'')" style="background:var(--s3);color:var(--t2);border:1px solid var(--b2);padding:2px 7px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">✏</button>'
        +'<button onclick="bkModalIpotekCikar(''+bkId+'',''+ip.id+'')" style="background:rgba(239,68,68,.1);color:var(--r);border:1px solid rgba(239,68,68,.3);padding:2px 7px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">✕</button>'
        +'</td></tr>';
    });
    html+='</tbody></table>';
    ...
    liste.innerHTML='<div style="color:var(--r);font-size:12px;padding:10px">Hata: '+e.message+'</div>';
  }
}

function bkModalIpotekEkle(){
  const bkId=window._bkModalEditId;
  const b=bkId?_bkData.find(x=>String(x.id)===String(bkId)):null;
  window._bkIpotekYukleCallback=bkId;
  ipotekEkleAc(b?b.banka:'');
}

function bkModalIpotekDuzenle(ipId){
  window._bkIpotekYukleCallback=window._bkModalEditId;
  ipotekDuzenle(ipId);
  document.getElementById('ipt-mod').style.display='flex';
}

async function bkModalIpotekCikar(bkId, ipId){
  if(!confirm('Bu taşınmazı bankadan ayır?\nKayıt silinmez, bağlantı kopar.')) return;
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b) return;
  b.ipotekler=(b.ipotekler||[]).filter(x=>String(x)!==String(ipId));
  try{
    const r=await api('/api/banka_kart/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});
    const d=await r.json();
    if(d.ok){ toast('Bağlantı kaldırıldı','i'); await bkModalIpotekYukle(bkId); }
    else toast('Hata','e');
  }catch(e){ toast('Hata','e'); }
}

function bkModalIpotekExcel(){
  const bkId=window._bkModalEditId;
  if(!bkId){ toast('Kart seçili değil','e'); return; }
  const a=document.createElement('a');
  a.href='/api/banka_kart/ipotek_excel?id='+encodeURIComponent(bkId);
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  a.download='ipotek_'+((b?b.banka:bkId)||bkId).replace(/[^a-zA-Z0-9_À-ž]/g,'_')+'.xlsx';
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  toast('Excel indiriliyor...','i');
}

async function bkModalIpotekYazdir(){
  const bkId=window._bkModalEditId;
  if(!bkId){ toast('Kart seçili değil','e'); return; }
  // bkIpotekYazdir fonksiyonunu çağır
  await bkIpotekYazdir(bkId);
}
...
async function bkIpotekDetayToggle(bkId){
  ...
  row.style.display='';
  await bkIpotekPanelYukle(bkId);
}

async function bkIpotekPanelYukle(bkId){
  const inner = document.getElementById('bkipt-inner-'+bkId);
  if(!inner) return;
  const b = _bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ inner.innerHTML='<div style="color:var(--r);font-size:12px;padding:8px">Banka bulunamadı</div>'; return; }

  inner.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px">⏳ Yükleniyor...</div>';

  try{
    const r = await api('/api/ipotek'); const d = await r.json();
    const tumIpotekler = d.records||[];
    // Bu bankaya bağlı ipotekler (ipotekler[] dizisi veya banka adı eşleşmesi)
    const ipotekIds = (b.ipotekler||[]).map(x=>String(x));
    const bankaIpotekleri = tumIpotekler.filter(ip=>
      ipotekIds.includes(String(ip.id)) ||
      (ip.banka && ip.banka.toLowerCase()===b.banka.toLowerCase())
    );

    const fm=n=>n?(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}):'—';
    const fmtl=n=>{const v=parseFloat(n)||0;return v>0?'₺'+fm(v):'—';};
    const totExp = bankaIpotekleri.reduce((s,ip)=>s+(parseFloat(ip.exp_deger)||0),0);
    const totIpt = bankaIpotekleri.reduce((s,ip)=>s+(parseFloat(ip.tutar)||0),0);
    const fark = totExp - totIpt;
    ...
    // ── ÖZET KARTLAR ──
    html+=`<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px;align-items:center">
      <div style="background:var(--s2);border-radius:var(--rad);padding:8px 14px;font-size:11px;min-width:120px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">TOPLAM EKSPERTİZ</div>
        <div style="color:var(--acc);font-weight:700;font-family:var(--mono)">${fmtl(totExp)}</div>
      </div>
      <div style="background:var(--s2);border-radius:var(--rad);padding:8px 14px;font-size:11px;min-width:120px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">TOPLAM İPOTEK</div>
        <div style="color:var(--am);font-weight:700;font-family:var(--mono)">${fmtl(totIpt)}</div>
      </div>
      <div style="background:var(--s2);border:1px solid ${farkRenk};border-radius:var(--rad);padding:8px 14px;font-size:11px;min-width:120px">
        <div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">${fark>=0?'EKSPERTİZ FAZLASI ✓':'EKSPERTİZ EKSİĞİ ⚠'}</div>
        <div style="color:${farkRenk};font-weight:700;font-family:var(--mono)">${fmtl(Math.abs(fark))}</div>
      </div>
      <div style="margin-left:auto;display:flex;gap:6px;flex-wrap:wrap">
        <button onclick="bkIpotekEkle('`+bkId+`')" style="background:var(--g);color:#fff;border:none;padding:6px 14px;border-radius:var(--rad);cursor:pointer;font-size:12px;font-weight:600">+ Taşınmaz Ekle</button>
        <button onclick="bkIpotekExcel('`+bkId+`')" style="background:rgba(16,185,129,.15);color:var(--g);border:1px solid rgba(16,185,129,.3);padding:6px 12px;border-radius:var(--rad);cursor:pointer;font-size:11px">📊 Excel</button>
        <button onclick="bkIpotekYazdir('`+bkId+`')" style="background:rgba(79,156,249,.12);color:var(--acc);border:1px solid rgba(79,156,249,.3);padding:6px 12px;border-radius:var(--rad);cursor:pointer;font-size:11px">🖨 Yazdır</button>
      </div>
    </div>`;

    // ── İPOTEK TABLOSU ──
    if(!bankaIpotekleri.length){
      html+=`<div style="color:var(--t3);font-size:12px;padding:16px;text-align:center;background:var(--s2);border-radius:var(--rad)">
        Bu bankaya ait taşınmaz kaydı yok. "+ Taşınmaz Ekle" ile ekleyebilirsiniz.
      </div>`;
    } else {
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:800px">
        <thead><tr style="background:var(--s2);color:var(--t2);font-size:11px">
          <th style="padding:7px 10px;text-align:left">Sahibi</th>
          <th style="padding:7px 10px;text-align:left">Konum / Nitelik</th>
          <th style="padding:7px 10px;text-align:right">İpotek ₺</th>
          <th style="padding:7px 10px;text-align:right">Ekspertiz ₺</th>
          <th style="padding:7px 10px;text-align:right">Fark</th>
          <th style="padding:7px 10px;text-align:center">İp. Tarihi</th>
          <th style="padding:7px 10px;text-align:center">DASK</th>
          <th style="padding:7px 10px;text-align:center;width:80px">İşlem</th>
        </tr></thead>
        <tbody>`;
      bankaIpotekleri.forEach(function(ip,idx){
        const fk=((parseFloat(ip.exp_deger)||0)-(parseFloat(ip.tutar)||0));
        const fkR=fk>=0?'var(--g)':'var(--r)';
        const fkTxt=(fk>=0?'+':'')+fmtl(Math.abs(fk));
        html+=`<tr style="${idx%2?'background:rgba(255,255,255,.015)':''}">
          <td style="padding:7px 10px;font-weight:600">${ip.sahip||'—'}</td>
          <td style="padding:7px 10px;color:var(--t3);font-size:11px">${ip.il||''} ${ip.ilce?'/ '+ip.ilce:''} · ${ip.nitelik||'—'}</td>
          <td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:var(--am)">${fmtl(ip.tutar)}</td>
          <td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:var(--acc)">${fmtl(ip.exp_deger)}</td>
          <td style="padding:7px 10px;text-align:right;font-family:var(--mono);font-weight:600;color:${fkR}">${fkTxt}</td>
          <td style="padding:7px 10px;text-align:center;font-size:11px">${ip.ipotek_tarih||'—'}</td>
          <td style="padding:7px 10px;text-align:center;font-size:11px;color:${ip.dask_vade?'var(--t2)':'var(--t3)'}">${ip.dask_vade||'—'}</td>
          <td style="padding:7px 10px;text-align:center">
            <button onclick="bkIpotekDuzenle('`+bkId+`','`+ip.id+`')" style="background:var(--s3);color:var(--t2);border:1px solid var(--b2);padding:3px 8px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">✏</button>
            <button onclick="bkIpotekCikar('`+bkId+`','`+ip.id+`')" style="background:rgba(239,68,68,.1);color:var(--r);border:1px solid rgba(239,68,68,.3);padding:3px 8px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">✕</button>
          </td>
        </tr>`;
      });
...
// Banka kartından doğrudan ipotek ekle (o bankayı ön seçimle)
function bkIpotekEkle(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b) return;
  // İpotek modunu aç, bankayı ön seç
  ipotekEkleAc(b.banka);
  // Kaydedilince banka kartı panelini yenile
  window._bkIpotekYukleCallback = bkId;
}

// Banka kartından ipotek düzenle
function bkIpotekDuzenle(bkId, ipId){
  window._bkIpotekYukleCallback = bkId;
  ipotekDuzenle(ipId);
  document.getElementById('ipt-mod').style.display='flex';
}

// Banka kartından ipotek çıkar (sadece banka bağlantısını kopar, ipoteği silmez)
async function bkIpotekCikar(bkId, ipId){
  if(!confirm('Bu taşınmazı bankadan ayırmak istiyor musunuz?\nTaşınmaz kaydı silinmez, sadece bu bankadan bağlantısı kopar.')) return;
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b) return;
  b.ipotekler=(b.ipotekler||[]).filter(x=>String(x)!==String(ipId));
  try{
    const r=await api('/api/banka_kart/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});
    const d=await r.json();
    if(d.ok){ toast('Bağlantı kaldırıldı','i'); await bkIpotekPanelYukle(bkId); }
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){ toast('Hata','e'); }
}

// ── BANKA KARTI İPOTEK ÇIKTI FONKSİYONLARI ────────────────────────────────

async function bkIpotekExcel(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ toast('Banka bulunamadı','e'); return; }
  const a=document.createElement('a');
  a.href=`/api/banka_kart/ipotek_excel?id=${encodeURIComponent(bkId)}`;
  a.download=`ipotek_${(b.banka||bkId).replace(/\s/g,'_')}.xlsx`;
  a.click();
  toast('Excel indiriliyor...','i');
}

async function bkIpotekPDF(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ toast('Banka bulunamadı','e'); return; }
  const a=document.createElement('a');
  a.href=`/api/banka_kart/ipotek_pdf?id=${encodeURIComponent(bkId)}`;
  a.download=`ipotek_${(b.banka||bkId).replace(/\s/g,'_')}.pdf`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  toast('PDF indiriliyor...','i');
}

async function bkIpotekYazdir(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ toast('Banka bulunamadı','e'); return; }
  ...
  try{
    const r=await api('/api/ipotek'); const d=await r.json();
    const ids=(b.ipotekler||[]).map(x=>String(x));
    ipotekler=(d.records||[]).filter(ip=>ids.includes(String(ip.id)));
  }catch(e){ toast('Veri çekme hatası','e'); return; }

  const fm=n=>n?(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}):'—';
  const fmtl=n=>{const v=parseFloat(n)||0;return v>0?'₺'+fm(v):'—';};
  const totExp=ipotekler.reduce((s,ip)=>s+(ip.exp_deger||0),0);
  const totIpt=ipotekler.reduce((s,ip)=>s+(ip.tutar||0),0);
  const fark=totExp-totIpt;

  // Satırları string concat ile oluştur (nested backtick yok)
  let satirlar='';
  ipotekler.forEach(function(ip,i){
    const fk=(ip.exp_deger||0)-(ip.tutar||0);
    const fkCls=fk>=0?'fazla':'eksik';
    const fkTxt=(fk>=0?'+':'-')+fmtl(Math.abs(fk));
    const bgc=i%2===0?'#fff':'#f9fafb';
    satirlar+='<tr style="background:'+bgc+'">'
      +'<td>'+(i+1)+'</td>'
      +'<td style="font-weight:600">'+(ip.sahip||'—')+'</td>'
      +'<td>'+(ip.il||'')+(ip.ilce?' / '+ip.ilce:'')+'</td>'
      +'<td>'+(ip.adres||'—')+'</td>'
      +'<td>'+(ip.nitelik||'—')+'</td>'
      +'<td class="num">'+fmtl(ip.tutar)+'</td>'
      +'<td class="num">'+fmtl(ip.exp_deger)+'</td>'
      +'<td class="'+fkCls+'">'+fkTxt+'</td>'
      +'<td>'+(ip.ipotek_tarih||'—')+'</td>'
      +'<td>'+(ip.dask_vade||'—')+'</td>'
      +'</tr>';
  });

  const banka=b.banka||'';
  const sube=b.sube?(' — '+b.sube):'';
  const firma=b.firma||'';
  const tarih=new Date().toLocaleDateString('tr-TR');
  const farkRenk=fark>=0?'#059669':'#dc2626';
  const farkEtk=fark>=0?'EKSPERTİZ FAZLASI':'EKSPERTİZ EKSİĞİ';
  const farkTxt=(fark>=0?'+':'-')+'₺'+fm(Math.abs(fark));
  const totTxt=ipotekler.length+' taşınmaz';

  let html='<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8">'
    +'<title>İpotek Listesi - '+banka+'</title>'
    +'<style>'
    +'body{font-family:Segoe UI,sans-serif;font-size:12px;color:#1a1a1a;margin:20px}'
    +'h2{font-size:16px;margin-bottom:4px;color:#1e3a5f}'
    +'h3{font-size:13px;color:#374151;margin-bottom:14px}'
    +'.ozet{display:flex;gap:14px;margin-bottom:14px;flex-wrap:wrap}'
    +'.met{background:#f3f4f6;border-radius:8px;padding:10px 16px;min-width:130px}'
    +'.met-lbl{font-size:10px;color:#6b7280;font-weight:600;margin-bottom:3px}'
    +'.met-val{font-size:14px;font-weight:700;color:#1e3a5f}'
    +'table{width:100%;border-collapse:collapse;font-size:11px}'
    +'th{background:#1e3a5f;color:#fff;padding:7px 10px;text-align:left}'
    +'td{padding:7px 10px;border-bottom:1px solid #e5e7eb}'
    +'.fazla{color:#059669;font-weight:600}.eksik{color:#dc2626;font-weight:600}'
    +'.num{text-align:right;font-family:monospace}'
    +'tfoot td{background:#1e3a5f;color:#fff;font-weight:700;padding:7px 10px}'
    +'@media print{body{margin:8px}}'
    +'</style></head><body>'
    +'<h2>🏦 '+banka+sube+' İpotek Listesi</h2>'
    +'<h3>'+firma+' · '+tarih+'</h3>'
    +'<div class="ozet">'
    +'<div class="met"><div class="met-lbl">TOPLAM EKSPERTİZ</div><div class="met-val">'+fmtl(totExp)+'</div></div>'
    +'<div class="met"><div class="met-lbl">TOPLAM İPOTEK</div><div class="met-val">'+fmtl(totIpt)+'</div></div>'
    +'<div class="met" style="border:2px solid '+farkRenk+'">'
    +'<div class="met-lbl">'+farkEtk+'</div>'
    +'<div class="met-val" style="color:'+farkRenk+'">'+farkTxt+'</div></div>'
    +'</div>'
    +'<table><thead><tr>'
    +'<th>No</th><th>Sahibi</th><th>İl/İlçe</th><th>Adres</th><th>Nitelik</th>'
    +'<th class="num">İpotek ₺</th><th class="num">Ekspertiz ₺</th><th>Fark</th>'
    +'<th>İp.Tarihi</th><th>DASK</th>'
    +'</tr></thead>'
    +'<tbody>'+satirlar+'</tbody>'
    +'<tfoot><tr>'
    +'<td colspan="5">TOPLAM ('+totTxt+')</td>'
    +'<td class="num">'+fmtl(totIpt)+'</td>'
    +'<td class="num">'+fmtl(totExp)+'</td>'
    +'<td>'+farkTxt+'</td>'
    +'<td colspan="2"></td>'
    +'</tr></tfoot></table>'
  ...
  else toast('Popup engelleyici! Tarayıcı ayarlarından popup izni verin.','e');
}
...
async function renderIpotek(){
  // Banka listesini de yükle (BANKA_KART_DATA gerekli)
  if(!_bkData || !_bkData.length){
    try{ const r=await api('/api/banka_kart'); const d=await r.json(); _bkData=d.records||[]; }catch(e){}
  }
...
function iptBankaDoldur(secili){
  const sel=document.getElementById('ipt-banka'); if(!sel) return;
  // BANKA_KART_DATA'dan benzersiz banka listesi
  const bankalar=[...new Set((_bkData||[]).map(b=>b.banka).filter(Boolean))].sort();
  sel.innerHTML='<option value="">-- Banka Seçin --</option>'
    +bankalar.map(b=>`<option value="${b}"${b===secili?' selected':''}>${b}</option>`).join('');
  // Listede yoksa ekstra option ekle
  if(secili && !bankalar.includes(secili)){
    sel.innerHTML+=`<option value="${secili}" selected>${secili}</option>`;
  }
}

function ipotekEkleAc(onBanka){
  _ipotekEditId=null;
...
function ipotekDuzenle(id){
  ...
  iptBankaDoldur(r.banka); f('ipt-tutar',r.tutar);f('ipt-ipotek-tarih',r.ipotek_tarih);
...
async function ipotekKaydet(){
  ...
  try{
    const r=await api('/api/ipotek/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(kayit)});
    const d=await r.json();
    if(d.ok){
      toast('Kaydedildi','s');
      // Banka kartı ile bağlantı kur: seçilen banka adına göre kart bul ve ipoteği bağla
      const secilenBanka=kayit.banka;
      if(secilenBanka){
        const bkKart=_bkData.find(b=>b.banka===secilenBanka);
        if(bkKart){
          if(!bkKart.ipotekler) bkKart.ipotekler=[];
          const ipId=String(d.id||kayit.id);
          if(!bkKart.ipotekler.map(x=>String(x)).includes(ipId)){
            bkKart.ipotekler.push(ipId);
            await api('/api/banka_kart/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(bkKart)});
          }
        }
      }
      ipotekKapat();
      renderIpotek();
      // Banka kartı panelini yenile
      if(window._bkIpotekYukleCallback){
        const cbId=window._bkIpotekYukleCallback;
        window._bkIpotekYukleCallback=null;
        const row=document.getElementById('bkipt-'+cbId);
        if(row && row.style.display!=='none') await bkIpotekPanelYukle(cbId);
        await renderBankaKart();
      }
      // Modal içindeki ipotek panelini de yenile
      if(window._bkModalEditId){
        await bkModalIpotekYukle(window._bkModalEditId);
      }
    }else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata: '+e.message,'e');}

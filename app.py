...
async function bkModalIpotekYukle(bkId){
  ...
  liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px;text-align:center">Yukleniyor...</div>';
  if(ozet) ozet.innerHTML='';

  const b = _bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px;text-align:center">Kaydedilmemis kart.</div>'; return; }
  ...
  try{
    ...
    const fm=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
    const ftl=n=>{const v=parseFloat(n)||0;return v>0?'TL'+fm(v):'TL0,00';};
    ...
    const farkEtkt = fark>=0 ? 'FAZLA' : 'EKSIK';

    // OZET - string concat
    if(ozet) ozet.innerHTML=''
      +'<div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">'
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">TASINMAZ</div>'
      +'<div style="color:var(--acc);font-weight:700">'+bankaIpt.length+' adet</div></div>'
      +'<div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">'
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">TOPLAM IPOTEK</div>'
      +'<div style="color:var(--am);font-weight:700;font-family:var(--mono)">'+ftl(totIpt)+'</div></div>'
      +'<div style="background:var(--s2);border-radius:var(--rad);padding:6px 12px;font-size:11px">'
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">EKSPERTIZ</div>'
      +'<div style="color:var(--acc);font-weight:700;font-family:var(--mono)">'+ftl(totExp)+'</div></div>'
      +'<div style="background:var(--s2);border:1px solid '+farkRenk+';border-radius:var(--rad);padding:6px 12px;font-size:11px">'
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono)">'+farkEtkt+'</div>'
      +'<div style="color:'+farkRenk+';font-weight:700;font-family:var(--mono)">'+ftl(Math.abs(fark))+'</div></div>';

    if(!bankaIpt.length){
      liste.innerHTML='<div style="color:var(--t3);font-size:12px;padding:16px;text-align:center">Bu bankaya bagli tasinmaz yok. Yukardaki "+ Tasinmaz Ekle" ile ekleyebilirsiniz.</div>';
      return;
    }

    var html=''
      +'<table style="width:100%;border-collapse:collapse;font-size:11px">'
      +'<thead><tr style="background:var(--s2);color:var(--t3);font-size:10px">'
      +'<th style="padding:6px 8px;text-align:left">Sahibi</th>'
      +'<th style="padding:6px 8px;text-align:left">Konum / Nitelik</th>'
      +'<th style="padding:6px 8px;text-align:right">Ipotek</th>'
      ...
      +'<th style="padding:6px 8px;text-align:center;width:70px">Islem</th>'
      +'</tr></thead><tbody>';

    bankaIpt.forEach(function(ip,i){
      var fk=(parseFloat(ip.exp_deger)||0)-(parseFloat(ip.tutar)||0);
      var fkR=fk>=0?'var(--g)':'var(--r)';
      var rowBg=i%2?'background:rgba(255,255,255,.015)':'';
      html+=''
        +'<tr style="border-bottom:1px solid var(--b1);'+rowBg+'">'
...
async function bkIpotekPanelYukle(bkId){
  ...
  if(!inner) return;
  const b = _bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ inner.innerHTML='<div style="color:var(--r);font-size:12px;padding:8px">Banka bulunamadi</div>'; return; }

  inner.innerHTML='<div style="color:var(--t3);font-size:12px;padding:12px">Yukleniyor...</div>';
  ...
  try{
    ...
    const fm=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
    const ftl=n=>{var v=parseFloat(n)||0;return v>0?'TL'+fm(v):'TL0,00';};
    const totExp = bankaIpt.reduce((s,ip)=>s+(parseFloat(ip.exp_deger)||0),0);
    const totIpt = bankaIpt.reduce((s,ip)=>s+(parseFloat(ip.tutar)||0),0);
    const fark = totExp - totIpt;
    const farkRenk = fark>=0 ? 'var(--g)' : 'var(--r)';
    ...
    var html = ''
      ...
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">TOPLAM EKSPERTIZ</div>'
      +'<div style="color:var(--acc);font-weight:700;font-family:var(--mono)">'+ftl(totExp)+'</div></div>'
      +'<div style="background:var(--s2);border-radius:var(--rad);padding:8px 14px;font-size:11px;min-width:120px">'
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">TOPLAM IPOTEK</div>'
      +'<div style="color:var(--am);font-weight:700;font-family:var(--mono)">'+ftl(totIpt)+'</div></div>'
      ...
      +'<div style="color:var(--t3);font-size:9.5px;font-family:var(--mono);margin-bottom:2px">'+(fark>=0?'EKSPERTIZ FAZLASI':'EKSPERTIZ EKSIGI')+'</div>'
      +'<div style="color:'+farkRenk+';font-weight:700;font-family:var(--mono)">'+ftl(Math.abs(fark))+'</div></div>'
      +'<div style="margin-left:auto;display:flex;gap:6px;flex-wrap:wrap">'
      +'<button onclick="bkIpotekEkle(''+bkId+'')" style="background:var(--g);color:#fff;border:none;padding:6px 14px;border-radius:var(--rad);cursor:pointer;font-size:12px;font-weight:600">+ Tasinmaz Ekle</button>'
      ...
      +'<button onclick="bkIpotekYazdir(''+bkId+'')" style="background:rgba(79,156,249,.12);color:var(--acc);border:1px solid rgba(79,156,249,.3);padding:6px 12px;border-radius:var(--rad);cursor:pointer;font-size:11px">Yazdir</button>'
      +'</div></div>';

    if(!bankaIpt.length){
      html+='<div style="color:var(--t3);font-size:12px;padding:16px;text-align:center;background:var(--s2);border-radius:var(--rad)">Bu bankaya ait tasinmaz kaydi yok.</div>';
    } else {
      html+=''
        +'<div style="overflow-x:auto">'
        ...
        +'<th style="padding:7px 10px;text-align:right">Ipotek TL</th>'
        +'<th style="padding:7px 10px;text-align:right">Ekspertiz TL</th>'
        +'<th style="padding:7px 10px;text-align:right">Fark</th>'
        +'<th style="padding:7px 10px;text-align:center">Ip. Tarihi</th>'
        +'<th style="padding:7px 10px;text-align:center">DASK</th>'
        +'<th style="padding:7px 10px;text-align:center;width:80px">Islem</th>'
        +'</tr></thead><tbody>';
      bankaIpt.forEach(function(ip,i){
        var fk=((parseFloat(ip.exp_deger)||0)-(parseFloat(ip.tutar)||0));
        var fkR=fk>=0?'var(--g)':'var(--r)';
        var fkTxt=(fk>=0?'+':'')+ftl(Math.abs(fk));
        var rowBg=i%2?'background:rgba(255,255,255,.015)':'';
        html+=''
          +'<tr style="'+rowBg+'">'
          ...
          +'<td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:var(--am)">'+ftl(ip.tutar)+'</td>'
          +'<td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:var(--acc)">'+ftl(ip.exp_deger)+'</td>'
          ...
          +'<button onclick="bkIpotekDuzenle(''+bkId+'',''+ip.id+'')" style="background:var(--s3);color:var(--t2);border:1px solid var(--b2);padding:3px 8px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">Duzenle</button>'
          +'<button onclick="bkIpotekCikar(''+bkId+'',''+ip.id+'')" style="background:rgba(239,68,68,.1);color:var(--r);border:1px solid rgba(239,68,68,.3);padding:3px 8px;border-radius:4px;cursor:pointer;font-size:10px;margin:1px">Cikar</button>'
          +'</td></tr>';
      });

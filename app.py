...
KK_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nakit_akis_kk.json")
KK_DATA = []   # Kredi kartı kartları: [{id, banka, kart_no, limit, kesim_gun, odeme_gun, gecikme_faiz, harcamalar:[...]}]

def load_kk():
    global KK_DATA
    if os.path.exists(KK_FILE):
        try:
            with open(KK_FILE, 'r', encoding='utf-8') as f:
                KK_DATA = json.load(f)
            print(f"  {len(KK_DATA)} kredi karti yuklendi")
        except: KK_DATA = []

def save_kk():
    try:
        with open(KK_FILE, 'w', encoding='utf-8') as f:
            json.dump(KK_DATA, f, ensure_ascii=False, indent=2)
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(KK_DATA, ensure_ascii=False, indent=2)
            # daemon=False: Render deploy sonrası process kapanmadan önce GitHub'a yazılsın
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_kk.json', d,
                f'KK - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=False).start()
    except Exception as e:
        print(f"KK kayit hatasi: {e}")
...
def save_banka_kart():
    try:
        ...
        if GITHUB_TOKEN:
            import threading as _thr
            _data = json.dumps(BANKA_KART_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_banka_kart.json', _data,
                f'Banka karti - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_ipotek():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(IPOTEK_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_ipotek.json', d,
                f'save_ipotek - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nakit_akis_settings.json")

# Startup: dosya dizininin yazılabilir olduğunu kontrol et
try:
    _test_dir = os.path.dirname(os.path.abspath(__file__))
    _test_file = os.path.join(_test_dir, '.write_test')
    with open(_test_file, 'w') as _tf: _tf.write('ok')
    os.remove(_test_file)
    print(f"  Dizin yazilabilir: {_test_dir}")
except Exception as _we:
    print(f"  UYARI: Dizin yazma testi basarisiz: {_we}")
    # /tmp kullan
    USERS_FILE    = "/tmp/nakit_akis_users.json"
    SETTINGS_FILE = "/tmp/nakit_akis_settings.json"
    print(f"  Fallback: USERS_FILE={USERS_FILE}")

# ── STATELESS SESSION (HMAC imzalı token - sunucu yeniden başlasa da geçerli) ──
SESSIONS   = {}
_RESTORING = False  # GitHub restore sırasında sync'i engelle  # Kara liste (çıkış yapılan tokenlar)
SESSION_TTL = 365 * 24 * 3600  # 1 yıl
REVOKED_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nakit_akis_revoked.json")
REVOKED_TOKENS = set()  # Çıkış yapılan tokenlar
_LOGIN_ATTEMPTS = {}   # {ip: [timestamp, ...]} brute-force koruması
_LOGIN_MAX_ATTEMPTS = 10  # Max başarısız deneme
_LOGIN_LOCKOUT_SEC = 300   # 5 dakika kilitleme

def _get_secret():
    # SECRET: önce SESSION_SECRET env var, sonra GITHUB_TOKEN, son çare sabit değer
    secret = os.environ.get('SESSION_SECRET','') or GITHUB_TOKEN or 'ulusal-finans-secret-2024'
    return secret.encode('utf-8')

def load_sessions():
    global REVOKED_TOKENS
    # Kara listeyi lokal'den yükle
    if os.path.exists(REVOKED_FILE):
        try:
            with open(REVOKED_FILE, 'r') as f:
                REVOKED_TOKENS = set(json.load(f))
            print(f"  {len(REVOKED_TOKENS)} iptal edilmiş token yuklendi")
        except: REVOKED_TOKENS = set()
    # GitHub'dan da yükle (deploy sonrası)
    if GITHUB_TOKEN:
        try:
            data = github_dosya_oku('nakit_akis_revoked.json')
            if data and isinstance(data, list):
                REVOKED_TOKENS.update(data)
        except: pass

def save_sessions():
    # Eski compat - artık gerekmiyor
    pass

def _sign_token(payload_str):
    import hmac as _hmac, hashlib as _hs
    sig = _hmac.new(_get_secret(), payload_str.encode(), _hs.sha256).hexdigest()
    return sig

def create_session(username):
    import base64 as _b64
    # Token içine expire ekle - sunucu bağımsız çalışır
    exp = int(time.time()) + SESSION_TTL
    payload = json.dumps({'u': username, 't': int(time.time()), 'exp': exp}, separators=(',',':'))
...
def get_session(token):
    ...
    # Kara listede mi?
    if token in REVOKED_TOKENS: return None
    # Token formatını kontrol et
    try:
        import base64 as _b64, hmac as _hmac, hashlib as _hs
        parts = token.split('.')
        if len(parts) != 2: return None
        payload_b64, sig = parts[0], parts[1]
        # İmzayı doğrula
        expected = _sign_token(payload_b64)
        if not _hmac.compare_digest(expected, sig): return None
        # Payload'ı çöz
        payload = json.loads(_b64.urlsafe_b64decode(payload_b64 + '=='))
        username = payload.get('u','')
        if not username: return None
        # Token içindeki expire kontrolü
        exp = payload.get('exp', 0)
        if exp and time.time() > exp: return None
        return {'username': username, 'expires': exp or (time.time() + SESSION_TTL)}
    except:
        return None
...
def revoke_token(token):
    ...
    try:
        data = list(REVOKED_TOKENS)
        with open(REVOKED_FILE, 'w') as f:
            json.dump(data, f)
        # GitHub'a da kaydet
        if GITHUB_TOKEN:
            import threading as _rt
            _rt.Thread(target=lambda: github_dosya_yukle(
                'nakit_akis_revoked.json',
                json.dumps(data),
                'Logout'
            ), daemon=True).start()
    except: pass
...
def load_users():
    # Sadece diskten oku - GitHub çağrısı YOK (her API isteğinde çağrıldığı için)
...
def save_users(users):
    ...
    try:
        ...
        print(f"  save_users OK: {len(users)} kullanici → {USERS_FILE}")
    except Exception as e:
        print(f"  save_users DISK HATASI: {e}")

    # 2. GitHub'a arka planda yedekle (non-daemon: process kapanmadan önce tamamlanır)
    if GITHUB_TOKEN:
        _snap = dict(users)
        def _gh():
            try:
                icerik = json.dumps(_snap, ensure_ascii=False, indent=2)
                ok = github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_users.json', icerik, 'Kullanici guncelleme')
                print(f"  save_users GitHub: {'OK' if ok else 'BASARISIZ'}")
            except Exception as e:
                print(f"  save_users GitHub HATA: {e}")
        t = threading.Thread(target=_gh, daemon=False)
        t.start()
...
def save_settings(s):
    try:
        ...
        print(f"save_settings disk hatasi: {e}")
    def _github_sync():
        try:
            if GITHUB_TOKEN:
                icerik = json.dumps(s, ensure_ascii=False, indent=2)
                github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_settings.json', icerik, 'Ayarlar guncelleme')
        except Exception as e:
            print(f"save_settings github hatasi: {e}")
    threading.Thread(target=_github_sync, daemon=True).start()



# create_session ve get_session yukarida HMAC tabanli olarak tanimlandi - tekrari kaldirildi
...
def github_api_dosya_yukle_binary(dosya_yolu, icerik_b64, mesaj):
    """Binary dosyayı (base64) GitHub'a yükle"""
    ...
    try:
        ...
        print(f'GitHub binary yukle hatasi: {e}'); return False
...
def github_yedek_al():
    ...
    dosyalar = {
        ...
        f'{BACKUP_PREFIX}nakit_akis_banka_kart.json':  json.dumps(BANKA_KART_DATA, ensure_ascii=False, indent=2),
        f'{BACKUP_PREFIX}nakit_akis_docs_meta.json':   json.dumps(DOCS_META, ensure_ascii=False, indent=2),
        f'{BACKUP_PREFIX}nakit_akis_hatirlatma.json':  json.dumps(HATIRLATMALAR, ensure_ascii=False, indent=2),
        f'{BACKUP_PREFIX}nakit_akis_otel.json':        json.dumps(OTEL_DATA, ensure_ascii=False, indent=2),
        f'{BACKUP_PREFIX}nakit_akis_kk.json':           json.dumps(KK_DATA, ensure_ascii=False, indent=2),
        f'{BACKUP_PREFIX}nakit_akis_taksitli.json':     json.dumps(TAKSITLI_DATA, ensure_ascii=False, indent=2),
    }
...
def banka_kart_kullanim_guncelle():
    """Gider tablosu + KK_DATA'dan banka kartı kullanımlarını hesapla.
    Manuel girilen değerler (manuel_kullanim_*) korunur.
    Kredi Kartı alanı KK_DATA'dan otomatik senkronize edilir.
    """
    try:
        tur_map = {
            'TAKSİTLİ KREDİ': 'taksitli', 'TAKSİTLİ': 'taksitli',
            'ROTATİF': 'rotatif', 'ROTATİF KREDİ': 'rotatif',
            'KMH': 'kmh', 'KMH HESABI': 'kmh',
            'KREDİ KARTI': 'kredi_karti', 'KREDI KARTI': 'kredi_karti',
            'TEMİNAT MEKTUBU': 'teminat', 'TEMINAT MEKTUBU': 'teminat',
            'DİĞER': 'diger', 'DIGER': 'diger',
        }

        # TAKSITLI_DATA'dan banka bazlı taksitli kredi bakiyelerini hesapla
        # {banka_adi_upper: kalan_borclar_toplami}
        taksitli_banka_map = {}
        for tk in TAKSITLI_DATA:
            b = (tk.get('banka') or '').upper().strip()
            if not b: continue
            # Ödenmemiş taksit toplamı = kalan borç
            kalan = sum(
                float(t.get('tutar', 0)) for t in tk.get('taksitler', [])
                if t.get('durum') != 'ÖDENDİ'
            )
            taksitli_banka_map[b] = round(taksitli_banka_map.get(b, 0) + kalan, 2)

        # KK_DATA'dan banka bazlı kredi kartı kullanımını ve limitini önceden hesapla
        # {banka_adi_upper: {kullanim: X, limit: Y}}
        kk_banka_map = {}
        for kk in KK_DATA:
            b = (kk.get('banka') or '').upper().strip()
            if not b: continue
            harcamalar = kk.get('harcamalar', [])
            # Sadece gerçek harcamalar (ödeme kayıtları kullanımı düşürür)
            kullanim = round(sum(
                float(h.get('tutar', 0)) for h in harcamalar
                if not h.get('odendi') and h.get('tur') != 'odeme'
            ), 2)
            limit = float(kk.get('limit', 0) or 0)
            if b not in kk_banka_map:
                kk_banka_map[b] = {'kullanim': 0.0, 'limit': 0.0}
            kk_banka_map[b]['kullanim'] = round(kk_banka_map[b]['kullanim'] + kullanim, 2)
            kk_banka_map[b]['limit']    = round(kk_banka_map[b]['limit'] + limit, 2)

        for kart in BANKA_KART_DATA:
            banka_adi = (kart.get('banka') or '').upper().strip()
            firma = kart.get('firma', '')

            # Gider tablosundan bu banka için kullanımları hesapla
            gider_toplam = {}
            for rec in DATA:
                rec_banka = (rec.get('yer') or rec.get('banka') or '').upper().strip()
                rec_firma = rec.get('firma', '')
                rec_tur = (rec.get('tur') or '').upper().strip()
                durum = (rec.get('durum') or '').upper()
                if rec_banka != banka_adi: continue
                if firma and rec_firma and rec_firma != firma: continue
                if durum in ('ÖDENDİ', 'ODENDI', 'İPTAL', 'IPTAL'): continue
                alan = None
                for k, v in tur_map.items():
                    if k in rec_tur or rec_tur in k:
                        alan = v; break
                if not alan: continue
                tutar = float(rec.get('tutar') or 0)
                gider_toplam[alan] = round(gider_toplam.get(alan, 0) + tutar, 2)

            # Her alan için güncelleme stratejisi:
            for alan in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
                if alan == 'kredi_karti':
                    # KK_DATA'dan otomatik senkronize et
                    kk_info = kk_banka_map.get(banka_adi)
                    if kk_info is not None:
                        kart['kullanim_kredi_karti'] = kk_info['kullanim']
                        # KK limitini de banka kartına yaz (limit boşsa)
                        if float(kart.get('limit_kredi_karti', 0) or 0) == 0 and kk_info['limit'] > 0:
                            kart['limit_kredi_karti'] = kk_info['limit']
                        kart['manuel_kullanim_kredi_karti'] = kk_info['kullanim']
                    elif alan in gider_toplam:
                        kart['kullanim_kredi_karti'] = gider_toplam[alan]
                    else:
                        # Manuel değeri koru
                        manuel = kart.get('manuel_kullanim_kredi_karti')
                        if manuel is not None:
                            kart['kullanim_kredi_karti'] = float(manuel)
                        # Aksi halde mevcut değer kalır
                elif alan == 'taksitli':
                    # TAKSITLI_DATA'dan otomatik senkronize et (ödenmemiş bakiye)
                    tak_kalan = taksitli_banka_map.get(banka_adi)
                    if tak_kalan is not None:
                        kart['kullanim_taksitli'] = tak_kalan
                    elif alan in gider_toplam:
                        kart[f'kullanim_{alan}'] = gider_toplam[alan]
                elif alan in gider_toplam:
                    kart[f'kullanim_{alan}'] = gider_toplam[alan]
                else:
                    # Manuel değeri koru (sıfırlama)
                    manuel = kart.get(f'manuel_kullanim_{alan}')
                    if manuel is not None:
                        kart[f'kullanim_{alan}'] = float(manuel)
                    # Aksi halde mevcut değer olduğu gibi kalır

        save_banka_kart()
    except Exception as e:
        print(f"Banka kart kullanim guncelleme hatasi: {e}")
        import traceback; traceback.print_exc()

def _docs_meta_github_sync():
    """DOCS_META'yı GitHub'a anlık yedekle"""
    if not GITHUB_TOKEN or _RESTORING: return
    import threading as _t
    _dm = json.dumps(DOCS_META, ensure_ascii=False, indent=2)
    _t.Thread(target=lambda d=_dm: github_dosya_yukle(
        f'{BACKUP_PREFIX}nakit_akis_docs_meta.json', d,
        f'Dokuman meta - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
    ), daemon=True).start()

def _dosyaya_kaydet_json(dosya_yolu, veri):
    """Sadece diske yaz, GitHub sync yok (restore sırasında kullanılır)"""
    try:
        os.makedirs(os.path.dirname(dosya_yolu), exist_ok=True)
        with open(dosya_yolu, 'w', encoding='utf-8') as f:
            json.dump(veri, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"Disk yazma hatasi ({dosya_yolu}): {e}")

def github_yedekten_yukle():
    global DATA, GELIR_DATA, BANKA_DATA, KREDI_DATA, CARI_DATA, NOTLAR, IPOTEK_DATA, _RESTORING
    global BANKA_KART_DATA, HATIRLATMALAR, DOCS_META, OTEL_DATA, KK_DATA
    if not GITHUB_TOKEN: return False
    _RESTORING = True
    ...
    dosya_map = {
        ...
        f'{BACKUP_PREFIX}nakit_akis_banka_kart.json': 'banka_kart',
        f'{BACKUP_PREFIX}nakit_akis_docs_meta.json':  'docs_meta',
        f'{BACKUP_PREFIX}nakit_akis_hatirlatma.json': 'hatirlatma',
        f'{BACKUP_PREFIX}nakit_akis_otel.json':       'otel',
        f'{BACKUP_PREFIX}nakit_akis_kk.json':         'kk',
    }
    ...
    for yol, tur in dosya_map.items():
        try:
            ...
            if veri is None:
                print(f"  {tur}: GitHub'da bulunamadi, atlanıyor")
                continue
            # OTEL_DATA dict formatındadır, list kontrolünü atla
            if tur != 'otel':
                if not isinstance(veri, list):
                    print(f"  {tur}: Beklenmeyen format ({type(veri)}), atlanıyor")
                    continue
                if len(veri) == 0:
                    print(f"  {tur}: Bos liste geldi, mevcut veri korunuyor")
                    continue  # BOŞ LİSTE GELİRSE MEVCUT VERİYİ KORU!
            if   tur=='gider':
                DATA[:]=veri; _dosyaya_kaydet_json(SAVE_FILE, DATA)
            elif tur=='gelir':
                GELIR_DATA[:]=veri; _dosyaya_kaydet_json(GELIR_SAVE_FILE, GELIR_DATA)
            elif tur=='banka':
                BANKA_DATA[:]=veri; _dosyaya_kaydet_json(BANKA_SAVE_FILE, BANKA_DATA)
            elif tur=='kredi':
                KREDI_DATA[:]=veri; _dosyaya_kaydet_json(KREDI_SAVE_FILE, KREDI_DATA)
            elif tur=='cari':
                CARI_DATA[:]=veri; _dosyaya_kaydet_json(CARI_SAVE_FILE, CARI_DATA)
            elif tur=='notlar':
                NOTLAR[:]=veri; _dosyaya_kaydet_json(NOTLAR_FILE, NOTLAR)
            elif tur=='ipotek':
                IPOTEK_DATA[:]=veri; _dosyaya_kaydet_json(IPOTEK_FILE, IPOTEK_DATA)
            elif tur=='banka_kart':
                BANKA_KART_DATA[:]=veri; _dosyaya_kaydet_json(BANKA_KART_FILE, BANKA_KART_DATA)
            elif tur=='hatirlatma':
                HATIRLATMALAR[:]=veri; _dosyaya_kaydet_json(HATIRLATMA_FILE, HATIRLATMALAR)
            elif tur=='docs_meta':
                DOCS_META[:]=veri; _dosyaya_kaydet_json(DOCS_META_FILE, DOCS_META)
            elif tur=='otel':
                if isinstance(veri, dict):
                    OTEL_DATA.update(veri)
                    _dosyaya_kaydet_json(OTEL_DATA_FILE, OTEL_DATA)
                else:
                    OTEL_DATA[:]=veri; _dosyaya_kaydet_json(OTEL_DATA_FILE, OTEL_DATA)
            elif tur=='kk':
                KK_DATA[:]=veri; _dosyaya_kaydet_json(KK_FILE, KK_DATA)
            elif dosya_adi=='nakit_akis_taksitli.json':
                TAKSITLI_DATA[:]=veri; _dosyaya_kaydet_json(TAKSITLI_FILE, TAKSITLI_DATA)
            print(f"  {tur}: {len(veri)} kayit yuklendi")
            yuklu += 1
        except Exception as _e:
            print(f"  HATA {tur}: {_e}")
            import traceback; traceback.print_exc()

    # Kullanıcıları GitHub'dan geri yükle (deploy'da silinmesin)
    try:
        users_veri = github_dosya_oku(f'{BACKUP_PREFIX}nakit_akis_users.json')
        if users_veri and isinstance(users_veri, dict) and len(users_veri) > 0:
            # GitHub'daki versiyon daha fazla kullanıcı içeriyorsa onu kullan
            lokal_users = {}
            if os.path.exists(USERS_FILE):
                try:
                    with open(USERS_FILE, 'r', encoding='utf-8') as _uf:
                        lokal_users = json.load(_uf)
                except: pass
            # Merge: GitHub'daki kullanıcıları baz al, lokalde ekstra varsa ekle
            merged = dict(users_veri)
            for k, v in lokal_users.items():
                if k not in merged:
                    merged[k] = v
            _dosyaya_kaydet_json(USERS_FILE, merged)
            print(f"  users: {len(merged)} kullanici yuklendi (GitHub:{len(users_veri)} + lokal:{len(lokal_users)})")
        else:
            print(f"  users: GitHub'da bulunamadi, lokal kullaniliyor")
    except Exception as _ue:
        print(f"  HATA users: {_ue}")
        import traceback as _tb; _tb.print_exc()

    # Ayarları GitHub'dan geri yükle
    try:
        settings_veri = github_dosya_oku(f'{BACKUP_PREFIX}nakit_akis_settings.json')
        if settings_veri and isinstance(settings_veri, dict):
            _dosyaya_kaydet_json(SETTINGS_FILE, settings_veri)
            print(f"  settings: yuklendi")
    except Exception as _se:
        print(f"  HATA settings: {_se}")

    print(f"  GitHub yedeginden {yuklu} dosya yuklendi")
    _RESTORING = False

    # Belge dosyalarını da geri yükle
    try:
        import base64 as _b64
        belgeler_list = github_api('GET', f'/repos/{GITHUB_REPO}/contents/belgeler')
        if isinstance(belgeler_list, list):
            os.makedirs(DOCS_DIR, exist_ok=True)
            restored = 0
            for dosya_info in belgeler_list:
                if dosya_info.get('type') == 'file':
                    dosya_adi = dosya_info.get('name','')
                    local_path = os.path.join(DOCS_DIR, dosya_adi)
                    if not os.path.exists(local_path):
                        try:
                            dosya_detay = github_api('GET', f'/repos/{GITHUB_REPO}/contents/belgeler/{dosya_adi}')
                            if dosya_detay and 'content' in dosya_detay:
                                icerik = _b64.b64decode(dosya_detay['content'])
                                with open(local_path, 'wb') as f:
                                    f.write(icerik)
                                restored += 1
                        except: pass
            if restored: print(f"  {restored} belge dosyasi geri yuklendi")
    except Exception as _be:
        print(f"  Belgeler yuklenemedi: {_be}")

    return yuklu > 0


print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX26")
...
<script>
// Zaten giriş yapılmışsa login sayfasına gelmesin
(async function(){
  try{
...
async function login(){const u=document.getElementById('u').value.trim(),p=document.getElementById('p').value,e=document.getElementById('err');e.style.display='none';if(!u||!p){e.textContent='Kullanıcı adı ve şifre gerekli.';e.style.display='block';return;}const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});const d=await r.json();if(d.ok){if(d.token){try{sessionStorage.setItem('na_token',d.token);}catch(ex){}}window.location.replace('/');}else{e.textContent=d.msg||'Giriş başarısız.';e.style.display='block';}}</script></body></html>'''
...
# ── MULTIPART PARSER (cgi modülü Python 3.13'te kaldırıldı) ──────────────────

def parse_multipart_body(body, content_type):
    """
    cgi.FieldStorage yerine kullanılan saf Python multipart parser.
    Döndürür: {'field_name': value_or_bytes, 'file_field': (filename, bytes), ...}
    """
    import re
    result = {}
    # boundary'yi content-type header'ından çıkar
    m = re.search(r'boundary=([^\s;]+)', content_type)
    if not m:
        return result
    boundary = m.group(1).strip('"').encode()
    delimiter = b'--' + boundary
    # Body'yi parçalara böl
    ...
    for part in parts:
        ...
        # Content-Disposition'dan name ve filename'i al
        ...
        name = name_m.group(1)
        if filename_m:
            result[name] = (filename_m.group(1), content)
        else:
...
TAKSITLI_FILE = os.path.join(APP_DIR, "nakit_akis_taksitli.json")
TAKSITLI_DATA = []        # Taksitli krediler [{id, kredi_no, banka, firma, tur, ana_para, faiz_orani, vade_ay, baslangic, taksitler:[{no,tarih,anapara,faiz,bsmv,tutar,durum}]}]
...
def load_data():
    ...
    # DOCS_META yükle: önce lokal, yoksa GitHub'dan
    if os.path.exists(DOCS_META_FILE):
        try:
            with open(DOCS_META_FILE, 'r', encoding='utf-8') as f:
                DOCS_META = json.load(f)
            print(f"  {len(DOCS_META)} dokuman meta yuklendi")
        except:
            DOCS_META = []
    else:
        # Lokal dosya yok (yeni deploy) - GitHub'dan çek
        if GITHUB_TOKEN:
            try:
                veri = github_dosya_oku(f'{BACKUP_PREFIX}nakit_akis_docs_meta.json')
                if veri and isinstance(veri, list):
                    ...
                    print(f"  {len(DOCS_META)} dokuman meta GitHub'dan yuklendi")
            except Exception as _de:
                print(f"  Dokuman meta GitHub'dan yuklenemedi: {_de}")
                DOCS_META = []
        else:
            DOCS_META = []

    # Belgeler klasörü oluştur ve GitHub'dan geri yükle
    os.makedirs(DOCS_DIR, exist_ok=True)
    if GITHUB_TOKEN and DOCS_META:
        # Lokal'de olmayan dosyaları GitHub'dan indir
        import threading as _bt
        def _belgeler_yukle():
            import base64 as _b64
            try:
                for meta in DOCS_META:
                    dosya_adi = meta.get('dosya_adi','')
                    if not dosya_adi: continue
                    local_path = os.path.join(DOCS_DIR, dosya_adi)
                    if not os.path.exists(local_path):
                        try:
                            detay = github_api('GET', f'/repos/{GITHUB_REPO}/contents/belgeler/{dosya_adi}')
                            if detay and 'content' in detay:
                                icerik = _b64.b64decode(detay['content'])
                                with open(local_path, 'wb') as f:
                                    f.write(icerik)
                                print(f"  Belge geri yuklendi: {dosya_adi}")
                        except Exception as _fe:
                            print(f"  Belge yuklenemedi {dosya_adi}: {_fe}")
            except Exception as _e:
                print(f"  Belgeler geri yukleme hatasi: {_e}")
        _bt.Thread(target=_belgeler_yukle, daemon=True).start()
...
def save_data():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_data.json', d,
                f'save_data - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_gelir():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(GELIR_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_gelir.json', d,
                f'save_gelir - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_banka():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(BANKA_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_banka.json', d,
                f'save_banka - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def _taksitli_odeme_senkronize(kredi_id):
    """Kredi taksitlerini / bakiyesini DATA (ödeme tablosu) ile senkronize eder."""
    kredi = next((k for k in TAKSITLI_DATA if str(k.get('id'))==str(kredi_id)), None)
    if not kredi:
        print(f"  SYNC UYARI: kredi_id={kredi_id} bulunamadi")
        return
    tur = (kredi.get('tur') or '').upper().strip()
    doner_turler = ['ROTATİF','KMH','VİNOV','TEMİNAT MEKTUBU']
    is_doner = kredi.get('is_doner', False) or tur in doner_turler
    taksit_sayisi = len(kredi.get('taksitler', []))
    print(f"  SYNC: kredi={kredi.get('kredi_no')} tur={tur} is_doner={is_doner} taksit_sayisi={taksit_sayisi}")
    # Eski kayıtları temizle
    DATA[:] = [d for d in DATA if d.get('taksitli_id') != str(kredi_id)]

    if is_doner:
        kullanim  = float(kredi.get('kullanim') or 0)
        ana_para  = float(kredi.get('ana_para') or 0)
        faiz_yillik = float(kredi.get('faiz_orani') or 0)
        bsmv_oran   = float(kredi.get('bsmv_oran') or 5)
        vade_str    = kredi.get('vade_bitis','')
        firma       = kredi.get('firma','')
        banka       = kredi.get('banka','')
        kredi_no    = kredi.get('kredi_no','')

        # Vade bitiş tarihini parse et
        vade_dt = None
        try:
            vade_dt = datetime.strptime(vade_str, '%Y-%m-%d') if '-' in vade_str else datetime.strptime(vade_str, '%d.%m.%Y')
        except: pass

        AYLAR_L = ['Ocak','Şubat','Mart','Nisan','Mayıs','Haziran',
                   'Temmuz','Ağustos','Eylül','Ekim','Kasım','Aralık']

        def _tarih_kayit(tarih_str, anapara, faiz, bsmv, tutar, aciklama, taksit_no):
            try:
                dt2 = datetime.strptime(tarih_str, '%d.%m.%Y')
                ay2 = AYLAR_L[dt2.month-1]; yil2 = str(dt2.year)
            except:
                ay2 = ''; yil2 = ''
            DATA.append({
                'firma': firma, 'tur': tur, 'yer': banka, 'kredi_no': kredi_no,
                'tarih': tarih_str, 'ay': ay2, 'yil': yil2, 'durum': 'ÖDENMEDİ',
                'anapara': round(anapara,2), 'faiz': round(faiz,2),
                'bsmv': round(bsmv,2), 'tutar': round(tutar,2),
                'aciklama': aciklama, 'taksitli_id': str(kredi_id), 'taksit_no': taksit_no,
            })

        efektif = kullanim if kullanim > 0 else ana_para
        if tur == 'ROTATİF' and efektif > 0 and faiz_yillik > 0:
            # ROTATİF: Her çeyrek sonunda (Mart/Haziran/Eylül/Aralık) sadece FAİZ
            # Vade sonunda ANA PARA iadesi
            baslangic_str = kredi.get('kullanim_tarihi','') or kredi.get('vade_bitis','')
            try:
                bas_dt = datetime.strptime(baslangic_str, '%Y-%m-%d') if '-' in baslangic_str else datetime.strptime(baslangic_str, '%d.%m.%Y')
            except:
                bas_dt = datetime.now()
            ceyreksonu_aylar = [3, 6, 9, 12]  # Mart, Haziran, Eylül, Aralık
            faiz_gunluk = faiz_yillik / 100 / 365
            # Çeyrek sonu tarihlerini bul (başlangıç → vade bitiş arası)
            taksit_no = 1
            dt_iter = bas_dt
            while dt_iter <= vade_dt:
                if dt_iter.month in ceyreksonu_aylar and dt_iter > bas_dt:
                    # Bu çeyrek sonu için faiz hesapla (yaklaşık 91 gün)
                    gun_sayisi = 91
                    faiz_tut = round(kullanim * faiz_yillik / 100 / 4, 2)
                    bsmv_tut = round(faiz_tut * bsmv_oran / 100, 2)
                    tarih_str2 = dt_iter.strftime(f'%d.%m.%Y')
                    _tarih_kayit(tarih_str2, 0, faiz_tut, bsmv_tut, faiz_tut+bsmv_tut,
                                 f'ROTATİF çeyrek faiz ({dt_iter.strftime("%b %Y")})', taksit_no)
                    taksit_no += 1
                if dt_iter.month == 12: dt_iter = datetime(dt_iter.year+1, 1, 1)
                else: dt_iter = datetime(dt_iter.year, dt_iter.month+1, 1)
            # Vade sonunda ana para
            if vade_dt:
                vade_tarih_str = vade_dt.strftime('%d.%m.%Y')
                _tarih_kayit(vade_tarih_str, efektif, 0, 0, efektif,
                             f'ROTATİF vade sonu — ana para iadesi', taksit_no)

        elif tur == 'KMH' and efektif > 0 and faiz_yillik > 0:
            # KMH: Her ayın sonunda aylık faiz + anapara ödemesi
            # Kullanılan tutarı vade boyunca eşit taksitlerle öde
            try:
                bas_str = kredi.get('kullanim_tarihi','')
                bas_dt = datetime.strptime(bas_str, '%Y-%m-%d') if '-' in bas_str else datetime.strptime(bas_str, '%d.%m.%Y')
            except:
                bas_dt = datetime.now()
            vade_ay = int(kredi.get('vade_ay') or 12)
            if not vade_ay: vade_ay = 12
            faiz_aylik = faiz_yillik / 100 / 12
            # Eşit taksit (anüite) hesabı
            if faiz_aylik > 0:
                taksit_tut = efektif * (faiz_aylik*(1+faiz_aylik)**vade_ay) / ((1+faiz_aylik)**vade_ay-1)
            else:
                taksit_tut = efektif / vade_ay
            kalan_ana = efektif
            dt_iter = datetime(bas_dt.year, bas_dt.month, 1)
            for i in range(1, vade_ay+1):
                # Ayın son günü
                if dt_iter.month == 12:
                    son_gun = datetime(dt_iter.year+1, 1, 1) - timedelta(days=1)
                else:
                    son_gun = datetime(dt_iter.year, dt_iter.month+1, 1) - timedelta(days=1)
                faiz_tut  = round(kalan_ana * faiz_aylik, 2)
                bsmv_tut  = round(faiz_tut * bsmv_oran / 100, 2)
                ana_tut   = round(taksit_tut - faiz_tut - bsmv_tut, 2) if i < vade_ay else round(kalan_ana, 2)
                top_tut   = round(ana_tut + faiz_tut + bsmv_tut, 2)
                kalan_ana -= ana_tut
                _tarih_kayit(son_gun.strftime('%d.%m.%Y'), ana_tut, faiz_tut, bsmv_tut, top_tut,
                             f'KMH taksit {i}/{vade_ay}', i)
                if dt_iter.month == 12: dt_iter = datetime(dt_iter.year+1, 1, 1)
                else: dt_iter = datetime(dt_iter.year, dt_iter.month+1, 1)

        else:
            # Diğer döner krediler (VİNOV, TEMİNAT MEKTUBU vb.) — tek satır
            if efektif <= 0: return
            tarih_str2 = vade_dt.strftime('%d.%m.%Y') if vade_dt else ''
            faiz_tek = round(efektif * faiz_yillik / 100, 2) if faiz_yillik > 0 else 0
            bsmv_tek = round(faiz_tek * bsmv_oran / 100, 2)
            _tarih_kayit(tarih_str2, efektif, faiz_tek, bsmv_tek, efektif+faiz_tek+bsmv_tek,
                         tur + ' kullanim', 0)
    else:
        # Taksitli krediler: her taksit ayrı satır
        mevcut = {d.get('taksit_no'): d.get('durum','ÖDENMEDİ')
                  for d in DATA if d.get('taksitli_id')==str(kredi_id)}
        for t in kredi.get('taksitler', []):
            tarih_str = t.get('tarih','')
            try:
                dt = datetime.strptime(tarih_str, '%d.%m.%Y')
                ay = ['Ocak','Şubat','Mart','Nisan','Mayıs','Haziran',
                      'Temmuz','Ağustos','Eylül','Ekim','Kasım','Aralık'][dt.month-1]
                yil = str(dt.year)
            except:
                ay = ''; yil = ''
            durum = mevcut.get(t.get('no'), t.get('durum','ÖDENMEDİ'))
            DATA.append({
                'firma':           kredi.get('firma',''),
                'tur':             tur if tur else 'TAKSİTLİ KREDİ',
                'yer':             kredi.get('banka',''),
                'kredi_no':        kredi.get('kredi_no',''),
                'tarih':           tarih_str,
                'ay':              ay,
                'yil':             yil,
                'durum':           durum,
                'anapara':         round(float(t.get('anapara',0)),2),
                'faiz':            round(float(t.get('faiz',0)),2),
                'bsmv':            round(float(t.get('bsmv',0)),2),
                'tutar':           round(float(t.get('tutar',0)),2),
                'aciklama':        f"Taksit {t.get('no','')} / {len(kredi.get('taksitler',[]))}",
                'taksitli_id':     str(kredi_id),
                'taksit_no':       t.get('no',0),
                'kullanim_tarihi': kredi.get('kullanim_tarihi',''),
                'odeme_tarihi':    t.get('odeme_tarihi',''),
            })

def load_taksitli():
    global TAKSITLI_DATA
    if os.path.exists(TAKSITLI_FILE):
        try:
            with open(TAKSITLI_FILE, 'r', encoding='utf-8') as f:
                TAKSITLI_DATA = json.load(f)
            print(f"  {len(TAKSITLI_DATA)} taksitli kredi yuklendi")
        except: TAKSITLI_DATA = []

def save_taksitli():
    try:
        with open(TAKSITLI_FILE, 'w', encoding='utf-8') as f:
            json.dump(TAKSITLI_DATA, f, ensure_ascii=False, indent=2)
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(TAKSITLI_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_taksitli.json', d,
                f'Taksitli kredi - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
        print(f"Taksitli kredi kayit hatasi: {e}")
...
def save_kredi():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(KREDI_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_kredi.json', d,
                f'save_kredi - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_cari():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(CARI_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_cari.json', d,
                f'save_cari - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_notlar():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(NOTLAR, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_notlar.json', d,
                f'save_notlar - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_otel_data():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(OTEL_DATA, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_otel.json', d,
                f'save_otel_data - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
def save_hatirlatmalar():
    ...
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            import threading as _thr
            _d = json.dumps(HATIRLATMALAR, ensure_ascii=False, indent=2)
            _thr.Thread(target=lambda d=_d: github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_hatirlatma.json', d,
                f'save_hatirlatmalar - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
            ), daemon=True).start()
    except Exception as e:
...
# Duplike save_banka / save_kredi kaldirildi — GitHub backup iceren versiyonlar yukarida tanimlandi
...
def import_excel(filepath, sheet_hint=None):
    ...
    try:
        ...
        col_map = {
            ...
            'kredi_no':  ['KREDİ/ÇEK NO','KREDI/CEK NO','KREDİ NO','KREDI NO','ÇEK NO','CEK NO','KREDI','LOAN NO','LOAN NUMBER'],
        ...
        for _, row in df.iterrows():
            ...
            records.append({
                ...
                'kredi_no': str(g('kredi_no')).strip() if g('kredi_no') not in ['','nan','None',None] else '',
...
def apply_filter(params):
    ...
    if params.get('yer'):      result = [d for d in result if d.get('yer','').strip().upper() == params['yer'].strip().upper()]

    if params.get('kredi_no'): result = [d for d in result if str(d.get('kredi_no','')).strip() == str(params['kredi_no']).strip()]
    ...
    if params.get('q'):
        ...
        result = [d for d in result if q in
                  ...
                  (d.get('yer','') + d.get('firma','') + d.get('tur','') + d.get('kredi_no','') + d.get('aciklama','')).lower()]
...
def tcmb_kur_cek(tarih_str=None):
    ...
    try:
        ...
        # Alternatif API dene
        try:
            return tcmb_kur_alternatif(tarih_str)
        except:
            pass
...
def tcmb_kur_alternatif(tarih_str=None):
    """Çoklu kaynak: Frankfurter → ExchangeRate-API → open.er-api"""
    import urllib.request as _ur, json as _json
    
    isim_map = {
        'USD':'ABD Doları','EUR':'Euro','GBP':'İngiliz Sterlini',
        'CHF':'İsviçre Frangı','JPY':'Japon Yeni','SAR':'Suudi Arabistan Riyali',
        'AED':'BAE Dirhemi','DKK':'Danimarka Kronu','SEK':'İsveç Kronu',
        'NOK':'Norveç Kronu','CAD':'Kanada Doları','AUD':'Avustralya Doları',
        'CNY':'Çin Yuanı','RUB':'Rus Rublesi','KWD':'Kuveyt Dinarı',
    }
    ...
    tarih_iso = _parse_tarih(tarih_str)

    # KAYNAK 1: Frankfurter.app (ECB)
    try:
        url = f"https://api.frankfurter.app/{tarih_iso or 'latest'}?from=TRY"
        data = _fetch(url)
        kurlar = {}
        for kod, oran in data.get('rates', {}).items():
            if oran and oran > 0:
                v = round(1.0/oran, 4)
                kurlar[kod] = {'kod':kod,'isim':isim_map.get(kod,kod),'isim_en':kod,
                               'alis':v,'satis':v,'efektif_alis':v,'efektif_satis':v,'birim':1}
        if kurlar:
            return {'ok':True,'tarih':data.get('date',tarih_str or ''),'kurlar':kurlar,
                    'url':url,'uyari':'TCMB erişilemedi — Frankfurter.app (ECB) verileri'}
    except Exception as e1:
        print(f"Frankfurter hata: {e1}")

    # KAYNAK 2: open.er-api.com
    try:
        url2 = "https://open.er-api.com/v6/latest/TRY"
        data2 = _fetch(url2)
        kurlar = {}
        for kod, oran in data2.get('rates', {}).items():
            if oran and oran > 0 and kod != 'TRY':
                v = round(1.0/oran, 4)
                kurlar[kod] = {'kod':kod,'isim':isim_map.get(kod,kod),'isim_en':kod,
                               'alis':v,'satis':v,'efektif_alis':v,'efektif_satis':v,'birim':1}
        if kurlar:
            tarih_x = data2.get('time_last_update_utc','')[:10] or (tarih_str or '')
            return {'ok':True,'tarih':tarih_x,'kurlar':kurlar,
                    'url':url2,'uyari':'TCMB erişilemedi — open.er-api.com verileri'}
    except Exception as e2:
        print(f"open.er-api hata: {e2}")

    # KAYNAK 3: exchangerate-api.com (ücretsiz)
    try:
        ...
        kurlar = {}
        if try_rate:
            for kod, usd_rate in rates.items():
                if usd_rate and kod != 'TRY':
                    v = round(try_rate/usd_rate, 4)
                    kurlar[kod] = {'kod':kod,'isim':isim_map.get(kod,kod),'isim_en':kod,
                                   'alis':v,'satis':v,'efektif_alis':v,'efektif_satis':v,'birim':1}
        if kurlar:
            return {'ok':True,'tarih':tarih_str or datetime.now().strftime('%d.%m.%Y'),
                    'kurlar':kurlar,'url':url3,'uyari':'TCMB erişilemedi — exchangerate-api.com verileri'}
    except Exception as e3:
        print(f"exchangerate-api hata: {e3}")

    return {'ok':False,'hata':'Tüm kur kaynakları başarısız oldu. Lütfen daha sonra tekrar deneyin.','tarih':tarih_str or ''}

# ── LOGO TIGER SQL BAĞLANTISI ────────────────────────────────────────────────

def logo_connect():

    """Logo Tiger SQL Server bağlantısı. pymssql (önce) veya pyodbc kullanır."""
    ...
    # pymssql ile bağlan (Render.com dahil tüm ortamlarda çalışır)
    ...
    try:
        ...
        pass  # pymssql yoksa pyodbc dene

    except Exception as e:

        raise Exception(f"Logo bağlantı hatası (pymssql): {e}")



    # pyodbc fallback (lokal Windows ortamı için)
    ...
    try:
        ...
        raise Exception(
            ...
            "SQL Server bağlantısı için pymssql kurulu değil. "

            "Render.com: requirements.txt dosyasında 'pymssql' olduğundan emin olun ve yeniden deploy edin."
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args): pass  # HTTP access log sustur
    def log_error(self, format, *args):
        print(f"[HTTP ERROR] {format % args}")  # Hataları göster
        ...
        try:
            ...
            if token and get_session(token):
                self.send_header('Set-Cookie',
                    f'na_token={token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000; Secure')
        except: pass
        self.end_headers()
    ...
    def do_GET(self):
        ...
        if path == '/login':
            # Zaten oturumu varsa ana sayfaya yönlendir - geri tuşu sorununu çözer
            if check_auth(self):
        ...
        if path == '/api/logout':
            ...
            if not token: token = self.headers.get('X-Token','')
        ...
        if path == '/api/me':
            ...
            self.send_json({'ok':True,'username':sess['username'],'ad':u.get('ad',sess['username']),'rol':u.get('rol','user'),'izinler':u.get('izinler',None)}); return

        if path == '/api/token':
            # sessionStorage'a token yüklemek için — mevcut cookie token'ını döndür
            token = get_token_from_request(self)
            if not token: token = self.headers.get('X-Token','')
            if token and get_session(token):
                self.send_json({'ok':True,'token':token}); return
            self.send_json({'ok':False},401); return

        if path == '/api/settings/get':
            self.send_json({'ok': True, 'settings': load_settings()}); return
        ...
        if path == '/api/taksitli/sablon':
            # GET olarak da çalışsın (indirme linki için)
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                import openpyxl, io
                from openpyxl.styles import Font, PatternFill, Alignment
                wb = openpyxl.Workbook()
                ws1 = wb.active; ws1.title = 'Taksitli Krediler'
                hdrs1 = ['Firma','Banka','Kredi No','Kredi Türü','Ana Para (₺)',
                         'Faiz % (Yıllık)','Vade (Ay)','İlk Taksit Tarihi','BSMV %','Açıklama']
                ...
                for c,h in enumerate(hdrs1,1):
                    ...
                    cell.alignment=Alignment(horizontal='center')
                ornekler1=[
                    ['ULUSAL','YAPIKREDİ','246324480007','TAKSİTLİ KREDİ',5000000,45.5,36,'10.01.2024',5,''],
                    ['ABC','FİNANSBANK','16497','KONUT KREDİSİ',2000000,38.0,120,'15.03.2023',5,''],
                    ['ULUSAL','TEB','130244124','TAKSİTLİ KREDİ',3000000,42.0,48,'01.06.2023',5,''],
                ]
                for r,row in enumerate(ornekler1,2):
                    for c,v in enumerate(row,1): ws1.cell(row=r,column=c,value=v)
                for col,w in zip(['A','B','C','D','E','F','G','H','I','J'],[10,16,18,18,16,14,10,18,8,25]):
                    ws1.column_dimensions[col].width=w
                ws1.auto_filter.ref=ws1.dimensions; ws1.freeze_panes='A2'
                ws2 = wb.create_sheet('Döner Krediler')
                hdrs2=['Firma','Banka','Kredi No','Kredi Türü','Limit (₺)',
                       'Kullanılan (₺)','Faiz % (Yıllık)','Komisyon %','Vade Bitiş','Açıklama']
                for c,h in enumerate(hdrs2,1):
                    cell=ws2.cell(row=1,column=c,value=h)
                    cell.fill=hfill; cell.font=hfont
                    cell.alignment=Alignment(horizontal='center')
                ornekler2=[
                    ['ULUSAL','AKBANK','ROT-001','ROTATİF',10000000,3500000,45.0,0.5,'31.12.2026',''],
                    ['ABC','GARANTİ','KMH-002','KMH',5000000,0,42.0,0,'30.06.2026',''],
                    ['ULUSAL','YAPIKREDİ','VNV-003','VİNOV',2000000,2000000,44.0,0,'15.04.2026',''],
                ]
                for r,row in enumerate(ornekler2,2):
                    for c,v in enumerate(row,1): ws2.cell(row=r,column=c,value=v)
                for col,w in zip(['A','B','C','D','E','F','G','H','I','J'],[10,16,14,12,16,16,14,10,14,25]):
                    ws2.column_dimensions[col].width=w
                ws2.auto_filter.ref=ws2.dimensions; ws2.freeze_panes='A2'
                buf=io.BytesIO(); wb.save(buf); buf.seek(0); data=buf.getvalue()
                self.send_response(200)
                self.send_header('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
                self.send_header('Content-Disposition','attachment; filename="kredi_yonetimi_sablon.xlsx"')
        ...
        if path == '/api/gider/excel_sablon':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                import openpyxl
                from openpyxl.styles import Font, PatternFill, Alignment
                wb = openpyxl.Workbook()
                ws = wb.active; ws.title = 'Gider Kayıtları'
                hdrs = ['GRUP FİRMASI','ÖDEME TÜRÜ','ÖDEME YAPILACAK YER','KREDİ/ÇEK NO',
                        'VADE - ÖDEME TARİHİ','ÖDEME AYI','DURUMU','ÖDEME ANAPARA',
                        'ÖDEME FAİZ','ÖDEME BSMV','ÖDEME TUTARI','AÇIKLAMA']
                hfill = PatternFill("solid", fgColor="1E3A5F")
                hfont = Font(bold=True, color="FFFFFF", name="Calibri", size=11)
                ...
                ornekler = [
                    ['ULUSAL','TAKSİTLİ KREDİ','ZİRAAT BANKASI','KRD-001','15.04.2026','Nisan','ÖDENMEDİ',100000,3500,525,104025,''],
                    ['ABC','ROTATİF','GARANTİ BBVA','ROT-001','20.04.2026','Nisan','ÖDENMEDİ',0,5000,750,5750,''],
                    ['ULUSAL','KMH','HALKBANK','KMH-001','10.04.2026','Nisan','ÖDENMEDİ',50000,2000,300,52300,''],
                    ['ULUSAL','KREDİ KARTI','GARANTİ BBVA','KK-2220','05.04.2026','Nisan','ÖDENMEDİ',0,0,0,15000,'Aylık harcama'],
                ]
                for r, row in enumerate(ornekler, 2):
                    for col, val in enumerate(row, 1):
                        ws.cell(row=r, column=col, value=val)
                for col, w in zip(['A','B','C','D','E','F','G','H','I','J','K','L'],[12,20,25,14,16,10,12,14,12,12,14,30]):
                    ws.column_dimensions[col].width = w
                ws.auto_filter.ref = ws.dimensions
                ws.freeze_panes = 'A2'
                ws2 = wb.create_sheet('Alan Açıklamaları')
                notlar = [
                    ('GRUP FİRMASI', 'ULUSAL, ABC, BRG, BRK veya firma adı'),
                    ('ÖDEME TÜRÜ', 'TAKSİTLİ KREDİ, ROTATİF, KMH, KREDİ KARTI, TEMİNAT MEKTUBU, DİĞER'),
                    ('ÖDEME YAPILACAK YER', 'Banka adı (ZİRAAT BANKASI, GARANTİ BBVA vb.)'),
                    ('KREDİ/ÇEK NO', 'Kredi referans no, çek no, kart no (isteğe bağlı)'),
                    ('VADE - ÖDEME TARİHİ', 'GG.AA.YYYY formatı (örn: 15.04.2026)'),
                    ('ÖDEME AYI', 'Türkçe ay adı: Ocak, Şubat ... Aralık'),
                    ('DURUMU', 'ÖDENDİ veya ÖDENMEDİ'),
                    ('ÖDEME ANAPARA', 'Sayısal tutar (TL) — yoksa 0'),
                    ('ÖDEME FAİZ', 'Sayısal tutar (TL) — yoksa 0'),
                    ('ÖDEME BSMV', 'Sayısal tutar (TL) — yoksa 0'),
                    ('ÖDEME TUTARI', 'Toplam tutar (Anapara+Faiz+BSMV) — zorunlu'),
                    ('AÇIKLAMA', 'İsteğe bağlı not'),
                ]
                ws2.cell(row=1, column=1, value='Sütun').font = Font(bold=True)
                ws2.cell(row=1, column=2, value='Açıklama').font = Font(bold=True)
                for i, (k, v) in enumerate(notlar, 2):
                ...
                self.send_file(out.getvalue(), 'gider_sablon.xlsx',
        ...
        if path == '/api/kk/liste':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            result = []
            for kk in KK_DATA:
                harcamalar = kk.get('harcamalar', [])
                # Sadece gerçek harcamalar (ödeme kayıtları hariç)
                kullanilan = round(sum(
                    float(h.get('tutar', 0)) for h in harcamalar
                    if not h.get('odendi') and h.get('tur') != 'odeme'
                ), 2)
                limit = float(kk.get('limit', 0) or 0)
                kalan = round(limit - kullanilan, 2)
                r = dict(kk)
                r['kullanilan'] = kullanilan
                r['kalan'] = kalan
                result.append(r)
            self.send_json({'ok':True, 'records': result}); return
        ...
        if path == '/':
            ...
            # Kredi özeti
            tot_limit = sum(float(k.get(f'limit_{a}',0) or 0) for k in BANKA_KART_DATA for a in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger'])
            tot_kull  = sum(float(k.get(f'kullanim_{a}',0) or 0) for k in BANKA_KART_DATA for a in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger'])
            self.send_json({
                ...
                'kredi_toplam_limit': round(tot_limit,2),
                'kredi_toplam_kull': round(tot_kull,2),
                'kredi_kalan': round(tot_limit-tot_kull,2),
            ...
            if cache_key in KUR_CACHE:
                cached = dict(KUR_CACHE[cache_key])
            ...
            # Tatil/hafta sonu: TCMB boş dönerse önceki iş günlerini dene (en fazla 5 geri)
            if not result.get('ok') or not result.get('kurlar'):
                from datetime import datetime as _dt, timedelta as _td
                try:
                    baz = _dt.strptime(tarih, '%d.%m.%Y') if tarih else _dt.now()
                except: baz = _dt.now()
                for geri in range(1, 6):
                    onceki = (baz - _td(days=geri)).strftime('%d.%m.%Y')
                    ck2 = onceki
                    if ck2 in KUR_CACHE:
                        result = dict(KUR_CACHE[ck2])
                        result['from_cache'] = True
                        result['uyari'] = result.get('uyari','') + f' | {onceki} tarihli kur (TCMB bugün kapalı)'
                        break
                    r2 = tcmb_kur_cek(onceki)
                    if r2.get('ok') and r2.get('kurlar'):
                        r2['uyari'] = r2.get('uyari','') + f' | {onceki} tarihli kur (TCMB bugün kapalı)'
                        result = r2
                        KUR_CACHE[ck2] = r2
                        break

            if result.get('ok') and result.get('kurlar'):
                KUR_CACHE[cache_key] = result
                if len(KUR_CACHE) > 30:
            ...
            # Banka kartlarından canlı limitleri hesapla
            tur_adlar = {
            ...
            canli = []
            for kart in BANKA_KART_DATA:
                for alan, tur_adi in tur_adlar.items():
                    limit = float(kart.get(f'limit_{alan}') or 0)
                    if limit <= 0: continue
                    kull = float(kart.get(f'kullanim_{alan}') or 0)
                    faiz = float(kart.get(f'faiz_{alan}') or 0)
                    canli.append({
                        'id': f'bk_{kart["id"]}_{alan}',
                        'firma': kart.get('firma',''),
                        'banka': kart.get('banka',''),
                        'tur': tur_adi,
                        'limit': limit,
                        'kullanilan': kull,
                        'kalan_limit': round(limit - kull, 2),
                        'faiz_yillik': faiz,
                        'faiz_aylik': round(faiz/12, 4),
                        'aktif': True,
                        'kaynak': 'banka_kart',
                        'banka_kart_id': kart['id'],
                    })
            manuel = [k for k in KREDI_DATA if k.get('kaynak') != 'banka_kart']
            self.send_json({'records': canli + manuel})
            ...
            # Kullanılabilir kredi limitleri — önce banka kartlarından, sonra manuel KREDI_DATA
            tur_adlar_lk = {
                'taksitli':'TAKSİTLİ KREDİ','rotatif':'ROTATİF',
                'kmh':'KMH','kredi_karti':'KREDİ KARTI',
                'teminat':'TEMİNAT MEKTUBU','diger':'DİĞER'
            ...
            # 1. Banka kartı limitlerinden
            for kart in BANKA_KART_DATA:
                for alan, tur_adi in tur_adlar_lk.items():
                    limit = float(kart.get(f'limit_{alan}') or 0)
                    if limit <= 0: continue
                    kull  = float(kart.get(f'kullanim_{alan}') or 0)
                    kalan = round(limit - kull, 2)
                    if kalan <= 0: continue
                    faiz  = float(kart.get(f'faiz_{alan}') or 0)
                    kredi_kullanilabilir.append({
                        ...
                        'faiz_yillik': faiz,
                        'faiz_aylik':  round(faiz/12, 4),
                        'bitis':       kart.get('bitis_tarihi',''),
                        'ad':          f"{kart.get('banka','')} - {tur_adi}",
                        'kaynak':      'banka_kart',
                    })
            # 2. Manuel KREDI_DATA (banka_kart kaynağı olmayan)
            for kr in KREDI_DATA:
                if kr.get('kaynak') == 'banka_kart': continue
            ...
            # Belirtilen ay/yıl(lar) veya tarih aralığı için finansman ihtiyacını hesapla
            ay_raw   = qs.get('ay',  [''])[0]   # eski tekli (geriye uyumluluk)
            yil_raw  = qs.get('yil', [''])[0]   # eski tekli
            aylar_raw  = qs.get('aylar',  [''])[0]   # çoklu: "Ocak,Şubat"
            yillar_raw = qs.get('yillar', [''])[0]   # çoklu: "2025,2026"
            tarih_bas  = qs.get('tarih_bas', [''])[0]  # YYYY-MM-DD
            tarih_bit  = qs.get('tarih_bit', [''])[0]

            # Normalize: çoklu varsa onu kullan, yoksa tekliyi
            aylar  = [a.strip() for a in aylar_raw.split(',')  if a.strip()] if aylar_raw  else ([ay_raw]  if ay_raw  else [])
            yillar = [y.strip() for y in yillar_raw.split(',') if y.strip()] if yillar_raw else ([yil_raw] if yil_raw else [])
            ...
            def rec_matches(rec):
                t = rec.get('tarih', '')
                t_iso = tarih_to_iso(t)
                rec_yil = t[6:10] if len(t) >= 10 else ''
                rec_ay  = rec.get('ay', '')
                # Tarih aralığı filtresi
                if tarih_bas and t_iso and t_iso < tarih_bas: return False
                if tarih_bit and t_iso and t_iso > tarih_bit: return False
                # Yıl filtresi
                if yillar and rec_yil not in yillar: return False
                # Ay filtresi
                if aylar  and rec_ay  not in aylar:  return False
                return True

            gider_bekl = sum(
                r.get('tutar', 0) for r in DATA
                if r.get('durum') != 'ÖDENDİ' and rec_matches(r)
            )
            gelir_bekl = sum(
                r.get('tutar', 0) for r in GELIR_DATA
                if r.get('durum') != 'TAHSİL EDİLDİ' and rec_matches(r)
            )
            ...
            # Kaynak 1: KREDI_DATA — sadece banka kartından gelmeyen manuel kayıtlar
            kullanilabilir = []
            # Banka kartındaki banka adlarını topla — bunlar Kaynak 2'den gelecek, tekrar ekleme
            bk_bankalar = set()
            for kart in BANKA_KART_DATA:
                b = (kart.get('banka') or '').upper().strip()
                if b: bk_bankalar.add(b)

            for k in KREDI_DATA:
                # banka_kart kaynağından gelen ya da banka kartında zaten olan kaydı atlat
                if k.get('kaynak') == 'banka_kart': continue
                banka_ust = (k.get('banka') or '').upper().strip()
                if banka_ust in bk_bankalar: continue  # Banka kartında zaten var
                bos = float(k.get('limit',0) or 0) - float(k.get('kullanilan',0) or 0)
                if bos <= 0: continue
                faiz_yillik = float(k.get('faiz_yillik', k.get('faiz', 0)) or 0)
                kullanilabilir.append({
                    'id':         k.get('id',''),
                    'banka':      k.get('banka',''),
                    'tur':        k.get('tur',''),
                    'firma':      k.get('firma',''),
                    'limit':      float(k.get('limit',0) or 0),
                    'kullanilan': float(k.get('kullanilan',0) or 0),
                    'bos':        round(bos,2),
                    'faiz':       round(float(k.get('faiz',0) or 0)*100, 4) if float(k.get('faiz',0) or 0) < 1 else float(k.get('faiz',0) or 0),
                    'faiz_yillik': faiz_yillik,
                    'aciklama':   k.get('aciklama',''),
                    'kaynak':     'kredi',
                })

            # Kaynak 2: BANKA_KART_DATA — tüm limit türleri (kredi_karti dahil KK ile senkron)
            TUR_ADI = {
                'taksitli':'Taksitli Kredi','rotatif':'Rotatif Kredi',
                'kmh':'KMH','kredi_karti':'Kredi Kartı',
                'teminat':'Teminat Mektubu','diger':'Diğer'
            }
            for kart in BANKA_KART_DATA:
                for tur in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
                    limit = float(kart.get(f'limit_{tur}', 0) or 0)
                    kull  = float(kart.get(f'kullanim_{tur}', 0) or 0)
                    bos   = limit - kull
                    if bos <= 0: continue
                    faiz_raw    = float(kart.get(f'faiz_{tur}', 0) or 0)
                    faiz_yillik = faiz_raw * 100 if faiz_raw < 1 else faiz_raw
                    faiz_aylik  = faiz_yillik / 12
                    kullanilabilir.append({
                        'id':         f"{kart.get('id','')}_{tur}",
                        'banka':      kart.get('banka',''),
                        'tur':        TUR_ADI.get(tur, tur),
                        'firma':      kart.get('firma',''),
                        'limit':      limit,
                        'kullanilan': kull,
                        'bos':        round(bos, 2),
                        'faiz':       round(faiz_aylik, 4),
                        'faiz_yillik': round(faiz_yillik, 4),
                        'aciklama':   kart.get('notlar',''),
                        'kaynak':     'banka_kart',
                    })

            # NOT: KK_DATA ayrı eklenmez — kredi_karti alanı banka_kart_kullanim_guncelle()
            # tarafından KK_DATA'dan banka kartına zaten senkronize edilmektedir.

            # Faiz oranına göre sırala (en ucuz önce)
            kullanilabilir.sort(key=lambda x: x.get('faiz', 0))
            ...
            for k in kullanilabilir:
                ...
                faiz_aylik_oran = float(k.get('faiz', 0) or 0)
                # faiz değeri: eğer >1 ise yüzde olarak gelmiş (örn 3.5), /100 yapmak lazım
                # eğer <1 ise zaten oran olarak gelmiş (örn 0.035)
                if faiz_aylik_oran > 1:
                    faiz_aylik_oran = faiz_aylik_oran / 100
                aylik_faiz = round(kullan * faiz_aylik_oran, 2)
                faiz_yillik = float(k.get('faiz_yillik', k.get('faiz', 0)) or 0)
                if faiz_yillik < 1:
                    faiz_yillik = round(faiz_yillik * 100, 4)
                oneriler.append({
                    'banka':        k['banka'],
                    'tur':          k['tur'],
                    'firma':        k['firma'],
                    ...
                    'faiz':         round(float(k.get('faiz', 0) or 0) if float(k.get('faiz', 0) or 0) < 1
                                    else float(k.get('faiz', 0) or 0) / 100, 4),
                    'faiz_yuzde':   round(faiz_yillik / 12, 4),  # aylık %
                    'faiz_yillik':  round(faiz_yillik, 2),
                    'kullan':       round(kullan, 2),
                    'aylik_faiz':   aylik_faiz,
                    'yillik_faiz_tutar': round(kullan * faiz_yillik / 100, 2),
                    'oncelik':      len(oneriler) + 1,
                    'kaynak':       k.get('kaynak', 'kredi'),
                })
            ...
            self.send_json({
                ...
                'tahmini_yillik':       round(tahmini_maliyet * 12, 2),
    ...
    def do_POST(self):
        try:
            ...
            print(f"[do_POST UNCAUGHT] path={self.path}\n{tb}")
            try:
                self.send_json({'ok':False,'msg':'Sunucu hatasi: '+str(_top_e)}, 500)
            except Exception:
                pass

    def _do_POST_inner(self):

        global DATA,GELIR_DATA,BANKA_DATA,KREDI_DATA,CARI_DATA,SOURCE_FILE,SOURCE_SHEET,AUTO_RELOAD,NOTLAR,API_CONFIG,LOGO_CONFIG,AI_CONFIG,OTEL_DATA,OTEL_CONFIG,HATIRLATMALAR,GMAIL_CONFIG,WHAPI_CONFIG,WHAPI_MSGS,DOCS_META,BANKA_KART_DATA,IPOTEK_DATA
        ...
        if self.path == '/api/login':
            ...
            try:
                ...
                now_ts = time.time()
                _LOGIN_ATTEMPTS.setdefault(client_ip, [])
                # Eski denemeleri temizle
                _LOGIN_ATTEMPTS[client_ip] = [t for t in _LOGIN_ATTEMPTS[client_ip] if now_ts - t < _LOGIN_LOCKOUT_SEC]
                if len(_LOGIN_ATTEMPTS[client_ip]) >= _LOGIN_MAX_ATTEMPTS:
                    kalan = int(_LOGIN_LOCKOUT_SEC - (now_ts - _LOGIN_ATTEMPTS[client_ip][0]))
                    self.send_json({'ok':False,'msg':f'Çok fazla başarısız deneme. {kalan} saniye bekleyin.'},429)
                    return
                ...
                if u and u['password'] == hash_password(password):
                    ...
                    self.send_header('Set-Cookie',f'na_token={token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000; Secure')

                    self.end_headers()

                    self.wfile.write(json.dumps({'ok':True,'username':username,'ad':u.get('ad',''),'token':token},ensure_ascii=False).encode('utf-8'))

                else:

                    # Başarısız denemeyi kaydet
                    _LOGIN_ATTEMPTS.setdefault(client_ip, []).append(now_ts)
                    kalan_hak = _LOGIN_MAX_ATTEMPTS - len(_LOGIN_ATTEMPTS[client_ip])
                    msg = 'Kullanici adi veya sifre hatali.'
                    if kalan_hak <= 3: msg += f' ({kalan_hak} hak kaldı)'
                    self.send_json({'ok':False,'msg':msg},401)
        ...
        if self.path == '/api/users/add':
            try:
                print(f"[users/add] istek alindi, sess={sess}")
                if not sess:
                    self.send_json({'ok':False,'msg':'Oturum bulunamadi. Sayfayi yenileyip tekrar deneyin.'},401); return
                users = load_users()
                caller_rol = users.get(sess['username'],{}).get('rol','')
                print(f"[users/add] caller={sess['username']} rol={caller_rol}")
                if caller_rol != 'admin':
                    self.send_json({'ok':False,'msg':'Sadece admin ekleyebilir. Roliniz: '+caller_rol}); return
                p = json.loads(body)
                uname=p.get('username','').strip(); pw=p.get('password',''); ad=p.get('ad','').strip(); rol=p.get('rol','user')
                print(f"[users/add] eklenecek: uname={repr(uname)} pw_len={len(pw)} rol={rol}")
                if not uname: self.send_json({'ok':False,'msg':'Kullanici adi bos.'}); return
                if not pw: self.send_json({'ok':False,'msg':'Sifre bos.'}); return
                if uname in users: self.send_json({'ok':False,'msg':'Bu kullanici zaten var: '+uname}); return
                users[uname]={'password':hash_password(pw),'ad':ad or uname,'rol':rol,'created_at':datetime.now().strftime('%d.%m.%Y %H:%M')}
                save_users(users)
                print(f"[users/add] basarili: {uname} eklendi")
                self.send_json({'ok':True,'msg':uname+' eklendi'})
            except Exception as _e:
                import traceback; traceback.print_exc()
                self.send_json({'ok':False,'msg':'Sunucu hatasi: '+str(_e)},500)
            return
        ...
        if self.path == '/api/users/delete':
            try:
                if not sess: self.send_json({'ok':False,'msg':'Oturum yok'},401); return
                users = load_users()
        ...
        if self.path == '/api/users/list':
            try:
                ...
                lst=[{'username':k,'ad':v.get('ad',''),'rol':v.get('rol','user'),'created_at':v.get('created_at',''),'izinler':v.get('izinler',None)} for k,v in users.items()]
                self.send_json({'ok':True,'users':lst})
            except Exception as _e:
                self.send_json({'ok':False,'msg':'Sunucu hatasi: '+str(_e)},500)
            return
        ...
        if self.path == '/api/settings/save':
            try:
                ...
                users = load_users()
                if users.get(sess['username'],{}).get('rol') != 'admin':
                    self.send_json({'ok':False,'msg':'Sadece admin ayar degistirebilir.'}); return
                p = json.loads(body)
                s = load_settings()
                s.update(p)
                save_settings(s)
        ...
        if self.path == '/api/users/update':
            try:
                ...
                p = json.loads(body)
                target = p.get('username', sess['username'])
                is_admin = users.get(sess['username'],{}).get('rol') == 'admin'
                if target != sess['username'] and not is_admin:
                    self.send_json({'ok':False,'msg':'Yetersiz yetki.'}); return
                if target not in users:
                    self.send_json({'ok':False,'msg':'Kullanici bulunamadi.'}); return
                if 'ad' in p: users[target]['ad'] = p['ad']
                if 'rol' in p and is_admin and target != 'admin': users[target]['rol'] = p['rol']
                if 'izinler' in p and is_admin: users[target]['izinler'] = p['izinler']
                save_users(users)
                self.send_json({'ok': True})
            except Exception as _e:
                import traceback; traceback.print_exc()
        ...
        if self.path == '/api/users/me_update':
            try:
                if not sess: self.send_json({'ok':False,'msg':'Oturum yok'},401); return
                users = load_users()
                p = json.loads(body)
                uname = sess['username']
                if 'ad' in p: users[uname]['ad'] = p['ad']
                save_users(users)
                self.send_json({'ok': True})
            except Exception as _e:
                self.send_json({'ok':False,'msg':'Sunucu hatasi: '+str(_e)},500)
        ...
        if self.path == '/api/kk/liste':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            # Her karta kullanılan tutarı ve kalan limiti hesapla
            result = []
            for kk in KK_DATA:
                harcamalar = kk.get('harcamalar', [])
                # Sadece gerçek harcamalar (ödeme kayıtları hariç)
        ...
        if self.path == '/api/kk/kaydet':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                kayit = json.loads(body)
                if not kayit.get('id'):
                    kayit['id'] = str(int(datetime.now().timestamp()*1000))
                # harcamalar listesi zaten içinde gelir
                mevcut = next((i for i,r in enumerate(KK_DATA) if str(r.get('id'))==str(kayit.get('id'))), None)
                if mevcut is not None: KK_DATA[mevcut] = kayit
                else: KK_DATA.append(kayit)
                save_kk()
                self.send_json({'ok':True})
            except Exception as e: self.send_json({'ok':False,'msg':str(e)})
            return

        if self.path == '/api/kk/sil':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                p = json.loads(body)
                KK_DATA[:] = [r for r in KK_DATA if str(r.get('id')) != str(p.get('id'))]
                save_kk()
                self.send_json({'ok':True})
            except Exception as e: self.send_json({'ok':False,'msg':str(e)})
            return

        if self.path == '/api/kk/harcama/ekle':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                p = json.loads(body)
                kk_id = str(p.get('kk_id'))
                harcama = p.get('harcama', {})
                if not harcama.get('id'):
                    harcama['id'] = str(int(datetime.now().timestamp()*1000))
                idx = next((i for i,r in enumerate(KK_DATA) if str(r.get('id'))==kk_id), None)
                if idx is None: self.send_json({'ok':False,'msg':'Kart bulunamadı'}); return
                if 'harcamalar' not in KK_DATA[idx]: KK_DATA[idx]['harcamalar'] = []
                mevcut_h = next((j for j,h in enumerate(KK_DATA[idx]['harcamalar']) if str(h.get('id'))==str(harcama.get('id'))), None)
                if mevcut_h is not None: KK_DATA[idx]['harcamalar'][mevcut_h] = harcama
                else: KK_DATA[idx]['harcamalar'].append(harcama)
                save_kk()
                # Banka kartı kredi_karti kullanımını güncelle
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True})
            except Exception as e: self.send_json({'ok':False,'msg':str(e)})
            return

        if self.path == '/api/kk/harcama/sil':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                p = json.loads(body)
                kk_id = str(p.get('kk_id'))
                harcama_id = str(p.get('harcama_id'))
                idx = next((i for i,r in enumerate(KK_DATA) if str(r.get('id'))==kk_id), None)
                if idx is None: self.send_json({'ok':False,'msg':'Kart bulunamadı'}); return
                KK_DATA[idx]['harcamalar'] = [h for h in KK_DATA[idx].get('harcamalar',[]) if str(h.get('id'))!=harcama_id]
                save_kk()
                # Banka kartı kullanımını güncelle
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True})
            except Exception as e: self.send_json({'ok':False,'msg':str(e)})
            return
        ...
        if self.path == '/api/banka_kart/kaydet':
            ...
            try:
                ...
                # Manuel girilen kullanım değerlerini kalıcı olarak sakla
                for alan in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
                    v = kayit.get(f'kullanim_{alan}')
                    if v is not None:
                        kayit[f'manuel_kullanim_{alan}'] = float(v)
                ...
                save_banka_kart()
                # KK senkronizasyonunu tetikle (kredi kartı limiti güncel kalsın)
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True})
        ...
        if self.path == '/api/ipotek/kaydet':
            ...
            try:
                ...
                kayit = json.loads(body)
                ipotek_id = str(kayit.get('id'))
                yeni_banka = (kayit.get('banka') or '').strip()

                # Eski kaydı bul
                eski = next((r for r in IPOTEK_DATA if str(r.get('id')) == ipotek_id), None)
                eski_banka = (eski.get('banka') or '').strip() if eski else ''

                mevcut = next((i for i,r in enumerate(IPOTEK_DATA) if str(r.get('id')) == ipotek_id), None)
                if mevcut is not None: IPOTEK_DATA[mevcut] = kayit
                else: IPOTEK_DATA.append(kayit)

                save_ipotek()

                # Banka kartı ipotek listesini senkronize et
                banka_degisti = (yeni_banka != eski_banka) or (eski is None)
                if banka_degisti:
                    if eski_banka:
                        for kart in BANKA_KART_DATA:
                            if (kart.get('banka') or '').strip() == eski_banka:
                                mevcut_ipt = [str(x) for x in (kart.get('ipotekler') or [])]
                                if ipotek_id in mevcut_ipt:
                                    kart['ipotekler'] = [x for x in mevcut_ipt if x != ipotek_id]
                    if yeni_banka and yeni_banka not in ('DİĞER', ''):
                        for kart in BANKA_KART_DATA:
                            if (kart.get('banka') or '').strip() == yeni_banka:
                                mevcut_ipt = [str(x) for x in (kart.get('ipotekler') or [])]
                                if ipotek_id not in mevcut_ipt:
                                    kart['ipotekler'] = mevcut_ipt + [ipotek_id]
                    save_banka_kart()

                self.send_json({'ok': True, 'banka_sync': banka_degisti})
        ...
        if self.path == '/api/ipotek/sil':
            ...
            try:
                ...
                p = json.loads(body)
                ipotek_id = str(p.get('id'))

                silinen = next((r for r in IPOTEK_DATA if str(r.get('id')) == ipotek_id), None)

                IPOTEK_DATA[:] = [r for r in IPOTEK_DATA if str(r.get('id')) != ipotek_id]
                save_ipotek()

                # Banka kartlarından da temizle
                if silinen:
                    guncellendi = False
                    for kart in BANKA_KART_DATA:
                        mevcut_ipt = [str(x) for x in (kart.get('ipotekler') or [])]
        ...
        if self.path == '/api/ipotek/excel_yukle':
            ...
            try:
                ...
                form=parse_multipart_body(body, ctype)

                file_val=form.get('file')

                if not file_val or not isinstance(file_val, tuple):
                    self.send_json({'ok':False,'msg':'Dosya bulunamadi'}); return

                filename, file_bytes = file_val
        ...
        if self.path == '/api/source/set':
            ...
            try:
                ...
                with open(DOCS_META_FILE, 'w', encoding='utf-8') as f:
                    json.dump(DOCS_META, f, ensure_ascii=False, indent=2)
                # GitHub'a hem meta hem dosya içeriğini yükle
                if GITHUB_TOKEN:
                    import threading as _thr2
                    _dm = json.dumps(DOCS_META, ensure_ascii=False, indent=2)
                    _dosya_data_kopi = dosya_data
                    _dosya_adi_kopi = dosya_adi_disk
                    def _github_yukle_dosya():
                        # Meta kaydet
                        github_dosya_yukle(
                            f'{BACKUP_PREFIX}nakit_akis_docs_meta.json', _dm,
                            f'Dokuman meta - {datetime.now().strftime("%d.%m.%Y %H:%M")}'
                        )
                        # Dosya içeriğini base64 olarak GitHub'a yükle
                        try:
                            import base64 as _b64
                            icerik_b64 = _b64.b64encode(_dosya_data_kopi).decode('utf-8')
                            github_api_dosya_yukle_binary(
                                f'belgeler/{_dosya_adi_kopi}',
                                icerik_b64,
                                f'Dokuman eklendi: {_dosya_adi_kopi}'
                            )
                        except Exception as _e:
                            print(f'GitHub dosya yukle hatasi: {_e}')
                    _thr2.Thread(target=_github_yukle_dosya, daemon=True).start()
            ...
            try:
                ...
                if doc:
                    ...
                    silinen_dosya = doc.get('dosya_adi','')
                    DOCS_META[:] = [d for d in DOCS_META if d.get('id') != p.get('id')]
                    with open(DOCS_META_FILE, 'w', encoding='utf-8') as f:
                        json.dump(DOCS_META, f, ensure_ascii=False, indent=2)
                    # GitHub'dan da sil
                    if GITHUB_TOKEN and silinen_dosya:
                        import threading as _thr3
                        def _sil_github():
                            try:
                                mevcut = github_api('GET', f'/repos/{GITHUB_REPO}/contents/belgeler/{silinen_dosya}')
                                if mevcut and 'sha' in mevcut:
                                    github_api('DELETE', f'/repos/{GITHUB_REPO}/contents/belgeler/{silinen_dosya}',
                                        {'message': f'Belge silindi: {silinen_dosya}', 'sha': mevcut['sha']})
                            except: pass
                            # Meta güncelle
                            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_docs_meta.json',
                                json.dumps(DOCS_META, ensure_ascii=False, indent=2),
                                f'Dokuman meta guncellendi')
                        _thr3.Thread(target=_sil_github, daemon=True).start()
            ...
            p = json.loads(body); idx = p.get('idx')
            odeme_tarihi = p.get('odeme_tarihi','')

            if 0 <= idx < len(DATA):

                DATA[idx]['durum'] = 'ÖDENDİ' if DATA[idx].get('durum')!='ÖDENDİ' else 'ÖDENMEDİ'
                # Ödeme tarihi — sadece ÖDENDİ yapılıyorsa kaydet
                if DATA[idx]['durum'] == 'ÖDENDİ' and odeme_tarihi:
                    DATA[idx]['odeme_tarihi'] = odeme_tarihi
                elif DATA[idx]['durum'] == 'ÖDENMEDİ':
                    DATA[idx].pop('odeme_tarihi', None)
                save_data()
                banka_kart_kullanim_guncelle()
                ...
                self.send_json({'ok':True,'durum':DATA[idx]['durum'],
                               'odeme_tarihi':DATA[idx].get('odeme_tarihi',''),
                               'excel':excel_ok})
        ...
        elif self.path == '/api/gider/excel_yukle':
            # Gider tablosuna Excel'den toplu veri yükle (mevcut dataya EKLE veya REPLACE)
            try:
                import tempfile
                ctype = self.headers.get('Content-Type','')
                form = parse_multipart_body(body, ctype)
                ...
                if not file_val or not isinstance(file_val, tuple):
                    self.send_json({'ok':False,'msg':'Dosya bulunamadi'}); return
                filename, file_bytes = file_val
                mod = form.get('mod', 'ekle')  # 'ekle' veya 'replace'
                ext = os.path.splitext(filename)[1].lower()
                tmp = tempfile.NamedTemporaryFile(suffix=ext, delete=False)
                tmp.write(file_bytes); tmp.close()
                records, count, sheet = import_excel(tmp.name)
                os.unlink(tmp.name)
                if count == 0:
                    self.send_json({'ok':False,'msg':f'Kayıt bulunamadı — sayfa: {sheet}'}); return
                if mod == 'replace':
                    DATA[:] = records
                else:
                    # Mükerrer kontrolü: tarih+firma+tutar kombinasyonu aynıysa ekleme
                    mevcut_keys = set(f"{d.get('tarih')}-{d.get('firma')}-{d.get('tutar')}" for d in DATA)
                    yeni = [r for r in records if f"{r.get('tarih')}-{r.get('firma')}-{r.get('tutar')}" not in mevcut_keys]
                    DATA.extend(yeni)
                    count = len(yeni)
                save_data()
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True,'msg':f'{count} kayıt {"yüklendi" if mod=="replace" else "eklendi"} (toplam: {len(DATA)})'})
            except Exception as e:
                import traceback; traceback.print_exc()
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/gider/toplu_guncelle':
            # Seçili kayıtları toplu güncelle
            try:
                p = json.loads(body)
                idxler = p.get('idxler', [])          # güncelllenecek DATA indexleri
                alanlar = p.get('alanlar', {})         # {alan: deger} — sadece dolu olanlar
                if not idxler:
                    self.send_json({'ok':False,'msg':'Hiç kayıt seçilmedi'}); return
                guncellendi = 0
                for idx in idxler:
                    if 0 <= idx < len(DATA):
                ...
                save_data()
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True,'msg':f'{guncellendi} kayıt güncellendi'})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/gider/toplu_sil':
            # Seçili kayıtları toplu sil (index listesi büyükten küçüğe sıralanarak silinir)
            try:
                p = json.loads(body)
                idxler = p.get('idxler', [])
                if not idxler:
                    self.send_json({'ok':False,'msg':'Hiç kayıt seçilmedi'}); return
                # Büyükten küçüğe sırala ki index kayması olmasın
                for idx in sorted(set(idxler), reverse=True):
                    if 0 <= idx < len(DATA):
                        DATA.pop(idx)
                save_data()
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True,'msg':f'{len(idxler)} kayıt silindi'})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return
        ...
        elif self.path == '/api/taksitli/liste':
            self.send_json({'ok':True,'records': TAKSITLI_DATA})
            return

        elif self.path == '/api/taksitli/sablon':
            # Kredi yönetimi Excel şablonu indir
            try:
                import openpyxl
                from openpyxl.styles import Font, PatternFill, Alignment, PatternFill
                wb = openpyxl.Workbook()

                # ── Sayfa 1: Taksitli Krediler ──
                ws1 = wb.active; ws1.title = 'Taksitli Krediler'
                hdrs1 = ['Firma','Banka','Kredi No','Kredi Türü','Ana Para (₺)',
                         'Faiz % (Yıllık)','Vade (Ay)','İlk Taksit Tarihi','BSMV %','Açıklama']
                hfill = PatternFill("solid", fgColor="1E3A5F")
                ...
                ornekler1 = [
                    ['ULUSAL','YAPIKREDİ','246324480007','TAKSİTLİ KREDİ',5000000,45.5,36,'10.01.2024',5,''],
                    ['ABC','FİNANSBANK','16497','KONUT KREDİSİ',2000000,38.0,120,'15.03.2023',5,'Konut alımı'],
                    ['ULUSAL','TEB','130244124','TAKSİTLİ KREDİ',3000000,42.0,48,'01.06.2023',5,''],
                ]
                for r,row in enumerate(ornekler1,2):
                ...
                # ── Sayfa 2: Döner Krediler (KMH/ROTATİF/VİNOV) ──
                ws2 = wb.create_sheet('Döner Krediler')
                hdrs2 = ['Firma','Banka','Kredi No','Kredi Türü','Limit (₺)',
                         'Kullanılan (₺)','Faiz % (Yıllık)','Komisyon %','Vade Bitiş','Açıklama']
                for c,h in enumerate(hdrs2,1):
                ...
                ornekler2 = [
                    ['ULUSAL','AKBANK','ROT-001','ROTATİF',10000000,3500000,45.0,0.5,'31.12.2026',''],
                    ['ABC','GARANTİ','KMH-002','KMH',5000000,0,42.0,0,'30.06.2026','Kredili Mevduat'],
                    ['ULUSAL','YAPIKREDİ','VNV-003','VİNOV',2000000,2000000,44.0,0,'15.04.2026','Spot kredi'],
                ]
                for r,row in enumerate(ornekler2,2):
                    for c,v in enumerate(row,1):
                ...
                # ── Sayfa 3: Açıklamalar ──
                ws3 = wb.create_sheet('Alan Açıklamaları')
                aciklamalar = [
                    ('Alan','Açıklama','Örnek'),
                    ('Firma','Grup firması kodu','ULUSAL, ABC, BRG, BRK'),
                    ('Banka','Bankanın tam adı','YAPIKREDİ, TEB, GARANTİ BBVA'),
                    ('Kredi No','Bankanın kredi referans numarası','246324480007'),
                    ('Kredi Türü','TAKSİTLİ KREDİ / KONUT / TAŞIT / İHTİYAÇ','TAKSİTLİ KREDİ'),
                    ('Ana Para','Kredi ana para tutarı (₺)','5000000'),
                    ('Faiz %','Yıllık faiz oranı (sayı olarak)','45.5'),
                    ('Vade (Ay)','Toplam vade ay sayısı','36'),
                    ('İlk Taksit','İlk taksit tarihi (GG.AA.YYYY)','10.01.2024'),
                    ('BSMV %','BSMV oranı, genelde 5','5'),
                    ('Döner - Limit','Kredi limiti tutarı','10000000'),
                    ('Döner - Kullanılan','Mevcut kullanılan tutar','3500000'),
                ]
                hfill3=PatternFill("solid",fgColor="0F2027")
                for c,h in enumerate(['Alan','Açıklama','Örnek'],1):
                    cell=ws3.cell(row=1,column=c,value=h)
                    cell.fill=hfill3; cell.font=Font(bold=True,color="FFFFFF",name="Calibri",size=11)
                for r,row in enumerate(aciklamalar[1:],2):
                    for c,v in enumerate(row,1):
                        ws3.cell(row=r,column=c,value=v)
                for col,w in zip(['A','B','C'],[20,50,30]):
        ...
        elif self.path == '/api/taksitli/excel_yukle':
            # Excel'den kredi yükle (Taksitli + Döner)
            try:
                import tempfile
                ctype = self.headers.get('Content-Type','')
                form = parse_multipart_body(body, ctype)
                file_val = form.get('file')
                if not file_val or not isinstance(file_val, tuple):
                    self.send_json({'ok':False,'msg':'Dosya bulunamadı'}); return
                filename, file_bytes = file_val
                ext = os.path.splitext(filename)[1].lower()
                tmp = tempfile.NamedTemporaryFile(suffix=ext, delete=False)
                tmp.write(file_bytes); tmp.close()
                eklenen = 0
                hata_list = []
                def nc(s): return str(s).upper().strip().replace('İ','I').replace('Ş','S').replace('Ğ','G').replace('Ü','U').replace('Ö','O').replace('Ç','C').replace(' ','')
                for sayfa, is_doner_sheet in [('Taksitli Krediler', False), ('Döner Krediler', True)]:
                    try:
                        import pandas as pd
                        df = pd.read_excel(tmp.name, sheet_name=sayfa, dtype=str).fillna('')
                        for _, row in df.iterrows():
                            def g(col_keywords):
                                for k in col_keywords:
                                    for col in df.columns:
                                        if nc(k) in nc(col) or nc(col) in nc(k):
                                            v = row.get(col,'')
                                            return '' if str(v) in ('nan','None','') else str(v)
                                return ''
                            firma = g(['FIRMA','GRUP'])
                            banka = g(['BANKA'])
                            kredi_no = g(['KREDI NO','KREDI_NO','KRD NO'])
                            tur = g(['TUR','TYPE'])
                            if not firma or not banka: continue
                            if is_doner_sheet:
                                kredi = {
                                    'id': str(int(datetime.now().timestamp()*1000) + eklenen),
                                    'firma': firma, 'banka': banka, 'kredi_no': kredi_no,
                                    'tur': tur or 'ROTATİF',
                                    'ana_para': float(g(['LIMIT','ANA PARA','ANPARA']).replace(',','.') or 0),
                                    'kullanim': float(g(['KULLANILAN','KULLANIM']).replace(',','.') or 0),
                                    'faiz_orani': float(g(['FAIZ','FAİZ']).replace(',','.') or 0),
                                    'komisyon_oran': float(g(['KOMISYON']).replace(',','.') or 0),
                                    'vade_bitis': g(['VADE BİTİŞ','VADE BİTİS','BITIS']),
                                    'aciklama': g(['ACIKLAMA','AÇIKLAMA']),
                                    'is_doner': True, 'taksitler': []
                                }
                            else:
                                ana_para = float(g(['ANA PARA','ANPARA','LIMIT']).replace(',','.') or 0)
                                faiz = float(g(['FAIZ','FAİZ']).replace(',','.') or 0)
                                vade = int(float(g(['VADE']).replace(',','.') or 0))
                                bas_str = g(['ILK TAKSIT','İLK TAKSİT','BASLANGIC','BAŞLANGIÇ'])
                                bsmv = float(g(['BSMV']).replace(',','.') or 5)
                                if not ana_para or not vade: continue
                                # Taksit hesapla
                                taksitler = []
                                if faiz > 0 and bas_str:
                                    try:
                                        if '.' in bas_str: bas_dt = datetime.strptime(bas_str,'%d.%m.%Y')
                                        elif '-' in bas_str: bas_dt = datetime.strptime(bas_str,'%Y-%m-%d')
                                        else: bas_dt = None
                                        if bas_dt:
                                            faiz_aylik = faiz/100/12
                                            taksit_tut = ana_para*(faiz_aylik*(1+faiz_aylik)**vade)/((1+faiz_aylik)**vade-1)
                                            kalan = ana_para
                                            for i in range(1, vade+1):
                                                t_tarih = bas_dt.strftime('%d.%m.%Y')
                                                faiz_t = round(kalan*faiz_aylik,2)
                                                bsmv_t = round(faiz_t*bsmv/100,2)
                                                ana_t = round(taksit_tut-faiz_t-bsmv_t,2) if i<vade else round(kalan,2)
                                                taksitler.append({'no':i,'tarih':t_tarih,'anapara':ana_t,'faiz':faiz_t,'bsmv':bsmv_t,'tutar':round(ana_t+faiz_t+bsmv_t,2),'durum':'ÖDENMEDİ'})
                                                kalan -= ana_t
                                                bas_dt = datetime(bas_dt.year + (bas_dt.month)//12, (bas_dt.month)%12+1, bas_dt.day) if bas_dt.month < 12 else datetime(bas_dt.year+1,1,bas_dt.day)
                                    except: pass
                                kredi = {
                                    'id': str(int(datetime.now().timestamp()*1000) + eklenen),
                                    'firma': firma, 'banka': banka, 'kredi_no': kredi_no,
                                    'tur': tur or 'TAKSİTLİ KREDİ',
                                    'ana_para': ana_para, 'faiz_orani': faiz, 'vade_ay': vade,
                                    'bsmv_oran': bsmv, 'aciklama': g(['ACIKLAMA','AÇIKLAMA']),
                                    'is_doner': False, 'taksitler': taksitler
                                }
                            TAKSITLI_DATA.append(kredi)
                            _taksitli_odeme_senkronize(kredi['id'])
                            eklenen += 1
                    except Exception as ex:
                        hata_list.append(f"{sayfa}: {ex}")
                os.unlink(tmp.name)
                save_taksitli(); save_data(); banka_kart_kullanim_guncelle()
                msg = f'{eklenen} kredi eklendi'
                if hata_list: msg += f' (Uyarı: {"; ".join(hata_list)})'
                self.send_json({'ok':True,'msg':msg,'count':eklenen})
            except Exception as e:
                import traceback; traceback.print_exc()
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/taksitli/taksit_satir_guncelle':
            # Tek taksit satırının tarih/tutarını güncelle
            try:
                p = json.loads(body)
                kredi_id = str(p.get('kredi_id',''))
                taksit_no = int(p.get('taksit_no',0))
                for k in TAKSITLI_DATA:
                    if str(k.get('id')) == kredi_id:
                        for t in k.get('taksitler',[]):
                            if t.get('no') == taksit_no:
                                if p.get('tarih'): t['tarih'] = p['tarih']
                                if p.get('anapara') is not None: t['anapara'] = float(p['anapara'])
                                if p.get('faiz') is not None: t['faiz'] = float(p['faiz'])
                                if p.get('bsmv') is not None: t['bsmv'] = float(p['bsmv'])
                                if p.get('tutar') is not None: t['tutar'] = float(p['tutar'])
                                break
                        break
                save_taksitli()
                _taksitli_odeme_senkronize(kredi_id)
                save_data()
                self.send_json({'ok':True})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
        ...
        elif self.path == '/api/taksitli/kaydet':
            # Yeni taksitli kredi kaydet ve taksitlerini ödeme tablosuna ekle
            try:
                p = json.loads(body)
                kredi_id = p.get('id') or str(int(datetime.now().timestamp()*1000))
                p['id'] = kredi_id
                # Mevcut kaydı güncelle veya yeni ekle
                idx = next((i for i,k in enumerate(TAKSITLI_DATA) if str(k.get('id'))==str(kredi_id)), -1)
                if idx >= 0:
                    TAKSITLI_DATA[idx] = p
                else:
                    TAKSITLI_DATA.append(p)
                save_taksitli()
                # Ödeme tablosuna taksitleri senkronize et
                _taksitli_odeme_senkronize(kredi_id)
                save_data()  # Senkronize edilen ödeme tablosunu kaydet
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True,'id':kredi_id,'msg':'Kredi kaydedildi ve ödeme tablosuna aktarıldı'})
            except Exception as e:
                import traceback; traceback.print_exc()
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/taksitli/sil':
            try:
                p = json.loads(body)
                kredi_id = str(p.get('id',''))
                TAKSITLI_DATA[:] = [k for k in TAKSITLI_DATA if str(k.get('id')) != kredi_id]
                save_taksitli()
                # Ödeme tablosundan da kaldır (sadece ödenmemiş olanları)
                DATA[:] = [d for d in DATA if not (d.get('taksitli_id')==kredi_id and d.get('durum')!='ÖDENDİ')]
                save_data()
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True})
            except Exception as e:
        ...
        elif self.path == '/api/taksitli/tarihler_guncelle':
            try:
                p = json.loads(body)
                kredi_id = str(p.get('kredi_id',''))
                yeni_taksitler = p.get('taksitler',[])
                for k in TAKSITLI_DATA:
                    if str(k.get('id')) == kredi_id:
                        # Sadece tarihleri güncelle, diğer alanlar korunsun
                        tarih_map = {t.get('no'): t.get('tarih','') for t in yeni_taksitler}
                        for t in k.get('taksitler',[]):
                            if t.get('no') in tarih_map:
                                t['tarih'] = tarih_map[t['no']]
                        break
                save_taksitli()
                _taksitli_odeme_senkronize(kredi_id)
                save_data()
                self.send_json({'ok':True,'msg':f'{len(yeni_taksitler)} taksit tarihi güncellendi'})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/taksitli/taksit_satir_sil':
            try:
                p = json.loads(body)
                kredi_id = str(p.get('kredi_id',''))
                taksit_no = int(p.get('taksit_no',0))
                for k in TAKSITLI_DATA:
                    if str(k.get('id')) == kredi_id:
                        k['taksitler'] = [t for t in k.get('taksitler',[]) if t.get('no') != taksit_no]
                        # No'ları sırala
                        for i,t in enumerate(k['taksitler'],1): t['no']=i
                        break
                save_taksitli()
                _taksitli_odeme_senkronize(kredi_id)
                save_data(); banka_kart_kullanim_guncelle()
                self.send_json({'ok':True})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return

        elif self.path == '/api/taksitli/taksit_guncelle':
            # Tek taksit ödendi/ödenmedi güncelle → ödeme tablosuna yansıt
            try:
                p = json.loads(body)
                kredi_id = str(p.get('kredi_id',''))
                taksit_no = int(p.get('taksit_no',0))
                durum = p.get('durum','ÖDENMEDİ')
                odeme_tarihi = p.get('odeme_tarihi','')
                if not odeme_tarihi and durum == 'ÖDENDİ':
                    odeme_tarihi = datetime.now().strftime('%d.%m.%Y')
                for k in TAKSITLI_DATA:
                    if str(k.get('id')) == kredi_id:
                        for t in k.get('taksitler',[]):
                            if t.get('no') == taksit_no:
                                ...
                                if durum == 'ÖDENDİ':
                                    ...
                                    t.pop('odeme_tarihi', None)
                                break
                        break
                save_taksitli()
                # Ödeme tablosundaki ilgili kaydı güncelle
                for d in DATA:
                    if d.get('taksitli_id')==kredi_id and d.get('taksit_no')==taksit_no:
                        d['durum'] = durum
                        if durum == 'ÖDENDİ':
                            d['odeme_tarihi'] = odeme_tarihi
                        else:
                            d.pop('odeme_tarihi', None)
                        break
                save_data()
                banka_kart_kullanim_guncelle()
                self.send_json({'ok':True,'odeme_tarihi':odeme_tarihi})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            return
...
.lay{display:flex;flex:1;min-height:0;overflow-y:hidden;overflow-x:visible}
...
.nav-sec{font-size:12px;color:var(--t3);padding:12px 16px 4px;letter-spacing:.1em;text-transform:uppercase;font-weight:600;font-family:var(--mono)}

.nav{display:flex;align-items:center;gap:9px;padding:8px 16px;cursor:pointer;color:var(--t2);font-size:12.5px;transition:all .12s;border-left:2px solid transparent;user-select:none}

.nav:hover{color:var(--t1);background:var(--s2)}.nav.on{color:var(--acc);background:var(--s2);border-left-color:var(--acc)}

.main{flex:1;overflow-y:auto;overflow-x:auto;padding:20px;height:100%;box-sizing:border-box}

.view{display:none;animation:fadein .15s ease;width:100%;min-height:0;min-width:0}.view.on{display:block;width:100%;min-height:50px}
...
.mlb{font-size:12px;color:var(--t2);font-family:var(--mono);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px}

.mvl{font-family:var(--mono);font-size:19px;font-weight:600;line-height:1}

.msb{font-size:12px;color:var(--t3);margin-top:4px;font-family:var(--mono)}
...
.mcp{font-size:12px;color:var(--r);margin-top:2px;font-family:var(--mono)}
...
.ay-tab{background:transparent;border:none;border-bottom:2px solid transparent;color:var(--t2);padding:8px 16px;font-size:12.5px;font-weight:600;cursor:pointer;transition:all .15s;font-family:var(--sans);margin-bottom:-1px}
.ay-tab:hover{color:var(--t1)}
.ay-tab.on{color:var(--acc);border-bottom-color:var(--acc)}
.ay-panel{animation:fadeIn .15s ease}
.ay-row{display:flex;align-items:center;justify-content:space-between;gap:10px}
.ay-lbl{font-size:12px;color:var(--t2);flex:1}
.ay-color{width:36px;height:28px;border:1px solid var(--b1);border-radius:var(--rad);background:transparent;cursor:pointer;padding:2px}
.ay-hex{font-family:var(--mono);font-size:11px;color:var(--t3);width:60px}
.tema-btn{background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:7px 14px;color:var(--t2);font-size:12px;cursor:pointer;transition:all .15s;display:flex;align-items:center;gap:6px}
.tema-btn:hover{border-color:var(--acc);color:var(--t1)}
.izin-row{display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid var(--b1)}
.izin-row:last-child{border-bottom:none}
.izin-lbl{font-size:12px;color:var(--t2);flex:1}
.izin-chk{accent-color:var(--acc);width:14px;height:14px;cursor:pointer}
.uk-card{background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:12px 14px;margin-bottom:8px;display:flex;align-items:center;gap:10px}
.uk-name{font-size:13px;font-weight:600;color:var(--t1)}
.uk-rol{font-size:11px;padding:2px 8px;border-radius:10px;font-weight:600}
.uk-rol.admin{background:rgba(251,191,36,.15);color:var(--am)}
.uk-rol.user{background:rgba(79,156,249,.12);color:var(--acc)}
@keyframes fadeIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}
...
/* ── FONT BOYUTLARI (50+ yaş) ─────────────────────────────── */
body{font-size:15px}
.nav{font-size:14px!important;padding:9px 16px!important}
.nav-sec{font-size:12px!important;letter-spacing:.08em}
.ptit{font-size:24px!important;font-weight:700}
.psub{font-size:14px!important}
.mlb{font-size:13px!important}
.mvl{font-size:22px!important}
.msb{font-size:13px!important}
.met{padding:14px 18px!important}
table{font-size:13px!important}
th{font-size:12px!important}
td{font-size:13px!important}
.btn{font-size:13px!important;padding:8px 14px!important}
.bsm{font-size:12px!important;padding:6px 10px!important}
.fi,.fi2,.fse,.si{font-size:14px!important;padding:9px 12px!important}
.flb{font-size:13px!important;font-weight:600;margin-bottom:5px}
.bdg{font-size:12px!important;padding:3px 9px!important}
.num{font-size:13px!important}
.pgn button{font-size:13px!important;padding:5px 10px!important}
.mtit{font-size:16px!important}
label{font-size:14px!important}
input,select,textarea{font-size:14px!important}
.src-card .ptit{font-size:18px!important}
.hdr{height:54px!important}
.logo{font-size:16px!important}

/* Aylık özet kartlar */
.mo-kart{font-size:14px!important}
.mo-tutar{font-size:18px!important}

/* Tablo header'ları */
thead th{font-size:12px!important;font-weight:700}
tbody td{font-size:13px!important;line-height:1.5}
...
.tc-ipotek::-webkit-scrollbar { width: 12px; height: 12px; }
.tc-ipotek::-webkit-scrollbar-track { background: var(--s3); border-radius: 6px; }
.tc-ipotek::-webkit-scrollbar-thumb { background: #4f9cf9; border-radius: 6px; border: 2px solid var(--s3); }
.tc-ipotek::-webkit-scrollbar-thumb:hover { background: #2563eb; }
.tc-ipotek::-webkit-scrollbar-corner { background: var(--s3); }
.tc-ipotek table { min-width: 2400px !important; width: max-content !important; }

/* Tüm sayfa scrollbar'ları */
::-webkit-scrollbar { width: 10px; height: 10px; }
::-webkit-scrollbar-track { background: var(--s2); }
::-webkit-scrollbar-thumb { background: #4f9cf9; border-radius: 5px; border: 2px solid var(--s2); }
::-webkit-scrollbar-thumb:hover { background: #2563eb; }
::-webkit-scrollbar-corner { background: var(--s2); }
* { scrollbar-width: thin; scrollbar-color: #4f9cf9 var(--s2); }

/* ── MOBİL RESPONSIVE ───────────────────────────────────── */
.mob-menu-btn{display:none;background:none;border:none;color:var(--t1);font-size:24px;cursor:pointer;padding:4px 8px;line-height:1;flex-shrink:0}
...
@media(max-width:768px){
  ...
  .hdr .btn{font-size:11px;padding:5px 9px}
  .sbar{position:fixed;left:-224px;top:0;bottom:0;z-index:200;width:220px;transition:left .25s ease;box-shadow:4px 0 24px rgba(0,0,0,.6)}
  .sbar.open{left:0}
  .lay{position:relative}
  .main{padding:12px 10px}
  .ptit{font-size:17px!important}
  .psub{font-size:12px!important}
  .mets{grid-template-columns:1fr 1fr!important;gap:8px!important}
  .met{padding:10px 12px!important}
  .mvl{font-size:17px!important}
  .mlb{font-size:11px!important}
  .tbr{flex-wrap:wrap!important;gap:6px!important}
  .si{width:100%!important}
  .fi,.fse,.fi2{font-size:13px!important}
  .btn{font-size:12px!important;padding:6px 10px!important}
  .bsm{font-size:11px!important;padding:5px 8px!important}
  .tc{overflow-x:auto;-webkit-overflow-scrolling:touch}
  table{font-size:11px!important}
  th,td{padding:5px 6px!important;white-space:nowrap}
  .mod{width:95%!important;max-width:none!important;max-height:88vh!important}
  .fg{grid-template-columns:1fr!important}
  .nav{font-size:13px!important;padding:9px 14px!important}
  .nav-sec{font-size:12px!important}
  .src-card{padding:14px!important}
}
@media(max-width:400px){
  .mets{grid-template-columns:1fr!important}
}
...
<div class="mob-overlay" id="mob-overlay" onclick="mobMenuKapat()"></div>
<div class="hdr">
  <button class="mob-menu-btn" onclick="mobMenuAc()" aria-label="Menü">☰</button>
  <div class="logo">NAKİT<em>/</em>AKİŞ <em style="font-size:12px">v3</em></div>

  <div class="hdr-sep"></div>

  <div class="hdr-src" id="hdr-src" style="display:none">
...
<nav class="sbar" id="sbar">
  ...
  <div class="nav"    id="nv-kk"        onclick="showView('kk')">💳 KREDİ KARTLARI</div>

  <div class="nav"    id="nv-taksitli" onclick="showView('taksitli')">📋 KREDİ YÖNETİMİ</div>

  <div class="nav"    id="nv-tbl"    onclick="showView('tbl')" style="color:#E24B4A">− ÖDEME TABLOSU</div>

  <div class="nav"    id="nv-gelir"  onclick="showView('gelir')">＋ TAHSİLAT TABLOSU</div>
  ...
  <div class="nav"    id="nv-ayarlar" onclick="showView('ayarlar')">⚙ AYARLAR</div>
...
<div class="view on" id="vw-db">
  ...
  <div class="mets">
    ...
    <div class="met"><div class="met-stripe" style="background:#f87171"></div>
      <div class="mlb">Kredi Kullanım</div>
      <div class="mvl" id="st-kredi-kull" style="color:#f87171">₺0</div>
      <div class="msb" id="st-kredi-pct">— kullanım</div>
    </div>
    <div class="met"><div class="met-stripe" style="background:#34d399"></div>
      <div class="mlb">Kullanılabilir Limit</div>
      <div class="mvl" id="st-kredi-kalan" style="color:#34d399">₺0</div>
      <div class="msb">kalan kredi limiti</div>
    </div>
  </div>
...
<div class="view" id="vw-tbl">
  ...
  <div class="ptit">Ödeme Tablosu</div>
  ...
  <!-- Tür Bazlı Özet — ÜSTTE -->
  <div id="tur-ozet-wrap-top" style="display:none;margin-bottom:12px">
    <div id="tur-ozet-grid-top" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:8px"></div>
    <div id="tur-ozet-toplam-top" style="padding:10px 14px;background:var(--s2);border-radius:8px;border:1px solid var(--b1);display:flex;flex-wrap:wrap;gap:16px;font-size:12px;font-family:var(--mono)"></div>
  </div>

  <!-- Filtre Satırı 1: Temel filtreler -->
  <div class="tbr" style="flex-wrap:wrap;gap:6px;margin-bottom:4px">
  ...
  <!-- Filtre Satırı 2: Kredi/Çek No + Tarih + Hızlı Butonlar -->
  <div class="tbr" style="flex-wrap:wrap;gap:6px">

    <select class="fi" id="fkn" onchange="doFilter()" style="font-family:var(--mono);font-size:11px;min-width:160px"><option value="">Tüm Kredi/Çek No</option></select>
    ...
    <div style="display:flex;align-items:center;gap:4px">
      ...
      <button class="btn bgh bsm" onclick="giderTumFiltreSifirla()" title="Tüm filtreleri temizle" style="padding:4px 7px;font-size:11px;color:var(--r)">✕ Temizle</button>

      <button class="btn bgh bsm" onclick="hizliTarih('gider','bu-ay')" style="font-size:12px;padding:3px 6px">Bu Ay</button>

      <button class="btn bgh bsm" onclick="hizliTarih('gider','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('gider','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
    ...
    <button class="btn bgh bsm" onclick="giderExcelSablonIndir()" title="Excel şablonu indir">📋 Şablon</button>

    <label class="btn bgh bsm" style="cursor:pointer;margin:0" title="Excel'den toplu yükle">
      ⬆ Excel Yükle
      <input type="file" id="gider-excel-input" accept=".xlsx,.xls" style="display:none" onchange="giderExcelYukleAc(this)">
    </label>

    <button class="btn bgh bsm" id="toplu-guncelle-btn" onclick="topluGuncelleAc()" style="display:none">
      ✏️ Toplu Güncelle (<span id="secili-sayi">0</span>)
    </button>

    <button class="btn bsm" id="toplu-sil-btn" onclick="topluSil()" style="display:none;background:var(--r);color:#fff;border:none">
      🗑 Seçilenleri Sil (<span id="secili-sayi-sil">0</span>)
    </button>
  ...
  <div class="tc">
    ...
    <table>
      ...
      <thead><tr>
        ...
        <th style="width:30px"><input type="checkbox" id="tbl-sec-hepsi" title="Tümünü seç" onchange="tblSecHepsi(this.checked)"></th>
  ...
  <!-- Alt özet kaldırıldı - sadece üstte gösteriliyor -->
  <div id="tur-ozet-wrap" style="display:none">
    <div id="tur-ozet-grid" style="display:none"></div>
    <div id="tur-ozet-toplam" style="display:none"></div>
  </div>

  <!-- Excel Yükleme Modal -->
  <div class="ov" id="gider-excel-mod" style="display:none">
    <div class="mod" style="max-width:520px;width:96%">
      <div class="mtit">⬆ Excel'den Gider Yükle<button class="mclose" onclick="document.getElementById('gider-excel-mod').style.display='none'">✕</button></div>
      <div style="padding:16px;display:flex;flex-direction:column;gap:12px">
        <div style="background:var(--s3);border-radius:8px;padding:12px;font-size:12px;color:var(--t2)">
          <strong>📋 Şablon formatında hazırlayın:</strong><br>
          Sütunlar: Grup Firması, Ödeme Türü, Ödeme Yapılacak Yer, Kredi/Çek No, Tarih, Ay, Durum, Anapara, Faiz, BSMV, Tutar, Açıklama
          <br><br>
          <button class="btn bgh bsm" onclick="giderExcelSablonIndir()" style="font-size:11px">📥 Örnek Şablon İndir</button>
        </div>
        <div>
          <label style="font-size:12px;color:var(--t3);font-weight:600;display:block;margin-bottom:6px">Yükleme Modu</label>
          <div style="display:flex;gap:10px">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px">
              <input type="radio" name="gider-mod" id="gider-mod-ekle" value="ekle" checked>
              ➕ Mevcut dataya ekle (mükerrer kontrolü yapılır)
            </label>
          </div>
          <div style="display:flex;gap:10px;margin-top:6px">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px">
              <input type="radio" name="gider-mod" id="gider-mod-replace" value="replace">
              🔄 Tüm datayı değiştir (mevcut kayıtlar silinir!)
            </label>
          </div>
        </div>
        <div id="gider-excel-dosya-adi" style="font-size:12px;color:var(--acc);display:none"></div>
        <div id="gider-excel-msg" style="font-size:13px;color:var(--t3)"></div>
        <div style="display:flex;gap:8px">
          <button class="btn bp" id="gider-excel-yukle-btn" onclick="giderExcelYukle()" style="flex:1;display:none">⬆ Yükle</button>
          <button class="btn bo" onclick="document.getElementById('gider-excel-mod').style.display='none'">İptal</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="ov" id="toplu-guncelle-mod" style="display:none">
    <div class="mod" style="max-width:560px;width:96%">
      <div class="mtit">✏️ Toplu Güncelleme<button class="mclose" onclick="document.getElementById('toplu-guncelle-mod').style.display='none'">✕</button></div>
      <div style="padding:16px">
        <div style="font-size:12px;color:var(--t3);margin-bottom:14px;padding:8px 12px;background:var(--s3);border-radius:6px">
          ⚠️ <strong id="toplu-secili-bilgi">0 kayıt</strong> seçildi. Sadece dolu bıraktığınız alanlar güncellenir.
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            <label class="flb">Firma</label>
            <select class="fi2" id="tg-firma" style="width:100%">
              <option value="">— Değiştirme —</option>
              <option>ULUSAL</option><option>ABC</option><option>BRG</option><option>BRK</option>
            </select>
          </div>
          <div>
            <label class="flb">Ödeme Türü</label>
            <select class="fi2" id="tg-tur" style="width:100%">
              <option value="">— Değiştirme —</option>
              <option>TAKSİTLİ KREDİ</option><option>ROTATİF</option><option>KMH</option>
              <option>KREDİ KARTI</option><option>TEMİNAT MEKTUBU</option><option>DİĞER</option>
            </select>
          </div>
          <div>
            <label class="flb">Ödeme Yeri (Banka)</label>
            <select class="fi2" id="tg-yer" style="width:100%">
              <option value="">— Değiştirme —</option>
            </select>
          </div>
          <div>
            <label class="flb">Kredi / Çek No</label>
            <input class="fi2" id="tg-kredi-no" placeholder="Boşsa değiştirmez">
          </div>
          <div>
            <label class="flb">Durum</label>
            <select class="fi2" id="tg-durum" style="width:100%">
              <option value="">— Değiştirme —</option>
              <option>ÖDENDİ</option><option>ÖDENMEDİ</option>
            </select>
          </div>
          <div>
            <label class="flb">Açıklama</label>
            <input class="fi2" id="tg-aciklama" placeholder="Boşsa değiştirmez">
          </div>
        </div>
        <div style="margin-top:14px;display:flex;gap:8px">
          <button class="btn bp" onclick="topluGuncelleKaydet()" style="flex:1">💾 Güncelle</button>
          <button class="btn bo" onclick="document.getElementById('toplu-guncelle-mod').style.display='none'">İptal</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="view" id="vw-docs">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
    ...
    <div id="docs-drop-zone"
      ...
      <div style="font-size:12px;margin-top:4px;color:var(--t3)">Tüm dosya türleri desteklenir</div>
...
<div class="view" id="vw-gelir">
  ...
  <div class="ptit">Tahsilat Tablosu</div>
  ...
  <div class="tbr" style="flex-wrap:wrap;gap:6px">
    ...
    <div style="display:flex;align-items:center;gap:4px">
      ...
      <button class="btn bgh bsm" onclick="hizliTarih('gelir','bu-ay')" style="font-size:12px;padding:3px 6px">Bu Ay</button>

      <button class="btn bgh bsm" onclick="hizliTarih('gelir','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('gelir','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">
      ...
      <div style="display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap">
        ...
        <div style="display:flex;flex-direction:column;gap:3px">
          <label style="font-size:10px;color:var(--t3);font-weight:600">YIL (çoklu)</label>
          <select class="fi" id="fn-yil" multiple size="3" style="font-size:12px;min-width:90px"><option value="">Tüm dönem</option></select>
        </div>

        <div style="display:flex;flex-direction:column;gap:3px">
          <label style="font-size:10px;color:var(--t3);font-weight:600">AY (çoklu)</label>
          <select class="fi" id="fn-ay" multiple size="3" style="font-size:12px;min-width:100px"><option value="">Tüm aylar</option></select>
        </div>

        <div style="display:flex;flex-direction:column;gap:3px">
          <label style="font-size:10px;color:var(--t3);font-weight:600">BAŞLANGIÇ</label>
          <input class="fi" type="date" id="fn-tarih-bas" style="font-size:12px;width:130px">
        </div>

        <div style="display:flex;flex-direction:column;gap:3px">
          <label style="font-size:10px;color:var(--t3);font-weight:600">BİTİŞ</label>
          <input class="fi" type="date" id="fn-tarih-bit" style="font-size:12px;width:130px">
        </div>

        <div style="display:flex;flex-direction:column;gap:3px;justify-content:flex-end">
          <label style="font-size:10px;color:transparent">_</label>
          <div style="display:flex;gap:6px">
            <button class="btn bp bsm" onclick="hesaplaOneri()">Analiz Et</button>
            <button class="btn bo bsm" onclick="fnFiltreTemizle()" title="Filtreleri temizle">✕ Temizle</button>
          </div>
        </div>

      </div>

      <div style="font-size:10px;color:var(--t3);margin-top:4px">💡 Çoklu seçim için <kbd style="background:var(--s3);border:1px solid var(--b1);border-radius:3px;padding:1px 4px">Ctrl</kbd> (Mac: <kbd style="background:var(--s3);border:1px solid var(--b1);border-radius:3px;padding:1px 4px">⌘</kbd>) basılı tutarak tıklayın. Tarih aralığı seçilirse yıl/ay filtresi devre dışı kalır.</div>
...
<div class="view" id="vw-banka-api">
  ...
  <div id="batab-ekstre-view">
    ...
    <div style="margin-top:20px">
      ...
      <div style="font-family:var(--mono);font-size:12px;color:var(--t3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Son Yüklenen Ekstre Bakiyeleri</div>
...
<div class="view" id="vw-kur">
  ...
  <div style="margin-top:20px" id="kur-gecmis-wrap" style="display:none">
    ...
    <div style="font-family:var(--mono);font-size:12px;color:var(--t3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Sorgulanan Tarihler</div>
...
<div class="view" id="vw-logo">
  ...
  <div id="logo-ayarlar" style="display:none;background:var(--s2);border:1px solid var(--b2);border-radius:var(--radl);padding:18px;margin-bottom:14px">
    ...
    <div style="margin-top:12px">
      ...
      <div style="font-size:12px;font-weight:600;color:var(--acc);margin-bottom:8px">🏢 Firma Listesi
        ...
        <span style="font-size:12px;color:var(--t3);font-weight:400;margin-left:6px">Logo'da kaç firma varsa hepsini ekleyin (001, 002...)</span>
  ...
  <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:12px;margin-bottom:10px" id="logo-filter-bar">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
      ...
      <button class="btn bgh bsm" onclick="hizliTarih('logo','bu-ay')" style="font-size:12px;padding:3px 6px">Bu Ay</button>

      <button class="btn bgh bsm" onclick="hizliTarih('logo','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('logo','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--t3);margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">Firma Seçimi</div>
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--t3);margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">Yıl Seçimi</div>
...
<div class="view" id="vw-stok">
  ...
  <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:12px;margin-bottom:10px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
      ...
      <button class="btn bgh bsm" onclick="hizliTarih('stok','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('stok','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
    ...
    <div style="display:flex;gap:16px;flex-wrap:wrap">
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--t3);margin-bottom:4px">Firma:</div>
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--t3);margin-bottom:4px">Yıl:</div>
...
<div class="view" id="vw-cari-logo">
  ...
  <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:12px;margin-bottom:10px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
      ...
      <button class="btn bgh bsm" onclick="hizliTarih('cari','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('cari','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
    ...
    <div style="display:flex;gap:16px;flex-wrap:wrap">
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--t3);margin-bottom:4px">Yıl (dönem ciro için):</div>
...
<div class="ov" id="not-edit-mod">
  ...
  <div class="mod" style="width:560px;max-width:95vw">
    ...
    <div class="mac">
      ...
      <button class="btn bp" onclick="notEditKaydet()">✓ Kaydet <span style="opacity:.6;font-size:12px">(Ctrl+Enter)</span></button>
...
<div class="view" id="vw-db">
  ...
  <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:16px">
    ...
    <div style="display:flex;gap:8px;margin-bottom:10px">
      ...
      <button class="btn bp" onclick="dbSorguCalistir()" style="height:60px;align-self:flex-start">▶ Çalıştır<br><span style="font-size:12px;opacity:.7">Ctrl+Enter</span></button>
    ...
    <div style="display:grid;grid-template-columns:300px 1fr;gap:0;height:calc(100vh - 185px);border:1px solid var(--b1);border-radius:var(--rad);overflow:hidden">
      ...
      <div style="background:var(--s1);border-right:1px solid var(--b1);display:flex;flex-direction:column">
        ...
        <div style="padding:10px 12px;border-bottom:1px solid var(--b1);display:flex;gap:6px;align-items:center;background:var(--s2)">
          ...
          <span id="wa-toplam-okunmadi" style="display:none;background:var(--r);color:#fff;border-radius:10px;font-size:12px;padding:1px 7px;font-weight:700"></span>
      ...
      <div style="background:var(--s2);display:flex;flex-direction:column">
        ...
        <div id="wa-mesaj-baslik" style="padding:11px 16px;border-bottom:1px solid var(--b1);display:flex;align-items:center;gap:10px;background:var(--s1)">
          ...
          <div>
            ...
            <div style="font-size:12px;color:var(--t3)" id="wa-baslik-alt">Kalıcı mesaj geçmişi</div>
...
<div class="view" id="vw-satis">
  ...
  <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:12px;margin-bottom:14px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
      ...
      <button class="btn bgh bsm" onclick="hizliTarih('sa','bu-ay')" style="font-size:12px;padding:3px 6px">Bu Ay</button>

      <button class="btn bgh bsm" onclick="hizliTarih('sa','bu-yil')" style="font-size:12px;padding:3px 6px">Bu Yıl</button>

      <button class="btn bgh bsm" onclick="hizliTarih('sa','son-3')" style="font-size:12px;padding:3px 6px">Son 3 Ay</button>
...
<div class="view" id="vw-banka-kart">
  ...
  <div id="bk-liste" style="overflow-x:auto;overflow-y:auto;max-height:calc(100vh - 280px);border:1px solid var(--b1);border-radius:8px"></div>
...
<div class="view" id="vw-kk" style="min-height:0">

  <div class="ptit">💳 KREDİ KARTLARI</div>

  <!-- Özet kartları -->
  <div id="kk-ozet-bar" style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px"></div>

  <!-- Araç çubuğu -->
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
    <button class="btn bp bsm" onclick="kkYeniKartAc()">+ Yeni Kart</button>
    <div style="flex:1"></div>
    <input class="fi" id="kk-arama" placeholder="🔍 Banka / kart ara..." style="width:200px;font-size:12px" oninput="kkFiltrele()">
  </div>

  <!-- Kart listesi -->
  <div id="kk-list" style="max-height:75vh;overflow-y:auto;padding-right:4px"></div>

  <!-- Kart ekle/düzenle modal -->
  <div class="ov" id="kk-mod" style="display:none">
    <div class="mod" style="max-width:640px;width:96%;max-height:90vh;overflow-y:auto">
      <div class="mtit"><span id="kk-mod-baslik">Yeni Kredi Kartı</span><button class="mclose" onclick="kkKapat()">✕</button></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:16px">
        <div style="grid-column:1/-1"><label class="flb">Banka Adı *</label>
          <select class="fi2" id="kk-banka" style="width:100%">
            <option value="">— Seçiniz —</option>
          </select>
        </div>
        <div><label class="flb">Kart No (son 4 hane)</label><input class="fi2" id="kk-kart-no" placeholder="**** **** **** 1234" maxlength="19"></div>
        <div><label class="flb">Firma</label>
          <select class="fi2" id="kk-firma" style="width:100%">
            <option value="ULUSAL">ULUSAL</option><option value="ABC">ABC</option>
            <option value="BRG">BRG</option><option value="BRK">BRK</option><option value="DİĞER">DİĞER</option>
          </select>
        </div>
        <div><label class="flb">Limit (₺) *</label><input class="fi2" type="number" id="kk-limit" placeholder="0"></div>
        <div><label class="flb">Hesap Kesim Günü</label><input class="fi2" type="number" id="kk-kesim-gun" placeholder="25" min="1" max="31"></div>
        <div><label class="flb">Son Ödeme Günü</label><input class="fi2" type="number" id="kk-odeme-gun" placeholder="10" min="1" max="31"></div>
        <div><label class="flb">Gecikme Faizi % (yıllık)</label><input class="fi2" type="number" id="kk-gecikme-faiz" placeholder="48.00" step="0.01"></div>
        <div><label class="flb">Notlar</label><input class="fi2" id="kk-notlar" placeholder="Açıklama..."></div>
      </div>
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="kkKaydet()" style="flex:1">💾 Kaydet</button>
        <button class="btn bo" onclick="kkKapat()">İptal</button>
      </div>
    </div>
  </div>

  <!-- Harcama ekle/düzenle modal -->
  <div class="ov" id="kk-harcama-mod" style="display:none">
    <div class="mod" style="max-width:600px;width:96%">
      <div class="mtit"><span id="kk-harcama-baslik">Harcama / Ödeme Ekle</span><button class="mclose" onclick="kkHarcamaKapat()">✕</button></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:16px">
        <!-- İşlem türü - Harcama mı Ödeme mi -->
        <div style="grid-column:1/-1">
          <label class="flb">İşlem Türü</label>
          <div style="display:flex;gap:8px;margin-top:4px">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;padding:7px 14px;border:1.5px solid var(--acc);border-radius:var(--rad);font-size:12px;font-weight:600;flex:1;justify-content:center" id="kk-h-tur-harcama-lbl">
              <input type="radio" name="kk-h-tur" id="kk-h-tur-harcama" value="harcama" checked onchange="kkHarcamaTurDegis()">
              💳 Harcama
            </label>
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;padding:7px 14px;border:1.5px solid var(--b1);border-radius:var(--rad);font-size:12px;font-weight:600;flex:1;justify-content:center" id="kk-h-tur-odeme-lbl">
              <input type="radio" name="kk-h-tur" id="kk-h-tur-odeme" value="odeme" onchange="kkHarcamaTurDegis()">
              💰 Ödeme (limit artırır)
            </label>
          </div>
          <div id="kk-h-odeme-info" style="display:none;margin-top:6px;padding:8px 10px;background:rgba(52,211,153,.1);border:1px solid rgba(52,211,153,.3);border-radius:6px;font-size:11px;color:var(--g)">
            ✅ Ödeme kaydı yapıldığında bu tutar kadar limit serbest kalır ve kalan bakiye artar.
          </div>
        </div>
        <div><label class="flb">Tarih *</label><input class="fi2" type="date" id="kk-h-tarih"></div>
        <div><label class="flb" id="kk-h-firma-lbl">Harcama Yapılan Yer *</label><input class="fi2" id="kk-h-firma" placeholder="Market, Restoran..."></div>
        <div><label class="flb">Tutar (₺) *</label><input class="fi2" type="number" id="kk-h-tutar" placeholder="0.00" step="0.01"></div>
        <div id="kk-h-kategori-wrap"><label class="flb">Kategori</label>
          <select class="fi2" id="kk-h-kategori" style="width:100%">
            <option value="">—</option>
            <option>Market</option><option>Akaryakıt</option><option>Restoran/Kafe</option>
            <option>Fatura/Abonelik</option><option>Alışveriş</option><option>Sağlık</option>
            <option>Ulaşım</option><option>Konaklama</option><option>İş Gideri</option><option>Diğer</option>
          </select>
        </div>
        <div style="grid-column:1/-1"><label class="flb">Açıklama</label><input class="fi2" id="kk-h-aciklama" placeholder="Not..."></div>
      </div>
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="kkHarcamaKaydet()" style="flex:1" id="kk-h-kaydet-btn">💾 Kaydet</button>
        <button class="btn bo" onclick="kkHarcamaKapat()">İptal</button>
      </div>
    </div>
  </div>
...
<!-- KREDİ YÖNETİMİ VIEW -->
<div class="view" id="vw-taksitli" style="min-height:0">
  <div class="ptit">📋 KREDİ YÖNETİMİ</div>
  <div id="taksitli-ozet-bar" style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px"></div>
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
    <button class="btn bp bsm" onclick="taksitliYeniAc()">+ Yeni Kredi</button>
    <button class="btn bgh bsm" onclick="taksitliSablonIndir()">📥 Şablon İndir</button>
    <label class="btn bgh bsm" style="cursor:pointer;margin:0">
      ⬆ Excel Yükle
      <input type="file" id="taksitli-excel-input" accept=".xlsx,.xls" style="display:none" onchange="taksitliExcelYukle(this)">
    </label>
    <div style="flex:1"></div>
    <select class="fi" id="taksitli-filtre-firma" onchange="renderTaksitli()" style="font-size:12px">
      <option value="">Tüm Firmalar</option>
    </select>
    <select class="fi" id="taksitli-filtre-banka" onchange="renderTaksitli()" style="font-size:12px">
      <option value="">Tüm Bankalar</option>
    </select>
  </div>
  <div id="taksitli-list"></div>

  <!-- Kredi Ekle/Düzenle Modal -->
  <div class="ov" id="taksitli-mod" style="display:none">
    <div class="mod" style="max-width:700px;width:96%;max-height:92vh;overflow-y:auto">
      <div class="mtit"><span id="taksitli-mod-baslik">Yeni Taksitli Kredi</span>
        <button class="mclose" onclick="taksitliKapat()">✕</button></div>
      <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div><label class="flb">Firma *</label>
          <select class="fi2" id="tk-firma" style="width:100%">
            <option value="ULUSAL">ULUSAL</option><option value="ABC">ABC</option>
            <option value="BRG">BRG</option><option value="BRK">BRK</option><option value="DİĞER">DİĞER</option>
          </select></div>
        <div><label class="flb">Banka *</label>
          <select class="fi2" id="tk-banka" style="width:100%">
            <option value="">— Seçiniz —</option>
          </select></div>
        <div><label class="flb">Kredi No *</label>
          <input class="fi2" id="tk-kredi-no" placeholder="KRD-001" style="font-family:var(--mono)"></div>
        <div><label class="flb">Kredi Türü</label>
          <select class="fi2" id="tk-tur" style="width:100%" onchange="tkTurDegis()">
            <optgroup label="— Taksitli Krediler —">
              <option value="TAKSİTLİ KREDİ">TAKSİTLİ KREDİ</option>
              <option value="KONUT KREDİSİ">KONUT KREDİSİ</option>
              <option value="TAŞIT KREDİSİ">TAŞIT KREDİSİ</option>
              <option value="İHTİYAÇ KREDİSİ">İHTİYAÇ KREDİSİ</option>
            </optgroup>
            <optgroup label="— Döner Krediler —">
              <option value="ROTATİF">ROTATİF</option>
              <option value="KMH">KMH (Kredili Mevduat)</option>
              <option value="VİNOV">VİNOV (Spot Kredi)</option>
            </optgroup>
            <optgroup label="— Diğer —">
              <option value="TEMİNAT MEKTUBU">TEMİNAT MEKTUBU</option>
              <option value="DİĞER">DİĞER</option>
            </optgroup>
          </select></div>
        <div><label class="flb">Ana Para / Limit (₺) *</label>
          <input class="fi2" type="number" id="tk-ana-para" placeholder="1000000" oninput="taksitHesapla()"></div>
        <div><label class="flb">Faiz Oranı % (yıllık) *</label>
          <input class="fi2" type="number" id="tk-faiz" placeholder="45.00" step="0.01" oninput="taksitHesapla()"></div>

        <!-- Taksitli kredi alanları -->
        <div id="tk-alan-vade"><label class="flb">Vade (Ay) *</label>
          <input class="fi2" type="number" id="tk-vade" placeholder="36" min="1" max="360" oninput="taksitHesapla()"></div>
        <div id="tk-alan-kullanim-tarihi"><label class="flb" id="tk-kullanim-tarihi-lbl">Kullandırım Tarihi</label>
          <input class="fi2" type="date" id="tk-kullanim-tarihi" placeholder="Kredinin kullanıma alındığı tarih">
          <div id="tk-kullanim-tarihi-aciklama" style="font-size:11px;color:var(--t3);margin-top:2px"></div></div>
        <div id="tk-alan-baslangic"><label class="flb">İlk Taksit Tarihi *</label>
          <input class="fi2" type="date" id="tk-baslangic" oninput="taksitHesapla()"></div>
        <div id="tk-alan-bsmv"><label class="flb">BSMV % (varsayılan 5)</label>
          <input class="fi2" type="number" id="tk-bsmv" placeholder="5" step="0.01" value="5" oninput="taksitHesapla()"></div>

        <!-- Döner kredi özel alanları -->
        <div id="tk-alan-kullanim" style="display:none"><label class="flb">Kullanılan Tutar (₺)</label>
          <input class="fi2" type="number" id="tk-kullanim" placeholder="0" step="0.01"></div>
        <div id="tk-alan-vade-bit" style="display:none"><label class="flb">Vade / Bitiş Tarihi</label>
          <input class="fi2" type="date" id="tk-vade-bit"></div>
        <div id="tk-alan-yenileme" style="display:none"><label class="flb">Yenileme Sıklığı</label>
          <select class="fi2" id="tk-yenileme" style="width:100%">
            <option value="">— Seçiniz —</option>
        ...
        <div id="tk-alan-komisyon" style="display:none"><label class="flb">Komisyon % (yıllık)</label>
          <input class="fi2" type="number" id="tk-komisyon" placeholder="0.50" step="0.01"></div>

        <div style="grid-column:1/-1">
          <label class="flb">Açıklama</label>
          <input class="fi2" id="tk-aciklama" placeholder="Kredi açıklaması..."></div>
      </div>
      <!-- Hesaplanan taksit önizlemesi -->
      <div id="taksit-onizleme" style="padding:0 16px 12px;display:none">
        <div style="font-size:12px;font-weight:600;color:var(--acc);margin-bottom:8px">📊 Taksit Planı Önizlemesi</div>
        <div id="taksit-ozet-satir" style="display:flex;gap:16px;flex-wrap:wrap;font-size:12px;padding:8px 12px;background:var(--s3);border-radius:6px;margin-bottom:8px;font-family:var(--mono)"></div>
        <div style="max-height:240px;overflow-y:auto;border:1px solid var(--b1);border-radius:6px">
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead><tr style="background:var(--s2);position:sticky;top:0">
              <th style="padding:6px 8px;color:var(--t3);text-align:left">No</th>
              <th style="padding:6px 8px;color:var(--t3);text-align:left">Tarih</th>
              <th style="padding:6px 8px;color:var(--t3);text-align:right">Anapara</th>
              <th style="padding:6px 8px;color:var(--t3);text-align:right">Faiz</th>
              <th style="padding:6px 8px;color:var(--t3);text-align:right">BSMV</th>
              <th style="padding:6px 8px;color:var(--t3);text-align:right">Taksit</th>
            </tr></thead>
            <tbody id="taksit-onizleme-tbody"></tbody>
          </table>
        </div>
      </div>
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="taksitliKaydet()" style="flex:1">💾 Kaydet & Ödeme Tablosuna Aktar</button>
        <button class="btn bo" onclick="taksitliKapat()">İptal</button>
      </div>
    </div>
  </div>
</div>
...
<div class="view" id="vw-ipotek" style="min-height:0">
  ...
  <div class="tc-ipotek" style="max-height:calc(100vh - 310px);border-radius:10px;border:1px solid var(--b2)">
  <table style="border-collapse:collapse;font-size:11px;min-width:2400px;width:max-content">
    <thead style="position:sticky;top:0;z-index:5"><tr style="background:#1e3a5f;color:#fff">
...
<div class="ov" id="ipt-mod" style="display:none">
  ...
  <div class="mod" style="max-width:760px;width:96%;max-height:90vh;overflow-y:auto">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;padding:16px">
      ...
      <div><label class="flb">İpotek Bankası</label>
        <select class="fi2" id="ipt-banka" style="width:100%">
          <option value="">— Seçiniz —</option>
        </select>
      </div>
...
<div class="view" id="vw-ayarlar">
  <div class="ptit">⚙ AYARLAR</div>
  <div class="psub">Görünüm, hesap ve kullanıcı yönetimi</div>

  <!-- SEKMELER -->
  <div style="display:flex;gap:4px;margin-bottom:20px;border-bottom:1px solid var(--b1);padding-bottom:0">
    <button class="ay-tab on" id="aytab-gorunum"   onclick="ayTab('gorunum')">🎨 Görünüm</button>
    <button class="ay-tab"   id="aytab-hesap"      onclick="ayTab('hesap')">👤 Hesabım</button>
    <button class="ay-tab"   id="aytab-kullanici"  onclick="ayTab('kullanici')" id="aytab-kullanici-btn">👥 Kullanıcılar</button>
  </div>

  <!-- GÖRÜNÜM SEKMESİ -->
  <div class="ay-panel" id="ayp-gorunum">
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:16px;max-width:900px">

      <!-- Tema Önayarları -->
      <div class="src-card" style="grid-column:1/-1">
        <h3>🎨 Tema Önayarları</h3>
        <div style="display:flex;gap:10px;flex-wrap:wrap" id="tema-onayar-list">
          <button class="tema-btn" onclick="temayiUygula('koyu')"   style="--tc:#0d1117">🌑 Koyu (Varsayılan)</button>
          <button class="tema-btn" onclick="temayiUygula('gece')"   style="--tc:#090e18">🌙 Gece Mavisi</button>
          <button class="tema-btn" onclick="temayiUygula('orman')"  style="--tc:#0a1a0f">🌲 Orman</button>
          <button class="tema-btn" onclick="temayiUygula('bordo')"  style="--tc:#180a0a">🍷 Bordo</button>
          <button class="tema-btn" onclick="temayiUygula('gri')"    style="--tc:#111318">🩶 Çelik Gri</button>
          <button class="tema-btn" onclick="temayiUygula('mor')"    style="--tc:#110a1a">💜 Mor Gece</button>
        </div>
      </div>

      <!-- Arka Plan Renkleri -->
      <div class="src-card">
        <h3>🖼 Arka Plan & Panel Renkleri</h3>
        <div style="display:grid;gap:12px">
          <div class="ay-row"><label class="ay-lbl">Ana Arka Plan</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-bg"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-bg"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Kenar Çubuğu</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-s1"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-s1"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Panel (Açık)</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-s2"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-s2"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Panel (Koyu)</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-s3"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-s3"></span></div></div>
        </div>
      </div>

      <!-- Vurgu & Yazı Renkleri -->
      <div class="src-card">
        <h3>✨ Vurgu & Yazı Renkleri</h3>
        <div style="display:grid;gap:12px">
          <div class="ay-row"><label class="ay-lbl">Vurgu Rengi (Mavi)</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-acc" class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-acc"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Başlık Yazısı</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-t1"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-t1"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Alt Yazı</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-t2"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-t2"></span></div></div>
          <div class="ay-row"><label class="ay-lbl">Pasif Yazı</label><div style="display:flex;align-items:center;gap:8px"><input type="color" id="c-t3"  class="ay-color" oninput="canliOnizle()"><span class="ay-hex" id="h-t3"></span></div></div>
        </div>
      </div>
      ...
      <div class="src-card">
        ...
        <div style="display:grid;gap:14px">
          <div class="ay-row"><label class="ay-lbl">Genel Font Boyutu</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-fontsize" min="11" max="18" step="1" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-fontsize').textContent=this.value+'px'">
              <span id="v-fontsize" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:32px">14px</span>
            </div>
          </div>
          <div class="ay-row"><label class="ay-lbl">Menü Font Boyutu</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-navfont" min="10" max="15" step="1" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-navfont').textContent=this.value+'px'">
              <span id="v-navfont" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:32px">12px</span>
            </div>
          </div>
          <div class="ay-row"><label class="ay-lbl">Tablo Font Boyutu</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-tblfont" min="10" max="15" step="1" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-tblfont').textContent=this.value+'px'">
              <span id="v-tblfont" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:32px">12px</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Kenarlık & Köşe -->
      <div class="src-card">
        <h3>🔲 Kenarlık & Köşe Yuvarlama</h3>
        <div style="display:grid;gap:14px">
          <div class="ay-row"><label class="ay-lbl">Küçük Köşe Yarıçapı</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-rad" min="0" max="16" step="1" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-rad').textContent=this.value+'px'">
              <span id="v-rad" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:32px">6px</span>
            </div>
          </div>
          <div class="ay-row"><label class="ay-lbl">Büyük Köşe Yarıçapı</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-radl" min="0" max="20" step="1" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-radl').textContent=this.value+'px'">
              <span id="v-radl" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:32px">10px</span>
            </div>
          </div>
          <div class="ay-row"><label class="ay-lbl">Menü Genişliği</label>
            <div style="display:flex;align-items:center;gap:10px">
              <input type="range" id="r-sbarw" min="160" max="260" step="10" style="width:120px;accent-color:var(--acc)" oninput="canliOnizle();document.getElementById('v-sbarw').textContent=this.value+'px'">
              <span id="v-sbarw" style="font-family:var(--mono);font-size:12px;color:var(--acc);width:36px">200px</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Butonlar -->
      <div class="src-card" style="grid-column:1/-1;display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <button class="btn bp" onclick="gorunumKaydet()">💾 Kaydet</button>
        <button class="btn bo" onclick="gorunumSifirla()">↺ Sıfırla</button>
        <span id="gorunum-msg" style="font-size:12px;color:var(--g)"></span>
      </div>
  ...
  <!-- HESABIM SEKMESİ -->
  <div class="ay-panel" id="ayp-hesap" style="display:none">
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;max-width:700px">

      <!-- Profil Bilgileri -->
      <div class="src-card">
        <h3>👤 Profil Bilgileri</h3>
        <div style="display:grid;gap:12px">
          <div><label class="flb">Kullanıcı Adı</label><input class="fi2" id="hesap-uname" disabled style="opacity:.5;cursor:not-allowed"></div>
          <div><label class="flb">Ad Soyad</label><input class="fi2" id="hesap-ad" placeholder="Adınız Soyadınız"></div>
          <div><label class="flb">Rol</label><input class="fi2" id="hesap-rol" disabled style="opacity:.5;cursor:not-allowed"></div>
          <button class="btn bp bsm" onclick="profilKaydet()">💾 Profili Güncelle</button>
          <span id="profil-msg" style="font-size:12px;color:var(--g)"></span>
        </div>
      </div>
      ...
      <div class="src-card">
        ...
        <div style="display:grid;gap:12px">
          <div><label class="flb">Yeni Şifre</label><input class="fi2" id="hesap-sifre1" type="password" placeholder="min. 6 karakter"></div>
          <div><label class="flb">Tekrar</label><input class="fi2" id="hesap-sifre2" type="password" placeholder="tekrar girin"></div>
          <button class="btn bp bsm" onclick="hesapSifreDegistir()">🔒 Şifreyi Kaydet</button>
          <span id="sifre-msg" style="font-size:12px;color:var(--g)"></span>
        </div>
      </div>
  ...
  <div class="ay-panel" id="ayp-kullanici" style="display:none">
    <div style="max-width:800px">
      ...
      <div class="src-card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
          <div><label class="flb">Kullanıcı Adı *</label><input class="fi2" id="yeni-uname" placeholder="kullanici_adi"></div>
          <div><label class="flb">Ad Soyad</label><input class="fi2" id="yeni-ad" placeholder="Ali Yılmaz"></div>
          <div><label class="flb">Şifre *</label><input class="fi2" id="yeni-pw" type="password" placeholder="min. 6 karakter"></div>
          <div><label class="flb">Rol</label>
            <select class="fi2" id="yeni-rol">
              <option value="user">Kullanıcı</option>
              <option value="admin">Admin</option>
            </select>
          </div>
        </div>
        <button class="btn bp bsm" onclick="kullaniciEkle()">+ Kullanıcı Ekle</button>
        <span id="kullanici-ekle-msg" style="font-size:12px;margin-left:10px;color:var(--g)"></span>
      </div>

      <!-- Kullanıcı Listesi -->
      <div class="src-card">
        <h3>👥 Mevcut Kullanıcılar</h3>
        <div id="kullanici-liste"><div style="color:var(--t3);font-size:12px">Yükleniyor...</div></div>
      </div>
  ...
  <div class="mod">
    ...
    <div class="fg">
      ...
      <div class="fgr"><label class="flb">Ödeme Türü</label>
        ...
        <select class="fse" id="mtu"><option>TAKSİTLİ KREDİ</option><option>ROTATİF</option><option>KMH</option><option>VİNOV</option><option>TEMİNAT MEKTUBU</option><option>KONUT KREDİSİ</option><option>TAŞIT KREDİSİ</option><option>İHTİYAÇ KREDİSİ</option><option>ÇEK ÖDEMESİ</option><option>MAAŞ</option><option>CARİ ÖDEME</option><option>DİĞER</option></select></div>

      <div class="fgr ff"><label class="flb">Ödeme Yapılacak Yer / Banka</label>

        <input class="fi2" type="text" id="mye" placeholder="Banka seçin veya yazın..." list="mye-list" autocomplete="off" oninput="myeBankaSecildi()">
        <datalist id="mye-list"></datalist></div>
...
async function api(url,opts={}){
  ...
  // Token'ı sessionStorage'dan oku (HttpOnly cookie JS'den okunamaz)
  let tok = '';
  try { tok = sessionStorage.getItem('na_token') || ''; } catch(ex) {}
  if(!tok) {
    // Fallback: cookie'den oku (Secure olmayan ortamlarda çalışır)
    const cookieStr = document.cookie.split(';').map(c=>c.trim()).find(c=>c.startsWith('na_token='))||'';
    tok = cookieStr ? cookieStr.substring('na_token='.length) : '';
  }
  if(tok){
  ...
  if(resp.status===401 && url !== '/api/login'){
    console.warn('401 alındı:', url);
    // Token yenilemeyi dene
    try{
      const tr = await fetch('/api/token',{credentials:'include'});
      if(tr.ok){
        const td = await tr.json();
        if(td.token){
          sessionStorage.setItem('na_token', td.token);
          // Orijinal isteği tekrar dene
          if(!opts.headers) opts.headers={};
          opts.headers['X-Token'] = td.token;
          return fetch(url, opts);
        }
      }
    }catch(ex){ console.warn('Token yenileme başarısız:', ex); }
  }
...
async function loadStats(){
  ...
  const fm=n=>{n=parseFloat(n)||0;return'₺\u00a0'+n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});};
  ...
  // Kredi limitleri
  if(d.kredi_toplam_limit!==undefined){
    const klEl=document.getElementById('st-kredi-kalan');
    const kuEl=document.getElementById('st-kredi-kull');
    if(klEl) klEl.textContent=fm(d.kredi_kalan||0);
    if(kuEl) kuEl.textContent=fm(d.kredi_toplam_kull||0);
    const ksPct=d.kredi_toplam_limit>0?Math.round((d.kredi_toplam_kull||0)/d.kredi_toplam_limit*100):0;
    const ksEl=document.getElementById('st-kredi-pct');
    if(ksEl){ksEl.textContent=ksPct+'% kullanım';ksEl.style.color=ksPct>80?'var(--r)':ksPct>60?'var(--am)':'var(--g)';}
  }
...
function fmtM(n){ n=parseFloat(n)||0; if(n>=1e6) return (n/1e6).toFixed(2).replace('.',',')+' M ₺'; if(n>=1e3) return (n/1e3).toFixed(1).replace('.',',')+' B ₺'; return n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})+' ₺'; }
...
function populateFilters(firmas,turler,aylar,yillar){
  ...
  // Kredi/Çek No dropdown - allData'dan doldur
  const knSel = document.getElementById('fkn');
  if(knSel && allData?.length){
    const knler = [...new Set(allData.map(d=>d.kredi_no||'').filter(Boolean))].sort((a,b)=>a.localeCompare(b,'tr'));
    const curKn = knSel.value;
    knSel.innerHTML = '<option value="">Tüm Kredi/Çek No</option>' + knler.map(k=>`<option${k===curKn?' selected':''}>${k}</option>`).join('');
  }
...
function fmtNk(n){
  // Kuruşsuz format (tam sayılar için)
  n = Math.round(parseFloat(n)||0);
  return n.toLocaleString('tr-TR',{minimumFractionDigits:0,maximumFractionDigits:0});
}
function para(n){
  // ₺ işaretli tam format: 1.234.567,89
  n = parseFloat(n)||0;
  return '₺ '+n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
}
...
function renderTbl(){
  ...
  document.getElementById('tbody').innerHTML=filtered.slice(start,end).map(d=>{
    ...
    return`<tr>
      ...
      <td><input type="checkbox" class="tbl-sec-chk" data-idx="${idx}" onchange="tblSecDegis()"></td>

      <td><span class="bdg ${fc}">${d.firma}</span></td>

      <td class="clip" style="max-width:145px" title="${d.tur}">
        <span style="font-size:12px">${d.tur}</span>
        ${d.taksitli_id?`<span style="display:block;font-size:10px;color:var(--t3);font-family:var(--mono)">${d.aciklama||''}</span>`:''}
      </td>
      ...
      <td style="font-size:12px">${d.ay||'—'}<span style="font-family:var(--mono);font-size:12px;color:var(--t3);display:block;line-height:1">${d.tarih?d.tarih.slice(6,10):''}</span></td>

      <td><span class="bdg ${d.durum==='ÖDENDİ'?'bgi':'bri'}" 

        ondblclick="toggleStWithDate(${idx})"

        title="Çift tıkla: ÖDENDİ / ÖDENMEDİ değiştir"

        style="cursor:pointer;user-select:none" title="Çift tıkla: durum değiştir${d.odeme_tarihi?' · Ödeme: '+d.odeme_tarihi:''}">${d.durum||'—'}${d.durum==='ÖDENDİ'&&d.odeme_tarihi?`<span style='display:block;font-size:9px;color:var(--g);font-family:var(--mono)'>${d.odeme_tarihi}</span>`:''}</span></td>
      ...
      <td style="white-space:nowrap">
        ...
        <button class="btn bgh" style="color:var(--r)" onclick="${d.taksitli_id?'odemeTaksitSil':'delRec'}(${idx})" title="Sil">✕</button>
      </td>
  ...
  renderPgn();
  tblSecDegis(); // checkbox sayacını sıfırla
  renderTurOzet(); // Tür bazlı özet güncelle

}



function renderTurOzet(){
  const wrap    = document.getElementById('tur-ozet-wrap');
  const grid    = document.getElementById('tur-ozet-grid');
  const toplamDiv = document.getElementById('tur-ozet-toplam');
  const wrapTop = document.getElementById('tur-ozet-wrap-top');
  const gridTop = document.getElementById('tur-ozet-grid-top');
  const toplamTop = document.getElementById('tur-ozet-toplam-top');
  if(!wrap||!grid||!toplamDiv) return;
  if(!filtered.length){
  ...
  if(wrapTop) wrapTop.style.display='block';

  // Tür bazlı gruplama
  const gruplar = {};
  filtered.forEach(d=>{
    // Türü sadeleştir: "Taksitli Kredi Ödemesi" → "Taksitli Kredi" gibi kısa gruplar
    let tur = (d.tur||'Diğer').trim();
    // Benzer türleri birleştir
    if(/taksitli.*kred/i.test(tur)||/kred.*taksit/i.test(tur)) tur='Taksitli Kredi';
    else if(/rotati/i.test(tur)||/kmh/i.test(tur)) tur='Rotatif / KMH';
    else if(/maaş/i.test(tur)||/maas/i.test(tur)) tur='Maaş';
    else if(/çek/i.test(tur)||/cek.ode/i.test(tur)) tur='Çek Ödemesi';
    else if(/kira/i.test(tur)) tur='Kira';
    else if(/kredi.kart/i.test(tur)||/kk/i.test(tur)) tur='Kredi Kartı';
    else if(/vergi/i.test(tur)||/sgk/i.test(tur)||/sigorta/i.test(tur)) tur='Vergi / SGK';
    if(!gruplar[tur]) gruplar[tur]={tutar:0,anapara:0,faiz:0,sayi:0,odendi:0,bekliyor:0};
    gruplar[tur].tutar   += d.tutar||0;
    gruplar[tur].anapara += d.anapara||0;
    gruplar[tur].faiz    += d.faiz||0;
    gruplar[tur].sayi    ++;
    if(d.durum==='ÖDENDİ') gruplar[tur].odendi += d.tutar||0;
    else gruplar[tur].bekliyor += d.tutar||0;
  });

  const fmtN=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const renkler=['#1E3A5F','#0F6E56','#993C1D','#533AB7','#854F0B','#185FA5','#993556','#5F5E5A'];
  const turler = Object.entries(gruplar).sort((a,b)=>b[1].tutar-a[1].tutar);

  const _kartHtml = (tur,g,i,renk,pct) => `<div style="background:var(--s2);border:1px solid var(--b1);border-radius:8px;padding:10px 12px;border-left:3px solid ${renk}">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <span style="font-size:12px;font-weight:600;color:var(--t1)">${tur}</span>
        <span style="font-size:11px;color:var(--t3);background:var(--s3);padding:1px 6px;border-radius:10px">${g.sayi} kayıt</span>
      </div>
      <div style="font-size:13px;font-weight:700;font-family:var(--mono);color:${renk}">${fmtN(g.tutar)} ₺</div>
      <div style="margin-top:4px;height:4px;background:var(--s3);border-radius:2px">
        <div style="height:4px;background:${renk};border-radius:2px;width:${Math.min(100,pct).toFixed(1)}%;opacity:0.7"></div>
      </div>
      <div style="display:flex;gap:8px;margin-top:5px;font-size:11px;font-family:var(--mono)">
        ${g.anapara>0?`<span style="color:var(--t3)">A: ${fmtN(g.anapara)}</span>`:''}
        ${g.faiz>0?`<span style="color:var(--t3)">F: ${fmtN(g.faiz)}</span>`:''}
      </div>
      <div style="display:flex;gap:8px;margin-top:3px;font-size:11px">
        ${g.odendi>0?`<span style="color:#1D9E75">✓ ${fmtN(g.odendi)}</span>`:''}
        ${g.bekliyor>0?`<span style="color:#E24B4A">⏳ ${fmtN(g.bekliyor)}</span>`:''}
      </div>
    </div>`;

  const _gridHtml = turler.map(([tur,g],i)=>{
    const renk = renkler[i % renkler.length];
    const pct = g.tutar / filtered.reduce((s,d)=>s+(d.tutar||0),0) * 100;
    return _kartHtml(tur,g,i,renk,pct);
  }).join('');
  grid.innerHTML = _gridHtml;
  if(gridTop) gridTop.innerHTML = _gridHtml;

  // Genel toplam satırı
  const totTutar   = filtered.reduce((s,d)=>s+(d.tutar||0),0);
  const totAnapara = filtered.reduce((s,d)=>s+(d.anapara||0),0);
  const totFaiz    = filtered.reduce((s,d)=>s+(d.faiz||0),0);
  const totBsmv    = filtered.reduce((s,d)=>s+(d.bsmv||0),0);
  const totOdendi  = filtered.filter(d=>d.durum==='ÖDENDİ').reduce((s,d)=>s+(d.tutar||0),0);
  const totBekl    = filtered.filter(d=>d.durum!=='ÖDENDİ').reduce((s,d)=>s+(d.tutar||0),0);

  const _toplamHtml=`
    <span style="color:var(--t3)">${filtered.length} kayıt · ${turler.length} tür</span>
    <span style="color:var(--t2)">Anapara: <strong style="font-family:var(--mono)">${fmtN(totAnapara)} ₺</strong></span>
    <span style="color:var(--t2)">Faiz: <strong style="font-family:var(--mono)">${fmtN(totFaiz)} ₺</strong></span>
    ${totBsmv>0?`<span style="color:var(--t2)">BSMV: <strong>${fmtN(totBsmv)} ₺</strong></span>`:''}
    <span style="color:var(--t2)">💸 <strong style="color:var(--acc);font-size:13px">${fmtN(totTutar)} ₺</strong> toplam çıkış</span>
    <span style="margin-left:4px;padding-left:12px;border-left:1px solid var(--b1);color:#1D9E75">✅ ${fmtN(totOdendi)} ₺</span>
    <span style="color:#E24B4A">⏳ ${fmtN(totBekl)} ₺</span>
  `;
  toplamDiv.innerHTML = _toplamHtml;
  if(toplamTop) toplamTop.innerHTML = _toplamHtml;
}
...
function myeBankaListesiDoldur(turFiltre){
  const dl = document.getElementById('mye-list');
  if(!dl) return;
  // BANKA_KART_DATA'dan bankları çek - önce yüklenmiş mi kontrol et
  if(typeof _bkData !== 'undefined' && _bkData.length){
    const tur_map = {
      'TAKSİTLİ KREDİ ÖDEMESİ':'taksitli','ROTATİF':'rotatif','KMH':'kmh',
      'KREDİ KARTI ÖDEMESİ':'kredi_karti','TEMİNAT MEKTUBU':'teminat'
    };
    const alan = tur_map[turFiltre];
    const bankalar = _bkData
      .filter(b => !alan || parseFloat(b['limit_'+alan]||0) > 0)
      .map(b => b.banka).filter(Boolean);
    dl.innerHTML = [...new Set(bankalar)].map(b=>`<option value="${b}">`).join('');
  }
}

function myeBankaSecildi(){
  const tur = document.getElementById('mtu')?.value || '';
  myeBankaListesiDoldur(tur);
}

// ── ÖDEME TABLOSUNDAN TAKSİT DÜZENLE/SİL ──────────────────────────────────────
function odemeTaksitDuzenle(idx){
  const d = allData[idx];
  if(!d||!d.taksitli_id) return;
  // Kredi Yönetimi'ndeki taksit düzenleme modalını aç
  document.getElementById('ts-kredi-id').value = d.taksitli_id;
  document.getElementById('ts-taksit-no').value = d.taksit_no;
  document.getElementById('ts-no').value = d.taksit_no;
  // Tarih DD.MM.YYYY → YYYY-MM-DD
  try{const[g,m,y]=(d.tarih||'').split('.');document.getElementById('ts-tarih').value=`${y}-${m}-${g}`;}
  catch{document.getElementById('ts-tarih').value='';}
  document.getElementById('ts-anapara').value = d.anapara||0;
  document.getElementById('ts-faiz').value = d.faiz||0;
  document.getElementById('ts-bsmv').value = d.bsmv||0;
  document.getElementById('ts-tutar').value = d.tutar||0;
  // Kaydedince ödeme tablosunu da yenile
  _taksitSatirSonraReload = true;
  document.getElementById('taksit-satir-mod').style.display='flex';
}

let _taksitSatirSonraReload = false;

async function odemeTaksitSil(idx){
  const d = allData[idx];
  if(!d||!d.taksitli_id){delRec(idx);return;}
  if(!confirm(`${d.taksit_no}. taksit satırı silinecek. Emin misiniz?`)) return;
  try{
    const r=await api('/api/taksitli/taksit_satir_sil',{method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({kredi_id:d.taksitli_id,taksit_no:d.taksit_no})});
    const res=await r.json();
    if(res.ok){toast('Silindi','s');reload();}
    else toast('Hata: '+res.msg,'e');
...
async function saveRec(){
  ...
  if((await r.json()).ok){
    ...
    // Taksitli kredi satırı ise — değişiklikleri Kredi Yönetimi'ne de yansıt
    const mevcut = editIdx!==null ? allData[editIdx] : null;
    if(mevcut && mevcut.taksitli_id && mevcut.taksit_no){
      // Taksit satırı güncellendi — backend record/edit zaten DATA'yı güncelledi
      // Kredi Yönetimi'nde de güncelle (arka planda)
      api('/api/taksitli/taksit_satir_guncelle',{method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
          kredi_id: mevcut.taksitli_id,
          taksit_no: mevcut.taksit_no,
          tarih: rec.tarih,
          anapara: rec.anapara,
          faiz: rec.faiz,
          bsmv: rec.bsmv,
  ...
  for(const key of keys){
    ...
    if(g.yil!==prevYil){
      ...
      html+=`<div style="grid-column:1/-1;display:flex;align-items:baseline;gap:10px;padding:8px 0 4px;border-bottom:1px solid var(--b2);margin-bottom:2px">
        ...
        <span style="font-family:var(--mono);font-size:12px;color:var(--t3)">Toplam: ${fm(yilTot)}</span>
    ...
    html+=`<div class="mca${isActive?' sel':''}" onclick="showMoDet('${g.ay}','${g.yil}','${key}')">
      ...
      ${g.pend>0
        ...
        : '<div style="font-size:12px;color:var(--g);margin-top:3px;font-family:var(--mono)">✓ Tümü ödendi</div>'
...
async function showMoDet(ay, yil, key){
  ...
  const fm=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
      ...
      <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:11px">
        ...
        ${Object.entries(byFirma).sort((a,b)=>b[1]-a[1]).map(([k,v])=>{
          ...
          return `<div style="display:flex;align-items:center;gap:7px;margin-bottom:6px">
            ...
            <span style="font-size:12px;color:var(--t3);min-width:32px;text-align:right">${pctF}%</span>
      ...
      <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:11px">
        ...
        ${Object.entries(byTur).sort((a,b)=>b[1]-a[1]).map(([k,v])=>{
          ...
          return `<div style="display:flex;align-items:center;gap:7px;margin-bottom:6px">
            ...
            <span style="font-size:12px;color:var(--t2);min-width:90px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${k}">${k.length>14?k.slice(0,14)+'…':k}</span>
            ...
            <span style="font-size:12px;color:var(--t3);min-width:32px;text-align:right">${pctT}%</span>
...
function filterDetTable(){
  ...
  const totFaiz=filtered.reduce((s,d)=>s+(d.faiz||0),0);
  const totBsmv=filtered.reduce((s,d)=>s+(d.bsmv||0),0);
  const totOdendi=filtered.filter(d=>d.durum==='ÖDENDİ').reduce((s,d)=>s+d.tutar,0);
  const totBekliyor=filtered.filter(d=>d.durum!=='ÖDENDİ').reduce((s,d)=>s+d.tutar,0);
  ...
  if(footer) footer.innerHTML=`
    <div style="display:flex;flex-wrap:wrap;gap:18px;align-items:center;padding:2px 0">
      <span style="color:var(--t3)">${filtered.length} kayıt</span>
      <span style="color:var(--t2)">Anapara: <strong style="color:var(--t1);font-family:var(--mono)">${fmtN(totAnapara)} ₺</strong></span>
      <span style="color:var(--t2)">Faiz: <strong style="color:var(--t1);font-family:var(--mono)">${fmtN(totFaiz)} ₺</strong></span>
      ${totBsmv>0?`<span style="color:var(--t2)">BSMV: <strong style="color:var(--t1);font-family:var(--mono)">${fmtN(totBsmv)} ₺</strong></span>`:''}
      <span style="color:var(--t2)">💸 Toplam Nakit Çıkışı: <strong style="color:var(--acc);font-family:var(--mono);font-size:13px">${fmtN(tot)} ₺</strong></span>
      <span style="margin-left:4px;padding-left:14px;border-left:1px solid var(--b1);color:var(--t2)">✅ Ödendi: <strong style="color:var(--g);font-family:var(--mono)">${fmtN(totOdendi)} ₺</strong></span>
      <span style="color:var(--t2)">⏳ Bekliyor: <strong style="color:var(--am);font-family:var(--mono)">${fmtN(totBekliyor)} ₺</strong></span>
    </div>`;
...
function showView(v){
  ...
  if(v==='taksitli'){renderTaksitli();}

  if(v==='kullanicilar'){setTimeout(loadKullanicilar,100);}

  if(v==='ayarlar'){setTimeout(()=>{ayarGorunumDoldur();},100);}
...
async function reload(){
  ...
  try {
    ...
    const fm2=n=>{n=parseFloat(n)||0;return'₺\u00a0'+n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});};
...
function giderTarihSifirla(){ document.getElementById('ftarih-bas').value=''; document.getElementById('ftarih-bit').value=''; const fkn=document.getElementById('fkn'); if(fkn) fkn.value=''; doFilter(); }

function giderTumFiltreSifirla(){
  // Tüm filtreleri sıfırla
  ['fs'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  ['fay','ffr','ftr','fyer','fkn','fyil','fdu'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.value='';
  });
  document.getElementById('ftarih-bas').value='';
  document.getElementById('ftarih-bit').value='';
  doFilter();
  toast('Filtreler temizlendi','i');
}
...
async function renderNakit(){
  ...
  ozet.forEach((o,idx)=>{
    ...
    if(o.yil!==prevYil){
      ...
      rows.push(`<tr><td colspan="10" style="background:var(--s3);font-family:var(--mono);font-size:12px;color:var(--acc);font-weight:700;padding:6px 10px;border-bottom:1px solid var(--b2)">${o.yil}</td></tr>`);
...
async function toggleNakitDet(idx){
  ...
  const fmtR=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  if(o.gelir_kalemler&&o.gelir_kalemler.length>0){
    ...
    const gRows=o.gelir_kalemler.map((k,ki)=>{
      ...
      const satirRows=(k.satirlar||[]).map(s=>`<tr style="background:rgba(16,185,129,.04);border-bottom:1px solid rgba(16,185,129,.1)">
        ...
        <td colspan="2" style="padding:3px 8px 3px 28px;font-size:11px;color:var(--t2)">
          ...
          ${s.aciklama?`<span style="color:var(--t3);margin-left:6px;font-size:12px">${s.aciklama.slice(0,40)}</span>`:''}
      ...
      return `<tr style="border-bottom:1px solid var(--b1);cursor:${k.satirlar?.length?'pointer':'default'}"
        ...
        <td style="padding:5px 8px;font-size:12px">
          ...
          ${k.satirlar?.length?`<span id="${kId}-ic" style="color:var(--t3);font-size:12px;margin-right:4px">▶</span>`:''}${k.tur}
    ...
    gHtml=`<div style="padding:12px 12px 4px">
      ...
      <div style="font-family:var(--mono);font-size:12px;color:var(--g);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">● GELİR KALEMLERİ</div>

      <table style="width:100%;border-collapse:collapse;font-size:12px">

        <thead><tr style="background:rgba(16,185,129,.08)">

          <th style="padding:4px 8px;text-align:left;font-size:12px;color:var(--g-d);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Tür</th>

          <th style="padding:4px 8px;font-size:12px;color:var(--g-d);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Firma</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--g-d);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Adet</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--g-d);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Toplam ₺</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--g-d);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Tahsil ₺</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--am);font-family:var(--mono);border-bottom:1px solid rgba(16,185,129,.2)">Bekl. ₺</th>
  ...
  if(o.gider_kalemler&&o.gider_kalemler.length>0){
    ...
    const dRows=turList.map((t,ti)=>{
      ...
      const satirRows = allSatirlar.map((s,si)=>{
        ...
        return `<tr id="${rowId}" style="background:${isDone?'rgba(52,211,153,.04)':'rgba(248,113,113,.04)'};border-bottom:1px solid var(--b1)">
          ...
          <td style="padding:4px 8px"><span style="font-size:12px;color:${FC2[s.firma]||'var(--t2)'};">${s.firma||'—'}</span></td>
          ...
          <td style="padding:4px 8px;text-align:center">
            ...
            <button onclick="nakitToggleDurum(${s.rec_idx},'${rowId}')"
              ...
              style="font-size:12px;padding:2px 10px;border-radius:4px;border:none;cursor:pointer;font-weight:600;
      ...
      return `<tr style="border-bottom:1px solid var(--b1);cursor:${allSatirlar.length?'pointer':'default'}"
        ...
        <td style="padding:5px 8px;font-size:12px;font-weight:500">
          ...
          ${allSatirlar.length?`<span id="${kId}-ic" style="color:var(--t3);font-size:12px;margin-right:4px">▼</span>`:''}${t.tur}
        ...
        <td style="padding:5px 8px;text-align:right;font-family:var(--mono);font-size:12px;color:var(--t3)">${pct}%
    ...
    dHtml=`<div style="padding:4px 12px 12px">
      ...
      <div style="font-family:var(--mono);font-size:12px;color:var(--r);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">● GİDER KALEMLERİ</div>

      <table style="width:100%;border-collapse:collapse;font-size:12px">

        <thead><tr style="background:rgba(248,113,113,.08)">

          <th style="padding:4px 8px;text-align:left;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Ödeme Türü</th>

          <th style="padding:4px 8px;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Firma(lar)</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Adet</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Toplam ₺</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Ödenen ₺</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--am);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Bekl. ₺</th>

          <th style="padding:4px 8px;text-align:right;font-size:12px;color:var(--r-d);font-family:var(--mono);border-bottom:1px solid rgba(248,113,113,.2)">Pay</th>
...
async function renderLikidite(){
  ...
  liki.forEach((o,idx)=>{
    ...
    rows.push(`<tr style="${!o.pozitif?'background:rgba(248,113,113,.03)':''};cursor:${hasOner?'pointer':'default'}" onclick="${hasOner?`toggleLikiDet(${idx})`:''}">
      ...
      <td style="text-align:center;font-size:12px;color:var(--t3)" id="liki-exp-${idx}">${hasOner?'▶':''}</td>
...
function toggleLikiDet(idx){
  ...
  inner.innerHTML=`
    ...
    <div style="padding:14px 16px">
      ...
      <div style="font-family:var(--mono);font-size:12px;color:var(--acc);text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        ...
        <thead><tr style="background:rgba(79,156,249,.1)">
          ...
          <th style="padding:5px 9px;text-align:left;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">#</th>

          <th style="padding:5px 9px;text-align:left;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Firma</th>

          <th style="padding:5px 9px;text-align:left;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Banka</th>

          <th style="padding:5px 9px;text-align:left;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Kredi Türü</th>

          <th style="padding:5px 9px;text-align:left;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Kredi/Kart No</th>

          <th style="padding:5px 9px;text-align:right;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Kullanılacak ₺</th>

          <th style="padding:5px 9px;text-align:right;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Yıllık Faiz %</th>

          <th style="padding:5px 9px;text-align:right;font-size:12px;color:var(--am);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Tahmini Faiz ₺</th>

          <th style="padding:5px 9px;text-align:right;font-size:12px;color:var(--acc);font-family:var(--mono);border-bottom:1px solid rgba(79,156,249,.2)">Kalan Limit ₺</th>
        ...
        <tbody>
          ...
          ${o.oneriler.map((k,ki)=>`<tr style="border-bottom:1px solid var(--b1);background:${ki%2?'rgba(255,255,255,.01)':''}">
            ...
            <td style="padding:5px 9px;font-family:var(--mono);font-size:12px;color:var(--t3)">${ki+1}</td>
          ...
          <tr style="border-top:1px solid var(--b2);background:rgba(79,156,249,.05)">
            ...
            <td colspan="5" style="padding:5px 9px;font-family:var(--mono);font-size:12px;color:var(--t2)">TOPLAM KREDİ KULLANIMI</td>
...
let krediList=[];
let _krediSaveTimer=null;
function saveKrediDebounced(){ clearTimeout(_krediSaveTimer); _krediSaveTimer=setTimeout(()=>saveKredi(),800); }
...
function drawKrediTbl(){
  ...
  if(!krediList.length){
    ...
    tbody.innerHTML='<tr><td colspan="10" style="padding:20px;text-align:center;color:var(--t3);font-size:12px">Banka Kartları bölümünden bankalarınızı ekleyin — limitler otomatik buraya yansır</td></tr>';
...
async function renderFinans(){
  ...
  if(fnYil&&d.yillar){
    ...
    const curVals=[...fnYil.selectedOptions].map(o=>o.value);

    fnYil.innerHTML='';

    d.yillar.forEach(y=>{const o=document.createElement('option');o.value=y;o.text=y;if(curVals.includes(y))o.selected=true;fnYil.appendChild(o);});

  }

  if(fnAy){

    const curVals=[...fnAy.selectedOptions].map(o=>o.value);

    fnAy.innerHTML='';

    AYLAR.forEach(m=>{const o=document.createElement('option');o.value=m;o.text=m;if(curVals.includes(m))o.selected=true;fnAy.appendChild(o);});

  }

}

function fnFiltreTemizle(){
  const fnYil=document.getElementById('fn-yil');
  const fnAy=document.getElementById('fn-ay');
  if(fnYil){[...fnYil.options].forEach(o=>o.selected=false);}
  if(fnAy){[...fnAy.options].forEach(o=>o.selected=false);}
  const bas=document.getElementById('fn-tarih-bas');
  const bit=document.getElementById('fn-tarih-bit');
  if(bas) bas.value='';
  if(bit) bit.value='';
}
...
function updateBankaToplam(){
  ...
  if(el) el.textContent='₺ '+(parseFloat(top)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  list.innerHTML=krediData.map((k,i)=>`
    ...
    <div style="background:var(--s3);border:1px solid var(--b1);border-radius:var(--rad);padding:9px;margin-bottom:8px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:5px;align-items:center">
        ...
        <div>
          ...
          <input class="fi2" type="number" style="font-size:11.5px;padding:4px 7px;font-family:var(--mono)" value="${k.limit||0}"
            ...
            oninput="krediData[${i}].limit=parseFloat(this.value)||0;updateKrediToplam();saveKrediDebounced()">
        ...
        <div>
          ...
          <input class="fi2" type="number" style="font-size:11.5px;padding:4px 7px;font-family:var(--mono)" value="${k.kullanilan||0}"
            ...
            oninput="krediData[${i}].kullanilan=parseFloat(this.value)||0;updateKrediToplam();saveKrediDebounced()">
        ...
        <div>
          ...
          <div style="font-size:12px;font-family:var(--mono);color:var(--acc);padding:4px 7px;background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad)">
            ...
            ${((k.limit||0)-(k.kullanilan||0)).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})}
        ...
        <div>
          ...
          <input class="fi2" type="number" step="0.01" style="font-size:11.5px;padding:4px 7px;font-family:var(--mono)" value="${k.faiz||0}"
            ...
            oninput="krediData[${i}].faiz=parseFloat(this.value)||0;saveKrediDebounced()">
      ...
      <div style="margin-top:5px">
        ...
        <div style="font-size:12px;color:var(--t3);font-family:var(--mono);margin-top:2px">
...
function updateKrediToplam(){
  ...
  if(el) el.textContent='₺ '+(parseFloat(bos)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
...
async function hesaplaOneri(){
  ...
  const fnYil=document.getElementById('fn-yil');
  const fnAy=document.getElementById('fn-ay');
  const yillar=fnYil?[...fnYil.selectedOptions].map(o=>o.value).filter(Boolean):[];
  const aylar=fnAy?[...fnAy.selectedOptions].map(o=>o.value).filter(Boolean):[];
  const tarihBas=document.getElementById('fn-tarih-bas')?.value||'';
  const tarihBit=document.getElementById('fn-tarih-bit')?.value||'';

  const btn=document.querySelector('#vw-finans .btn.bp');
  if(btn){btn.textContent='⏳';btn.disabled=true;}

  const params=new URLSearchParams();
  if(aylar.length)  params.set('aylar',aylar.join(','));
  if(yillar.length) params.set('yillar',yillar.join(','));
  if(tarihBas)      params.set('tarih_bas',tarihBas);
  if(tarihBit)      params.set('tarih_bit',tarihBit);
  ...
  if(tarihBas||tarihBit){
    ...
    const yStr=yillar.length?yillar.join(', '):'Tüm yıllar';
    const aStr=aylar.length?aylar.join(', '):'Tüm aylar';
    donemEtiketi=`📅 ${yStr} · ${aStr}`;
  }

  document.getElementById('fn-mets').innerHTML=`
    <div style="grid-column:1/-1;font-size:11px;color:var(--acc);font-weight:600;padding:2px 0 6px">${donemEtiketi}</div>
    <div class="met"><div class="met-stripe" style="background:var(--r)"></div>
    ...
    <div class="met"><div class="met-stripe" style="background:var(--acc2)"></div>
      <div class="mlb">Kullanılabilir Limit</div>
      <div class="mvl" style="color:var(--acc2);font-size:15px">₺${fmtN(d.kullanilabilir_toplam)}</div></div>
    ${d.net_acik>0?`
    <div class="met"><div class="met-stripe" style="background:var(--am)"></div>
      <div class="mlb">Tahmini Aylık Faiz</div>
      <div class="mvl" style="color:var(--am);font-size:15px">₺${fmtN(d.tahmini_maliyet)}</div></div>
    <div class="met"><div class="met-stripe" style="background:var(--r)"></div>
      <div class="mlb">Tahmini Yıllık Faiz</div>
      <div class="mvl" style="color:var(--r);font-size:15px">₺${fmtN(d.tahmini_yillik||d.tahmini_maliyet*12)}</div></div>`:''}
  `;
  ...
  if(d.oneriler&&d.oneriler.length>0){
    ...
    onHtml=`<div style="font-size:12px;font-weight:600;color:var(--t1);margin-bottom:8px;display:flex;flex-wrap:wrap;gap:12px">
      <span>Önerilen Plan — <span style="color:var(--acc)">₺${fmtN(d.toplam_kullanilacak)}</span> kullanım</span>
      <span>Aylık Faiz: <span style="color:var(--am)">₺${fmtN(d.tahmini_maliyet)}</span></span>
      <span>Yıllık Faiz: <span style="color:var(--r)">₺${fmtN(d.tahmini_yillik||d.tahmini_maliyet*12)}</span></span>
    </div>

    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:8px;margin-bottom:10px">

      ${d.oneriler.map(o=>{
        const faizAylik = o.faiz_yuzde || (o.faiz < 1 ? o.faiz*100 : o.faiz);
        const faizYillik = o.faiz_yillik || faizAylik*12;
        const kaynak = o.kaynak==='banka_kart' ? '🏦 Banka Kartı' : '📋 Kredi Listesi';
        const renkBorder = o.kaynak==='banka_kart' ? 'var(--g)' : 'var(--acc)';
        return `
        <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:11px;border-left:3px solid ${renkBorder}">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
            <div style="font-family:var(--mono);font-size:11px;color:var(--acc)">ÖNCELİK ${o.oncelik}</div>
            <span style="font-size:10px;color:var(--t3);font-family:var(--mono)">${kaynak}</span>
          </div>
          <div style="font-weight:600;font-size:13px;margin-bottom:1px">${o.banka}</div>
          <div style="font-size:11px;color:var(--t2);margin-bottom:8px">${o.tur} · <span style="color:var(--acc2)">${o.firma}</span></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-family:var(--mono);font-size:11px;background:var(--s3);border-radius:var(--rad);padding:8px">
            <span style="color:var(--t3)">Kullanılacak</span>
            <span style="color:var(--acc);font-weight:600;text-align:right">₺${fmtN(o.kullan)}</span>
            <span style="color:var(--t3)">Kalan limit</span>
            <span style="color:var(--t2);text-align:right">₺${fmtN(o.bos)}</span>
            <span style="color:var(--t3)">Faiz (aylık)</span>
            <span style="color:var(--t1);text-align:right">%${(faizAylik).toFixed(2)}</span>
            <span style="color:var(--t3)">Faiz (yıllık)</span>
            <span style="color:var(--t1);text-align:right">%${(faizYillik).toFixed(2)}</span>
            <span style="color:var(--t3)">Aylık maliyet</span>
            <span style="color:var(--am);font-weight:600;text-align:right">₺${fmtN(o.aylik_faiz)}</span>
            <span style="color:var(--t3)">Yıllık maliyet</span>
            <span style="color:var(--r);text-align:right">₺${fmtN(o.yillik_faiz_tutar||o.aylik_faiz*12)}</span>
          </div>
          <div style="margin-top:7px;height:3px;background:var(--b1);border-radius:2px;overflow:hidden">
            <div style="height:100%;width:${o.bos>0?Math.min(100,Math.round(o.kullan/o.bos*100)):100}%;background:${renkBorder};border-radius:2px"></div>
          </div>
          <div style="font-size:10.5px;color:var(--t3);font-family:var(--mono);margin-top:2px">
  ...
  // Tüm kullanılabilir krediler tablosu - veriler zaten d içinde
  const allD = d;

  const ktDiv=document.getElementById('fn-kredi-tablo');
  const tumLimitler = (d.kullanilabilir||[]).filter(k=>k.bos>0);

  if(tumLimitler.length>0){
    ktDiv.innerHTML=`
      <div style="font-size:12px;font-weight:600;color:var(--t2);margin-bottom:7px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        <span>Tüm Kullanılabilir Limitler — Düşük Faizden Yükseğe</span>
        <span style="font-weight:400;color:var(--t3);font-size:11px">🏦 Banka Kartları (tüm kredi türleri) &nbsp;|&nbsp; 📋 Manuel Kredi Listesi</span>
      </div>
      <div class="tc">
        <table style="font-size:12px">
          <thead><tr>
            <th>Banka</th><th>Firma</th><th>Tür</th><th>Kaynak</th>
            <th class="num">Toplam Limit ₺</th>
            <th class="num">Kullanılan ₺</th>
            <th class="num">Boş Limit ₺</th>
            <th class="num">Faiz % / ay</th>
            <th class="num">Faiz % / yıl</th>
            <th class="num">Aylık Maliyet ₺</th>
            <th class="num">Yıllık Maliyet ₺</th>
          </tr></thead>
          <tbody>
            ${tumLimitler.map(k=>{
              const faizAy = k.faiz<1 ? k.faiz*100 : k.faiz;
              const faizYil = k.faiz_yillik || faizAy*12;
              const ayMal = k.bos * faizAy / 100;
              const yilMal = ayMal * 12;
              const isBK = k.kaynak==='banka_kart';
              const kaynak = isBK
                ? '<span style="color:var(--g);font-size:10px;font-weight:600">🏦 Banka Kartı</span>'
                : '<span style="color:var(--acc);font-size:10px;font-weight:600">📋 Kredi Listesi</span>';
              return `<tr>
                <td><strong>${k.banka}</strong></td>
                <td><span class="bdg ${FC[k.firma]||'bbi'}">${k.firma||'—'}</span></td>
                <td style="font-size:11px;color:var(--t2)">${k.tur}</td>
                <td style="text-align:center">${kaynak}</td>
                <td class="num" style="font-family:var(--mono);color:var(--t2)">${fmtN(k.limit)}</td>
                <td class="num" style="font-family:var(--mono);color:var(--r)">${fmtN(k.kullanilan)}</td>
                <td class="num" style="color:var(--acc);font-family:var(--mono);font-weight:700">${fmtN(k.bos)}</td>
                <td class="num" style="font-family:var(--mono)">${faizAy.toFixed(2)}%</td>
                <td class="num" style="font-family:var(--mono)">${faizYil.toFixed(2)}%</td>
                <td class="num" style="color:var(--am);font-family:var(--mono)">${fmtN(ayMal)}</td>
                <td class="num" style="color:var(--r);font-family:var(--mono)">${fmtN(yilMal)}</td>
              </tr>`;}).join('')}
          </tbody>
          <tfoot><tr style="background:var(--s3);font-weight:700">
            <td colspan="6" style="padding:7px 10px;color:var(--t2);font-size:11px">TOPLAM KULLANILABİLİR LİMİT</td>
            <td class="num" style="color:var(--g);font-family:var(--mono);font-size:13px">₺${fmtN(tumLimitler.reduce((s,k)=>s+k.bos,0))}</td>
            <td colspan="4"></td>
          </tr></tfoot>
        </table>
      </div>`;
...
async function renderCariOzet(){
  ...
  tbody.innerHTML = ozet.map(o=>{
    ...
    return `<tr style="cursor:pointer" onclick="filterDetayByFirma('${o.cari_firma}')">
      ...
      <td><span class="bdg ${FC[o.grup_firma]||'bbi'}" style="font-size:12px">${o.grup_firma||'—'}</span></td>
...
async function renderCariDetay(){
  ...
  document.getElementById('cari-detay-tbody').innerHTML=page.map(rec=>{
    ...
    return `<tr>
      ...
      <td><span class="bdg ${FC[rec.grup_firma]||'bbi'}" style="font-size:12px">${rec.grup_firma||'—'}</span></td>
...
async function importBankaExcel(){
  ...
  try {
    ...
    const r = await api('/api/banka/import_excel', {method:'POST', body: formData, credentials:'include'});
...
function kurOncekiIsGunuDene(){
  // Seçili tarihten geriye git, iş günü bul
  const inp = document.getElementById('kur-tarih');
...
async function kurCek(){
  ...
  try {
    ...
    infoEl.innerHTML=`
      <span style="color:${d.uyari?'var(--am)':'var(--g)'}">${d.uyari?'⚠ ECB':'✓ TCMB'}</span>
      <span style="color:var(--t2)">${tarih} tarihi kurları</span>
      <span style="color:var(--t3)">${Object.keys(kurlar).length} para birimi</span>
      ${d.from_cache?'<span style="color:var(--am);font-size:12px">📦 Önbellekten</span>':''}
      ${d.uyari?`<span style="color:var(--am);font-size:11px">⚠ ${d.uyari}</span>`:''}
    `;
      ...
      .map(k => {
        ...
        return `<div class="met" style="cursor:default">
          ...
          <div style="font-family:var(--mono);font-size:11px;font-weight:700;color:${renk};margin-bottom:6px">${k}
            ...
            ${kur.birim>1?`<span style="font-size:12px;color:var(--t3)">(${kur.birim} birim)</span>`:''}

          </div>

          <div style="font-size:12px;color:var(--t3);margin-bottom:8px">${kur.isim||''}</div>
          ...
          <div style="font-size:12px;color:var(--t3);margin-top:3px;font-family:var(--mono)">
...
function renderLogoTablo(){
  ...
  const firmaOzetHTML = Object.entries(firmaOzet).map(([f,v])=>
    ...
    `<span style="font-size:12px;color:var(--t3)">${f}: <strong>₺${fmtN(v)}</strong></span>`).join(' · ');
  ...
  document.getElementById('logo-tbody').innerHTML = page.map((r,i)=>{
    ...
    return `<tr style="${isIade?'opacity:.7':''}">
      ...
      <td style="text-align:center;font-size:12px;color:var(--t3);cursor:pointer" onclick="logoDetay('${r.id}','${r.fatura_no}')" title="Kalemleri gör">▶</td>
      ...
      <td style="font-size:12px;color:var(--t3)" class="clip" title="${r.logo_firma_adi||''}">${r.logo_firma_adi||r.logo_firma_no||'—'}</td>
      ...
      <td><span class="bdg ${isIade?'bri':'bgi'}" style="font-size:12px">${r.tip_adi||'—'}</span></td>
...
async function logoDetay(id, faturaNo){
  ...
  detInner.innerHTML=`
    ...
    <div style="font-family:var(--mono);font-size:12px;color:var(--acc);margin-bottom:8px">📋 ${faturaNo} — Fatura Kalemleri</div>

    <table style="width:100%;border-collapse:collapse;font-size:11.5px">

      <thead><tr style="background:var(--s3)">

        <th style="padding:4px 8px;text-align:left;color:var(--t3);font-size:12px">Stok Kodu</th>

        <th style="padding:4px 8px;text-align:left;color:var(--t3);font-size:12px">Stok Adı</th>

        <th style="padding:4px 8px;text-align:right;color:var(--t3);font-size:12px">Miktar</th>

        <th style="padding:4px 8px;text-align:left;color:var(--t3);font-size:12px">Birim</th>

        <th style="padding:4px 8px;text-align:right;color:var(--t3);font-size:12px">Birim Fiyat ₺</th>

        <th style="padding:4px 8px;text-align:right;color:var(--t3);font-size:12px">KDV %</th>

        <th style="padding:4px 8px;text-align:right;color:var(--t3);font-size:12px">Net Tutar ₺</th>
...
function renderStokTablo(){
  ...
  document.getElementById('stok-tbody').innerHTML=page.map(s=>`<tr>
    ...
    <td style="font-size:12px;color:var(--t3)">${s.logo_firma_adi||s.logo_firma_no||'—'}</td>
...
function renderCariLogoTablo(){
  ...
  document.getElementById('cari-logo-tbody').innerHTML=page.map(c=>`<tr>
    ...
    <td><span class="bdg ${tipRenk[c.cari_tipi]||'bbi'}" style="font-size:12px">${(c.cari_tipi||'—').slice(0,8)}</span></td>

    <td style="font-size:12px;color:var(--t3)">${c.logo_firma_adi||'—'}</td>
...
async function gmailYukle(){
  ...
  document.getElementById('gmail-tbody').innerHTML=_gmailMails.map(m=>`
    ...
    <tr style="cursor:pointer;${m.okunmadi?'font-weight:600':''}" onclick="gmailOku('${m.id}')">
      ...
      <td style="text-align:center;width:20px">${m.okunmadi?'<span style="color:var(--acc);font-size:11px">●</span>':''}</td>
...
function _renderNotlar(){
  ...
  document.getElementById('not-listesi').innerHTML=filtered.map(n=>{
    ...
    const guncTag = n.guncellendi
      ...
      ? `<span style="color:var(--am);font-size:12px" title="Güncellendi: ${n.guncellendi}">✎</span>`
    ...
    return `<div style="background:${r.bg};border:1px solid ${r.border};border-radius:var(--radl);padding:14px;position:relative;display:flex;flex-direction:column;gap:0;transition:box-shadow .15s" onmouseenter="this.style.boxShadow='0 4px 16px rgba(0,0,0,.25)'" onmouseleave="this.style.boxShadow='none'">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        <div style="font-size:12px;color:var(--t3);font-family:var(--mono)">${n.tarih||''} ${guncTag}</div>
...
async function dbDurumYukle(){
  ...
  document.getElementById('db-status-grid').innerHTML=`
    ...
    <div class="met"><div class="met-stripe" style="background:var(--acc)"></div>
      ...
      <div class="mvl" style="color:var(--acc);font-size:20px">${(parseFloat(toplam)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
  ...
  document.getElementById('db-tablo-detay').innerHTML = Object.entries(tblMap).map(([k,v])=>`
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:12px 14px;display:flex;align-items:center;gap:12px">
      ...
      <button class="btn bgh bsm" onclick="dbHazirSorgu('SELECT * FROM ${k} LIMIT 50')" style="font-size:12px">Gör</button>
  ...
  document.getElementById('otel-bugun-checkin-card').innerHTML=`
    ...
    ${d.bugun_checkin_list.length
      ...
      ? d.bugun_checkin_list.map(r=>`<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--b1);font-size:12px">
          ...
          <div><strong>${r.misafir_adi||'—'}</strong> <span style="color:var(--t3);font-size:12px">Oda ${r.oda||'?'}</span></div>

          <div style="display:flex;gap:6px;align-items:center">

            <span style="font-size:12px;color:var(--t3)">${r.kanal||''}</span>

            ${r.telefon?`<button onclick="waHizliAc('${r.telefon}','${r.misafir_adi}')" class="btn bgh" style="font-size:12px;padding:2px 6px">💬</button>`:''}
  ...
  Object.entries(aylar).forEach(([ay, gunler])=>{
    ...
    gunler.forEach(g=>{
      ...
      html += `<div title="${g.tarih}: %${oran} dolu (${g.dolu}/${odaSayisi} oda)"
        ...
        <div style="font-size:12px;font-weight:600;color:#fff;line-height:1">${gunNo}</div>

        <div style="font-size:11px;color:rgba(255,255,255,.8);line-height:1">${oran>0?'%'+oran:''}</div>
...
function renderOtelTablo(){
  ...
  document.getElementById('otel-tbody').innerHTML=page.map(r=>{
    ...
    return `<tr>
      ...
      <td style="font-size:11px"><span class="bdg bbi" style="font-size:12px">${r.kanal||'—'}</span></td>

      <td class="num" style="font-family:var(--mono);font-size:12px;color:var(--g)">${fmtN(parseFloat(r.tutar||0))}</td>

      <td><span class="bdg ${durumRenk[r.durum]||'bbi'}" style="font-size:12px;cursor:pointer" ondblclick="otelDurumToggle(${r.id})">${r.durum||'—'}</span></td>
...
async function otelExcelImport(){
  ...
  const r = await api('/api/banka/import_ekstre',{method:'POST',body:formData,credentials:'include'});
...
function waGonderRender(){
  ...
  document.getElementById('wa-hizli-sablon').innerHTML=_waSablonlar.slice(0,6).map(s=>`
    ...
    <button class="btn bgh" onclick="waSablonHizliUygula(${s.id})"
      ...
      <span style="font-size:12px;color:var(--t3);margin-right:6px">${s.kategori}</span>${s.ad}
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--radl);padding:12px">
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:6px">
        ...
        <div>
          ...
          <span class="bdg bbi" style="font-size:12px;margin-left:6px">${s.kategori}</span>

        </div>

        <div style="display:flex;gap:4px">

          <button class="btn bgh" onclick="waSablonKullan(${s.id})" style="font-size:12px;padding:2px 6px">Kullan</button>

          <button class="btn bgh" onclick="waSablonDuzenle(${s.id})" style="font-size:12px;padding:2px 6px">✏</button>

          <button class="btn bgh" onclick="waSablonSil(${s.id})" style="font-size:12px;padding:2px 6px;color:var(--r)">✕</button>
  ...
  document.getElementById('wa-kisi-listesi').innerHTML=filtered.map(k=>`
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:10px 12px;display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:1;min-width:0">
        ...
        ${k.grup?`<span class="bdg bbi" style="font-size:12px">${k.grup}</span>`:''}
      ...
      <div style="display:flex;gap:4px;flex-shrink:0">
        ...
        <button class="btn bgh" onclick="waKisiDuzenle('${k.telefon}')" style="font-size:12px;padding:3px 6px">✏</button>

        <button class="btn bgh" onclick="waKisiSilFn('${k.telefon}')" style="font-size:12px;padding:3px 6px;color:var(--r)">✕</button>
...
function waSohbetListeRender(){
  ...
  if(!girdi.length){
    ...
    listEl.innerHTML='<div style="font-size:12px;color:var(--t3);padding:24px;text-align:center">'+(Object.keys(_waSohbetler).length===0?'💬 Henüz mesaj yok<br><span style="font-size:12px;margin-top:6px;display:block">Bağlantı sekmesinden token girip mesajlaşmaya başlayın</span>':'🔍 Arama sonucu bulunamadı')+'</div>';
      ...
      <div style="display:flex;align-items:flex-start;gap:8px">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
            ...
            <div style="font-size:12px;color:${okunmadi?'var(--g)':'var(--t3)'};flex-shrink:0;margin-left:4px">${zaman}</div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center">
            ...
            ${okunmadi?`<span style="background:var(--g);color:#000;border-radius:10px;font-size:12px;padding:1px 6px;font-weight:700;flex-shrink:0">${okunmadi}</span>`:''}
...
function waMesajlarRender(msgs, baslik){
  ...
  listEl.innerHTML=sorted.map(m=>{
    ...
    if(tarih&&tarih!==lastDate){ lastDate=tarih; dateSep=`<div style="text-align:center;margin:10px 0"><span style="background:var(--s3);color:var(--t3);font-size:12px;padding:3px 12px;border-radius:10px">${tarih}</span></div>`; }
    ...
    return dateSep+`<div style="display:flex;justify-content:${benden?'flex-end':'flex-start'}">
      ...
      <div style="max-width:72%;background:${benden?'#1a4a8a':'var(--s3)'};border-radius:${benden?'14px 14px 4px 14px':'14px 14px 14px 4px'};padding:9px 13px;box-shadow:0 1px 3px rgba(0,0,0,.2)">
        ...
        ${!benden?'<div style="font-size:12px;color:var(--acc);font-weight:600;margin-bottom:3px">'+(m.from_name||baslik)+'</div>':''}

        <div style="font-size:13px;color:var(--t1);word-break:break-word;line-height:1.5">${metin}</div>

        <div style="font-size:12px;color:${benden?'rgba(255,255,255,.5)':'var(--t3)'};margin-top:4px;text-align:right">${zaman}${benden?' ✓✓':''}</div>
...
async function renderHatirlatma(){
  ...
  if(nvHat){
    ...
    if(d.acil>0){
      ...
      nvHat.innerHTML=`🔔 HATIRLATICI <span style="background:var(--r);color:#fff;border-radius:10px;padding:1px 6px;font-size:12px;margin-left:4px">${d.acil}</span>`;
        ...
        <div style="display:flex;align-items:center;gap:10px;background:var(--s2);border:1px solid ${h.gecti&&!h.tamamlandi?'rgba(239,68,68,.3)':'var(--b1)'};border-radius:var(--rad);padding:10px 12px;margin-bottom:6px;${h.tamamlandi?'opacity:.55':''}">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
              ...
              <span style="color:${oncelikRenk};font-size:12px">${oncelikIcon}</span>
          ...
          <div style="display:flex;gap:4px;flex-shrink:0">
            ...
            <button class="btn bgh" onclick="hatDuzenle(${h.id})" style="font-size:12px;padding:3px 7px">✏</button>

            <button class="btn bgh" onclick="hatSil(${h.id})" style="font-size:12px;padding:3px 7px;color:var(--r)">✕</button>
...
function saUrunRender(){
  ...
  document.getElementById('sa-urun-tbody').innerHTML = page.map((u,i)=>{
    ...
    return `<tr>
      ...
      <td style="font-size:12px;color:var(--t3)">${u.grup||'—'}</td>

      <td style="font-size:12px;color:var(--t3)">${u.firma}</td>
      ...
      <td class="num">
        ...
        <div style="display:flex;align-items:center;gap:4px">
          ...
          <span style="font-size:12px;font-family:var(--mono);width:32px;text-align:right">%${pay}</span>
...
function saMarjRender(){
  ...
  document.getElementById('sa-marj-tbody').innerHTML=page.map(r=>{
    ...
    return `<tr style="background:${marjBg}">
      ...
      <td style="font-size:12px;color:var(--t3)">${r.logo_firma_adi||'—'}</td>
...
function saStokRender(){
  ...
  document.getElementById('sa-stok-tbody').innerHTML=page.map(r=>{
    ...
    return `<tr style="${bak<0?'background:rgba(239,68,68,.06)':''}">
      ...
      <td style="font-size:12px;color:var(--t3)">${r.stok_grubu||'—'}</td>
      ...
      <td><span class="bdg ${durumCls}" style="font-size:12px">${durumTxt}</span></td>
...
async function renderDocs(){
  ...
  try { await api('/api/docs/temizle_duplicate',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}); } catch(e){}
...
async function docsListeYukle(){
  ...
  try {
    ...
    const r = await api('/api/docs/list?q='+encodeURIComponent(q)+'&kategori='+encodeURIComponent(kat));
...
function docsStatsGuncelle(toplam, docs){
  ...
  ].map(([ic,lb,vl,cl])=>'<div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:8px 14px;display:flex;align-items:center;gap:10px"><span style="font-size:20px">'+ic+'</span><div><div style="font-size:12px;color:var(--t3)">'+lb+'</div><div style="font-size:14px;font-weight:700;color:'+cl+'">'+vl+'</div></div></div>').join('');
...
function docsGridRender(docs){
  ...
  docs.forEach(d=>{
    ...
    div.innerHTML =
      ...
      +'<div style="margin-top:6px"><span style="background:var(--s4);color:var(--acc);font-size:12px;padding:2px 8px;border-radius:10px">'+(d.kategori||'Genel')+'</span></div>'

      +(d.not?'<div style="font-size:12px;color:var(--t3);margin-top:4px;font-style:italic">'+d.not.slice(0,40)+'</div>':'')

      +'<div style="font-size:12px;color:var(--t3);margin-top:6px">'+fmtB(d.boyut)+' · '+(d.tarih||'').slice(0,10)+'</div>'
...
async function docsYukle(files){
  ...
  for(let i=0;i<files.length;i++){
    ...
    try {
      ...
      // sessionStorage'dan token oku (HttpOnly cookie JS'den okunamaz)
      let tokenVal = '';
      try { tokenVal = sessionStorage.getItem('na_token') || ''; } catch(ex) {}
      if(!tokenVal) {
        const tokenCookie = document.cookie.split(';').map(c=>c.trim()).find(c=>c.startsWith('na_token='));
        tokenVal = tokenCookie ? tokenCookie.split('=').slice(1).join('=') : '';
      }
      if(tokenVal) fd.append('_token', tokenVal);

      const r = await fetch('/api/docs/yukle',{method:'POST',body:fd,credentials:'include',headers:tokenVal?{'X-Token':tokenVal}:{}});
...
async function docsEditKaydet(){
  ...
  try {
    ...
    const r=await api('/api/docs/not_guncelle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:_docsEditId,ad,not:nt,kategori:kat})});
...
async function docsSil(docId, ad){
  ...
  try {
    ...
    const r=await api('/api/docs/sil',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:docId})});
...
async function initApp(){
  ...
  // sessionStorage'da token yoksa /api/token'dan al
  try{
    const stored = sessionStorage.getItem('na_token');
    if(!stored){
      const tr = await fetch('/api/token', {credentials:'include'});
      if(tr.ok){ const td = await tr.json(); if(td.token) sessionStorage.setItem('na_token', td.token); }
    }
  }catch(ex){}

  try {

    const fm=function(n){n=parseFloat(n)||0;return'₺\u00a0'+n.toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});};
    ...
    // Ayarlar sayfası - kullanıcılar sekmesi admin kontrolü
    setTimeout(async function(){
      try{
        const rm=await api('/api/me', {credentials:'include'}); const dm=await rm.json();
        if(dm.ok){
          _meData=dm;
          // Admin değilse kullanıcılar sekmesini gizle
          const kulTab=document.getElementById('aytab-kullanici');
          const kulPan=document.getElementById('ayp-kullanici');
          if(dm.rol!=='admin'){
            if(kulTab) kulTab.style.display='none';
            if(kulPan) kulPan.style.display='none';
          }
          // İzin kısıtlaması uygula
          if(dm.izinler && Array.isArray(dm.izinler)){
            document.querySelectorAll('.nav[id^=nv-]').forEach(el=>{
              const vid=el.id.replace('nv-','');
              if(!dm.izinler.includes(vid)) el.style.display='none';
            });
          }
        }
      }catch(e){}
...
// Geri tuşuyla login'e dönmeyi engelle
...
function bkListeRender(){
  ...
  if(!_bkFiltered.length){
    container.innerHTML='<div style="color:var(--t3);padding:30px;text-align:center">Henüz banka kartı eklenmedi — yukarıdan ekleyin</div>'; return;
  }
  const fm=n=>n?(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}):'—';
  const fmtl=n=>{const v=parseFloat(n)||0; return v>0?'₺'+fm(v):'—';};
  const pct=(k,l)=>l>0?Math.min(100,Math.round(k/l*100)):0;
  const renkPct=p=>p>80?'var(--r)':p>60?'var(--am)':'var(--g)';

  let html=`<table style="width:100%;border-collapse:collapse;font-size:13px;min-width:900px">
    <thead style="position:sticky;top:0;z-index:10">
      <tr style="background:#1e3a5f;color:#fff;font-size:12px">
        <th style="padding:10px 12px;text-align:left;white-space:nowrap">BANKA / ŞUBE</th>
        <th style="padding:10px 12px;text-align:left">FİRMA</th>
        <th style="padding:10px 12px;text-align:left">MÜDÜR / PERSONEL</th>
        <th style="padding:10px 12px;text-align:right;white-space:nowrap">TOPLAM LİMİT</th>
        <th style="padding:10px 12px;text-align:right;white-space:nowrap">KULLANIM</th>
        <th style="padding:10px 12px;text-align:right;white-space:nowrap">KALAN</th>
        <th style="padding:10px 8px;text-align:center;white-space:nowrap">KUL.%</th>
        <th style="padding:10px 8px;text-align:center">HESAP/İPOTEK</th>
        <th style="padding:10px 8px;text-align:center">İŞLEM</th>
      </tr>
    </thead>
    <tbody>`;

  _bkFiltered.forEach((b,bi)=>{
    const totL=bkToplam(b,'limit'), totK=bkToplam(b,'kullanim'), totKal=totL-totK;
    const p=pct(totK,totL);
    const rp=renkPct(p);
    const bg=bi%2===0?'background:var(--bg2)':'';
    const hesaplar=b.hesaplar||[];
    const ipotek=(b.ipotekler||[]).length;

    // Limit türleri özeti (sadece dolulara)
    const turler=BK_TURLER.filter(t=>parseFloat(b['limit_'+t])>0).map(t=>{
      const l=parseFloat(b['limit_'+t])||0;
      const k=parseFloat(b['kullanim_'+t])||0;
      const kal=l-k;
      return `<div style="display:flex;justify-content:space-between;gap:8px;font-size:11px;padding:2px 0">
        ...
        <span style="font-family:var(--mono);color:${kal>0?'var(--g)':'var(--r)'}">${fmtl(kal)}</span>
      </div>`;
    }).join('');

    html+=`<tr style="${bg};border-bottom:1px solid var(--b1)" id="bkrow-${b.id}">
      <!-- Banka/Şube -->
      <td style="padding:10px 12px;vertical-align:top">
        <div style="font-weight:700;font-size:14px;color:var(--t1)">🏦 ${b.banka||'—'}</div>
        ${b.sube?`<div style="font-size:12px;color:var(--t3);margin-top:2px">${b.sube}</div>`:''}
        ${turler?`<div style="margin-top:6px;background:var(--s3);border-radius:6px;padding:6px 8px">${turler}</div>`:''}
      </td>
      <!-- Firma -->
      <td style="padding:10px 12px;vertical-align:top">
        <span style="background:var(--s3);padding:3px 10px;border-radius:12px;font-size:12px;font-weight:600">${b.firma||'—'}</span>
      </td>
      <!-- İletişim -->
      <td style="padding:10px 12px;vertical-align:top;min-width:160px">
        ${b.mudur_ad?`<div style="font-size:12px"><span style="color:var(--am);font-size:10px;font-weight:700">MÜDÜR</span><br>
          <span style="font-weight:600">${b.mudur_ad}</span>
          ${b.mudur_tel?`<br><a href="tel:${b.mudur_tel}" style="color:var(--acc);font-size:11px">📞 ${b.mudur_tel}</a>`:''}
        </div>`:''}
        ${b.personel_ad?`<div style="font-size:12px;margin-top:4px"><span style="color:var(--g);font-size:10px;font-weight:700">PERSONEL</span><br>
          <span style="font-weight:600">${b.personel_ad}</span>
          ${b.personel_tel?`<br><a href="tel:${b.personel_tel}" style="color:var(--acc);font-size:11px">📞 ${b.personel_tel}</a>`:''}
        </div>`:''}
        ${!b.mudur_ad&&!b.personel_ad?'<span style="color:var(--t3);font-size:12px">—</span>':''}
      </td>
      <!-- Toplam Limit -->
      <td style="padding:10px 12px;text-align:right;vertical-align:top;font-family:var(--mono);font-size:13px;font-weight:600;color:var(--acc)">${fmtl(totL)}</td>
      <!-- Kullanım -->
      <td style="padding:10px 12px;text-align:right;vertical-align:top;font-family:var(--mono);font-size:13px;color:var(--am)">
        ${fmtl(totK)}
        <div style="font-size:10px;color:var(--t3);margin-top:2px">gider tablosundan</div>
      </td>
      <!-- Kalan -->
      <td style="padding:10px 12px;text-align:right;vertical-align:top;font-family:var(--mono);font-size:13px;font-weight:700;color:${totKal>0?'var(--g)':'var(--r)'}">${fmtl(totKal)}</td>
      <!-- Kullanım % -->
      <td style="padding:10px 8px;text-align:center;vertical-align:top">
        <div style="font-size:13px;font-weight:700;color:${rp}">${p}%</div>
        <div style="height:6px;background:var(--s3);border-radius:3px;margin-top:4px;overflow:hidden;width:60px;margin-left:auto;margin-right:auto">
          <div style="height:100%;width:${p}%;background:${rp};border-radius:3px"></div>
        </div>
      </td>
      <!-- Hesap/İpotek -->
      <td style="padding:10px 8px;text-align:center;vertical-align:top">
        ${hesaplar.length?`<div style="font-size:12px;color:var(--t2)">💳 ${hesaplar.length} hesap</div>`:''}
        <div style="font-size:12px;color:var(--am);margin-top:2px;cursor:pointer;text-decoration:underline" onclick="bkIpotekDetayToggle('${b.id}')">🏠 ${ipotek} ipotek ${ipotek?'▼':''}</div>
        ${!hesaplar.length&&!ipotek?'<span style="color:var(--t3);font-size:12px">—</span>':''}
      </td>
      <!-- İşlemler -->
      <td style="padding:10px 8px;text-align:center;vertical-align:top;white-space:nowrap">
        <div style="display:flex;flex-direction:column;gap:4px;align-items:center">
          <button onclick="bankaKartDuzenle('${b.id}')" style="background:var(--s3);color:var(--t2);border:1px solid var(--b2);padding:4px 10px;border-radius:5px;cursor:pointer;font-size:11px;width:80px">✏️ Düzenle</button>
          <button onclick="bkAktarKredi('${b.id}')" style="background:rgba(16,185,129,.15);color:var(--g);border:1px solid rgba(16,185,129,.3);padding:4px 10px;border-radius:5px;cursor:pointer;font-size:11px;width:80px">⬆ Kredi</button>
          <button onclick="bkIpotekYonet('${b.id}')" style="background:rgba(245,158,11,.15);color:var(--am);border:1px solid rgba(245,158,11,.3);padding:4px 10px;border-radius:5px;cursor:pointer;font-size:11px;width:80px">🏠 İpotek</button>
          <button onclick="bkSil('${b.id}')" style="background:rgba(239,68,68,.1);color:var(--r);border:1px solid rgba(239,68,68,.3);padding:4px 10px;border-radius:5px;cursor:pointer;font-size:11px;width:80px">🗑 Sil</button>
        </div>
      </td>
    </tr>
    <tr id="bkipt-${b.id}" style="display:none">
      <td colspan="9" style="padding:0;background:var(--s3)">
        <div style="padding:12px 16px" id="bkipt-inner-${b.id}">
          <div style="color:var(--t3);font-size:12px;padding:8px">Yükleniyor...</div>
        </div>
      </td>
    </tr>`;
  });
...
async function bkIpotekDetayToggle(bkId){
  const row = document.getElementById('bkipt-'+bkId);
  const inner = document.getElementById('bkipt-inner-'+bkId);
  if(!row) return;
  ...
  // Banka bilgisi
  const b = _bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ inner.innerHTML='<div style="color:var(--r);font-size:12px;padding:8px">Banka bulunamadı</div>'; return; }

  const ipotekIds = (b.ipotekler||[]).map(x=>String(x));
  if(!ipotekIds.length){
    inner.innerHTML=`<div style="color:var(--t3);font-size:12px;padding:8px">
      Bu bankaya bağlı ipotek yok — <button onclick="bkIpotekYonet('${bkId}')" 
        style="background:var(--acc);color:#fff;border:none;padding:3px 10px;border-radius:4px;cursor:pointer;font-size:11px">🏠 İpotek Ekle</button>
    </div>`;
    return;
  }

  // İpotek verilerini çek
  try{
    const r = await api('/api/ipotek');
    const d = await r.json();
    const tumIpotekler = d.records||[];
    const bankaIpotekleri = tumIpotekler.filter(ip=>ipotekIds.includes(String(ip.id)));

    if(!bankaIpotekleri.length){
      inner.innerHTML='<div style="color:var(--t3);font-size:12px;padding:8px">Bağlı ipotek kaydı bulunamadı</div>';
      return;
    }
    ...
    const totExp = bankaIpotekleri.reduce((s,ip)=>s+(ip.exp_deger||0),0);
    const totIpt = bankaIpotekleri.reduce((s,ip)=>s+(ip.tutar||0),0);
    const fark = totExp - totIpt;
    const farkRenk = fark>=0?'var(--g)':'var(--r)';
    const farkLabel = fark>=0?'Ekspertiz Fazlası ✓':'Ekspertiz Eksiği ⚠';

    inner.innerHTML=`
      <!-- Özet -->
      <div style="display:flex;gap:16px;margin-bottom:10px;flex-wrap:wrap">
        <div style="background:var(--s2);border-radius:6px;padding:8px 14px;font-size:12px">
          <div style="color:var(--t3);font-size:10px">TOPLAM EKSPERTİZ</div>
          <div style="color:var(--acc);font-weight:700;font-family:var(--mono)">${fmtl(totExp)}</div>
        </div>
        <div style="background:var(--s2);border-radius:6px;padding:8px 14px;font-size:12px">
          <div style="color:var(--t3);font-size:10px">TOPLAM İPOTEK LİMİT</div>
          <div style="color:var(--am);font-weight:700;font-family:var(--mono)">${fmtl(totIpt)}</div>
        </div>
        <div style="background:var(--s2);border-radius:6px;padding:8px 14px;font-size:12px;border:1px solid ${farkRenk}">
          <div style="color:var(--t3);font-size:10px">${farkLabel}</div>
          <div style="color:${farkRenk};font-weight:700;font-family:var(--mono)">${fmtl(Math.abs(fark))}</div>
        </div>
      </div>
      <!-- Tablo -->
      <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:700px">
        <thead><tr style="background:var(--s2);color:var(--t2);font-size:11px">
          <th style="padding:6px 8px;text-align:left">Sahibi</th>
          <th style="padding:6px 8px;text-align:left">Adres / Nitelik</th>
          <th style="padding:6px 8px;text-align:right">İpotek Tutarı</th>
          <th style="padding:6px 8px;text-align:right">Ekspertiz</th>
          <th style="padding:6px 8px;text-align:center">Durum</th>
          <th style="padding:6px 8px;text-align:center">DASK Vade</th>
        </tr></thead>
        <tbody>
          ${bankaIpotekleri.map((ip,idx)=>{
            const exp=ip.exp_deger||0, tut=ip.tutar||0;
            const fk=exp-tut;
            const fkR=fk>=0?'var(--g)':'var(--r)';
            const fkL=fk>=0?`+${fmtl(fk)} fazla`:`${fmtl(Math.abs(fk))} eksik`;
            return `<tr style="${idx%2?'background:rgba(255,255,255,.02)':''}">
              <td style="padding:6px 8px;font-weight:600">${ip.sahip||'—'}</td>
              <td style="padding:6px 8px;color:var(--t3)">${ip.il||''} ${ip.ilce||''} · ${ip.nitelik||'—'}</td>
              <td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--am)">${fmtl(tut)}</td>
              <td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--acc)">${fmtl(exp)}</td>
              <td style="padding:6px 8px;text-align:center">
                <span style="font-size:11px;font-weight:600;color:${fkR}">${fkL}</span>
              </td>
              <td style="padding:6px 8px;text-align:center;font-size:11px;color:${ip.dask_vade?'var(--t2)':'var(--t3)'}">${ip.dask_vade||'—'}</td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>
    ...
    inner.innerHTML=`<div style="color:var(--r);font-size:12px;padding:8px">Hata: ${e.message}</div>`;
  }
}
...
async function bkIpotekYonet(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ toast('Banka bulunamadı','e'); return; }

  // Tüm ipotekleri çek (sadece bu bankayı değil)
  let allIpotekler=[];
  try{
    const r=await api('/api/ipotek');
    const d=await r.json();
    allIpotekler=d.records||[];
  }catch(e){ toast('İpotek listesi alınamadı','e'); return; }

  const mevcutIds=(b.ipotekler||[]).map(x=>String(x));

  // Eski modal varsa kaldır
  const eskiMod=document.getElementById('bk-ipt-modal');
  if(eskiMod) eskiMod.remove();

  const modal=document.createElement('div');
  modal.id='bk-ipt-modal';
  modal.className='ov';
  modal.style.display='flex';

  const satirlar = allIpotekler.length
    ? allIpotekler.map(ip=>`
        <label style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--s2);border-radius:8px;margin-bottom:6px;cursor:pointer;border:1px solid ${mevcutIds.includes(String(ip.id))?'var(--acc)':'transparent'}">
          <input type="checkbox" value="${ip.id}" ${mevcutIds.includes(String(ip.id))?'checked':''} style="accent-color:var(--acc);width:16px;height:16px">
          <div style="flex:1;min-width:0">
            ...
            ${ip.banka?`<span style="font-size:11px;color:var(--am);margin-left:6px">[${ip.banka}]</span>`:''}
          </div>
          <span style="font-family:var(--mono);font-size:12px;color:var(--am);white-space:nowrap">₺${(parseFloat(ip.tutar)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
        </label>`)
      .join('')
    : '<p style="color:var(--t3);text-align:center;padding:20px">Henüz ipotek kaydı yok.<br><small>İpotek Takibi bölümünden ekleyin.</small></p>';

  modal.innerHTML=`<div class="mod" style="max-width:680px;width:94%;max-height:85vh;display:flex;flex-direction:column">
    <div class="mtit">
      <span>🏠 ${b.banka} — İpotek Bağla</span>
      <button class="mclose" onclick="document.getElementById('bk-ipt-modal').remove()">✕</button>
    </div>
    <div style="padding:12px 16px;font-size:13px;color:var(--t3)">
      İşaretlenen ipotekler bu bankaya bağlanır. Toplam ${allIpotekler.length} ipotek kayıtlı.
    </div>
    <div style="padding:0 16px 8px;overflow-y:auto;flex:1">${satirlar}</div>
    <div style="padding:12px 16px;display:flex;gap:10px;border-top:1px solid var(--b1)">
      <button class="btn bp" style="flex:1" onclick="bkIpotekKaydetYeni('${bkId}')">💾 Kaydet</button>
      <button class="btn bo" onclick="document.getElementById('bk-ipt-modal').remove()">İptal</button>
    </div>
  </div>`;
...
async function bkIpotekKaydetYeni(bkId){
  const modal=document.getElementById('bk-ipt-modal');
  if(!modal){ toast('Modal bulunamadı','e'); return; }
  ...
  const kayit={...b, ipotekler:secilen};

  try{
    const r=await api('/api/banka_kart/kaydet',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify(kayit)
    });
    if(r.status===401){ toast('Oturum süresi doldu, sayfayı yenileyin','e'); return; }
    const d=await r.json();
    if(d.ok){
      toast(secilen.length+' ipotek bağlandı ✓','s');
      modal.remove();
      renderBankaKart();
    } else {
      toast('Kayıt başarısız: '+(d.msg||'Sunucu hatası'),'e');
    }
  }catch(e){
    console.error('bkIpotekKaydet hata:',e);
    toast('Hata: '+e.message,'e');
  }
}

// bkIpotekKaydet - bkIpotekKaydetYeni ile değiştirildi
...
async function renderIpotek(){
  // Banka kart verisi yoksa yükle (dropdown için gerekli)
  if(!_bkData||_bkData.length===0){
    try{const rb=await api('/api/banka_kart',{credentials:'include'});const db=await rb.json();_bkData=db.records||[];}catch(e){}
  }
  try{const r=await api('/api/ipotek', {credentials:'include'});const d=await r.json();_ipotekData=d.records||[];_ipotekFiltered=[..._ipotekData];ipotekFiltreleriDoldur();ipotekTabloRender();ipotekOzetGuncelle();}catch(e){console.error('Ipotek:',e);}
}
...
function ipotekTabloRender(){
  ...
  tbody.innerHTML=_ipotekFiltered.map((r,i)=>{
    ...
    return `<tr style="${bg}">
      ...
      <td style="${td};font-size:12px">${r.kod||'—'}</td>
      ...
      <td style="${td};text-align:center;font-size:12px">${r.ipotek_tarih||'—'}</td>

      <td style="${td};text-align:right;font-family:var(--mono);color:#0ea5e9"><strong>${fm(r.exp_deger)}</strong></td>

      <td style="${td};text-align:center;font-size:12px">${r.exp_tarih||'—'}</td>

      <td style="${td};font-size:12px">${r.tbl_tip||'—'}</td>

      <td style="${td};font-size:12px">${r.dask_no||'—'}</td><td style="${td};text-align:center;font-size:12px">${r.dask_vade||'—'}</td>

      <td style="${td};font-size:12px">${r.konut_no||'—'}</td><td style="${td};text-align:center;font-size:12px">${r.konut_vade||'—'}</td>

      <td style="${td};text-align:center;white-space:nowrap">

        <button onclick="ipotekDuzenle('${r.id}')" style="background:#e0f2fe;color:#0369a1;border:none;padding:3px 7px;border-radius:4px;cursor:pointer;font-size:12px">✏️</button>

        <button onclick="ipotekSil('${r.id}')" style="background:#fee2e2;color:#dc2626;border:none;padding:3px 7px;border-radius:4px;cursor:pointer;font-size:12px">🗑️</button>
...
function ipotekOtoCap(){
  ...
  if(farkEl){farkEl.textContent=(fark>=0?'+':'')+('₺'+Math.abs(fark).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}));farkEl.style.color=fark>=0?'#10b981':'#ef4444';}

  if(durumEl){durumEl.textContent=fark>=0?'Ekspertiz fazla':'İpotek fazla';durumEl.style.color=fark>=0?'#166534':'#dc2626';}

}

function _ipotekDoldurBankaDd(secili){
  const sel=document.getElementById('ipt-banka');
  if(!sel) return;
  // Banka kartlarından benzersiz banka adlarını topla
  const kartBankalar=[...new Set((_bkData||[]).map(k=>k.banka).filter(Boolean))].sort();
  sel.innerHTML='<option value="">— Seçiniz —</option>';
  kartBankalar.forEach(b=>{
  ...
  // Diğer / Boş seçeneği
  const oDiger=document.createElement('option');
  oDiger.value='DİĞER'; oDiger.text='Diğer';
  if(secili==='DİĞER') oDiger.selected=true;
  sel.appendChild(oDiger);
  const oYok=document.createElement('option');
  oYok.value=''; oYok.text='Banka Yok / Boş';
  // Seçili değer banka listesinde yoksa Diğer seç
  if(secili && secili!=='DİĞER' && !kartBankalar.includes(secili)){
    // Mevcut değeri de ekle
    const oMevcut=document.createElement('option');
    oMevcut.value=secili; oMevcut.text=secili+' (mevcut)';
    oMevcut.selected=true;
    sel.insertBefore(oMevcut, oDiger);
  }
}
...
function ipotekEkleAc(){
  ...
  ['ipt-sno','ipt-kod','ipt-sahip','ipt-il','ipt-ilce','ipt-adres','ipt-ada','ipt-parsel','ipt-bb','ipt-kat','ipt-blok','ipt-nitelik','ipt-tasınmaz-tip','ipt-dask-no','ipt-dask-vade','ipt-konut-no','ipt-konut-vade'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
...
function ipotekDuzenle(id){
  ...
  _ipotekDoldurBankaDd(r.banka||'');
  f('ipt-tutar',r.tutar);f('ipt-ipotek-tarih',r.ipotek_tarih);
...
async function ipotekKaydet(){
  ...
  try{const r=await api('/api/ipotek/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(kayit)});const d=await r.json();if(d.ok){toast('Kaydedildi','s');ipotekKapat();renderIpotek();}else toast('Hata: '+(d.msg||''),'e');}catch(e){toast('Hata: '+e.message,'e');}
...
async function ipotekSil(id){
  ...
  try{const r=await api('/api/ipotek/sil',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});const d=await r.json();if(d.ok){toast('Silindi','i');renderIpotek();}else toast('Hata','e');}catch(e){toast('Hata','e');}
...
async function ipotekExcelYukle(input){
  ...
  try{const r=await api('/api/ipotek/excel_yukle',{method:'POST',body:formData,credentials:'include'});const d=await r.json();if(d.ok){toast(d.msg||'Yuklendi','s');renderIpotek();}else toast('Hata: '+(d.msg||''),'e');}catch(e){toast('Hata: '+e.message,'e');}
...
async function cikisYap(){
  ...
  try{ sessionStorage.removeItem('na_token'); }catch(ex){}
...
async function loadCurrentUser(){
  try{
    const r=await api('/api/me', {credentials:'include'}); const d=await r.json();
    if(d.ok){
      _meData = d;
      const lbl=document.getElementById('nav-user-label');
      if(lbl) lbl.textContent='👤 '+d.ad+' ('+d.username+')';
    }
  }catch(e){}
}
...
function ayTab(tab){
  ['gorunum','hesap','kullanici'].forEach(t=>{
    const btn=document.getElementById('aytab-'+t);
    const pan=document.getElementById('ayp-'+t);
    if(btn) btn.classList.toggle('on', t===tab);
    if(pan) pan.style.display = t===tab ? '' : 'none';
  });
  if(tab==='hesap') ayarHesapDoldur();
  if(tab==='kullanici') loadKullanicilar();
  if(tab==='gorunum') ayarGorunumDoldur();
}

// showView hook - ayarlar açılınca görünüm sekmesini başlat
...
// Tema önayarları
const TEMALAR = {
  koyu:  {bg:'#0d1117',s1:'#161c26',s2:'#1c2333',s3:'#21293a',acc:'#4f9cf9',t1:'#e2e8f0',t2:'#94a3b8',t3:'#4a5568',rad:6,radl:10,fs:14,nfs:12,tfs:12,sw:200},
  gece:  {bg:'#090e18',s1:'#0f1628',s2:'#141e38',s3:'#1a2645',acc:'#60a5fa',t1:'#e0eaff',t2:'#7b93c4',t3:'#374569',rad:6,radl:10,fs:14,nfs:12,tfs:12,sw:200},
  orman: {bg:'#0a1a0f',s1:'#0f2116',s2:'#132a1b',s3:'#183522',acc:'#34d399',t1:'#d1fae5',t2:'#6ee7b7',t3:'#2d6a4f',rad:8,radl:12,fs:14,nfs:12,tfs:12,sw:200},
  bordo: {bg:'#180a0a',s1:'#261010',s2:'#301515',s3:'#3d1c1c',acc:'#f87171',t1:'#fee2e2',t2:'#fca5a5',t3:'#7f1d1d',rad:6,radl:10,fs:14,nfs:12,tfs:12,sw:200},
  gri:   {bg:'#111318',s1:'#181c24',s2:'#1f242f',s3:'#252b38',acc:'#818cf8',t1:'#e2e8f0',t2:'#8b9ab4',t3:'#404860',rad:4,radl:8, fs:14,nfs:12,tfs:12,sw:200},
  mor:   {bg:'#110a1a',s1:'#1a1028',s2:'#221438',s3:'#2a1a44',acc:'#a78bfa',t1:'#ede9fe',t2:'#c4b5fd',t3:'#5b21b6',rad:8,radl:14,fs:14,nfs:12,tfs:12,sw:200},
};

function ayarGorunumDoldur(){
  const root = getComputedStyle(document.documentElement);
  const g = id => root.getPropertyValue('--'+id).trim();
  const setC = (id, val) => { const el=document.getElementById('c-'+id); if(el) el.value=rgbToHex(val)||val; const h=document.getElementById('h-'+id); if(h) h.textContent=(rgbToHex(val)||val).toUpperCase(); };
  setC('bg',g('bg')); setC('s1',g('s1')); setC('s2',g('s2')); setC('s3',g('s3'));
  setC('acc',g('acc')); setC('t1',g('t1')); setC('t2',g('t2')); setC('t3',g('t3'));
  const fs = parseInt(document.body.style.fontSize)||14;
  const rfs=document.getElementById('r-fontsize'); if(rfs){rfs.value=fs;document.getElementById('v-fontsize').textContent=fs+'px';}
  // nav font
  const nfsSample = parseInt(g('--nav-fs'))||12;
  const rnfs=document.getElementById('r-navfont'); if(rnfs){rnfs.value=nfsSample;document.getElementById('v-navfont').textContent=nfsSample+'px';}
  // tbl font
  const tfsSample = parseInt(g('--tbl-fs'))||12;
  const rtfs=document.getElementById('r-tblfont'); if(rtfs){rtfs.value=tfsSample;document.getElementById('v-tblfont').textContent=tfsSample+'px';}
  // rad
  const radv=parseInt(g('rad'))||6;
  const rrad=document.getElementById('r-rad'); if(rrad){rrad.value=radv;document.getElementById('v-rad').textContent=radv+'px';}
  const radlv=parseInt(g('radl'))||10;
  const rradl=document.getElementById('r-radl'); if(rradl){rradl.value=radlv;document.getElementById('v-radl').textContent=radlv+'px';}
  // sbar width
  const sw=parseInt(g('sbar-w'))||200;
  const rsw=document.getElementById('r-sbarw'); if(rsw){rsw.value=sw;document.getElementById('v-sbarw').textContent=sw+'px';}
}
...
function canliOnizle(){
  const gv = id => { const el=document.getElementById(id); return el?el.value:''; };
  const root=document.documentElement;
  if(gv('c-bg'))  root.style.setProperty('--bg', gv('c-bg'));
  if(gv('c-s1'))  root.style.setProperty('--s1', gv('c-s1'));
  if(gv('c-s2'))  root.style.setProperty('--s2', gv('c-s2'));
  if(gv('c-s3'))  root.style.setProperty('--s3', gv('c-s3'));
  if(gv('c-acc')) { root.style.setProperty('--acc', gv('c-acc')); root.style.setProperty('--acc-d', gv('c-acc')); }
  if(gv('c-t1'))  root.style.setProperty('--t1', gv('c-t1'));
  if(gv('c-t2'))  root.style.setProperty('--t2', gv('c-t2'));
  if(gv('c-t3'))  root.style.setProperty('--t3', gv('c-t3'));
  const fs = document.getElementById('r-fontsize'); if(fs) document.body.style.fontSize = fs.value+'px';
  const nfs = document.getElementById('r-navfont');
  if(nfs){ document.querySelectorAll('.nav').forEach(el=>el.style.fontSize=nfs.value+'px'); root.style.setProperty('--nav-fs',nfs.value+'px'); }
  const tfs = document.getElementById('r-tblfont');
  if(tfs){ document.querySelectorAll('td,th').forEach(el=>el.style.fontSize=tfs.value+'px'); root.style.setProperty('--tbl-fs',tfs.value+'px'); }
  const rad = document.getElementById('r-rad'); if(rad) root.style.setProperty('--rad', rad.value+'px');
  const radl = document.getElementById('r-radl'); if(radl) root.style.setProperty('--radl', radl.value+'px');
  const sw = document.getElementById('r-sbarw');
  if(sw){ const sbar=document.getElementById('sbar'); if(sbar) sbar.style.width=sw.value+'px'; root.style.setProperty('--sbar-w',sw.value+'px'); }
  // hex etiketleri güncelle
  ['bg','s1','s2','s3','acc','t1','t2','t3'].forEach(id=>{
    const el=document.getElementById('h-'+id); const cv=document.getElementById('c-'+id);
    if(el&&cv) el.textContent=cv.value.toUpperCase();
  });
}

function temayiUygula(ad){
  const t=TEMALAR[ad]; if(!t) return;
  const root=document.documentElement;
  root.style.setProperty('--bg',t.bg); root.style.setProperty('--s1',t.s1);
  root.style.setProperty('--s2',t.s2); root.style.setProperty('--s3',t.s3);
  root.style.setProperty('--acc',t.acc); root.style.setProperty('--acc-d',t.acc);
  root.style.setProperty('--t1',t.t1); root.style.setProperty('--t2',t.t2); root.style.setProperty('--t3',t.t3);
  root.style.setProperty('--rad',t.rad+'px'); root.style.setProperty('--radl',t.radl+'px');
  root.style.setProperty('--sbar-w',t.sw+'px');
  document.body.style.fontSize=t.fs+'px';
  document.querySelectorAll('.nav').forEach(el=>el.style.fontSize=t.nfs+'px');
  const sbar=document.getElementById('sbar'); if(sbar) sbar.style.width=t.sw+'px';
  ayarGorunumDoldur();
  toast('Tema uygulandı: '+ad,'s');
}
...
async function gorunumKaydet(){
  ...
  const gv=id=>{const el=document.getElementById(id);return el?el.value:''};
  const payload={
    bg:gv('c-bg'),s1:gv('c-s1'),s2:gv('c-s2'),s3:gv('c-s3'),
    acc:gv('c-acc'),t1:gv('c-t1'),t2:gv('c-t2'),t3:gv('c-t3'),
    font_size:document.getElementById('r-fontsize')?.value||'14',
    nav_font:document.getElementById('r-navfont')?.value||'12',
    tbl_font:document.getElementById('r-tblfont')?.value||'12',
    rad:document.getElementById('r-rad')?.value||'6',
    radl:document.getElementById('r-radl')?.value||'10',
    sbar_w:document.getElementById('r-sbarw')?.value||'200',
  };
  try{
    const r=await api('/api/settings/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const d=await r.json();
    const msg=document.getElementById('gorunum-msg');
    if(d.ok){
      if(msg){msg.textContent='✓ Kaydedildi';setTimeout(()=>msg.textContent='',3000);}
      localStorage.setItem('nakit_gorunum', JSON.stringify(payload));
      toast('Görünüm kaydedildi','s');
    } else {
      if(msg) msg.textContent='Hata: '+(d.msg||'');
    }
  }catch(e){toast('Hata','e');}
}

function gorunumSifirla(){
  if(!confirm('Varsayılan görünüme dönülsün mü?')) return;
  temayiUygula('koyu');
  localStorage.removeItem('nakit_gorunum');
  fetch('/api/settings/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bg:'',s1:'',s2:'',s3:'',acc:'',t1:'',t2:'',t3:'',font_size:'',nav_font:'',tbl_font:'',rad:'',radl:'',sbar_w:''})});
  toast('Sıfırlandı','i');
}

function gorunumUygula(s){
  if(!s) return;
  const root=document.documentElement;
  if(s.bg)  root.style.setProperty('--bg',s.bg);
  if(s.s1)  root.style.setProperty('--s1',s.s1);
  if(s.s2)  root.style.setProperty('--s2',s.s2);
  if(s.s3)  root.style.setProperty('--s3',s.s3);
  if(s.acc) {root.style.setProperty('--acc',s.acc);root.style.setProperty('--acc-d',s.acc);}
  if(s.t1)  root.style.setProperty('--t1',s.t1);
  if(s.t2)  root.style.setProperty('--t2',s.t2);
  if(s.t3)  root.style.setProperty('--t3',s.t3);
  if(s.rad)  root.style.setProperty('--rad',s.rad+'px');
  if(s.radl) root.style.setProperty('--radl',s.radl+'px');
  if(s.sbar_w){const sbar=document.getElementById('sbar');if(sbar)sbar.style.width=s.sbar_w+'px';root.style.setProperty('--sbar-w',s.sbar_w+'px');}
  if(s.font_size) document.body.style.fontSize=s.font_size+'px';
  if(s.nav_font)  document.querySelectorAll('.nav').forEach(el=>el.style.fontSize=s.nav_font+'px');
  if(s.tbl_font)  {root.style.setProperty('--tbl-fs',s.tbl_font+'px');}
}

// Sayfa yüklenince görünüm ayarlarını uygula
(async function(){
  try{
    const r=await api('/api/settings/get', {credentials:'include'}); const d=await r.json();
    if(d.ok && d.settings) gorunumUygula(d.settings);
  }catch(e){}
})();

// ── HESABIM ──────────────────────────────────────────────────────────────────

async function ayarHesapDoldur(){
  try{
    const r=await api('/api/me', {credentials:'include'}); const d=await r.json();
    if(d.ok){
      const uEl=document.getElementById('hesap-uname'); if(uEl) uEl.value=d.username||'';
      const aEl=document.getElementById('hesap-ad');    if(aEl) aEl.value=d.ad||'';
      const rEl=document.getElementById('hesap-rol');   if(rEl) rEl.value=d.rol||'user';
    }
  }catch(e){}
}

async function profilKaydet(){
  const ad=document.getElementById('hesap-ad')?.value.trim();
  if(!ad){toast('Ad boş olamaz','e');return;}
  try{
    const r=await api('/api/users/me_update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ad})});
    const d=await r.json();
    const msg=document.getElementById('profil-msg');
    if(d.ok){
      if(msg){msg.textContent='✓ Güncellendi';setTimeout(()=>msg.textContent='',3000);}
      loadCurrentUser();
      toast('Profil güncellendi','s');
    } else { if(msg) msg.textContent='Hata: '+(d.msg||''); }
  }catch(e){toast('Hata','e');}
}

async function hesapSifreDegistir(){
  const yeni=document.getElementById('hesap-sifre1')?.value;
  const tekrar=document.getElementById('hesap-sifre2')?.value;
  const msg=document.getElementById('sifre-msg');
  if(yeni!==tekrar){if(msg)msg.textContent='Şifreler eşleşmiyor';toast('Şifreler eşleşmiyor','e');return;}
  if(!yeni||yeni.length<6){if(msg)msg.textContent='En az 6 karakter';toast('En az 6 karakter','e');return;}
  try{
    const r=await api('/api/users/change_password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({new_password:yeni})});
    const d=await r.json();
    if(d.ok){
      if(msg){msg.textContent='✓ Şifre değiştirildi';setTimeout(()=>msg.textContent='',3000);}
      document.getElementById('hesap-sifre1').value='';
      document.getElementById('hesap-sifre2').value='';
      toast('Şifre değiştirildi','s');
    } else { if(msg) msg.textContent='Hata: '+(d.msg||''); }
  }catch(e){toast('Hata','e');}
}

// Eski fonksiyonlar (geriye dönük uyumluluk)
async function sifreDegistir(){hesapSifreDegistir();}
...
async function loadKullanicilar(){
  ...
  try{
    const r=await api('/api/users/list',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const d=await r.json();
    if(!d.ok){container.innerHTML='<div style="color:var(--t3);font-size:12px;padding:10px">'+(d.msg||'Yetki yok')+'</div>';return;}
    if(!d.users.length){container.innerHTML='<div style="color:var(--t3);font-size:12px;padding:10px">Kullanıcı yok</div>';return;}
    container.innerHTML=d.users.map(u=>`
      <div class="uk-card">
        <div style="flex:1">
          <span class="uk-name">${u.username}</span>
          ${u.ad?`<span style="color:var(--t2);font-size:12px;margin-left:6px">${u.ad}</span>`:''}
          <span class="uk-rol ${u.rol}">${u.rol==='admin'?'Admin':'Kullanıcı'}</span>
        </div>
        <span style="font-size:11px;color:var(--t3)">${u.created_at||''}</span>
        <button onclick="kullaniciIzinAc('${u.username}')" style="background:var(--s3);border:1px solid var(--b1);color:var(--t2);padding:4px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px">🔐 İzinler</button>
        ${u.username!=='admin'?`<button onclick="kullaniciSifreDlg('${u.username}')" style="background:var(--s3);border:1px solid var(--b1);color:var(--t2);padding:4px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px">🔑</button>
        <button onclick="kullaniciSil('${u.username}')" style="background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.3);color:var(--r);padding:4px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px">🗑</button>`:''}
      </div>`).join('');
  }catch(e){container.innerHTML='<div style="color:var(--r);font-size:12px;padding:10px">Hata</div>';}
}

async function kullaniciEkle(){
  const unameEl = document.getElementById('yeni-uname');
  const adEl    = document.getElementById('yeni-ad');
  const pwEl    = document.getElementById('yeni-pw');
  const rolEl   = document.getElementById('yeni-rol');
  const msg     = document.getElementById('kullanici-ekle-msg');

  if(!unameEl||!pwEl){ toast('Form elementleri bulunamadı','e'); return; }
  ...
  if(!uname){ toast('Kullanıcı adı zorunlu','e'); return; }
  if(!pw || pw.length < 6){ toast('Şifre en az 6 karakter olmalı','e'); return; }

  if(msg){ msg.style.color='var(--t3)'; msg.textContent='Ekleniyor...'; }

  try{
    const r = await api('/api/users/add', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({username:uname, ad:ad, password:pw, rol:rol})
    });
    if(!r.ok){ toast('Sunucu hatası: '+r.status,'e'); if(msg) msg.textContent=''; return; }
    const d = await r.json();
    if(d.ok){
      if(msg){ msg.style.color='var(--g)'; msg.textContent='✓ '+uname+' eklendi'; setTimeout(()=>{ if(msg) msg.textContent=''; },3000); }
      unameEl.value=''; pwEl.value=''; if(adEl) adEl.value='';
      loadKullanicilar();
      toast(uname+' eklendi','s');
    } else {
      if(msg){ msg.style.color='var(--r)'; msg.textContent='Hata: '+(d.msg||'Bilinmeyen hata'); }
      toast('Hata: '+(d.msg||''),'e');
    }
  } catch(e) {
    console.error('kullaniciEkle error:', e);
    if(msg){ msg.style.color='var(--r)'; msg.textContent='Bağlantı hatası'; }
    toast('Bağlantı hatası: '+e.message,'e');
  }
}
...
async function kullaniciSil(uname){
  ...
  try{
    const r=await api('/api/users/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:uname})});
    const d=await r.json();
    if(d.ok){toast('Silindi','i');loadKullanicilar();}
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata','e');}
}

async function kullaniciSifreDlg(uname){
  const yeni=prompt(uname+' için yeni şifre girin (min. 6 karakter):');
  if(!yeni) return;
  if(yeni.length<6){toast('En az 6 karakter','e');return;}
  try{
    const r=await api('/api/users/change_password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:uname,new_password:yeni})});
    const d=await r.json();
    if(d.ok) toast(uname+' şifresi değiştirildi','s');
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata','e');}
}
...
const MENU_IZINLER = [
  {k:'db',       ad:'Dashboard'},
  {k:'banka-kart',ad:'Banka Kartları'},
  {k:'tbl',      ad:'Gider Tablosu'},
  {k:'gelir',    ad:'Tahsilat Tablosu'},
  {k:'nakit',    ad:'Nakit Akış Dengesi'},
  {k:'finans',   ad:'Finansman Yönetimi'},
  {k:'liki',     ad:'Likidite & Finansman'},
  {k:'cari',     ad:'Cari Hesaplar'},
  {k:'banka-api',ad:'Banka API & Ekstre'},
  {k:'kur',      ad:'Döviz Kurları'},
  {k:'logo',     ad:'Faturalar (LOGO)'},
  {k:'satis',    ad:'Satış Analizi'},
  {k:'stok',     ad:'Stok Listesi (LOGO)'},
  {k:'cari-logo',ad:'Cari Listesi (LOGO)'},
  {k:'gmail',    ad:'Gmail'},
  {k:'docs',     ad:'Döküman Yönetimi'},
  {k:'notlar',   ad:'Notlar'},
  {k:'hat',      ad:'Hatırlatıcı'},
  {k:'ai',       ad:'AI Asistan'},
  {k:'wa',       ad:'WhatsApp'},
  {k:'otel',     ad:'Otel Yönetimi'},
  {k:'mo',       ad:'Aylık Özet'},
  {k:'ipotek',   ad:'İpotek Takibi'},
  {k:'src',      ad:'Kaynak Dosya'},
  {k:'exp',      ad:'Dışa Aktar'},
  {k:'import',   ad:'Veri İçe Aktar'},
];

let _izinDlgUser = null;

function kullaniciIzinAc(uname){
  _izinDlgUser = uname;
  // izinleri backend'den al
  api('/api/users/list',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})
  .then(r=>r.json()).then(d=>{
    if(!d.ok) return;
    const u = d.users.find(x=>x.username===uname);
    const izinler = u?.izinler || null; // null = tüm izinler açık
    // Modal oluştur
    let mevcut = document.getElementById('izin-modal');
    ...
    modal.innerHTML=`
      <div style="background:var(--s1);border:1px solid var(--b2);border-radius:var(--radl);padding:24px;width:420px;max-height:80vh;overflow-y:auto">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div style="font-size:14px;font-weight:600;color:var(--t1)">🔐 ${uname} — Menü Erişim İzinleri</div>
          <button onclick="document.getElementById('izin-modal').remove()" style="background:transparent;border:none;color:var(--t3);cursor:pointer;font-size:18px">✕</button>
        </div>
        <div style="font-size:11px;color:var(--t3);margin-bottom:12px">Kapalı menüler bu kullanıcıya görünmez. Boş bırakırsanız tüm menüler açık olur.</div>
        <div style="display:flex;gap:8px;margin-bottom:12px">
          <button onclick="izinTumunuSec(true)"  style="background:var(--s2);border:1px solid var(--b1);color:var(--t2);padding:4px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px">Tümünü Seç</button>
          <button onclick="izinTumunuSec(false)" style="background:var(--s2);border:1px solid var(--b1);color:var(--t2);padding:4px 10px;border-radius:var(--rad);cursor:pointer;font-size:11px">Tümünü Kaldır</button>
        </div>
        <div id="izin-list">
          ${MENU_IZINLER.map(m=>`
            <div class="izin-row">
              <input type="checkbox" class="izin-chk" id="izin-${m.k}" ${!izinler||izinler.includes(m.k)?'checked':''}>
              <label class="izin-lbl" for="izin-${m.k}">${m.ad}</label>
            </div>`).join('')}
        </div>
        <div style="display:flex;gap:10px;margin-top:16px">
          <button onclick="izinKaydet()" class="btn bp bsm" style="flex:1">💾 Kaydet</button>
          <button onclick="document.getElementById('izin-modal').remove()" class="btn bo bsm">İptal</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
  });
...
function izinTumunuSec(durum){
  MENU_IZINLER.forEach(m=>{
    const el=document.getElementById('izin-'+m.k);
    if(el) el.checked=durum;
  });
}

async function izinKaydet(){
  const secili = MENU_IZINLER.filter(m=>document.getElementById('izin-'+m.k)?.checked).map(m=>m.k);
  try{
    const r=await api('/api/users/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:_izinDlgUser, izinler:secili})});
    const d=await r.json();
    if(d.ok){
      document.getElementById('izin-modal')?.remove();
      toast('İzinler kaydedildi','s');
    } else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata','e');}
}
...
async function topluYukle(){
  ...
  for(const file of files){
    ...
    const r=await api('/api/import/json',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tur,data})});const d=await r.json();results.push({file:file.name,ok:d.ok,msg:d.msg});}
...
// ── GİDER TABLOSU TOPLU İŞLEMLER ─────────────────────────────────────────────

var _giderExcelDosya = null;

function giderExcelSablonIndir(){
  window.location.href='/api/gider/excel_sablon';
}

function giderExcelYukleAc(input){
  _giderExcelDosya = input.files[0];
  if(!_giderExcelDosya) return;
  document.getElementById('gider-excel-dosya-adi').textContent='📄 ' + _giderExcelDosya.name;
  document.getElementById('gider-excel-dosya-adi').style.display='block';
  document.getElementById('gider-excel-yukle-btn').style.display='block';
  document.getElementById('gider-excel-msg').textContent='Dosya hazır. Yükleme modunu seçip "Yükle" butonuna tıklayın.';
  document.getElementById('gider-excel-msg').style.color='var(--acc)';
  document.getElementById('gider-excel-mod').style.display='flex';
  // input'u sıfırla (aynı dosyayı tekrar seçebilmek için)
  input.value='';
}

async function giderExcelYukle(){
  if(!_giderExcelDosya){ toast('Dosya seçilmedi','e'); return; }
  const mod = document.querySelector('input[name="gider-mod"]:checked')?.value || 'ekle';
  if(mod==='replace' && !confirm('⚠️ Tüm mevcut gider verileri silinecek ve Excel\'deki verilerle değiştirilecek. Emin misiniz?')) return;
  const msg = document.getElementById('gider-excel-msg');
  msg.textContent='⏳ Yükleniyor...'; msg.style.color='var(--t3)';
  const fd = new FormData();
  fd.append('file', _giderExcelDosya);
  fd.append('mod', mod);
  try{
    const r = await api('/api/gider/excel_yukle',{method:'POST', body:fd, credentials:'include'});
    const d = await r.json();
    if(d.ok){
      msg.textContent='✅ ' + d.msg; msg.style.color='var(--g)';
      toast(d.msg,'s');
      setTimeout(()=>{ document.getElementById('gider-excel-mod').style.display='none'; reload(); },1200);
    } else {
      msg.textContent='❌ ' + d.msg; msg.style.color='var(--r)';
      toast('Hata: '+d.msg,'e');
    }
  }catch(e){
    msg.textContent='❌ Hata: '+e.message; msg.style.color='var(--r)';
    toast('Hata','e');
  }
}

// Checkbox seçim yönetimi
function tblSecHepsi(durum){
  document.querySelectorAll('.tbl-sec-chk').forEach(cb=>cb.checked=durum);
  tblSecDegis();
}

function tblSecDegis(){
  const secilen = [...document.querySelectorAll('.tbl-sec-chk:checked')];
  const sayi = secilen.length;
  const sayiEl = document.getElementById('secili-sayi');
  const sayiElSil = document.getElementById('secili-sayi-sil');
  const btn = document.getElementById('toplu-guncelle-btn');
  const btnSil = document.getElementById('toplu-sil-btn');
  if(sayiEl) sayiEl.textContent = sayi;
  if(sayiElSil) sayiElSil.textContent = sayi;
  if(btn) btn.style.display = sayi > 0 ? 'inline-flex' : 'none';
  if(btnSil) btnSil.style.display = sayi > 0 ? 'inline-flex' : 'none';
}

async function topluSil(){
  const secilen = [...document.querySelectorAll('.tbl-sec-chk:checked')];
  if(!secilen.length){ toast('Önce kayıt seçin','e'); return; }
  const idxler = secilen.map(cb=>parseInt(cb.dataset.idx));
  if(!confirm(idxler.length + ' kayıt kalıcı olarak silinecek. Emin misiniz?')) return;
  try{
    const r = await api('/api/gider/toplu_sil',{
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({idxler})
    });
    const d = await r.json();
    if(d.ok){
      toast(d.msg,'s');
      document.querySelectorAll('.tbl-sec-chk').forEach(cb=>cb.checked=false);
      document.getElementById('tbl-sec-hepsi').checked=false;
      tblSecDegis();
      reload();
    } else toast('Hata: '+d.msg,'e');
  }catch(e){ toast('Hata: '+e.message,'e'); }
...
function topluGuncelleAc(){
  const secilen = [...document.querySelectorAll('.tbl-sec-chk:checked')];
  if(!secilen.length){ toast('Önce kayıt seçin','e'); return; }
  document.getElementById('toplu-secili-bilgi').textContent = secilen.length + ' kayıt';
  // Toplu güncelleme modalındaki yer dropdown'ını doldur
  const yerSel = document.getElementById('tg-yer');
  yerSel.innerHTML='<option value="">— Değiştirme —</option>';
  const yerler=[...new Set(allData.map(d=>d.yer).filter(Boolean))].sort();
  yerler.forEach(y=>{const o=document.createElement('option');o.value=y;o.text=y;yerSel.appendChild(o);});
  // Banka kartlarındaki bankaları da ekle
  (_bkData||[]).forEach(k=>{
    if(k.banka&&!yerler.includes(k.banka)){
      const o=document.createElement('option');o.value=k.banka;o.text=k.banka;yerSel.appendChild(o);
    }
  });
  // Alanları sıfırla
  ['tg-firma','tg-tur','tg-yer','tg-durum'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  ['tg-kredi-no','tg-aciklama'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  document.getElementById('toplu-guncelle-mod').style.display='flex';
}

async function topluGuncelleKaydet(){
  const secilen = [...document.querySelectorAll('.tbl-sec-chk:checked')];
  if(!secilen.length){ toast('Hiç kayıt seçilmedi','e'); return; }
  const idxler = secilen.map(cb=>parseInt(cb.dataset.idx));
  const alanlar = {};
  const firma   = document.getElementById('tg-firma')?.value;
  const tur     = document.getElementById('tg-tur')?.value;
  const yer     = document.getElementById('tg-yer')?.value;
  const krediNo = document.getElementById('tg-kredi-no')?.value.trim();
  const durum   = document.getElementById('tg-durum')?.value;
  const aciklama= document.getElementById('tg-aciklama')?.value.trim();
  if(firma)    alanlar.firma    = firma;
  if(tur)      alanlar.tur      = tur;
  if(yer)      alanlar.yer      = yer;
  if(krediNo)  alanlar.kredi_no = krediNo;
  if(durum)    alanlar.durum    = durum;
  if(aciklama) alanlar.aciklama = aciklama;
  if(!Object.keys(alanlar).length){ toast('Güncellenecek alan seçilmedi','e'); return; }
  if(!confirm(`${idxler.length} kayıt güncellensin mi?\n\nGüncellenecek alanlar: ${Object.keys(alanlar).join(', ')}`)) return;
  try{
    const r = await api('/api/gider/toplu_guncelle',{
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({idxler, alanlar})
    });
    const d = await r.json();
    if(d.ok){
      toast(d.msg,'s');
      document.getElementById('toplu-guncelle-mod').style.display='none';
      // Seçimleri temizle
      document.querySelectorAll('.tbl-sec-chk').forEach(cb=>cb.checked=false);
      document.getElementById('tbl-sec-hepsi').checked=false;
      tblSecDegis();
...
async function githubYedekAl(){
  ...
  try{const r=await api('/api/yedek/github', {credentials:'include'});const d=await r.json();if(d.ok){toast('GitHub yedegi tamamlandi','s');}else toast('Hata: '+(d.msg||''),'e');}catch(e){toast('Hata','e');}
...
var _kkData = [], _kkEditId = null, _kkAktifKartId = null, _kkHarcamaEditId = null;

async function renderKK(){
  // Banka kart listesini yükle (dropdown için)
  if(!_bkData||_bkData.length===0){
    try{const rb=await api('/api/banka_kart',{credentials:'include'});const db=await rb.json();_bkData=db.records||[];}catch(e){}
  }
  try{
    const r=await api('/api/kk/liste',{credentials:'include'});
    const d=await r.json();
    _kkData=d.records||[];
    kkListeRender();  // kkOzetBar içerde çağrılıyor
  }catch(e){console.error('KK render hatası:',e);}
}

function kkFiltrele(){
  const q=(document.getElementById('kk-arama')?.value||'').toLowerCase();
  kkListeRender(q);
}

function kkListeRender(q=''){
  const list=document.getElementById('kk-list');
  if(!list)return;
  const fmt=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const filtered=q?_kkData.filter(k=>(k.banka||'').toLowerCase().includes(q)||(k.kart_no||'').includes(q)):_kkData;

  // Özet çubuğu için tüm kartlardan topla (filtre yoksa)
  {
    const isOdeme=h=>h.odendi===true||h.odendi==='true'||h.tur==='odeme';
    let totLim=0,totKull=0,totKalan=0;
    _kkData.forEach(k=>{
      const lim=parseFloat(k.limit||0);
      // Server değeri varsa kullan (en güvenilir)
      const kull=parseFloat(k.kullanilan!=null?k.kullanilan:
        (k.harcamalar||[]).filter(h=>!isOdeme(h)).reduce((s,h)=>s+parseFloat(h.tutar||0),0));
      const kal =parseFloat(k.kalan!=null?k.kalan:Math.max(0,lim-kull));
      totLim+=lim; totKull+=kull; totKalan+=kal;
    });
    const pct=totLim>0?Math.round(totKull/totLim*100):0;
    const br=pct>80?'#ef4444':pct>50?'#f59e0b':'#10b981';
    const el=document.getElementById('kk-ozet-bar');
    if(el) el.innerHTML=`
      <div class="met" style="border-left:3px solid var(--acc)">
        <div class="mlb">Toplam Kart</div>
        <div class="mvl" style="font-size:26px;color:var(--acc)">${_kkData.length}<span style="font-size:12px;font-weight:400;color:var(--t3);margin-left:4px">kart</span></div>
      </div>
      <div class="met" style="border-left:3px solid var(--acc)">
        <div class="mlb">Toplam Limit</div>
        <div class="mvl" style="color:var(--acc)">₺${fmt(totLim)}</div>
      </div>
      <div class="met" style="border-left:3px solid ${br}">
        <div class="mlb">Kullanılan <span style="font-size:10px;color:var(--t3)">(${pct}%)</span></div>
        <div class="mvl" style="color:var(--am)">₺${fmt(totKull)}</div>
        <div style="margin-top:5px;height:4px;background:var(--b1);border-radius:2px;overflow:hidden">
          <div style="height:100%;width:${pct}%;background:${br};border-radius:2px"></div>
        </div>
      </div>
      <div class="met" style="border-left:3px solid #10b981">
        <div class="mlb">💰 Kalan Limit <span style="font-size:10px;color:var(--g)">(Finansmana aktarıldı)</span></div>
        <div class="mvl" style="color:#10b981">₺${fmt(totKalan)}</div>
        <div style="font-size:10px;color:var(--t3);margin-top:3px">→ Finansman Yönetimi'nde kullanılabilir limit olarak görünür</div>
      </div>`;
  }

  if(!filtered.length){
    list.innerHTML='<div style="color:var(--t3);text-align:center;padding:40px;font-size:13px">Henüz kredi kartı eklenmedi.<br><small>Yukarıdaki "+ Yeni Kart" butonuyla ekleyebilirsiniz.</small></div>';
    return;
  }
  list.innerHTML=filtered.map(k=>{
    const limit=parseFloat(k.limit||0);
    // Server'dan gelen kullanilan/kalan değerlerini tercih et
    let kull, kalan;
    if(k.kullanilan!==undefined){
      kull  = parseFloat(k.kullanilan||0);
      kalan = parseFloat(k.kalan||0);
    } else {
      const isOdeme=h=>h.odendi===true||h.odendi==='true'||h.tur==='odeme';
      const hList=k.harcamalar||[];
      kull  = hList.filter(h=>!isOdeme(h)).reduce((s,h)=>s+parseFloat(h.tutar||0),0);
      kalan = Math.max(0,limit-kull);
    }
    // Harcama tablosu için sıralı liste (her zaman harcamalar key'inden)
    const harcamalar=[...(k.harcamalar||[])].sort((a,b)=>(b.tarih||'').localeCompare(a.tarih||''));
    const topHarcama=harcamalar.filter(h=>h.odendi!==true&&h.odendi!=='true'&&h.tur!=='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
    const topOdeme  =harcamalar.filter(h=>h.odendi===true||h.odendi==='true'||h.tur==='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
    const pct=limit>0?Math.min(100,Math.round(kull/limit*100)):0;
    const barRenk=pct>80?'#ef4444':pct>50?'#f59e0b':'#10b981';
    const kalanRenk=pct>80?'var(--r)':pct>50?'var(--am)':'var(--g)';
    // Kesim / ödeme tarihi hesapla
    const bugun=new Date();
    let kesimStr='', odemeStr='';
    if(k.kesim_gun){
      const kes=new Date(bugun.getFullYear(),bugun.getMonth(),parseInt(k.kesim_gun));
      if(kes<bugun) kes.setMonth(kes.getMonth()+1);
      kesimStr=kes.toLocaleDateString('tr-TR',{day:'numeric',month:'long'});
    }
    if(k.odeme_gun){
      const od=new Date(bugun.getFullYear(),bugun.getMonth(),parseInt(k.odeme_gun));
      if(od<bugun) od.setMonth(od.getMonth()+1);
      odemeStr=od.toLocaleDateString('tr-TR',{day:'numeric',month:'long'});
    }

    return `
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);margin-bottom:16px;overflow:hidden">

      <!-- ── KART ÜST BÖLÜM: Görsel kart + limit bilgisi ── -->
      <div style="padding:16px;display:grid;grid-template-columns:auto 1fr auto;gap:16px;align-items:start">

        <!-- Kart görseli -->
        <div style="width:80px;height:50px;background:linear-gradient(135deg,#1e3a5f 0%,#1d4ed8 60%,#7c3aed 100%);border-radius:8px;display:flex;flex-direction:column;justify-content:space-between;padding:7px 10px;box-shadow:0 4px 12px rgba(0,0,0,.3);flex-shrink:0">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:10px;color:rgba(255,255,255,.7);font-weight:600">${(k.banka||'').substring(0,8).toUpperCase()}</span>
            <span style="font-size:14px">💳</span>
          </div>
          <div style="font-size:9px;color:rgba(255,255,255,.6);font-family:monospace">${k.kart_no?'•••• '+k.kart_no.slice(-4):'•••• ••••'}</div>
        </div>

        <!-- Banka & detay bilgisi -->
        <div>
          <div style="font-weight:700;font-size:15px;margin-bottom:4px">${k.banka||'—'}
            <span style="font-size:11px;color:var(--t3);font-weight:400;margin-left:6px">${k.firma||''}</span>
          </div>
          <div style="display:flex;gap:14px;flex-wrap:wrap;font-size:11px;color:var(--t3)">
            ${kesimStr?`<span>✂️ Hesap kesim: <strong style="color:var(--t2)">${kesimStr}</strong></span>`:''}
            ${odemeStr?`<span>📅 Son ödeme: <strong style="color:var(--t2)">${odemeStr}</strong></span>`:''}
            <span>⚠️ Gecikme faizi: <strong style="color:var(--am)">%${parseFloat(k.gecikme_faiz||0).toFixed(2)}/yıl</strong></span>
          </div>
          <!-- Kullanım çubuğu -->
          <div style="margin-top:10px">
            <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:4px">
              <span style="color:var(--t3)">Kullanım: <strong style="color:var(--am)">₺${fmt(kull)}</strong></span>
              <span style="color:var(--t3)">Limit: <strong style="color:var(--t1)">₺${fmt(limit)}</strong></span>
            </div>
            <div style="height:8px;background:var(--b1);border-radius:4px;overflow:hidden">
              <div style="height:100%;width:${pct}%;background:${barRenk};border-radius:4px;transition:.4s"></div>
            </div>
            <div style="font-size:10px;color:var(--t3);margin-top:3px">${pct}% kullanıldı</div>
          </div>
        </div>

        <!-- Kalan limit kutusu + butonlar -->
        <div style="text-align:right">
          <div style="background:var(--s3);border:1px solid ${barRenk};border-radius:10px;padding:10px 16px;margin-bottom:8px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:2px">KALAN LİMİT</div>
            <div style="font-family:var(--mono);font-size:18px;font-weight:700;color:${kalanRenk}">₺${fmt(kalan)}</div>
          </div>
          <div style="display:flex;gap:6px;justify-content:flex-end">
            <button onclick="kkHarcamaEkleAc('${k.id}')" class="btn bp bsm" style="font-size:12px">+ Harcama</button>
            <button onclick="kkDuzenle('${k.id}')" class="btn bgh" style="padding:4px 9px;font-size:12px">✏️</button>
            <button onclick="kkSil('${k.id}')" class="btn bgh" style="padding:4px 9px;font-size:12px;color:var(--r)">🗑</button>
          </div>
        </div>
      </div>

      <!-- ── HARCAMA TABLOSU (accordion) ── -->
      <div style="border-top:1px solid var(--b1)">
        <div style="padding:10px 16px 8px;display:flex;align-items:center;justify-content:space-between;background:var(--s3);cursor:pointer"
             onclick="kkHarcamaToggle('kk-tbl-${k.id}', this)">
          <span style="font-size:12px;font-weight:700;color:var(--acc)">📋 HARCAMA KAYITLARI
            <span style="font-size:11px;font-weight:400;color:var(--t3);margin-left:6px">${harcamalar.length} adet · Toplam: <strong style="color:var(--am)">₺${fmt(topHarcama)}</strong></span>
          </span>
          <div style="display:flex;gap:8px;align-items:center">
            <button class="btn bp bsm" onclick="event.stopPropagation();kkHarcamaEkleAc('${k.id}')" style="font-size:11px">+ Yeni Harcama</button>
            <span style="font-size:16px;color:var(--t3)" id="kk-arr-${k.id}">▼</span>
          </div>
        </div>
        <div id="kk-tbl-${k.id}" style="display:none">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:12px">
            <thead style="position:sticky;top:0;z-index:1">
              <tr style="background:var(--s2)">
                <th style="padding:7px 10px;text-align:left;color:var(--t3);font-weight:600;white-space:nowrap">S.No</th>
                <th style="padding:7px 10px;text-align:left;color:var(--t3);font-weight:600;white-space:nowrap">Tarih</th>
                <th style="padding:7px 10px;text-align:left;color:var(--t3);font-weight:600">Firma / Ödeme Yeri</th>
                <th style="padding:7px 10px;text-align:right;color:var(--t3);font-weight:600;white-space:nowrap">Tutar (₺)</th>
                <th style="padding:7px 10px;text-align:left;color:var(--t3);font-weight:600">Kategori</th>
                <th style="padding:7px 10px;text-align:left;color:var(--t3);font-weight:600">Açıklama</th>
                <th style="padding:7px 10px;text-align:center;color:var(--t3);font-weight:600">İşlem</th>
              </tr>
            </thead>
            <tbody>
              ${harcamalar.map((h,i)=>{
                const isOdeme=h.odendi||h.tur==='odeme';
                const rowBg=isOdeme?'background:rgba(52,211,153,.07)':'';
                return `
              <tr style="border-bottom:1px solid var(--b1);${i%2===0&&!isOdeme?'background:var(--s2)':''}${isOdeme?'background:rgba(52,211,153,.07)':''}">
                <td style="padding:7px 10px;color:var(--t3);font-size:11px">${i+1}</td>
                <td style="padding:7px 10px;font-family:var(--mono);white-space:nowrap;color:var(--t2)">${h.tarih||'—'}</td>
                <td style="padding:7px 10px;font-weight:600;color:${isOdeme?'var(--g)':'var(--t1)'}">
                  ${isOdeme?'💰 ':''}${h.firma||'—'}
                </td>
                <td style="padding:7px 10px;text-align:right;font-family:var(--mono);color:${isOdeme?'var(--g)':'var(--am)'};font-weight:700">
                  ${isOdeme?'+':''}₺${fmt(h.tutar)}
                </td>
                <td style="padding:7px 10px;color:${isOdeme?'var(--g)':'var(--t3)'}">
                  <span style="${isOdeme?'background:rgba(52,211,153,.2);padding:1px 6px;border-radius:3px;font-size:10px;font-weight:600':''}">
                    ${isOdeme?'✅ Ödeme':h.kategori||'—'}
                  </span>
                </td>
                <td style="padding:7px 10px;color:var(--t2);font-size:11px;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${h.aciklama||''}</td>
                <td style="padding:7px 10px;text-align:center;white-space:nowrap">
                  <button onclick="kkHarcamaDuzenle('${k.id}','${h.id}')" class="btn bgh" style="padding:3px 8px;font-size:11px" title="Düzenle">✏️</button>
                  <button onclick="kkHarcamaSil('${k.id}','${h.id}')" class="btn bgh" style="padding:3px 8px;font-size:11px;color:var(--r)" title="Sil">🗑</button>
                </td>
              </tr>`}).join('')}
            </tbody>
          </table>
        ...
        <!-- Alt özet çubuğu -->
        <div style="padding:10px 16px;background:var(--s3);display:flex;gap:24px;flex-wrap:wrap;font-size:12px;border-top:1px solid var(--b1)">
          <span>💳 Harcama: <strong style="color:var(--am);font-family:var(--mono)">₺${fmt(topHarcama)}</strong></span>
          ${topOdeme>0?`<span>💰 Ödeme: <strong style="color:var(--g);font-family:var(--mono)">+₺${fmt(topOdeme)}</strong> <span style="font-size:10px;color:var(--t3)">(limit geri kazanıldı)</span></span>`:''}
          <span>📊 Net Kullanım: <strong style="color:var(--am);font-family:var(--mono)">₺${fmt(kull)}</strong></span>
          <span>💰 Limit: <strong style="color:var(--t1);font-family:var(--mono)">₺${fmt(limit)}</strong></span>
          <span>✅ Kalan: <strong style="color:${kalanRenk};font-family:var(--mono)">₺${fmt(kalan)}</strong></span>
          <span style="color:var(--t3)">(${100-pct}% boş)</span>
        </div>`
        :`<div style="padding:20px;text-align:center;color:var(--t3);font-size:12px">
          Henüz harcama eklenmedi. <button class="btn bp bsm" onclick="kkHarcamaEkleAc('${k.id}')" style="margin-left:8px;font-size:11px">+ İlk Harcamayı Ekle</button>
        </div>`}
        </div><!-- kk-tbl kapanış -->
      </div>
...
function kkToggle(id){
  // Uyumluluk için korundu
}

function kkHarcamaToggle(tblId, headerEl){
  const el = document.getElementById(tblId);
  if(!el) return;
  const open = el.style.display !== 'none';
  el.style.display = open ? 'none' : 'block';
  // Ok ikonunu güncelle
  const kartId = tblId.replace('kk-tbl-','');
  const arr = document.getElementById('kk-arr-'+kartId);
  if(arr) arr.textContent = open ? '▼' : '▲';
}

// kkOzetBar artık kkListeRender içinde hesaplanıyor
function _kkDoldurBankaDd(secili){
  const sel=document.getElementById('kk-banka');
  if(!sel)return;
  const bankalar=[...new Set((_bkData||[]).map(k=>k.banka).filter(Boolean))].sort();
  sel.innerHTML='<option value="">— Seçiniz —</option>';
  bankalar.forEach(b=>{
    const o=document.createElement('option');
    o.value=b;o.text=b;
  ...
  if(secili==='DİĞER')oDiger.selected=true;
  sel.appendChild(oDiger);
  if(secili&&secili!=='DİĞER'&&!bankalar.includes(secili)){
    const oM=document.createElement('option');
    oM.value=secili;oM.text=secili+' (mevcut)';oM.selected=true;
    sel.insertBefore(oM,oDiger);
  }
}

function kkYeniKartAc(){
  _kkEditId=null;
  document.getElementById('kk-mod-baslik').textContent='Yeni Kredi Kartı';
  ['kk-kart-no','kk-limit','kk-kesim-gun','kk-odeme-gun','kk-gecikme-faiz','kk-notlar'].forEach(id=>{
    const el=document.getElementById(id);if(el)el.value='';
  });
  document.getElementById('kk-firma').value='ULUSAL';
  _kkDoldurBankaDd('');
  document.getElementById('kk-mod').style.display='flex';
}

function kkDuzenle(id){
  const k=_kkData.find(x=>String(x.id)===String(id));if(!k)return;
  _kkEditId=id;
  document.getElementById('kk-mod-baslik').textContent='Kredi Kartı Düzenle';
  _kkDoldurBankaDd(k.banka||'');
  const f=(fid,val)=>{const el=document.getElementById(fid);if(el)el.value=val||'';};
  f('kk-kart-no',k.kart_no);f('kk-limit',k.limit);f('kk-kesim-gun',k.kesim_gun);
  f('kk-odeme-gun',k.odeme_gun);f('kk-gecikme-faiz',k.gecikme_faiz);f('kk-notlar',k.notlar);
  document.getElementById('kk-firma').value=k.firma||'ULUSAL';
  document.getElementById('kk-mod').style.display='flex';
}

function kkKapat(){document.getElementById('kk-mod').style.display='none';_kkEditId=null;}

async function kkKaydet(){
  const gv=id=>{const el=document.getElementById(id);return el?el.value.trim():'';};
  const banka=gv('kk-banka');
  if(!banka){toast('Banka seçiniz!','e');return;}
  const limit=parseFloat(gv('kk-limit'))||0;
  if(limit<=0){toast('Limit giriniz!','e');return;}
  // Mevcut kart varsa harcamaları koru
  const eskiKart=_kkEditId?_kkData.find(x=>String(x.id)===String(_kkEditId)):null;
  const kayit={
    id:_kkEditId||String(Date.now()),
    banka, firma:gv('kk-firma'),
    kart_no:gv('kk-kart-no'), limit,
    kesim_gun:parseInt(gv('kk-kesim-gun'))||null,
    odeme_gun:parseInt(gv('kk-odeme-gun'))||null,
    gecikme_faiz:parseFloat(gv('kk-gecikme-faiz'))||0,
    notlar:gv('kk-notlar'),
    harcamalar:eskiKart?eskiKart.harcamalar||[]:[]
  };
  try{
    const r=await api('/api/kk/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(kayit)});
    const d=await r.json();
    if(d.ok){toast('Kaydedildi ✓','s');kkKapat();renderKK();}
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata: '+e.message,'e');}
}

async function kkSil(id){
  if(!confirm('Bu kredi kartı ve tüm harcama kayıtları silinsin mi?'))return;
  try{
    const r=await api('/api/kk/sil',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
    const d=await r.json();
    if(d.ok){toast('Silindi','i');if(_kkAktifKartId===id)_kkAktifKartId=null;renderKK();}
    else toast('Hata','e');
  }catch(e){toast('Hata','e');}
}

function kkHarcamaTurDegis(){
  const isOdeme=document.getElementById('kk-h-tur-odeme').checked;
  document.getElementById('kk-h-odeme-info').style.display=isOdeme?'block':'none';
  document.getElementById('kk-h-firma-lbl').textContent=isOdeme?'Ödeme Yapılan Banka/Kaynak *':'Harcama Yapılan Yer *';
  document.getElementById('kk-h-firma').placeholder=isOdeme?'Banka transferi, havale...':'Market, Restoran...';
  document.getElementById('kk-h-kategori-wrap').style.display=isOdeme?'none':'block';
  document.getElementById('kk-harcama-baslik').textContent=isOdeme?'Kart Ödemesi Ekle':'Harcama Ekle';
  document.getElementById('kk-h-kaydet-btn').textContent=isOdeme?'💰 Ödeme Kaydet':'💾 Kaydet';
  // Seçili radio butonun border rengini güncelle
  document.getElementById('kk-h-tur-harcama-lbl').style.borderColor=isOdeme?'var(--b1)':'var(--acc)';
  document.getElementById('kk-h-tur-odeme-lbl').style.borderColor=isOdeme?'var(--g)':'var(--b1)';
}

function kkHarcamaEkleAc(kkId){
  _kkAktifKartId=kkId;_kkHarcamaEditId=null;
  document.getElementById('kk-harcama-baslik').textContent='Harcama / Ödeme Ekle';
  const bugun=new Date().toISOString().split('T')[0];
  ['kk-h-firma','kk-h-tutar','kk-h-aciklama'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  document.getElementById('kk-h-tarih').value=bugun;
  document.getElementById('kk-h-kategori').value='';
  // Harcama seçili başlat
  document.getElementById('kk-h-tur-harcama').checked=true;
  kkHarcamaTurDegis();
  document.getElementById('kk-harcama-mod').style.display='flex';
}

function kkHarcamaDuzenle(kkId,harcamaId){
  const kart=_kkData.find(x=>String(x.id)===String(kkId));
  const h=(kart?.harcamalar||[]).find(x=>String(x.id)===String(harcamaId));
  if(!kart||!h)return;
  _kkAktifKartId=kkId;_kkHarcamaEditId=harcamaId;
  const f=(fid,val)=>{const el=document.getElementById(fid);if(el)el.value=val||'';};
  f('kk-h-tarih',h.tarih);f('kk-h-firma',h.firma);f('kk-h-tutar',h.tutar);
  f('kk-h-kategori',h.kategori);f('kk-h-aciklama',h.aciklama);
  // Ödeme mi harcama mı?
  const isOdeme=h.odendi===true||h.tur==='odeme';
  if(isOdeme) document.getElementById('kk-h-tur-odeme').checked=true;
  else document.getElementById('kk-h-tur-harcama').checked=true;
  kkHarcamaTurDegis();
  document.getElementById('kk-harcama-mod').style.display='flex';
}

function kkHarcamaKapat(){document.getElementById('kk-harcama-mod').style.display='none';_kkHarcamaEditId=null;}

async function kkHarcamaKaydet(){
  const gv=id=>{const el=document.getElementById(id);return el?el.value.trim():'';};
  const tarih=gv('kk-h-tarih');const firma=gv('kk-h-firma');const tutar=parseFloat(gv('kk-h-tutar'))||0;
  const isOdeme=document.getElementById('kk-h-tur-odeme').checked;
  if(!tarih){toast('Tarih giriniz!','e');return;}
  if(!firma){toast(isOdeme?'Ödeme kaynağı giriniz!':'Firma giriniz!','e');return;}
  if(tutar<=0){toast('Tutar giriniz!','e');return;}
  const harcama={
    id:_kkHarcamaEditId||String(Date.now()),
    tarih, firma, tutar,
    tur: isOdeme?'odeme':'harcama',
    odendi: isOdeme,  // Ödeme kaydı limit düşürmez
    kategori: isOdeme?'Kart Ödemesi':gv('kk-h-kategori'),
    aciklama:gv('kk-h-aciklama')
  };
  try{
    const r=await api('/api/kk/harcama/ekle',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({kk_id:_kkAktifKartId,harcama})});
    const d=await r.json();
    if(d.ok){
      toast(isOdeme?'Ödeme kaydedildi — limit güncellendi ✓':'Harcama kaydedildi ✓','s');
      kkHarcamaKapat();renderKK();
    } else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata: '+e.message,'e');}
}

async function kkHarcamaSil(kkId,harcamaId){
  if(!confirm('Harcama kaydı silinsin mi?'))return;
  try{
    const r=await api('/api/kk/harcama/sil',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({kk_id:kkId,harcama_id:harcamaId})});
    const d=await r.json();
    if(d.ok){toast('Silindi','i');renderKK();}
    else toast('Hata','e');
  }catch(e){toast('Hata','e');}
}


// ── KREDİ YÖNETİMİ MODÜLÜ ─────────────────────────────────────────────────────
let _taksitliData = [];
let _taksitliEditId = null;
let _hesaplananTaksitler = [];

async function renderTaksitli(){
  // _bkData boşsa banka kartlarını yükle (banka dropdown için gerekli)
  if(!_bkData || !_bkData.length){
    try{
      const br=await api('/api/banka_kart'); const bd=await br.json();
      _bkData = bd.records||[];
    }catch(e){ _bkData=[]; }
  }
  const r = await api('/api/taksitli/liste'); const d = await r.json();
  _taksitliData = d.records || [];
  _renderTaksitliOzetBar();
  _renderTaksitliList();
  // Filtre dropdown'larını doldur
  const firmalar = [...new Set(_taksitliData.map(k=>k.firma).filter(Boolean))].sort();
  const bankalar = [...new Set(_taksitliData.map(k=>k.banka).filter(Boolean))].sort();
  const fFirma = document.getElementById('taksitli-filtre-firma');
  const fBanka = document.getElementById('taksitli-filtre-banka');
  const mevFirma = fFirma?.value; const mevBanka = fBanka?.value;
  if(fFirma){ fFirma.innerHTML='<option value="">Tüm Firmalar</option>'+firmalar.map(f=>`<option${f===mevFirma?' selected':''}>${f}</option>`).join(''); }
  if(fBanka){ fBanka.innerHTML='<option value="">Tüm Bankalar</option>'+bankalar.map(b=>`<option${b===mevBanka?' selected':''}>${b}</option>`).join(''); }
}

function _renderTaksitliOzetBar(){
  const bar = document.getElementById('taksitli-ozet-bar');
  if(!bar) return;
  const aktif = _taksitliData;
  const topAna = aktif.reduce((s,k)=>s+parseFloat(k.ana_para||0),0);
  const topKalan = aktif.reduce((s,k)=>{
    const bekl = (k.taksitler||[]).filter(t=>t.durum!=='ÖDENDİ').reduce((a,t)=>a+parseFloat(t.tutar||0),0);
    return s+bekl;
  },0);
  const topOdenen = aktif.reduce((s,k)=>{
    const od = (k.taksitler||[]).filter(t=>t.durum==='ÖDENDİ').reduce((a,t)=>a+parseFloat(t.tutar||0),0);
    return s+od;
  },0);
  const fmt=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  bar.innerHTML=`
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:14px 16px">
      <div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px">TOPLAM KREDİ</div>
      <div style="font-size:22px;font-weight:700;font-family:var(--mono);color:var(--t1)">${aktif.length} adet</div>
    </div>
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:14px 16px">
      <div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px">TOPLAM ANA PARA</div>
      <div style="font-size:18px;font-weight:700;font-family:var(--mono);color:var(--acc)">₺${fmt(topAna)}</div>
    </div>
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:14px 16px">
      <div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px">ÖDENDİ</div>
      <div style="font-size:18px;font-weight:700;font-family:var(--mono);color:#1D9E75">₺${fmt(topOdenen)}</div>
    </div>
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;padding:14px 16px">
      <div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px">KALAN BORÇ</div>
      <div style="font-size:18px;font-weight:700;font-family:var(--mono);color:#E24B4A">₺${fmt(topKalan)}</div>
    </div>`;
}

function _renderTaksitliList(){
  const list = document.getElementById('taksitli-list');
  if(!list) return;
  const fFirma = document.getElementById('taksitli-filtre-firma')?.value || '';
  const fBanka = document.getElementById('taksitli-filtre-banka')?.value || '';
  const fmt=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const filtered = _taksitliData.filter(k=>
    (!fFirma||k.firma===fFirma)&&(!fBanka||k.banka===fBanka)
  );
  if(!filtered.length){
    list.innerHTML='<div style="text-align:center;padding:40px;color:var(--t3);font-size:13px">Henüz kredi kaydı yok — "+ Yeni Kredi" ile ekleyin</div>';
    return;
  }
  list.innerHTML = filtered.map(k=>{
    const taksitler = k.taksitler||[];
    const isDoner = k.is_doner || ['ROTATİF','KMH','TEMİNAT MEKTUBU'].includes(k.tur);
    const odendi = taksitler.filter(t=>t.durum==='ÖDENDİ').length;
    const bekliyor = taksitler.filter(t=>t.durum!=='ÖDENDİ').length;
    const topTutar = isDoner ? parseFloat(k.ana_para||0) : taksitler.reduce((s,t)=>s+parseFloat(t.tutar||0),0);
    const odenenTutar = taksitler.filter(t=>t.durum==='ÖDENDİ').reduce((s,t)=>s+parseFloat(t.tutar||0),0);
    const kullanimTutar = isDoner ? parseFloat(k.kullanim||0) : odenenTutar;
    const kalanTutar = isDoner ? (parseFloat(k.ana_para||0)-parseFloat(k.kullanim||0)) : (topTutar-odenenTutar);
    const pct = isDoner
      ? (parseFloat(k.ana_para||0)>0 ? Math.round(parseFloat(k.kullanim||0)/parseFloat(k.ana_para||0)*100) : 0)
      : (taksitler.length ? Math.round(odendi/taksitler.length*100) : 0);
    const renk = isDoner
      ? (pct>80?'#E24B4A':pct>50?'#EF9F27':'#1D9E75')
      : (pct===100?'#1D9E75':pct>50?'#EF9F27':'#E24B4A');
    const bugun = new Date(); bugun.setHours(0,0,0,0);
    const yaklasan = taksitler
      .filter(t=>t.durum!=='ÖDENDİ')
      .map(t=>{try{const [g,m,y]=t.tarih.split('.');return{...t,dt:new Date(y,m-1,g)};}catch{return null;}})
      .filter(t=>t&&t.dt>=bugun)
      .sort((a,b)=>a.dt-b.dt)[0];
    return `
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;margin-bottom:12px;overflow:hidden">
      <!-- Başlık -->
      <div style="padding:14px 16px;display:grid;grid-template-columns:1fr auto auto;gap:12px;align-items:center;cursor:pointer"
           onclick="taksitliToggle('tbl-${k.id}')">
        <div>
          <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
            <span style="font-weight:700;font-size:14px">${k.banka||'—'}</span>
            <span class="bdg ${FC[k.firma]||'bbi'}">${k.firma||''}</span>
            <span style="font-size:11px;color:var(--acc);font-family:var(--mono)">${k.kredi_no||''}</span>
            <span style="font-size:11px;color:var(--t3);background:var(--s3);padding:2px 8px;border-radius:10px">${k.tur||'TAKSİTLİ KREDİ'}</span>
          </div>
          <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:6px;font-size:11px;color:var(--t3)">
            ${!isDoner&&k.kullanim_tarihi?`<span>📌 Kullandırım: <strong style="color:var(--t2)">${k.kullanim_tarihi}</strong></span>`:''}
            ${isDoner ? `
              <span>Limit: <strong style="color:var(--t2);font-family:var(--mono)">₺${fmt(k.ana_para)}</strong></span>
              <span>Kullanılan: <strong style="color:var(--am);font-family:var(--mono)">₺${fmt(k.kullanim||0)}</strong></span>
              <span>Kullanılabilir: <strong style="color:#1D9E75;font-family:var(--mono)">₺${fmt(kalanTutar)}</strong></span>
              <span>%${parseFloat(k.faiz_orani||0).toFixed(2)} yıllık</span>
              ${k.komisyon_oran?`<span>Komisyon: %${parseFloat(k.komisyon_oran).toFixed(2)}</span>`:''}
              ${k.vade_bitis?`<span>📅 Vade: ${k.vade_bitis}</span>`:''}
              ${k.yenileme_gun?`<span>🔄 ${k.yenileme_gun} günde yenileme</span>`:''}
            ` : `
              <span>Ana Para: <strong style="color:var(--t2);font-family:var(--mono)">₺${fmt(k.ana_para)}</strong></span>
              <span>%${parseFloat(k.faiz_orani||0).toFixed(2)} yıllık</span>
              <span>${taksitler.length} taksit</span>
              <span style="color:#1D9E75">✅ ${odendi} ödendi</span>
              <span style="color:#E24B4A">⏳ ${bekliyor} bekliyor</span>
              ${yaklasan?`<span style="color:var(--am)">📅 Sonraki: ${yaklasan.tarih} — ₺${fmt(yaklasan.tutar)}</span>`:''}
            `}
          </div>
          <!-- İlerleme çubuğu -->
          <div style="margin-top:8px;height:6px;background:var(--s3);border-radius:3px;overflow:hidden">
            <div style="height:100%;width:${pct}%;background:${renk};border-radius:3px;transition:.4s"></div>
          </div>
          <div style="font-size:10px;color:var(--t3);margin-top:2px">
            ${isDoner
              ? `${pct}% kullanıldı · Kullanılabilir: <strong style="color:${renk};font-family:var(--mono)">₺${fmt(kalanTutar)}</strong>`
              : `${pct}% tamamlandı · Kalan borç: <strong style="color:${renk};font-family:var(--mono)">₺${fmt(kalanTutar)}</strong>`
            }
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:10px;color:var(--t3)">${isDoner?'KULLANILABİLİR':'KALAN BORÇ'}</div>
          <div style="font-family:var(--mono);font-size:16px;font-weight:700;color:${renk}">₺${fmt(kalanTutar)}</div>
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0">
          ${!isDoner?`<button onclick="event.stopPropagation();taksitTarihAc('${k.id}')" class="btn bgh" style="padding:4px 9px;font-size:12px" title="Taksit tarihlerini düzenle">📅</button>`:''}
          <button onclick="event.stopPropagation();taksitliDuzenle('${k.id}')" class="btn bgh" style="padding:4px 9px;font-size:12px" title="Düzenle">✏️</button>
          <button onclick="event.stopPropagation();taksitliSil('${k.id}')" class="btn bgh" style="padding:4px 9px;font-size:12px;color:var(--r)" title="Sil">🗑</button>
        </div>
      </div>
      <!-- Detay (accordion) -->
      <div id="tbl-${k.id}" style="display:none;border-top:1px solid var(--b1)">
        ${isDoner ? `
        <!-- Döner Kredi Detay Kartı -->
        <div style="padding:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px">
          <div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">KREDİ LİMİTİ</div>
            <div style="font-family:var(--mono);font-size:15px;font-weight:700">₺${fmt(k.ana_para)}</div>
          </div>
          <div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">KULLANILAN</div>
            <div style="font-family:var(--mono);font-size:15px;font-weight:700;color:var(--am)">₺${fmt(k.kullanim||0)}</div>
          </div>
          <div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">KULLANILABİLİR</div>
            <div style="font-family:var(--mono);font-size:15px;font-weight:700;color:#1D9E75">₺${fmt(kalanTutar)}</div>
          </div>
          <div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">FAİZ (YILLIK)</div>
            <div style="font-family:var(--mono);font-size:15px;font-weight:700">%${parseFloat(k.faiz_orani||0).toFixed(2)}</div>
          </div>
          ${k.komisyon_oran?`<div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">KOMİSYON</div>
            <div style="font-family:var(--mono);font-size:15px;font-weight:700">%${parseFloat(k.komisyon_oran).toFixed(2)}</div>
          </div>`:''}
          ${k.vade_bitis?`<div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">VADE BİTİŞİ</div>
            <div style="font-size:13px;font-weight:700">${k.vade_bitis}</div>
          </div>`:''}
          ${k.yenileme_gun?`<div style="background:var(--s3);border-radius:8px;padding:12px">
            <div style="font-size:10px;color:var(--t3);margin-bottom:3px">YENİLEME</div>
            <div style="font-size:13px;font-weight:700">${k.yenileme_gun} gün</div>
          </div>`:''}
        </div>
        <div style="padding:10px 16px;font-size:11px;color:var(--t3);border-top:1px solid var(--b1);background:var(--s3)">
          ℹ️ Döner kredilerde taksit tablosu bulunmaz. Kullanım bilgilerini Banka Kartları ekranında limitler üzerinden takip edebilirsiniz.
        </div>
        ` : `
          ...
          <table style="width:100%;border-collapse:collapse;font-size:12px">
            <thead style="position:sticky;top:0;z-index:1">
              <tr style="background:var(--s3)">
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:center">No</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600">Tarih</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:right">Anapara</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:right">Faiz</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:right">BSMV</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:right">Taksit</th>
                <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:center">Durum</th>
              </tr>
            </thead>
            <tbody>
              ${taksitler.map(t=>{
                const od = t.durum==='ÖDENDİ';
                return `<tr style="border-bottom:1px solid var(--b1);${od?'opacity:.65':''}">
                  <td style="padding:6px 10px;text-align:center;color:var(--t3)">${t.no}</td>
                  <td style="padding:6px 10px;font-family:var(--mono);color:var(--t2)">${t.tarih||'—'}</td>
                  <td style="padding:6px 10px;text-align:right;font-family:var(--mono)">₺${fmt(t.anapara)}</td>
                  <td style="padding:6px 10px;text-align:right;font-family:var(--mono)">₺${fmt(t.faiz)}</td>
                  <td style="padding:6px 10px;text-align:right;font-family:var(--mono)">₺${fmt(t.bsmv)}</td>
                  <td style="padding:6px 10px;text-align:right;font-family:var(--mono);font-weight:700">₺${fmt(t.tutar)}</td>
                  <td style="padding:6px 10px;text-align:center;white-space:nowrap">
                    <span class="bdg ${od?'bgi':'bri'}" onclick="taksitDurumToggle('${k.id}',${t.no},'${od?'ÖDENMEDİ':'ÖDENDİ'}')"
                      style="cursor:pointer;user-select:none;margin-right:4px" title="Tıkla: durum değiştir">${od?'ÖDENDİ':'ÖDENMEDİ'}</span>
                    ${od&&t.odeme_tarihi?`<span style="display:block;font-size:9px;color:var(--g);font-family:var(--mono)">${t.odeme_tarihi}</span>`:''}
                    <button onclick="taksitDuzenleAc('${k.id}',${t.no-1})" class="btn bgh" style="padding:2px 6px;font-size:10px" title="Düzenle">✏️</button>
                    <button onclick="taksitSatirSil('${k.id}',${t.no})" class="btn bgh" style="padding:2px 6px;font-size:10px;color:var(--r)" title="Sil">✕</button>
                  </td>
                </tr>`;
              }).join('')}
            </tbody>
        ...
        <!-- Alt özet -->
        <div style="padding:10px 16px;background:var(--s3);display:flex;gap:20px;flex-wrap:wrap;font-size:12px;border-top:1px solid var(--b1);font-family:var(--mono)">
          <span>Toplam: <strong>₺${fmt(topTutar)}</strong></span>
          <span style="color:#1D9E75">Ödendi: <strong>₺${fmt(odenenTutar)}</strong></span>
          <span style="color:#E24B4A">Kalan: <strong>₺${fmt(kalanTutar)}</strong></span>
        </div>
        </div>`}
      </div>
    </div>`;
...
function _tkDoldurBankaDd(secili){
  const sel = document.getElementById('tk-banka');
  if(!sel) return;
  const bankalar = [...new Set((_bkData||[]).map(k=>k.banka).filter(Boolean))].sort();
  sel.innerHTML = '<option value="">— Seçiniz —</option>';
  bankalar.forEach(b=>{
  ...
  // Seçili değer listede yoksa "mevcut" olarak ekle
  if(secili && !bankalar.includes(secili)){
    const o = document.createElement('option');
    o.value = secili; o.text = secili + ' (mevcut)';
    sel.appendChild(o);
  }
  // Seçimi uygula
...
function tkTurDegis(){
  const tur = document.getElementById('tk-tur')?.value || '';
  const donerTurler = ['ROTATİF','KMH','VİNOV','TEMİNAT MEKTUBU'];
  const isDoner = donerTurler.includes(tur);
  // Taksitli alanlar
  ['tk-alan-vade','tk-alan-bsmv','tk-alan-kullanim-tarihi'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.style.display = isDoner ? 'none' : 'block';
  });
  document.getElementById('tk-alan-baslangic').querySelector('label').textContent =
    isDoner ? 'Başlangıç Tarihi' : 'İlk Taksit Tarihi *';
  // Döner kredi alanları
  ['tk-alan-kullanim','tk-alan-vade-bit','tk-alan-yenileme'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.style.display = isDoner ? 'block' : 'none';
  });
  document.getElementById('tk-alan-komisyon').style.display =
    (tur==='TEMİNAT MEKTUBU'||tur==='ROTATİF'||tur==='VİNOV') ? 'block' : 'none';
  // VİNOV ve KMH için vade de göster
  if(tur==='VİNOV'||tur==='KMH'){
    document.getElementById('tk-alan-vade').style.display='block';
  }
  if(tur==='VİNOV'){
    document.getElementById('tk-alan-bsmv').style.display='block';
  }
  // Tür bazlı açıklamalar
  const acEl = document.getElementById('tk-kullanim-tarihi-aciklama');
  if(acEl){
    if(tur==='ROTATİF') acEl.textContent = '📌 Çeyrek sonları (Mart/Haziran/Eylül/Aralık) faiz, vade sonunda ana para tahsil edilir.';
    else if(tur==='KMH') acEl.textContent = '📌 Her ayın son günü anapara + faiz ödemesi yapılır.';
    else if(tur==='VİNOV') acEl.textContent = '📌 Vade sonunda tek seferde anapara + faiz tahsil edilir.';
    else acEl.textContent = '';
  }
  const lblEl = document.getElementById('tk-kullanim-tarihi-lbl');
  if(lblEl){
    if(tur==='ROTATİF'||tur==='KMH') lblEl.textContent = 'Kullandırım Tarihi * (çeyrek/taksit başlangıcı)';
    else lblEl.textContent = 'Kullandırım Tarihi';
  }

  // Önizlemeyi güncelle
  if(!isDoner) taksitHesapla();
  else document.getElementById('taksit-onizleme').style.display='none';
}

function taksitliToggle(id){
  const el=document.getElementById(id);
  if(el) el.style.display=el.style.display==='none'?'block':'none';
}

function taksitliYeniAc(){
  _taksitliEditId=null;
  document.getElementById('taksitli-mod-baslik').textContent='Yeni Kredi';
  // Tüm alanları sıfırla
  ['tk-kredi-no','tk-ana-para','tk-faiz','tk-vade','tk-baslangic','tk-kullanim-tarihi','tk-aciklama',
   'tk-kullanim','tk-vade-bit','tk-komisyon'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.value='';
  });
  document.getElementById('tk-firma').value='ULUSAL';
  document.getElementById('tk-tur').value='TAKSİTLİ KREDİ';
  document.getElementById('tk-bsmv').value='5';
  document.getElementById('tk-yenileme').value='';
  document.getElementById('taksit-onizleme').style.display='none';
  _hesaplananTaksitler=[];
  // Banka dropdown'ı doldur
  _tkDoldurBankaDd('');
  // Form alanlarını türe göre göster/gizle
  tkTurDegis();
  document.getElementById('taksitli-mod').style.display='flex';
}

function taksitliKapat(){
  document.getElementById('taksitli-mod').style.display='none';
  _taksitliEditId=null; _hesaplananTaksitler=[];
}

async function taksitliDuzenle(id){
  const k=_taksitliData.find(x=>String(x.id)===String(id));
  if(!k)return;
  _taksitliEditId=id;
  document.getElementById('taksitli-mod-baslik').textContent='Kredi Düzenle';
  const s=(fid,val)=>{const el=document.getElementById(fid);if(el)el.value=val??'';};
  s('tk-firma',k.firma);s('tk-kredi-no',k.kredi_no);
  // Banka listesi yoksa önce yükle
  if(!_bkData||!_bkData.length){
    try{const br=await api('/api/banka_kart');const bd=await br.json();_bkData=bd.records||[];}catch{}
  }
  // Banka dropdown'ı doldur ve seçili değeri ata
  _tkDoldurBankaDd(k.banka||'');
  // Seçili değeri garantiye al — dropdown zaten seçili yapıyor ama tekrar kontrol
  const bnkSel=document.getElementById('tk-banka');
  if(bnkSel && k.banka){
    // Listede varsa seç, yoksa mevcut seçili kalacak (zaten _tkDoldurBankaDd halletti)
    const opts=[...bnkSel.options];
    const found=opts.find(o=>o.value===k.banka);
    if(found) bnkSel.value=k.banka;
  }
  s('tk-tur',k.tur);s('tk-ana-para',k.ana_para);s('tk-faiz',k.faiz_orani);
  s('tk-vade',k.vade_ay);s('tk-aciklama',k.aciklama);
  s('tk-bsmv',k.bsmv_oran||5);
  // Döner kredi alanları
  s('tk-kullanim',k.kullanim||0);
  s('tk-komisyon',k.komisyon_oran||0);
  // Vade bitiş: YYYY-MM-DD veya DD.MM.YYYY → input[date] için YYYY-MM-DD
  if(k.vade_bitis){
    let vb=k.vade_bitis;
    if(vb.includes('.')){const[g,m,y]=vb.split('.');vb=`${y}-${m}-${g}`;}
    s('tk-vade-bit',vb);
  } else s('tk-vade-bit','');
  s('tk-yenileme',k.yenileme_gun||'');
  // Başlangıç tarihi: taksitli → ilk taksit, döner → vade_bitis veya boş
  const donerTurler=['ROTATİF','KMH','VİNOV','TEMİNAT MEKTUBU'];
  const isDoner=donerTurler.includes((k.tur||'').toUpperCase());
  if(!isDoner && k.taksitler && k.taksitler.length){
    try{
      const [g,m,y]=k.taksitler[0].tarih.split('.');
      document.getElementById('tk-baslangic').value=`${y}-${m}-${g}`;
    }catch{}
  }
  // Form alanlarını türe göre göster/gizle
  tkTurDegis();
...
async function taksitliSil(id){
  if(!confirm('Bu kredi ve ilgili ödenmemiş taksitler silinecek. Emin misiniz?')) return;
  try{
    const r=await api('/api/taksitli/sil',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
    const d=await r.json();
    if(d.ok){toast('Kredi silindi','s');renderTaksitli();}
    else toast('Hata: '+d.msg,'e');
  }catch(e){toast('Hata','e');}
}

function taksitHesapla(){
  const anaPara = parseFloat(document.getElementById('tk-ana-para')?.value)||0;
  const faizYillik = parseFloat(document.getElementById('tk-faiz')?.value)||0;
  const vade = parseInt(document.getElementById('tk-vade')?.value)||0;
  const bsmvOran = parseFloat(document.getElementById('tk-bsmv')?.value)||5;
  const basStr = document.getElementById('tk-baslangic')?.value;
  if(!anaPara||!faizYillik||!vade||!basStr){
    document.getElementById('taksit-onizleme').style.display='none';
    return;
  }
  const faizAylik = faizYillik/100/12;
  // Eşit taksit (anüite) formülü
  let taksitTutari;
  if(faizAylik===0){ taksitTutari=anaPara/vade; }
  else { taksitTutari=anaPara*(faizAylik*Math.pow(1+faizAylik,vade))/(Math.pow(1+faizAylik,vade)-1); }
  const fmt=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const [y,m,g]=basStr.split('-');
  let bas=new Date(y,m-1,g);
  _hesaplananTaksitler=[];
  let kalanAna=anaPara;
  for(let i=1;i<=vade;i++){
    const tarih=`${String(bas.getDate()).padStart(2,'0')}.${String(bas.getMonth()+1).padStart(2,'0')}.${bas.getFullYear()}`;
    const faizTutar=kalanAna*faizAylik;
    const bsmv=faizTutar*bsmvOran/100;
    const anapara=i<vade?taksitTutari-faizTutar-bsmv:kalanAna;
    const topTaksit=anapara+faizTutar+bsmv;
    kalanAna-=anapara;
    _hesaplananTaksitler.push({
      no:i,tarih,
      anapara:Math.round(anapara*100)/100,
      faiz:Math.round(faizTutar*100)/100,
      bsmv:Math.round(bsmv*100)/100,
      tutar:Math.round(topTaksit*100)/100,
      durum:'ÖDENMEDİ'
    });
    bas=new Date(bas.getFullYear(),bas.getMonth()+1,bas.getDate());
  }
  const topToplam=_hesaplananTaksitler.reduce((s,t)=>s+t.tutar,0);
  const topFaiz=_hesaplananTaksitler.reduce((s,t)=>s+t.faiz,0);
  const topBsmv=_hesaplananTaksitler.reduce((s,t)=>s+t.bsmv,0);
  document.getElementById('taksit-ozet-satir').innerHTML=
    `<span>Aylık Taksit: <strong style="color:var(--acc)">₺${fmt(taksitTutari)}</strong></span>
     <span>Toplam Ödeme: <strong>₺${fmt(topToplam)}</strong></span>
     <span style="color:var(--r)">Toplam Faiz: <strong>₺${fmt(topFaiz)}</strong></span>
     <span>BSMV: <strong>₺${fmt(topBsmv)}</strong></span>`;
  document.getElementById('taksit-onizleme-tbody').innerHTML=
    _hesaplananTaksitler.map(t=>`
    <tr style="border-bottom:1px solid var(--b1)">
      <td style="padding:5px 8px;color:var(--t3)">${t.no}</td>
      <td style="padding:5px 8px;font-family:var(--mono)">${t.tarih}</td>
      <td style="padding:5px 8px;text-align:right;font-family:var(--mono)">₺${fmt(t.anapara)}</td>
      <td style="padding:5px 8px;text-align:right;font-family:var(--mono);color:var(--r)">₺${fmt(t.faiz)}</td>
      <td style="padding:5px 8px;text-align:right;font-family:var(--mono)">₺${fmt(t.bsmv)}</td>
      <td style="padding:5px 8px;text-align:right;font-family:var(--mono);font-weight:700">₺${fmt(t.tutar)}</td>
    </tr>`).join('');
  document.getElementById('taksit-onizleme').style.display='block';
  console.log('taksitHesapla: ', _hesaplananTaksitler.length, 'taksit hesaplandi');
}

async function taksitliKaydet(){
  const gv=id=>document.getElementById(id)?.value.trim()||'';
  const tur = gv('tk-tur');
  const donerTurler = ['ROTATİF','KMH','VİNOV','TEMİNAT MEKTUBU'];
  const isDoner = donerTurler.includes(tur); // VİNOV da döner kredi
  // Taksitli ise kaydetten önce taksitleri yeniden hesapla
  if(!isDoner){
    taksitHesapla();
    if(!_hesaplananTaksitler.length){
      toast('Taksit planı hesaplanamadı — Ana Para, Faiz, Vade ve İlk Taksit Tarihi giriniz','e');
      return;
    }
  }
  const kredi={
    id: _taksitliEditId||null,
    firma:gv('tk-firma'),banka:gv('tk-banka'),kredi_no:gv('tk-kredi-no'),
    tur,
    ana_para:parseFloat(gv('tk-ana-para'))||0,
    kullanim_tarihi:(()=>{const v=gv('tk-kullanim-tarihi');if(!v)return '';const[y,m,g]=v.split('-');return `${g}.${m}.${y}`;})(),
    faiz_orani:parseFloat(gv('tk-faiz'))||0,
    vade_ay:parseInt(gv('tk-vade'))||0,
    bsmv_oran:parseFloat(gv('tk-bsmv'))||5,
    aciklama:gv('tk-aciklama'),
    // Döner kredi alanları
    kullanim:parseFloat(gv('tk-kullanim'))||0,
    vade_bitis:gv('tk-vade-bit'),
    yenileme_gun:parseInt(gv('tk-yenileme'))||0,
    komisyon_oran:parseFloat(gv('tk-komisyon'))||0,
    is_doner: isDoner,
    taksitler: isDoner ? [] : _hesaplananTaksitler
  };
  if(!kredi.banka){toast('Banka adı giriniz','e');return;}
  if(!kredi.kredi_no){toast('Kredi numarası giriniz','e');return;}
  if(!kredi.ana_para){toast('Limit / Ana para giriniz','e');return;}
  // Düzenleme modunda mevcut ödendi durumlarını koru
  if(_taksitliEditId){
    const mevcut=_taksitliData.find(k=>String(k.id)===String(_taksitliEditId));
    if(mevcut&&mevcut.taksitler){
      const odendiMap={};
      mevcut.taksitler.forEach(t=>{if(t.durum==='ÖDENDİ')odendiMap[t.no]=true;});
      kredi.taksitler.forEach(t=>{if(odendiMap[t.no])t.durum='ÖDENDİ';});
    }
  }
  try{
    const r=await api('/api/taksitli/kaydet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(kredi)});
    const d=await r.json();
    if(d.ok){
      toast(d.msg,'s');
      taksitliKapat();
...
function taksitliSablonIndir(){
  const a=document.createElement('a');
  a.href='/api/taksitli/sablon';
  a.download='kredi_yonetimi_sablon.xlsx';
  a.click();
  toast('Şablon hazırlanıyor...','i');
}

async function taksitliExcelYukle(input){
  const file=input.files[0]; if(!file) return;
  if(!confirm(`"${file.name}" dosyasındaki krediler sisteme eklenecek. Devam edilsin mi?`)){input.value='';return;}
  const fd=new FormData(); fd.append('file',file);
  try{
    toast('Yükleniyor...','i');
    const r=await fetch('/api/taksitli/excel_yukle',{method:'POST',body:fd,credentials:'include'});
    const d=await r.json();
    if(d.ok){toast(d.msg,'s');renderTaksitli();}
    else toast('Hata: '+d.msg,'e');
  }catch(e){toast('Hata: '+e.message,'e');}
  input.value='';
}

function tsTutarHesapla(){
  const a=parseFloat(document.getElementById('ts-anapara')?.value)||0;
  const f=parseFloat(document.getElementById('ts-faiz')?.value)||0;
  const b=parseFloat(document.getElementById('ts-bsmv')?.value)||0;
  const el=document.getElementById('ts-tutar');
  if(el) el.value=Math.round((a+f+b)*100)/100;
}

function taksitDuzenleAc(krediId, taksitIdx){
  const k=_taksitliData.find(x=>String(x.id)===String(krediId));
  if(!k) return;
  const t=k.taksitler[taksitIdx];
  if(!t) return;
  document.getElementById('ts-kredi-id').value=krediId;
  document.getElementById('ts-taksit-no').value=t.no;
  document.getElementById('ts-no').value=t.no;
  // Tarih: DD.MM.YYYY → YYYY-MM-DD
  try{const[g,m,y]=t.tarih.split('.');document.getElementById('ts-tarih').value=`${y}-${m}-${g}`;}
  catch{document.getElementById('ts-tarih').value='';}
  document.getElementById('ts-anapara').value=t.anapara||0;
  document.getElementById('ts-faiz').value=t.faiz||0;
  document.getElementById('ts-bsmv').value=t.bsmv||0;
  document.getElementById('ts-tutar').value=t.tutar||0;
  document.getElementById('taksit-satir-mod').style.display='flex';
}

async function taksitSatirKaydet(){
  const krediId=document.getElementById('ts-kredi-id').value;
  const taksitNo=parseInt(document.getElementById('ts-taksit-no').value);
  const tarihRaw=document.getElementById('ts-tarih').value; // YYYY-MM-DD
  let tarih='';
  if(tarihRaw){const[y,m,g]=tarihRaw.split('-');tarih=`${g}.${m}.${y}`;}
  const p={kredi_id:krediId,taksit_no:taksitNo,tarih,
    anapara:parseFloat(document.getElementById('ts-anapara').value)||0,
    faiz:parseFloat(document.getElementById('ts-faiz').value)||0,
    bsmv:parseFloat(document.getElementById('ts-bsmv').value)||0,
    tutar:parseFloat(document.getElementById('ts-tutar').value)||0};
  try{
    const r=await api('/api/taksitli/taksit_satir_guncelle',{method:'POST',
      headers:{'Content-Type':'application/json'},body:JSON.stringify(p)});
    const d=await r.json();
    if(d.ok){
      toast('Güncellendi','s');
      document.getElementById('taksit-satir-mod').style.display='none';
      // Kredi Yönetimi ekranındaysa renderTaksitli, Ödeme Tablosu'ndaysa reload
      if(_taksitSatirSonraReload){ _taksitSatirSonraReload=false; reload(); }
      else renderTaksitli();
    } else toast('Hata: '+d.msg,'e');
  }catch(e){toast('Hata','e');}
}

async function taksitSatirSil(krediId, taksitNo){
  if(!confirm(`${taksitNo}. taksit satırı silinecek. Emin misiniz?`)) return;
  try{
    const r=await api('/api/taksitli/taksit_satir_sil',{method:'POST',
      headers:{'Content-Type':'application/json'},body:JSON.stringify({kredi_id:krediId,taksit_no:taksitNo})});
    const d=await r.json();
    if(d.ok){toast('Silindi','s');renderTaksitli();}
    else toast('Hata: '+d.msg,'e');
  }catch(e){toast('Hata','e');}
}

async function taksitDurumToggle(krediId, taksitNo, yeniDurum){
  // Ödendi yapılıyorsa tarih sor
  if(yeniDurum==='ÖDENDİ'){
    const tarih = prompt('Ödeme tarihi (GG.MM.YYYY) — boş bırakırsanız bugün kullanılır:',
      new Date().toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric'}));
    if(tarih===null) return; // iptal
    const odemeTarihi = tarih.trim() || new Date().toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric'});
    try{
      const r=await api('/api/taksitli/taksit_guncelle',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({kredi_id:krediId,taksit_no:taksitNo,durum:yeniDurum,odeme_tarihi:odemeTarihi})});
      const d=await r.json();
      if(d.ok){ renderTaksitli(); }
      else toast('Hata: '+d.msg,'e');
    }catch(e){toast('Hata','e');}
  } else {
    try{
      const r=await api('/api/taksitli/taksit_guncelle',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({kredi_id:krediId,taksit_no:taksitNo,durum:yeniDurum})});
      const d=await r.json();
      if(d.ok){ renderTaksitli(); }
      else toast('Hata: '+d.msg,'e');
    }catch(e){toast('Hata','e');}
...
// ── TAKSİT TARİH DÜZENLEME ────────────────────────────────────────────────────
let _ttTaksitler = [];

function taksitTarihAc(krediId){
  const k = _taksitliData.find(x=>String(x.id)===String(krediId));
  if(!k||!k.taksitler||!k.taksitler.length){ toast('Taksit bulunamadi','e'); return; }
  document.getElementById('tt-kredi-id').value = krediId;
  document.getElementById('tt-yeni-ilk-tarih').value = '';
  _ttTaksitler = k.taksitler.map(t=>({...t}));
  _ttRenderTablo();
  document.getElementById('taksit-tarih-mod').style.display='flex';
}

function _ttRenderTablo(){
  const fmt=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  document.getElementById('tt-tbody').innerHTML = _ttTaksitler.map((t,i)=>`
    <tr style="border-bottom:1px solid var(--b1);${t.durum==='ÖDENDİ'?'opacity:.6':''}">
      <td style="padding:6px 10px;text-align:center;color:var(--t3)">${t.no}</td>
      <td style="padding:6px 8px">
        <input type="date" value="${_ttTarihToInput(t.tarih)}"
          style="background:var(--s3);border:1px solid var(--b1);border-radius:4px;padding:3px 6px;font-size:11px;color:var(--t1);width:140px"
          onchange="_ttTarihDegis(${i},this.value)">
      </td>
      <td style="padding:6px 10px;text-align:right;font-family:var(--mono);font-size:11px">&#8378;${fmt(t.tutar)}</td>
      <td style="padding:6px 10px;text-align:center">
        <span class="bdg ${t.durum==='ÖDENDİ'?'bgi':'bri'}" style="font-size:10px">${t.durum==='ÖDENDİ'?'✓':'⏳'}</span>
      </td>
    </tr>`).join('');
}

function _ttTarihToInput(s){
  if(!s) return '';
  const p=s.split('.');
  return p.length===3?`${p[2]}-${p[1]}-${p[0]}`:'';
}

function _ttInputToTarih(s){
  if(!s) return '';
  const p=s.split('-');
  return p.length===3?`${p[2]}.${p[1]}.${p[0]}`:'';
}

function _ttTarihDegis(idx,val){
  if(_ttTaksitler[idx]) _ttTaksitler[idx].tarih=_ttInputToTarih(val);
}

function ttTumunuYenile(){
  const ilkStr=document.getElementById('tt-yeni-ilk-tarih').value;
  if(!ilkStr){toast('Ilk taksit tarihini giriniz','e');return;}
  const[y,m,g]=ilkStr.split('-');
  let bas=new Date(parseInt(y),parseInt(m)-1,parseInt(g));
  _ttTaksitler.forEach((t)=>{
    t.tarih=`${String(bas.getDate()).padStart(2,'0')}.${String(bas.getMonth()+1).padStart(2,'0')}.${bas.getFullYear()}`;
    bas=new Date(bas.getFullYear(),bas.getMonth()+1,bas.getDate());
  });
  _ttRenderTablo();
  toast(`${_ttTaksitler.length} taksit tarihi guncellendi — kaydetmeyi unutmayin`,'i');
}

async function ttKaydet(){
  const krediId=document.getElementById('tt-kredi-id').value;
  try{
    const r=await api('/api/taksitli/tarihler_guncelle',{method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({kredi_id:krediId,taksitler:_ttTaksitler})});
    const d=await r.json();
    if(d.ok){toast('Tarihler guncellendi','s');document.getElementById('taksit-tarih-mod').style.display='none';renderTaksitli();}
    else toast('Hata: '+d.msg,'e');
  }catch(e){toast('Hata','e');}
}
...
function toggleStWithDate(idx){
  const d=allData[idx];
  if(!d)return;
  if(d.durum==='ÖDENDİ'){toggleSt(idx);return;}
  _odendiIdx=idx;
  const bugun=new Date().toISOString().split('T')[0];
  document.getElementById('odendi-tarih-input').value=bugun;
  document.getElementById('odendi-tarih-mod').style.display='flex';
}

async function odendiTarihKaydet(){
  const idx=_odendiIdx; if(idx===null)return;
  const tv=document.getElementById('odendi-tarih-input').value;
  let odemeTarihi='';
  if(tv){const[y,m,g]=tv.split('-');odemeTarihi=`${g}.${m}.${y}`;}
  else{odemeTarihi=new Date().toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric'}).split('.').join('.');}
  document.getElementById('odendi-tarih-mod').style.display='none';
  _odendiIdx=null;
  try{
    const r=await api('/api/record/toggle',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({idx,odeme_tarihi:odemeTarihi})});
    const d=await r.json();
    if(d.ok){
      if(allData[idx]){allData[idx].durum=d.durum;if(d.odeme_tarihi)allData[idx].odeme_tarihi=d.odeme_tarihi;}
      filtered.forEach(r=>{if(r._idx===idx){r.durum=d.durum;if(d.odeme_tarihi)r.odeme_tarihi=d.odeme_tarihi;}});
      renderTbl();loadStats();
      toast('Odendi — '+odemeTarihi,'s');
    }else toast('Hata','e');
  }catch(e){toast('Hata','e');}
}

function odendiTarihIptal(){
  const idx=_odendiIdx; _odendiIdx=null;
  document.getElementById('odendi-tarih-mod').style.display='none';
  if(idx!==null)toggleSt(idx);
}
</script>

  <!-- Taksit Satırı Düzenle Modal -->
  <div class="ov" id="taksit-satir-mod" style="display:none">
    <div class="mod" style="max-width:480px;width:96%">
      <div class="mtit">Taksit Satırı Düzenle
        <button class="mclose" onclick="document.getElementById('taksit-satir-mod').style.display='none'">✕</button></div>
      <input type="hidden" id="ts-kredi-id">
      <input type="hidden" id="ts-taksit-no">
      <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div><label class="flb">Tarih</label>
          <input class="fi2" type="date" id="ts-tarih"></div>
        <div><label class="flb">Taksit No</label>
          <input class="fi2" type="number" id="ts-no" readonly style="opacity:.6"></div>
        <div><label class="flb">Anapara (₺)</label>
          <input class="fi2" type="number" id="ts-anapara" step="0.01" oninput="tsTutarHesapla()"></div>
        <div><label class="flb">Faiz (₺)</label>
          <input class="fi2" type="number" id="ts-faiz" step="0.01" oninput="tsTutarHesapla()"></div>
        <div><label class="flb">BSMV (₺)</label>
          <input class="fi2" type="number" id="ts-bsmv" step="0.01" oninput="tsTutarHesapla()"></div>
        <div><label class="flb">Toplam Taksit (₺)</label>
          <input class="fi2" type="number" id="ts-tutar" step="0.01" style="font-weight:700;color:var(--acc)"></div>
      </div>
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="taksitSatirKaydet()" style="flex:1">💾 Güncelle</button>
        <button class="btn bo" onclick="document.getElementById('taksit-satir-mod').style.display='none'">İptal</button>
      </div>
    </div>
  </div>

  <!-- Taksit Tarihleri Toplu Düzenleme Modal -->
  <div class="ov" id="taksit-tarih-mod" style="display:none">
    <div class="mod" style="max-width:560px;width:96%;max-height:88vh;overflow-y:auto">
      <div class="mtit">📅 Taksit Tarihlerini Düzenle
        <button class="mclose" onclick="document.getElementById('taksit-tarih-mod').style.display='none'">✕</button></div>
      <input type="hidden" id="tt-kredi-id">
      <div style="padding:12px 16px 8px;background:var(--s3);border-bottom:1px solid var(--b1);font-size:12px;color:var(--t3)">
        Tüm taksit tarihlerini güncellemek için ilk taksit tarihini değiştirin ya da her satırı ayrı ayrı düzenleyin.
      </div>
      <div style="padding:12px 16px;display:flex;gap:10px;align-items:flex-end;border-bottom:1px solid var(--b1)">
        <div style="flex:1">
          <label class="flb" style="font-size:11px">İlk Taksit Tarihi ile Tümünü Yeniden Hesapla</label>
          <input class="fi2" type="date" id="tt-yeni-ilk-tarih" style="width:100%">
        </div>
        <button class="btn bp bsm" onclick="ttTumunuYenile()" style="white-space:nowrap">🔄 Tümünü Güncelle</button>
      </div>
      <div style="max-height:380px;overflow-y:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead style="position:sticky;top:0;z-index:1">
            <tr style="background:var(--s2)">
              <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:center;width:50px">No</th>
              <th style="padding:7px 10px;color:var(--t3);font-weight:600">Tarih</th>
              <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:right">Taksit</th>
              <th style="padding:7px 10px;color:var(--t3);font-weight:600;text-align:center;width:60px">Durum</th>
            </tr>
          </thead>
      ...
      <div style="padding:12px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="ttKaydet()" style="flex:1">💾 Tarihleri Kaydet</button>
        <button class="btn bo" onclick="document.getElementById('taksit-tarih-mod').style.display='none'">İptal</button>
      </div>
    </div>
  </div>

  <!-- Ödendi Tarih Modal -->
  <div class="ov" id="odendi-tarih-mod" style="display:none">
    <div class="mod" style="max-width:360px;width:96%">
      <div class="mtit">✅ Ödeme Tarihi
        <button class="mclose" onclick="odendiTarihIptal()">✕</button></div>
      <div style="padding:16px">
        <div style="font-size:13px;color:var(--t2);margin-bottom:12px">
          Ödeme yapılan tarihi giriniz (opsiyonel):
        </div>
        <input class="fi2" type="date" id="odendi-tarih-input" style="width:100%;margin-bottom:4px">
        <div style="font-size:11px;color:var(--t3)">Boş bırakırsanız bugünün tarihi kullanılır.</div>
      </div>
      <div style="padding:0 16px 16px;display:flex;gap:10px">
        <button class="btn bp" onclick="odendiTarihKaydet()" style="flex:1">✅ Ödendi Yap</button>
        <button class="btn bgh" onclick="odendiTarihIptal()">Tarihsiz Geç</button>
      </div>
    </div>
  </div>
...
def main():
    ...
    # Taksitli kredileri ödeme tablosuna senkronize et (startup)
    for _tk in TAKSITLI_DATA:
        try:
            _taksitli_odeme_senkronize(_tk['id'])
        except Exception as _e:
            print(f"  Taksitli sync hatasi {_tk.get('kredi_no','?')}: {_e}")
    if TAKSITLI_DATA:
        print(f"  {len(TAKSITLI_DATA)} taksitli kredi odeme tablosuna senkronize edildi")

    load_users()



    # GitHub'dan her deploy'da yükle - arka planda (blocking olmadan)
    IS_RENDER = bool(os.environ.get('RENDER') or os.environ.get('PORT'))
    if GITHUB_TOKEN:
        def _restore_bg():
            import time as _t
            _t.sleep(0.5)
            print("  GitHub'dan veriler yukleniyor...")
            ok = github_yedekten_yukle()
            if ok:
                print("  GitHub yukleme TAMAMLANDI")
                # Taksitli kredileri ödeme tablosuna senkronize et
                for _tk in TAKSITLI_DATA:
                    try: _taksitli_odeme_senkronize(_tk['id'])
                    except: pass
                if TAKSITLI_DATA:
                    save_data()
                    print(f"  {len(TAKSITLI_DATA)} taksitli kredi senkronize edildi")
                # Veriler yüklendi, banka kart kullanımlarını hesapla
                banka_kart_kullanim_guncelle()
                print("  Banka kart kullanim hesaplandi")
            else:
                print("  GitHub yukleme basarisiz - lokal veriler kullaniliyor")
        threading.Thread(target=_restore_bg, daemon=True).start()
    else:
        # GitHub yoksa lokal verilerden hesapla
        banka_kart_kullanim_guncelle()

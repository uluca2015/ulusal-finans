...
<nav class="sbar" id="sbar">
  ...
  <div class="nav"    id="nv-tbl"    onclick="showView('tbl')">− ÖDEME TABLOSU</div>
    ...
    <div class="mod" style="max-width:700px;width:96%;max-height:92vh;overflow-y:auto">
      ...
      <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div><label class="flb">Kredi Türü</label>
          <select class="fi2" id="tk-tur" style="width:100%">
            <option>TAKSİTLİ KREDİ</option><option>KONUT KREDİSİ</option>
            <option>TAŞIT KREDİSİ</option><option>İHTİYAÇ KREDİSİ</option><option>DİĞER</option>
          </select></div>
        <div><label class="flb">Ana Para (₺) *</label>
...
function renderTurOzet(){
  ...
  grid.innerHTML = turler.map(([tur,g],i)=>{
    const renk = renkler[i % renkler.length];
    const pct = g.tutar / filtered.reduce((s,d)=>s+(d.tutar||0),0) * 100;
    return `<div style="background:var(--s2);border:1px solid var(--b1);border-radius:8px;padding:10px 12px;border-left:3px solid ${renk}">
...
function _renderTaksitliList(){
  ...
  list.innerHTML = filtered.map(k=>{
    ...
    const topTutar = taksitler.reduce((s,t)=>s+parseFloat(t.tutar||0),0);
    const odenenTutar = taksitler.filter(t=>t.durum==='ÖDENDİ').reduce((s,t)=>s+parseFloat(t.tutar||0),0);
    const kalanTutar = topTutar - odenenTutar;
    const pct = taksitler.length ? Math.round(odendi/taksitler.length*100) : 0;
    const renk = pct===100?'#1D9E75':pct>50?'#EF9F27':'#E24B4A';
    // Yaklaşan ödeme
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:10px;margin-bottom:12px;overflow:hidden">
      ...
      <div style="padding:14px 16px;display:grid;grid-template-columns:1fr auto auto;gap:12px;align-items:center;cursor:pointer"
        ...
        <div>
          ...
          <div style="font-size:10px;color:var(--t3);margin-top:2px">${pct}% tamamlandı · Kalan: <strong style="color:${renk};font-family:var(--mono)">₺${fmt(kalanTutar)}</strong></div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:10px;color:var(--t3)">KALAN BORÇ</div>

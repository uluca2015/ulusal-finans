      ...
      <div style="background:var(--s2);border-radius:10px;padding:14px;border:1px solid var(--b2)">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
          ...
          <div><label class="flb">Banka Adı *</label>
            ...
            <select clas<input class="fi2" type="text" id="bk-banka" list="bk-banka-list" placeholder="Banka adı yazın veya seçin..." autocomplete="off">
...
async function bankaKartKaydet(){
  ...
  try{
    ...
    if(r.status===401){ toast('Oturum sorunu - sayfa yenileniyor...','e'); setTimeout(()=>location.reload(),1500); return; }
    if(!r.ok && r.status!==200){ toast('Sunucu hatası: '+r.status,'e'); return; }
    let d; try{ d=await r.json(); }catch(e){ toast('Yanıt okunamadı','e'); return; }

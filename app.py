...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/logout':
            ...
            self.send_header('Set-Cookie','na_token=; Path=/; Max-Age=0'); self.end_headers(); return
        if path == '/api/me':
        ...
        if path == '/api/ipotek/excel':
            ...
            try:
                ...
                for ri,r in enumerate(IPOTEK_DATA,2):
                    vals=[r.get('sno',''),r.get('kod',''),r.get('sahip',''),r.get('il',''),r.get('ilce',''),r.get('adres',''),r.get('ada',''),r.get('parsel',''),r.get('bb',''),r.get('kat',''),r.get('blok',''),r.get('nitelik',''),r.get('banka',''),r.get('tutar',0),r.get('ipotek_tarih',''),r.get('exp_deger',0),r.get('exp_tarih',''),r.get('tasınmaz_tip',''),r.get('dask_no',''),r.get('dask_vade',''),r.get('konut_no',''),r.get('konut_vade','')]
    ...
    def do_POST(self):
        ...
        # Tüm diğer POST'lar için önce body oku, sonra auth kontrol et
        body = b''
        remaining = length
        while remaining > 0:
...
function ipotekTabloRender(){
  ...
  tbody.innerHTML=_ipotekFiltered.map((r,i)=>{
    ...
    return `<tr style="${bg}">
      ...
      <td style="${td};font-size:10px">${r.tasınmaz_tip||'—'}</td>
...
function ipotekDuzenle(id){
  ...
  f('ipt-exp-deger',r.exp_deger);f('ipt-exp-tarih',r.exp_tarih);f('ipt-tasınmaz-tip',r.tasınmaz_tip);
...
async function ipotekKaydet(){
  ...
  const kayit={id:_ipotekEditId||String(Date.now()),sno:gv('ipt-sno'),kod:gv('ipt-kod'),sahip:gv('ipt-sahip'),il:gv('ipt-il'),ilce:gv('ipt-ilce'),adres:gv('ipt-adres'),ada:gv('ipt-ada'),parsel:gv('ipt-parsel'),bb:gv('ipt-bb'),kat:gv('ipt-kat'),blok:gv('ipt-blok'),nitelik:gv('ipt-nitelik'),banka:gv('ipt-banka'),tutar:gn('ipt-tutar'),ipotek_tarih:gv('ipt-ipotek-tarih'),exp_deger:gn('ipt-exp-deger'),exp_tarih:gv('ipt-exp-tarih'),'tasınmaz_tip':gv('ipt-tasınmaz-tip'),dask_no:gv('ipt-dask-no'),dask_vade:gv('ipt-dask-vade'),konut_no:gv('ipt-konut-no'),konut_vade:gv('ipt-konut-vade')};

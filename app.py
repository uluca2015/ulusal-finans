...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX19-KK4")
...
function kkOzetBar(){
  ...
  const topLim   = _kkData.reduce((s,k)=>s+parseFloat(k.limit||0),0);
  const topKull  = _kkData.reduce((s,k)=>s+parseFloat(k.kullanilan||0),0);
  const topKalan = _kkData.reduce((s,k)=>s+parseFloat(k.kalan||0),0);
  ...
  document.getElementById('kk-ozet-bar').innerHTML=`
    <div class="met"><div class="met-stripe" style="background:var(--acc)"></div>
      ...
      <div class="mvl" style="font-size:22px;color:var(--acc)">${_kkData.length}</div></div>
    <div class="met"><div class="met-stripe" style="background:var(--acc)"></div>
      <div class="mlb">Toplam Limit</div>
      <div class="mvl" style="color:var(--acc)">₺${fmt(topLim)}</div></div>
    <div class="met"><div class="met-stripe" style="background:var(--am)"></div>
      <div class="mlb">Kullanılan</div>
      <div class="mvl" style="color:var(--am)">₺${fmt(topKull)}
        <span style="font-size:11px;color:var(--t3);font-weight:400"> (${kulPct}%)</span></div></div>
    <div class="met"><div class="met-stripe" style="background:var(--g)"></div>
      <div class="mlb">Kalan Limit</div>
      <div class="mvl" style="color:var(--g)">₺${fmt(topKalan)}</div></div>
  `;
}
...
function kkListeRender(q=''){
  ...
  list.innerHTML=filtered.map(k=>{
    ...
    // Kullanılan = sadece harcamalar (odendi=true olanlar düşülmüş)
    const harcamalar=[...( k.harcamalar||[])].sort((a,b)=>(b.tarih||'').localeCompare(a.tarih||''));
    const topHarcama=harcamalar.filter(h=>!h.odendi&&h.tur!=='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
    const topOdeme=harcamalar.filter(h=>h.odendi||h.tur==='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
    // kalan = limit - net kullanım (harcama - ödeme)
    const netKull=Math.max(0,topHarcama-topOdeme);
    const kalan=Math.max(0,limit-netKull);
    const kull=netKull;

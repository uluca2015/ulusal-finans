...
.main{flex:1;overflow-y:auto;overflow-x:hidden;padding:20px;height:100%;box-sizing:border-box}
...
<div class="view" id="vw-ipotek" style="min-height:0">
  ...
  <div class="tc-ipotek" style="overflow-x:auto;overflow-y:auto;max-height:calc(100vh - 310px);border-radius:10px;border:1px solid var(--b2)">
...
async function bkIpotekKaydet(bkId, modal){
  ...
  try{
    ...
    if(r.status===401){ toast('Oturum sorunu - sayfayı yenileyin','e'); return; }
    const d = await r.json();
    if(d.ok){
      ...
      toast('Kayıt hatası: '+(d.msg||'Bilinmeyen hata'),'e');
    }
  } catch(e){
    toast('Bağlantı hatası: '+e.message,'e');
  }

...
def save_users(users):
    ...
    try:
        ...
        print(f"  save_users DISK HATASI: {e} → /tmp fallback")
        # /tmp fallback
        try:
            with open('/tmp/nakit_akis_users.json', 'w', encoding='utf-8') as f:
                json.dump(users, f, ensure_ascii=False, indent=2)
        except Exception as e2:
            print(f"  save_users /tmp HATASI da: {e2}")
    # GitHub'a arka planda yedekle
    _users_snap = dict(users)
    def _github_sync():
        try:
            if GITHUB_TOKEN:
                icerik = json.dumps(_users_snap, ensure_ascii=False, indent=2)
                github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_users.json', icerik, 'Kullanici guncelleme')
                print(f"  save_users: GitHub yedegi OK")
        except Exception as e:
            print(f"  save_users github: {e}")
...
def github_yedekten_yukle():
    ...
    # Kullanıcıları ve ayarları da GitHub'dan geri yükle
    try:
        users_veri = github_dosya_oku(f'{BACKUP_PREFIX}nakit_akis_users.json')
        if users_veri and isinstance(users_veri, dict) and users_veri:
            _dosyaya_kaydet_json(USERS_FILE, users_veri)
            print(f"  users: {len(users_veri)} kullanici yuklendi")
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-23-FIX8")

...
def github_yedekten_yukle():
    ...
    global BANKA_KART_DATA, HATIRLATMALAR, DOCS_META, OTEL_DATA, OTEL_CONFIG
    if not GITHUB_TOKEN: return False
    _RESTORING = True
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            ay  = aylar[0]  if aylar  else ''
            yil = yillar[0] if yillar else ''
            self.send_json({
                'ay': ay, 'yil': yil,
...
async function initApp(){
  ...
  try {
    ...
    var _dbsub=document.getElementById('db-sub'); if(_dbsub) _dbsub.textContent='Yükleniyor...';
    ...
    document.getElementById('db-sub').textContent=(s.total_count||0)>0
      ...
      ? (s.total_count)+' kayıt yüklü · '+new Date().toLocaleString('tr-TR')
...
function bkListeRender(){
  ...
  if(!_bkFiltered.length){
    container.innerHTML='<div style="color:var(--t3);padding:30px;text-align:center">Henüz banka karti eklenmedi — yukaridan ekleyin</div>'; return;
  }
  ...
  _bkFiltered.forEach(function(b,bi){
    var totL=bkToplam(b,'limit'), totK=bkToplam(b,'kullanim'), totKal=totL-totK;
    var p=pct(totK,totL);
    var rp=renkPct(p);
    var bg=bi%2===0?'background:var(--bg2)':'';
    var hesaplar=b.hesaplar||[];
    var ipotek=(b.ipotekler||[]).length;

    var turler='';
    BK_TURLER.filter(t=>parseFloat(b['limit_'+t])>0).forEach(function(t){
      var l=parseFloat(b['limit_'+t])||0;
      var k=parseFloat(b['kullanim_'+t])||0;
      var kal=l-k;
      turler+='<div style="display:flex;justify-content:space-between;gap:8px;font-size:11px;padding:2px 0">'
    ...
    var iletisim='';
    if(b.mudur_ad){
      iletisim+='<div style="font-size:12px"><span style="color:var(--am);font-size:10px;font-weight:700">MÜDÜR</span><br>'
        +'<span style="font-weight:600">'+b.mudur_ad+'</span>';
      if(b.mudur_tel) iletisim+='<br><a href="tel:'+b.mudur_tel+'" style="color:var(--acc);font-size:11px">📞 '+b.mudur_tel+'</a>';
      iletisim+='</div>';
    }
    if(b.personel_ad){
      iletisim+='<div style="font-size:12px;margin-top:4px"><span style="color:var(--g);font-size:10px;font-weight:700">PERSONEL</span><br>'
        +'<span style="font-weight:600">'+b.personel_ad+'</span>';
      if(b.personel_tel) iletisim+='<br><a href="tel:'+b.personel_tel+'" style="color:var(--acc);font-size:11px">📞 '+b.personel_tel+'</a>';
      iletisim+='</div>';
    }
    if(!b.mudur_ad && !b.personel_ad) iletisim='<span style="color:var(--t3);font-size:12px">—</span>';

    html+=''
      +'<tr style="'+bg+';border-bottom:1px solid var(--b1)" id="bkrow-'+b.id+'">'
      ...
      +'</td>'
      +'<td style="padding:10px 12px;vertical-align:top;min-width:160px">'+iletisim+'</td>'
      ...
      +'<td style="padding:10px 12px;text-align:right;vertical-align:top;font-family:var(--mono);font-size:13px;color:var(--am)">'+fmtl(totK)+'</td>'
      ...
      +'<div style="height:100%;width:'+p+'%;background:'+rp+';border-radius:3px"></div></div>'
      +'</td>'
      +'<td style="padding:10px 8px;text-align:center;vertical-align:top">'
...
async function bkIpotekYonet(bkId){
  const b=_bkData.find(x=>String(x.id)===String(bkId));
  if(!b){ toast('Banka bulunamadi','e'); return; }

  const r=await api('/api/ipotek'); const d=await r.json();
  const allIpotekler=d.records||[];
  const baglı=(b.ipotekler||[]).map(x=>String(x));

  const satirlar = allIpotekler.length
    ? allIpotekler.map(ip=>{
        var checked=baglı.includes(String(ip.id))?'checked':'';
        var banka=ip.banka?'<span style="font-size:11px;color:var(--am);margin-left:6px">['+ip.banka+']</span>':'';
        return '<label style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-bottom:1px solid var(--b1);cursor:pointer;font-size:12px">'
          +'<input type="checkbox" value="'+ip.id+'" '+checked+' style="accent-color:var(--acc)">'
          +'<span><b>'+(ip.sahip||'—')+'</b> — '+(ip.il||'')+' '+(ip.ilce||'')+' '+(ip.nitelik||'')+banka+'</span>'
          +'</label>';
      }).join('')
    : '<div style="color:var(--t3);padding:16px;text-align:center">Henüz ipotek kaydı yok</div>';

  var existing=document.getElementById('bk-ipt-modal');
  ...
  modal.innerHTML=''
    +'<div class="mod" style="max-width:680px;width:94%;max-height:85vh;display:flex;flex-direction:column">'
    +'<div class="mtit"><span>'+b.banka+' — İpotek Bağla / Çöz</span>'
    +'<button class="mclose" onclick="document.getElementById('bk-ipt-modal').remove()">✕</button></div>'
    +'<div style="padding:10px 14px;font-size:12px;color:var(--t3);border-bottom:1px solid var(--b1)">'
    +'İşaretlenen ipotekler bu bankaya baglanir. Toplam '+allIpotekler.length+' ipotek kayitli.</div>'
    +'<div style="overflow-y:auto;flex:1">'+satirlar+'</div>'
    +'<div style="padding:12px 14px;display:flex;gap:8px;border-top:1px solid var(--b1)">'
    +'<button class="btn bp" style="flex:1" onclick="bkIpotekYonetKaydet(''+bkId+'')">Kaydet</button>'
...
function ipotekTabloRender(){
  var tbody=document.getElementById('ipt-tbody');
  if(!tbody)return;
  if(!_ipotekFiltered.length){
    tbody.innerHTML='<tr><td colspan="10" style="padding:20px;text-align:center;color:var(--t3)">Kayit bulunamadi</td></tr>';
    return;
  }
  const fm=n=>(parseFloat(n)||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  const ft=n=>{var v=parseFloat(n)||0;return v>0?'₺'+fm(v):'—';};

  tbody.innerHTML=_ipotekFiltered.map(function(r,i){
    var bg=i%2?'background:rgba(255,255,255,.015)':'';
    var fk=(parseFloat(r.exp_deger)||0)-(parseFloat(r.tutar)||0);
    var fkR=fk>=0?'var(--g)':'var(--r)';
    return '<tr style="'+bg+';border-bottom:1px solid var(--b1)">'
      +'<td style="padding:6px 8px;font-weight:600;white-space:nowrap">'+(r.sahip||'—')+'</td>'
      +'<td style="padding:6px 8px;font-size:11px">'+(r.il||'')+(r.ilce?' / '+r.ilce:'')+'</td>'
      +'<td style="padding:6px 8px;font-size:11px;color:var(--t3)">'+(r.nitelik||'—')+'</td>'
      +'<td style="padding:6px 8px;font-size:11px;color:var(--acc)">'+(r.banka||'—')+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--am)">'+ft(r.tutar)+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);color:var(--acc)">'+ft(r.exp_deger)+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-family:var(--mono);font-weight:600;color:'+fkR+'">'+(fk>=0?'+':'')+ft(Math.abs(fk))+'</td>'
      +'<td style="padding:6px 8px;text-align:center;font-size:11px">'+(r.ipotek_tarih||'—')+'</td>'
      +'<td style="padding:6px 8px;text-align:center;font-size:11px;color:'+(r.dask_vade?'var(--t2)':'var(--t3)')+'">'+(r.dask_vade||'—')+'</td>'
      +'<td style="padding:6px 8px;text-align:center;white-space:nowrap">'
      +'<button onclick="ipotekDuzenle(''+r.id+'')" style="background:var(--s3);color:var(--t2);border:1px solid var(--b2);padding:2px 8px;border-radius:4px;cursor:pointer;font-size:11px;margin:1px">✏</button>'
      +'<button onclick="ipotekSil(''+r.id+'')" style="background:rgba(239,68,68,.1);color:var(--r);border:1px solid rgba(239,68,68,.3);padding:2px 8px;border-radius:4px;cursor:pointer;font-size:11px;margin:1px">🗑</button>'
      +'</td></tr>';
  }).join('');
}
...
if __name__ == '__main__':
    ...
    # Render'da disk sıfırlanır - GitHub'dan tüm verileri geri yükle
    if os.environ.get('GITHUB_TOKEN'):
        print("  GitHub'dan veriler yukleniyor...")
        try:
            github_yedekten_yukle()
            print("  GitHub veri yukleme tamamlandi")
        except Exception as _ge:
            print(f"  GitHub veri yukleme hatasi: {_ge}")

    import threading
    t = threading.Thread(target=yedek_zamanlayici, daemon=True)
    t.start()

    server = HTTPServer(('0.0.0.0', PORT), AppHandler)
    print(f"  Sunucu baslatildi: http://0.0.0.0:{PORT}")
    try:

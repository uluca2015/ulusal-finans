...
def save_banka_kart():
    ...
    try:
        ...
        print(f"  save_banka_kart OK: {len(BANKA_KART_DATA)} kayit")
    except Exception as e:
        print(f"  save_banka_kart DISK HATASI: {e}")
    # 2. GitHub'a non-daemon thread ile yedekle (process kapanmadan tamamlanır)
    if GITHUB_TOKEN:
        try:
            _snap = json.dumps(BANKA_KART_DATA, ensure_ascii=False, indent=2)
            ok = github_dosya_yukle(
                f'{BACKUP_PREFIX}nakit_akis_banka_kart.json', _snap,
...
def save_data():
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_data.json', _d,
...
def save_kredi():
    try:
        ...
        if GITHUB_TOKEN and not _RESTORING:
            ...
            github_dosya_yukle(f'{BACKUP_PREFIX}nakit_akis_kredi.json', _d,

...
function kkListeRender(q=''){
  ...
  list.innerHTML=filtered.map(k=>{
    ...
    <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);margin-bottom:16px;overflow:hidden">
      ...
      <!-- ── HARCAMA TABLOSU (her zaman görünür) ── -->
      <div style="border-top:1px solid var(--b1)">
        <div style="padding:10px 16px 8px;display:flex;align-items:center;justify-content:space-between;background:var(--s3)">
          ...
          <button class="btn bp bsm" onclick="kkHarcamaEkleAc('${k.id}')" style="font-size:11px">+ Yeni Harcama</button>
        </div>
...
function kkToggle(id){
  // Artık toggle yok - harcamalar her zaman görünür
  // Fonksiyon uyumluluk için korundu
}

...
# -*- coding: utf-8 -*-
"""
NAKİT AKIŞ YÖNETİM SİSTEMİ  v3.1 (Web/Cloud Edition)
========================================================
Yenilik: Kullanıcı girişi, çok kullanıcı desteği, Render.com uyumlu
Gereksinim: pip install pandas openpyxl matplotlib
"""
...
def load_users():
    ...
    # Varsayılan admin kullanıcısı — ilk girişte değiştirilmeli!
    default = {
        "admin": {
...
def check_auth(handler):
    """Request authenticate mi? Session döndür veya None."""
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: linear-gradient(135deg, #1e3a5f 0%, #0f2027 100%);
         min-height: 100vh; display: flex; align-items: center; justify-content: center;
         font-family: 'Segoe UI', sans-serif; }
  .card { background: #fff; border-radius: 16px; padding: 40px 36px; width: 360px;
          box-shadow: 0 20px 60px rgba(0,0,0,0.4); }
  .logo { text-align: center; margin-bottom: 28px; }
  .logo h1 { color: #1e3a5f; font-size: 22px; font-weight: 700; margin-top: 8px; }
  ...
  button { width: 100%; padding: 12px; background: #1e3a5f; color: #fff;
           border: none; border-radius: 8px; font-size: 15px; font-weight: 600;
           cursor: pointer; transition: background .2s; margin-top: 4px; }
  button:hover { background: #2d5a9e; }
  .err { color: #dc2626; font-size: 13px; background: #fee2e2;
...
<div class="card">
  <div class="logo">
    <div class="icon">💰</div>
    <h1>Nakit Akış Yönetimi</h1>
    <p>Sisteme giriş yapın</p>
  </div>
  <div class="err" id="err">ERR</div>
  <div class="field"><label>Kullanıcı Adı</label>
    <input type="text" id="u" placeholder="kullanici_adi" autocomplete="username">
  </div>
...
async function login(){
  ...
  else{ e.textContent=d.msg||'Giriş başarısız.'; e.style.display='block'; }
}
</script>
</body>
</html>'''
...
def load_ipotek():
    ...
    if os.path.exists(IPOTEK_FILE):
        try:
            ...
            print(f"  {len(IPOTEK_DATA)} ipotek kaydı yüklendi")
        except: IPOTEK_DATA = []
...
# ── OTOMATİK GITHUB YEDEKLEME SİSTEMİ ───────────────────────────────────────
...
def github_dosya_yukle(dosya_yolu, icerik_str, mesaj):
    ...
    try:
        ...
        if sha: veri['sha'] = sha
        sonuc = github_api('PUT', f'/repos/{GITHUB_REPO}/contents/{dosya_yolu}', veri)
        return sonuc is not None
    except Exception as e:
        print(f"GitHub yukleme hatasi ({dosya_yolu}): {e}")
        return False
...
def github_dosya_oku(dosya_yolu):
    ...
    try:
        ...
        if sonuc and 'content' in sonuc:
            icerik = base64.b64decode(sonuc['content']).decode('utf-8')
            return json.loads(icerik)
    except Exception as e:
        print(f"GitHub okuma hatasi ({dosya_yolu}): {e}")
    return None

def github_yedek_al():
    if not GITHUB_TOKEN:
        print("  GITHUB_TOKEN tanimli degil, yedekleme atlandi")
        return False
    ...
    for yol, icerik in dosyalar.items():
        if github_dosya_yukle(yol, icerik, mesaj):
            basarili += 1
    print(f"  GitHub yedek: {basarili}/{len(dosyalar)} dosya yuklendi [{ts}]")
    return basarili > 0

def github_yedekten_yukle():
    global DATA, GELIR_DATA, BANKA_DATA, KREDI_DATA, CARI_DATA, NOTLAR, OTEL_DATA, HATIRLATMALAR
    if not GITHUB_TOKEN: return False
    print("  GitHub yedeginden veriler yukleniyor...")
    dosya_map = {
    ...
    for yol, tur in dosya_map.items():
        ...
        elif tur == 'otel':       OTEL_DATA = veri;     save_otel_data()
        elif tur == 'hatirlatma': HATIRLATMALAR = veri; save_hatirlatmalar()
        yuklu += 1
...
def yedek_zamanlayici():
    ...
    while True:
        ...
        print(f"  Sonraki otomatik yedek: {hedef.strftime('%d.%m.%Y %H:%M')}")
        _time.sleep(bekle)
        print("  Gece otomatik yedekleme basliyor...")
        github_yedek_al()
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/logout':
            ...
            self.send_header('Set-Cookie', 'na_token=; Path=/; Max-Age=0')
            self.end_headers()
            return
        if path == '/api/me':
            ...
            self.send_json({'ok': True, 'username': sess['username'],
                            'ad': u.get('ad', sess['username']), 'rol': u.get('rol','user')})
            return
        if path not in ('/api/login',):
            sess = check_auth(self)
            if not sess:
                # API isteklerine JSON 401, sayfa isteklerine redirect
                if path.startswith('/api/'):
                    self.send_json({'ok': False, 'msg': 'Oturum süresi doldu', 'redirect': '/login'}, 401)
                else:
                    self.send_response(302)
                    self.send_header('Location', '/login')
                    self.end_headers()
        ...
        if path == '/':
            ...
            try:
                ok = github_yedek_al()
                if ok:
                    self.send_json({"ok": True, "msg": "GitHub yedeklendi!"})
                else:
                    self.send_json({'ok': False, 'msg': 'Yedekleme basarisiz. GITHUB_TOKEN kontrol edin.'})
            except Exception as e:
                self.send_json({'ok': False, 'msg': str(e)})

        elif path == '/api/ipotek':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            self.send_json({'ok':True,'records':IPOTEK_DATA})

        elif path == '/api/ipotek/sablon':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False},401); return
            try:
                import openpyxl
                ...
                headers = [
                    ('S.No','sno',60), ('Kod','kod',80), ('Il','il',80),
                    ('Ilce','ilce',100), ('Mahalle','mahalle',120), ('Cadde/Sokak','cadde',140),
                    ('Ada','ada',60), ('Pafta','pafta',60), ('Parsel','parsel',60),
                    ('Banka','banka',120), ('Ipotek Degeri (TL)','ipotek_deger',130),
                    ('Ekspertiz Degeri (TL)','exp_deger',130), ('Ekspertiz Tarihi (GG.AA.YYYY)','exp_tarih',160),
                    ('Banka Kredi Limiti (TL)','kredi_limit',150), ('Aciklama','aciklama',150)
                ]
                for col, (h, _, w) in enumerate(headers, 1):
                    cell = ws.cell(row=1, column=col, value=h)
                    cell.fill = hdr_fill; cell.font = hdr_font
                    cell.alignment = Alignment(horizontal='center', vertical='center')
                    cell.border = border
                    ws.column_dimensions[chr(64+col)].width = w/7
                ws.row_dimensions[1].height = 30
                # Örnek satır
                ornek = ['1','IPT-001','Istanbul','Kadikoy','Moda Mah.','Moda Cad. No:5',
                         '123','15','42','Vakifbank','5000000','8000000','01.01.2024','6000000','Aciklama']
                ...
                # Açıklama notu
                ws.cell(row=3, column=1, value="* 2. satır örnek veridir, silebilirsiniz. Tarih formatı: GG.AA.YYYY")
                ws.cell(row=3, column=1).font = Font(italic=True, color="EF4444", size=9)
                ws.merge_cells('A3:O3')
                # Freeze
                ws.freeze_panes = 'A2'
                out = io.BytesIO(); wb.save(out)
                self.send_file(out.getvalue(), 'ipotek_takibi_sablon.xlsx',
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})

        elif path == '/api/ipotek/excel':
            ...
            try:
                import openpyxl
                from openpyxl.styles import Font, PatternFill, Alignment
                wb = openpyxl.Workbook(); ws = wb.active; ws.title = 'İpotek Takibi'
                hdr_fill = PatternFill("solid", fgColor="1E3A5F")
                hdr_font = Font(bold=True, color="FFFFFF", name="Calibri", size=10)
                headers = ['S.No','Kod','İl','İlçe','Mahalle','Cadde/Sokak','Ada','Pafta','Parsel',
                           'Banka','İpotek Değeri (₺)','Ekspertiz Değeri (₺)','Ekspertiz Tarihi',
                           'Banka Kredi Limiti (₺)','Limit Farkı (₺)','Limit Durumu','Açıklama']
                for col, h in enumerate(headers, 1):
                    cell = ws.cell(row=1, column=col, value=h)
                    cell.fill = hdr_fill; cell.font = hdr_font
                    cell.alignment = Alignment(horizontal='center')
                for r_idx, r in enumerate(IPOTEK_DATA, 2):
                    fark = (r.get('exp_deger',0) or 0) - (r.get('kredi_limit',0) or 0)
                    durum = 'LİMİT FAZLASI' if fark >= 0 else 'LİMİT EKSİ'
                    row_data = [r.get('sno',''), r.get('kod',''), r.get('il',''), r.get('ilce',''),
                                r.get('mahalle',''), r.get('cadde',''), r.get('ada',''),
                                r.get('pafta',''), r.get('parsel',''), r.get('banka',''),
                                r.get('ipotek_deger',0), r.get('exp_deger',0), r.get('exp_tarih',''),
                                r.get('kredi_limit',0), fark, durum, r.get('aciklama','')]
                    for col, val in enumerate(row_data, 1):
                        cell = ws.cell(row=r_idx, column=col, value=val)
                        if col in [11,12,14,15]:
                            cell.number_format = '#,##0'
                        fill_color = 'D5F5E3' if fark >= 0 else 'FADBD8'
                        cell.fill = PatternFill("solid", fgColor=fill_color)
                ws.auto_filter.ref = ws.dimensions
                ws.freeze_panes = 'A2'
                out = io.BytesIO(); wb.save(out)
                self.send_file(out.getvalue(),
                    f"ipotek_takibi_{datetime.now().strftime('%Y%m%d')}.xlsx",
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
            ...
            if not sess:
                self.send_json({'ok': False}, 401); return
            tur = qs.get('tur', ['gider'])[0]
            dosya_map = {
                'gider':  (DATA,       'nakit_akis_data.json'),
                'gelir':  (GELIR_DATA, 'nakit_akis_gelir.json'),
                'banka':  (BANKA_DATA, 'nakit_akis_banka.json'),
                'kredi':  (KREDI_DATA, 'nakit_akis_kredi.json'),
                'cari':   (CARI_DATA,  'nakit_akis_cari.json'),
                'notlar': (NOTLAR,     'nakit_akis_notlar.json'),
                'otel':   (OTEL_DATA,  'nakit_akis_otel.json'),
            }
            if tur not in dosya_map:
                self.send_json({'ok': False, 'msg': 'Bilinmeyen tür'}); return
            veri, dosya_adi = dosya_map[tur]
            body_bytes = json.dumps(veri, ensure_ascii=False, indent=2).encode('utf-8')
            self.send_response(200)
            self.send_header('Content-Type', 'application/json; charset=utf-8')
    ...
    def do_POST(self):
        global DATA,GELIR_DATA,BANKA_DATA,KREDI_DATA,CARI_DATA,SOURCE_FILE,SOURCE_SHEET,AUTO_RELOAD,NOTLAR,API_CONFIG,LOGO_CONFIG,AI_CONFIG,OTEL_DATA,OTEL_CONFIG,HATIRLATMALAR,GMAIL_CONFIG,WHAPI_CONFIG,WHAPI_MSGS,DOCS_META
        ...
        if self.path == '/api/ipotek/excel_yukle':
            sess = check_auth(self)
            if not sess: self.send_json({'ok':False,'msg':'Yetkisiz'},401); return
            try:
                import cgi, tempfile
                ...
                import pandas as pd
                df = pd.read_excel(tmp.name, header=0, dtype=str)
                df = df.fillna('')
                os.unlink(tmp.name)
                # Sütun eşleştirme
                col_map = {}
                for col in df.columns:
                    ...
                    cu = cu.replace('İ','I').replace('Ş','S').replace('Ğ','G').replace('Ü','U').replace('Ö','O').replace('Ç','C')
                    if 'S.NO' in cu or cu == 'SNO': col_map['sno'] = col
                    elif 'KOD' in cu: col_map['kod'] = col
                    elif cu in ('IL','İL') or (cu.startswith('IL') and 'CE' not in cu and 'CE' not in cu): col_map['il'] = col
                    elif 'ILCE' in cu or 'İLÇE' in cu: col_map['ilce'] = col
                    elif 'MAHALLE' in cu: col_map['mahalle'] = col
                    elif 'CADDE' in cu or 'SOKAK' in cu: col_map['cadde'] = col
                    elif 'ADA' in cu: col_map['ada'] = col
                    elif 'PAFTA' in cu: col_map['pafta'] = col
                    elif 'PARSEL' in cu: col_map['parsel'] = col
                    elif 'BANKA' in cu and 'KREDI' not in cu and 'LIMIT' not in cu: col_map['banka'] = col
                    elif 'IPOTEK' in cu and 'DEGER' in cu: col_map['ipotek_deger'] = col
                    elif 'EKSPERTIZ' in cu and 'TARIH' in cu: col_map['exp_tarih'] = col
                    elif 'EKSPERTIZ' in cu and 'DEGER' in cu: col_map['exp_deger'] = col
                    elif 'KREDI' in cu and 'LIMIT' in cu: col_map['kredi_limit'] = col
                    elif 'ACIKLAMA' in cu: col_map['aciklama'] = col
                kayitlar = []
                for _, row in df.iterrows():
                    def gv(field):
                        if field in col_map:
                            v = str(row.get(col_map[field], '')).strip()
                            return v if v not in ['nan','None',''] else ''
                        return ''
                    def gn(field):
                        v = gv(field).replace('.','').replace(',','.')
                        try: return float(v)
                        except: return 0
                    banka = gv('banka')
                    if not banka: continue
                    kayit = {
                        'id': str(int(datetime.now().timestamp()*1000) + len(kayitlar)),
                        'sno': gv('sno') or str(len(IPOTEK_DATA)+len(kayitlar)+1),
                        'kod': gv('kod'), 'il': gv('il'), 'ilce': gv('ilce'),
                        'mahalle': gv('mahalle'), 'cadde': gv('cadde'),
                        'ada': gv('ada'), 'pafta': gv('pafta'), 'parsel': gv('parsel'),
                        'banka': banka, 'ipotek_deger': gn('ipotek_deger'),
                        'exp_deger': gn('exp_deger'), 'exp_tarih': gv('exp_tarih'),
                        'kredi_limit': gn('kredi_limit'), 'aciklama': gv('aciklama'),
                    }
                    kayitlar.append(kayit)
                IPOTEK_DATA.extend(kayitlar)
                save_ipotek()
                self.send_json({'ok':True, 'count':len(kayitlar),
                    'msg':f'{len(kayitlar)} tasinmaz yuklenni'})
            except Exception as e:
                traceback.print_exc()
                self.send_json({'ok':False,'msg':str(e)})
            return
        ...
        if self.path == '/api/ipotek/kaydet':
            ...
            try:
                ...
                mevcut = next((i for i,r in enumerate(IPOTEK_DATA) if r.get('id')==kayit.get('id')), None)
                if mevcut is not None:
                    IPOTEK_DATA[mevcut] = kayit
                else:
                    IPOTEK_DATA.append(kayit)
                save_ipotek()
                self.send_json({'ok':True})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
        ...
        if self.path == '/api/ipotek/sil':
            ...
            try:
                p = json.loads(body)
                IPOTEK_DATA[:] = [r for r in IPOTEK_DATA if r.get('id') != p.get('id')]
                save_ipotek()
                self.send_json({'ok':True})
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
        ...
        if self.path == '/api/login':
            try:
                ...
                if u and u['password'] == hash_password(password):
                    ...
                    self.send_response(200)
                    self.send_header('Content-Type','application/json; charset=utf-8')
                    self.send_header('Set-Cookie', f'na_token={token}; Path=/; HttpOnly; Max-Age={SESSION_TTL}')
                    self.end_headers()
                    self.wfile.write(json.dumps({'ok': True, 'username': username, 'ad': u.get('ad','')}, ensure_ascii=False).encode('utf-8'))
                else:
                    self.send_json({'ok': False, 'msg': 'Kullanıcı adı veya şifre hatalı.'}, 401)
            except Exception as e:
        ...
        if self.path == '/api/users/add':
            ...
            if users.get(sess['username'],{}).get('rol') != 'admin':
                self.send_json({'ok': False, 'msg': 'Sadece admin kullanıcı ekleyebilir.'}); return
            p = json.loads(body)
            ...
            if not uname or not pw:
                self.send_json({'ok': False, 'msg': 'Kullanıcı adı ve şifre zorunlu.'}); return
            if uname in users:
                self.send_json({'ok': False, 'msg': 'Bu kullanıcı zaten var.'}); return
            users[uname] = {'password': hash_password(pw), 'ad': ad or uname, 'rol': rol,
                            'created_at': datetime.now().strftime('%d.%m.%Y %H:%M')}
            save_users(users)
            self.send_json({'ok': True})
            return
        ...
        if self.path == '/api/users/delete':
            ...
            if uname == 'admin':
                self.send_json({'ok': False, 'msg': 'Admin silinemez.'}); return
            if uname in users:
                del users[uname]
                save_users(users)
            self.send_json({'ok': True})
        ...
        if self.path == '/api/users/change_password':
            ...
            if len(new_pw) < 6:
                self.send_json({'ok': False, 'msg': 'Şifre en az 6 karakter olmalı.'}); return
            if target in users:
        ...
        if self.path == '/api/users/list':
            ...
            users = load_users()
            if users.get(sess['username'],{}).get('rol') != 'admin':
                self.send_json({'ok': False, 'msg': 'Sadece admin görebilir.'}); return
            lst = [{'username': k, 'ad': v.get('ad',''), 'rol': v.get('rol','user'),
                    'created_at': v.get('created_at','')} for k,v in users.items()]
            self.send_json({'ok': True, 'users': lst})
            return

        # ── AUTH KONTROL (diğer tüm POST'lar için) ──────────────────────────
        sess = check_auth(self)
        if not sess:
            self.send_json({'ok': False, 'msg': 'Oturum süresi doldu. Sayfayı yenileyin.'}, 401)
            return
...
<nav class="sbar">
  ...
  <div class="nav"    id="nv-tbl"    onclick="showView('tbl')">≡ Gider Tablosu</div>
  <div class="nav"    id="nv-gelir"  onclick="showView('gelir')">＋ Gelir Tablosu</div>
  <div class="nav"    id="nv-nakit"  onclick="showView('nakit')">⇌ Nakit Akış Dengesi</div>
  <div class="nav"    id="nv-finans" onclick="showView('finans')">⬡ Finansman Yönetimi</div>
  <div class="nav"    id="nv-liki"   onclick="showView('liki')">◎ Likidite & Finansman</div>
  <div class="nav"    id="nv-cari"   onclick="showView('cari')">⇄ Cari Hesaplar</div>
  <div class="nav"    id="nv-banka-api" onclick="showView('banka-api')">🔌 Banka API & Ekstre</div>
  <div class="nav"    id="nv-kur"      onclick="showView('kur')">💱 Döviz Kurları</div>
  <div class="nav"    id="nv-logo"     onclick="showView('logo')">🧾 Faturalar (Logo)</div>
  <div class="nav"    id="nv-satis"    onclick="showView('satis')">📈 Satış Analizi</div>
  <div class="nav"    id="nv-stok"     onclick="showView('stok')">📦 Stok Listesi (Logo)</div>
  <div class="nav"    id="nv-cari-logo" onclick="showView('cari-logo')">👥 Cari Listesi (Logo)</div>
  <div class="nav"    id="nv-gmail"    onclick="showView('gmail')">📧 Gmail</div>
  <div class="nav"    id="nv-docs"   onclick="showView('docs')">📁 Doküman Yönetimi</div>
  <div class="nav"    id="nv-notlar"   onclick="showView('notlar')">📝 Notlar</div>
  <div class="nav"    id="nv-hat"     onclick="showView('hat')">🔔 Hatırlatıcı</div>
  <div class="nav"    id="nv-ai"       onclick="showView('ai')">🤖 AI Asistan</div>
  <div class="nav"    id="nv-wa"      onclick="showView('wa')">💬 WhatsApp</div>
  <div class="nav"    id="nv-otel"    onclick="showView('otel')">🏨 Otel Yönetimi</div>
  <div class="nav"    id="nv-mo"     onclick="showView('mo')">◷ Aylık Özet</div>
  <div class="nav"    id="nv-ipotek"  onclick="showView('ipotek')">🏠 İpotek Takibi</div>
  <div class="nav-sec">Veri</div>
  <div class="nav"    id="nv-src"    onclick="showView('src')">⚙ Kaynak Dosya</div>
  <div class="nav"    id="nv-exp"    onclick="showView('exp')">⬇ Dışa Aktar</div>
  <div class="nav"    id="nv-import" onclick="showView('import')">⬆ Veri İçe Aktar</div>
  <div class="nav"    id="nv-db"     onclick="showView('db')">🗄 Veritabanı</div>
  <div class="nav-sec">Hesap</div>
  <div class="nav"    id="nv-kullanicilar" onclick="showView('kullanicilar')">👤 Kullanıcılar</div>
  <div class="nav"    id="nv-sifre"  onclick="showView('sifre')">🔑 Şifre Değiştir</div>
  <div class="nav"    onclick="cikisYap()" style="color:#f87171">🚪 Çıkış Yap</div>
...
<div class="view" id="vw-ipotek">
  <div class="ptit">🏠 İpotek Takibi</div>
  <div class="psub">Bankalara verilen ipotekli taşınmazların takibi</div>

  <!-- ÖZET KARTLAR -->
  <div class="mets" id="ipotek-mets" style="margin-bottom:20px">
    ...
    <div class="met"><div class="met-stripe" style="background:#f59e0b"></div><div class="mlb">Toplam Kredi Limiti</div><div class="mvl" id="ipt-toplam-limit" style="color:#f59e0b">₺0</div><div class="msb">toplam limit</div></div>
    <div class="met"><div class="met-stripe" style="background:#10b981"></div><div class="mlb">Limit Fazlası</div><div class="mvl" id="ipt-limit-fark" style="color:#10b981">₺0</div><div class="msb">ekspertiz - limit</div></div>
  </div>

  <!-- TOOLBAR -->
  <div class="tbr" style="margin-bottom:14px;flex-wrap:wrap;gap:8px">
    <input class="si" type="text" id="ipt-ara" placeholder="🔍 Ara (banka, il, ilçe...)" oninput="ipotekFiltrele()" style="width:220px">
    <select class="fi" id="ipt-banka-f" onchange="ipotekFiltrele()" style="width:160px">
      <option value="">Tüm Bankalar</option>
    </select>
    <select class="fi" id="ipt-il-f" onchange="ipotekFiltrele()" style="width:130px">
      <option value="">Tüm İller</option>
    </select>
    <select class="fi" id="ipt-durum-f" onchange="ipotekFiltrele()" style="width:150px">
      <option value="">Tüm Durumlar</option>
      <option value="fazla">Limit Fazlası</option>
      <option value="eksi">Limit Eksi</option>
    </select>
    <button class="btn bp bsm" onclick="ipotekEkleAc()">+ Yeni Taşınmaz</button>
    <button class="btn bo bsm" onclick="ipotekExcel()">⬇ Excel İndir</button>
    <button class="btn bo bsm" onclick="ipotekSablonIndir()" style="background:#6366f1;color:#fff;border:none">📋 Şablon İndir</button>
    <label class="btn bsm" style="background:#0ea5e9;color:#fff;border:none;cursor:pointer">
      ⬆ Excel Yükle
      <input type="file" id="ipt-excel-yukle" accept=".xlsx,.xls" style="display:none" onchange="ipotekExcelYukle(this)">
    </label>
  </div>

  <!-- TABLO -->
  <div style="overflow-x:auto">
  <table class="tbl" id="ipt-tablo" style="font-size:12px;min-width:1400px">
    <thead>
      <tr>
        ...
        <th style="width:110px">Mahalle</th>
        <th style="width:130px">Cadde/Sokak</th>
        <th style="width:70px">Ada</th>
        <th style="width:70px">Pafta</th>
        <th style="width:70px">Parsel</th>
        <th style="width:120px">Banka</th>
        <th style="width:120px">İpotek Değeri</th>
        <th style="width:120px">Ekspertiz Değeri</th>
        <th style="width:110px">Ekspertiz Tarihi</th>
        <th style="width:130px">Banka Kredi Limiti</th>
        <th style="width:110px">Limit Farkı</th>
        <th style="width:110px">Limit Durumu</th>
        <th style="width:80px">İşlem</th>
      </tr>
    </thead>
    <tbody id="ipt-tbody">
      <tr><td colspan="17" style="text-align:center;padding:30px;color:var(--t3)">Kayıt yok — + Yeni Taşınmaz butonuyla ekleyin</td></tr>
    </tbody>
  </table>
  </div>
</div>
...
<div class="ov" id="ipt-mod" style="display:none">
  <div class="mod" style="max-width:700px;width:95%">
    <div class="mtit">
      <span id="ipt-mod-baslik">Yeni Taşınmaz</span>
      <button class="mclose" onclick="ipotekKapat()">✕</button>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;padding:20px">

      <div class="fgr"><label class="flb">S.No *</label>
        <input class="fi2" type="text" id="ipt-sno" placeholder="1"></div>
      <div class="fgr"><label class="flb">Kod</label>
        <input class="fi2" type="text" id="ipt-kod" placeholder="IPT-001"></div>
      <div class="fgr"><label class="flb">İl *</label>
        <input class="fi2" type="text" id="ipt-il" placeholder="İstanbul"></div>

      <div class="fgr"><label class="flb">İlçe *</label>
        <input class="fi2" type="text" id="ipt-ilce" placeholder="Kadıköy"></div>
      <div class="fgr"><label class="flb">Mahalle</label>
        <input class="fi2" type="text" id="ipt-mahalle" placeholder="Moda Mah."></div>
      <div class="fgr"><label class="flb">Cadde/Sokak</label>
        <input class="fi2" type="text" id="ipt-cadde" placeholder="Moda Cad. No:5"></div>

      <div class="fgr"><label class="flb">Ada</label>
        <input class="fi2" type="text" id="ipt-ada" placeholder="123"></div>
      <div class="fgr"><label class="flb">Pafta</label>
        <input class="fi2" type="text" id="ipt-pafta" placeholder="15"></div>
      <div class="fgr"><label class="flb">Parsel</label>
        <input class="fi2" type="text" id="ipt-parsel" placeholder="42"></div>

      <div class="fgr"><label class="flb">Banka *</label>
        <input class="fi2" type="text" id="ipt-banka" placeholder="Vakıfbank"></div>
      <div class="fgr"><label class="flb">İpotek Değeri (₺)</label>
        <input class="fi2" type="number" id="ipt-ipotek-deger" placeholder="0" oninput="ipotekOtoCap()"></div>
      <div class="fgr"><label class="flb">Ekspertiz Değeri (₺) *</label>
        <input class="fi2" type="number" id="ipt-exp-deger" placeholder="0" oninput="ipotekOtoCap()"></div>

      <div class="fgr"><label class="flb">Ekspertiz Tarihi</label>
        <input class="fi2" type="date" id="ipt-exp-tarih"></div>
      <div class="fgr"><label class="flb">Banka Kredi Limiti (₺) *</label>
        <input class="fi2" type="number" id="ipt-kredi-limit" placeholder="0" oninput="ipotekOtoCap()"></div>
      <div class="fgr"><label class="flb">Açıklama</label>
        <input class="fi2" type="text" id="ipt-aciklama" placeholder="Ek bilgi..."></div>

      <!-- Otomatik hesaplanan alanlar -->
      <div class="fgr" style="background:#f0fdf4;border-radius:8px;padding:10px;grid-column:1/-1">
        <div style="font-size:12px;color:#6b7280;margin-bottom:6px">Otomatik Hesaplama</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div><span style="font-size:12px;color:#374151">Limit Farkı:</span>
            <strong id="ipt-fark-preview" style="color:#10b981;margin-left:8px">₺0</strong></div>
          <div><span style="font-size:12px;color:#374151">Limit Durumu:</span>
            <strong id="ipt-durum-preview" style="margin-left:8px">—</strong></div>
        </div>
      </div>
    </div>
...
<div class="view" id="vw-docs">
  <div class="ptit">📁 Doküman Yönetimi</div>
...
<div class="view" id="vw-exp">
  ...
  <div class="egrd">
    ...
    <div class="ec" style="grid-column:1/-1;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:18px">
      <div class="eico">💾</div><div class="etit" style="color:#166534">JSON Yedek Al (Önerilen!)</div>
      <div class="edsc" style="color:#166534">Tüm verilerinizi JSON formatında indirin. Render.com sıfırlanırsa bu dosyaları tekrar yükleyebilirsiniz.</div>
      <div class="ebr" style="flex-wrap:wrap;gap:8px">
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('gider')">📥 Gider JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('gelir')">📥 Gelir JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('banka')">📥 Banka JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('kredi')">📥 Kredi JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('cari')">📥 Cari JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('notlar')">📥 Notlar JSON</button>
        <button class="btn bsm" style="background:#166534;color:#fff" onclick="jsonIndir('otel')">📥 Otel JSON</button>
        <button class="btn bsm" style="background:#0f4c81;color:#fff" onclick="tumJsonIndir()">📦 Tümünü İndir</button>
        <button class="btn bsm" style="background:#7c3aed;color:#fff" onclick="githubYedekAl()">☁️ GitHub'a Yedekle</button>
      </div>
    </div>
  </div>
</div>
  ...
  <div style="max-width:600px">
    <div style="background:#f9fafb;border-radius:12px;padding:20px;margin-bottom:20px;border:1px solid #e5e7eb">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div><label style="font-size:12px;color:#6b7280">Kullanıcı Adı *</label>
          <input id="yeni-uname" type="text" placeholder="ornek_kullanici"
            style="width:100%;padding:8px 10px;border:1.5px solid #d1d5db;border-radius:7px;font-size:13px;margin-top:4px"></div>
        <div><label style="font-size:12px;color:#6b7280">Ad Soyad</label>
          <input id="yeni-ad" type="text" placeholder="Ali Yılmaz"
            style="width:100%;padding:8px 10px;border:1.5px solid #d1d5db;border-radius:7px;font-size:13px;margin-top:4px"></div>
        <div><label style="font-size:12px;color:#6b7280">Şifre *</label>
          <input id="yeni-pw" type="password" placeholder="min. 6 karakter"
            style="width:100%;padding:8px 10px;border:1.5px solid #d1d5db;border-radius:7px;font-size:13px;margin-top:4px"></div>
        <div><label style="font-size:12px;color:#6b7280">Rol</label>
          <select id="yeni-rol" style="width:100%;padding:8px 10px;border:1.5px solid #d1d5db;border-radius:7px;font-size:13px;margin-top:4px">
...
<div class="view" id="vw-sifre">
  <div class="ptit">🔑 Şifre Değiştir</div>
  ...
  <div style="max-width:360px;background:#f9fafb;border-radius:12px;padding:24px;border:1px solid #e5e7eb">
    <div style="margin-bottom:14px"><label style="font-size:13px;font-weight:600;color:#374151">Yeni Şifre</label>
      <input id="yeni-sifre" type="password" placeholder="min. 6 karakter"
        style="width:100%;padding:10px 12px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;margin-top:6px"></div>
    <div style="margin-bottom:18px"><label style="font-size:13px;font-weight:600;color:#374151">Yeni Şifre (Tekrar)</label>
      <input id="yeni-sifre2" type="password" placeholder="tekrar girin"
        style="width:100%;padding:10px 12px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;margin-top:6px"></div>
...
function openAdd(){
  ...
  ['mye','mta','man','mfa','mbs','mto','mac'].forEach(id=>document.getElementById(id).value='');
...
async function saveRec(){
  ...
  const rec={firma:document.getElementById('mfr').value,tur:document.getElementById('mtu').value,
    yer:document.getElementById('mye').value,tarih:ts,ay,durum:document.getElementById('mdu').value,
...
function showView(v){
  ...
  const vwEl=document.getElementById('vw-'+v);
  if(vwEl) vwEl.classList.add('on');
  const n=document.getElementById('nv-'+v);if(n)n.classList.add('on');
  // Sayfayı en üste kaydır
  const mainEl=document.querySelector('.main');
  if(mainEl){ mainEl.scrollTop=0; mainEl.scrollTo(0,0); }
  window.scrollTo(0,0);
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        ...
        <tbody>
          ...
          <tr style="border-top:1px solid var(--b2);background:rgba(79,156,249,.05)">
            <td colspan="4" style="padding:5px 9px;font-family:var(--mono);font-size:10px;color:var(--t2)">TOPLAM KREDİ KULLANIMI</td>
...
function renderKur(){
  ...
  document.getElementById('kur-tarih').value = yyyy+'-'+mm+'-'+dd;

  // Otomatik bugünkü kurları çek
  kurCek();
}
...
async function renderHatirlatma(){
  ...
  if(nvHat){
    if(d.acil>0){
      nvHat.innerHTML=`🔔 Hatırlatıcı <span style="background:var(--r);color:#fff;border-radius:10px;padding:1px 6px;font-size:10px;margin-left:4px">${d.acil}</span>`;
    } else {
      nvHat.innerHTML=`🔔 Hatırlatıcı`;
    }
  }

  // Özet
  const ozEl = document.getElementById('hat-ozet');
  if(ozEl) ozEl.innerHTML = `<span style="color:var(--am)">${d.bugun_bekleyen} bugün</span> · <span style="color:var(--r)">${d.acil} bekleyen</span>`;
...
async function docsYukle(files){
  ...
  for(let i=0;i<files.length;i++){
    ...
    fd.append('not', '');
    try {
      const r = await fetch('/api/docs/yukle',{method:'POST',body:fd});
      const d = await r.json();
...
async function initApp(){
  try {
    ...
    document.getElementById('st-tc').textContent=(s.total_count||0)+' kayit';
    document.getElementById('st-paid').textContent=fm(s.odendi||0);
    document.getElementById('st-pc').textContent=(s.odendi_count||0)+' kayit';
    document.getElementById('st-pend').textContent=fm(s.bekleyen||0);
    document.getElementById('st-bc').textContent=(s.bekleyen_count||0)+' kayit';
    document.getElementById('st-soon').textContent=fm(s.soon||0);
    document.getElementById('st-sc').textContent=(s.soon_count||0)+' kayit - 30 gun';
    document.getElementById('db-sub').textContent=(s.total_count||0)>0
      ? (s.total_count)+' kayit yuklu'
      : 'Veri yok - Veri Ice Aktar menusu ile yukleyin';
    populateFilters(s.firmas||[],s.turler||[],s.aylar||[],s.yillar||[]);
    loadSourceStatus();
...
async function githubYedekAl(){
  ...
  btn.disabled = true;
  try{
    const r = await fetch('/api/yedek/github');
    const d = await r.json();
    if(d.ok){ toast('✅ ' + d.msg, 's'); }
    else{ toast('❌ ' + d.msg, 'e'); }
  }catch(e){ toast('Hata: ' + e.message, 'e'); }
  btn.textContent = "☁️ GitHub'a Yedekle";
  btn.disabled = false;
}

async function jsonIndir(tur){
  window.location.href = '/api/export/json?tur=' + tur;
}

async function tumJsonIndir(){
  const turler = ['gider','gelir','banka','kredi','cari','notlar','otel'];
  for(const tur of turler){
    await new Promise(r => setTimeout(r, 400));
    const a = document.createElement('a');
    a.href = '/api/export/json?tur=' + tur;
    a.click();
  }
  toast('Tüm JSON dosyaları indiriliyor...','s');
}
...
var _ipotekData = [];
var _ipotekFiltered = [];
var _ipotekEditId = null;

async function renderIpotek(){
  try{
    const r = await fetch('/api/ipotek');
    const d = await r.json();
    _ipotekData = d.records || [];
    _ipotekFiltered = [..._ipotekData];
    ipotekFiltreleriDoldur();
    ipotekTabloRender();
    ipotekOzetGuncelle();
  }catch(e){ console.error('Ipotek yuklenemedi:', e); }
}

function ipotekFiltreleriDoldur(){
  const bankaSel = document.getElementById('ipt-banka-f');
  const ilSel = document.getElementById('ipt-il-f');
  const bankalar = [...new Set(_ipotekData.map(r=>r.banka).filter(Boolean))].sort();
  const iller = [...new Set(_ipotekData.map(r=>r.il).filter(Boolean))].sort();
  bankaSel.innerHTML = '<option value="">Tüm Bankalar</option>' + bankalar.map(b=>`<option>${b}</option>`).join('');
  ilSel.innerHTML = '<option value="">Tüm İller</option>' + iller.map(i=>`<option>${i}</option>`).join('');
}
...
function ipotekFiltrele(){
  ...
  const durum = document.getElementById('ipt-durum-f').value;
  _ipotekFiltered = _ipotekData.filter(r=>{
    if(banka && r.banka !== banka) return false;
    if(il && r.il !== il) return false;
    if(durum === 'fazla' && (r.exp_deger - r.kredi_limit) <= 0) return false;
    if(durum === 'eksi' && (r.exp_deger - r.kredi_limit) > 0) return false;
    if(ara){
      const hay = (r.banka+r.il+r.ilce+r.mahalle+r.cadde+r.ada+r.parsel+r.kod+r.aciklama||'').toLowerCase();
...
function ipotekOzetGuncelle(){
  ...
  document.getElementById('ipt-toplam-exp').textContent = fm(_ipotekFiltered.reduce((s,r)=>s+(r.exp_deger||0),0));
  document.getElementById('ipt-toplam-limit').textContent = fm(_ipotekFiltered.reduce((s,r)=>s+(r.kredi_limit||0),0));
  const fark = _ipotekFiltered.reduce((s,r)=>s+((r.exp_deger||0)-(r.kredi_limit||0)),0);
  const el = document.getElementById('ipt-limit-fark');
  el.textContent = fm(Math.abs(fark));
  el.style.color = fark >= 0 ? '#10b981' : '#ef4444';
  el.parentElement.querySelector('.mlb').textContent = fark >= 0 ? 'Limit Fazlası' : 'Limit Eksi';
}

function ipotekTabloRender(){
  const fm = n => n ? '₺'+(n||0).toLocaleString('tr-TR',{minimumFractionDigits:0,maximumFractionDigits:0}) : '—';
  const tbody = document.getElementById('ipt-tbody');
  if(!_ipotekFiltered.length){
    tbody.innerHTML = '<tr><td colspan="17" style="text-align:center;padding:30px;color:var(--t3)">Kayıt bulunamadı</td></tr>';
    return;
  }
  tbody.innerHTML = _ipotekFiltered.map((r,i)=>{
    const fark = (r.exp_deger||0) - (r.kredi_limit||0);
    const farkRenk = fark >= 0 ? '#10b981' : '#ef4444';
    const durum = fark >= 0 ? 'LİMİT FAZLASI' : 'LİMİT EKSİ';
    const durumRenk = fark >= 0 ? '#dcfce7' : '#fee2e2';
    const durumYazi = fark >= 0 ? '#166534' : '#dc2626';
    return `<tr>
      ...
      <td>${r.mahalle||'—'}</td>
      <td>${r.cadde||'—'}</td>
      <td style="text-align:center">${r.ada||'—'}</td>
      <td style="text-align:center">${r.pafta||'—'}</td>
      <td style="text-align:center">${r.parsel||'—'}</td>
      <td><strong>${r.banka||'—'}</strong></td>
      <td style="text-align:right;font-family:var(--mono)">${fm(r.ipotek_deger)}</td>
      <td style="text-align:right;font-family:var(--mono);color:#0ea5e9"><strong>${fm(r.exp_deger)}</strong></td>
      <td style="text-align:center;font-size:11px">${r.exp_tarih||'—'}</td>
      <td style="text-align:right;font-family:var(--mono);color:#f59e0b"><strong>${fm(r.kredi_limit)}</strong></td>
      <td style="text-align:right;font-family:var(--mono);color:${farkRenk}"><strong>${fark>=0?'+':''}${fm(fark)}</strong></td>
      <td style="text-align:center">
        <span style="background:${durumRenk};color:${durumYazi};padding:3px 8px;border-radius:12px;font-size:10px;font-weight:700">${durum}</span>
      </td>
      <td style="text-align:center">
        <button onclick="ipotekDuzenle('${r.id}')" style="background:#e0f2fe;color:#0369a1;border:none;padding:3px 8px;border-radius:5px;cursor:pointer;font-size:11px;margin-right:2px">Düz</button>
        <button onclick="ipotekSil('${r.id}')" style="background:#fee2e2;color:#dc2626;border:none;padding:3px 8px;border-radius:5px;cursor:pointer;font-size:11px">Sil</button>
      </td>
    </tr>`;
  }).join('');
}
...
function ipotekOtoCap(){
  ...
  const limit = parseFloat(document.getElementById('ipt-kredi-limit').value)||0;
  const fark = exp - limit;
  const farkEl = document.getElementById('ipt-fark-preview');
  const durumEl = document.getElementById('ipt-durum-preview');
  const fm = n => '₺'+Math.abs(n).toLocaleString('tr-TR',{minimumFractionDigits:0});
  farkEl.textContent = (fark>=0?'+':'-')+fm(fark);
  farkEl.style.color = fark>=0 ? '#10b981' : '#ef4444';
  durumEl.textContent = fark>=0 ? 'LİMİT FAZLASI' : 'LİMİT EKSİ';
  durumEl.style.color = fark>=0 ? '#166534' : '#dc2626';
}

function ipotekEkleAc(){
  _ipotekEditId = null;
  document.getElementById('ipt-mod-baslik').textContent = 'Yeni Taşınmaz Ekle';
  ['ipt-sno','ipt-kod','ipt-il','ipt-ilce','ipt-mahalle','ipt-cadde',
   'ipt-ada','ipt-pafta','ipt-parsel','ipt-banka','ipt-aciklama'].forEach(id=>{
    document.getElementById(id).value='';
  });
  ['ipt-ipotek-deger','ipt-exp-deger','ipt-kredi-limit'].forEach(id=>{
    document.getElementById(id).value='';
  });
  document.getElementById('ipt-exp-tarih').value='';
  document.getElementById('ipt-fark-preview').textContent='₺0';
  document.getElementById('ipt-durum-preview').textContent='—';
  document.getElementById('ipt-mod').style.display='flex';
}

function ipotekDuzenle(id){
  const r = _ipotekData.find(x=>x.id==id);
  if(!r) return;
  _ipotekEditId = id;
  document.getElementById('ipt-mod-baslik').textContent = 'Taşınmaz Düzenle';
  document.getElementById('ipt-sno').value = r.sno||'';
  document.getElementById('ipt-kod').value = r.kod||'';
  document.getElementById('ipt-il').value = r.il||'';
  document.getElementById('ipt-ilce').value = r.ilce||'';
  document.getElementById('ipt-mahalle').value = r.mahalle||'';
  document.getElementById('ipt-cadde').value = r.cadde||'';
  document.getElementById('ipt-ada').value = r.ada||'';
  document.getElementById('ipt-pafta').value = r.pafta||'';
  document.getElementById('ipt-parsel').value = r.parsel||'';
  document.getElementById('ipt-banka').value = r.banka||'';
  document.getElementById('ipt-ipotek-deger').value = r.ipotek_deger||'';
  document.getElementById('ipt-exp-deger').value = r.exp_deger||'';
  document.getElementById('ipt-exp-tarih').value = r.exp_tarih||'';
  document.getElementById('ipt-kredi-limit').value = r.kredi_limit||'';
  document.getElementById('ipt-aciklama').value = r.aciklama||'';
  ipotekOtoCap();
  document.getElementById('ipt-mod').style.display='flex';
}

function ipotekKapat(){
  document.getElementById('ipt-mod').style.display='none';
  _ipotekEditId = null;
}

async function ipotekKaydet(){
  const kayit = {
    id: _ipotekEditId || Date.now().toString(),
    sno: document.getElementById('ipt-sno').value.trim(),
    kod: document.getElementById('ipt-kod').value.trim(),
    il: document.getElementById('ipt-il').value.trim(),
    ilce: document.getElementById('ipt-ilce').value.trim(),
    mahalle: document.getElementById('ipt-mahalle').value.trim(),
    cadde: document.getElementById('ipt-cadde').value.trim(),
    ada: document.getElementById('ipt-ada').value.trim(),
    pafta: document.getElementById('ipt-pafta').value.trim(),
    parsel: document.getElementById('ipt-parsel').value.trim(),
    banka: document.getElementById('ipt-banka').value.trim(),
    ipotek_deger: parseFloat(document.getElementById('ipt-ipotek-deger').value)||0,
    exp_deger: parseFloat(document.getElementById('ipt-exp-deger').value)||0,
    exp_tarih: document.getElementById('ipt-exp-tarih').value,
    kredi_limit: parseFloat(document.getElementById('ipt-kredi-limit').value)||0,
    aciklama: document.getElementById('ipt-aciklama').value.trim(),
  };
  if(!kayit.banka){ toast('Banka zorunlu!','e'); return; }
  try{
    const r = await fetch('/api/ipotek/kaydet', {method:'POST',
      headers:{'Content-Type':'application/json'}, body:JSON.stringify(kayit)});
    const d = await r.json();
    if(d.ok){ toast('Kaydedildi','s'); ipotekKapat(); renderIpotek(); }
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){ toast('Hata: '+e.message,'e'); }
}

async function ipotekSil(id){
  if(!confirm('Bu taşınmaz silinsin mi?')) return;
  try{
    const r = await fetch('/api/ipotek/sil', {method:'POST',
      headers:{'Content-Type':'application/json'}, body:JSON.stringify({id})});
    const d = await r.json();
    if(d.ok){ toast('Silindi','i'); renderIpotek(); }
    else toast('Hata: '+(d.msg||''),'e');
  }catch(e){ toast('Hata: '+e.message,'e'); }
}

async function ipotekSablonIndir(){
  window.location.href = '/api/ipotek/sablon';
}
...
async function ipotekExcelYukle(input){
  ...
  try{
    const r = await fetch('/api/ipotek/excel_yukle', {method:'POST', body: formData});
    const d = await r.json();
    if(d.ok){
      toast(d.msg || (d.count + ' kayıt yüklendi'),'s');
      renderIpotek();
    } else toast('Hata: '+(d.msg||''),'e');
  }catch(e){ toast('Hata: '+e.message,'e'); }
  input.value='';
...
async function ipotekExcel(){
  window.location.href = '/api/ipotek/excel';
}
...
async function loadKullanicilar(){
  const container=document.getElementById('kullanici-liste');
  if(!container) return;
  try{
    ...
    container.innerHTML=d.users.map(u=>`
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f9fafb;border-radius:8px;margin-bottom:8px">
        <div style="flex:1">
          <strong>${u.username}</strong> &nbsp;<span style="color:#6b7280;font-size:12px">${u.ad}</span>
          &nbsp;<span style="background:${u.rol==='admin'?'#fef3c7':'#e0f2fe'};color:${u.rol==='admin'?'#92400e':'#0369a1'};padding:2px 8px;border-radius:12px;font-size:11px">${u.rol}</span>
        </div>
        <span style="font-size:11px;color:#9ca3af">${u.created_at}</span>
        ${u.username!=='admin'?`<button onclick="kullaniciSil('${u.username}')" style="background:#fee2e2;color:#dc2626;border:none;padding:4px 10px;border-radius:6px;cursor:pointer;font-size:12px">Sil</button>`:''}
      </div>`).join('');
  }catch(e){container.innerHTML='<p style="color:#ef4444">Hata: '+e.message+'</p>';}
}
...
async function kullaniciEkle(){
  ...
  const r=await fetch('/api/users/add',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username:uname,ad,password:pw,rol})});
  const d=await r.json();
  if(d.ok){toast('Kullanıcı eklendi ✓','s');document.getElementById('yeni-uname').value='';
    document.getElementById('yeni-ad').value='';document.getElementById('yeni-pw').value='';
    loadKullanicilar();}
  else toast('Hata: '+(d.msg||''),'e');
}
...
async function kullaniciSil(uname){
  ...
  const r=await fetch('/api/users/delete',{method:'POST',headers:{'Content-Type':'application/json'},
...
async function sifreDegistir(){
  ...
  const tekrar=document.getElementById('yeni-sifre2').value;
  if(yeni!==tekrar){toast('Şifreler eşleşmiyor','e');return;}
  if(yeni.length<6){toast('En az 6 karakter','e');return;}
  const r=await fetch('/api/users/change_password',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({new_password:yeni})});
  const d=await r.json();
  if(d.ok){toast('Şifre değiştirildi ✓','s');document.getElementById('yeni-sifre').value='';document.getElementById('yeni-sifre2').value='';}
  else toast('Hata: '+(d.msg||''),'e');
}

// Sayfa değişince kullanıcı bölümünü yükle - showView'a ek hook
document.addEventListener('click', function(e){
  const el = e.target.closest('[onclick]');
  if(el){
    const fn = el.getAttribute('onclick');
    if(fn && fn.includes("showView('kullanicilar')")) setTimeout(loadKullanicilar, 100);
  }
});
...
def main():
    ...
    # Render.com / production ortamında eski instance kapatma atlanır
    IS_RENDER = os.environ.get('RENDER') or os.environ.get('PORT')
    if not IS_RENDER:
        kill_old_instances()
        import time; time.sleep(0.5)

    # Kullanıcıları yükle / ilk kurulumda oluştur
    users = load_users()
    ...
    # GitHub yedeginden yukle (local veri yoksa) - arka planda yap
    if GITHUB_TOKEN and len(DATA) == 0:
        print("  Local veri yok, GitHub yedegi arka planda kontrol ediliyor...")
        def _github_restore():
            import time as _t; _t.sleep(2)
            github_yedekten_yukle()
        threading.Thread(target=_github_restore, daemon=True).start()
    elif GITHUB_TOKEN and len(DATA) > 0:
        print(f"  Local veri mevcut: {len(DATA)} kayit")

    if SOURCE_FILE:
        reload_from_source(force=True)

    # PORT: Render.com PORT env değişkenini kullanır, yoksa 5678
    PORT = int(os.environ.get('PORT', 5678))
    HOST = '0.0.0.0'  # Render için 0.0.0.0 gerekli
    ...
    print("=" * 55)
    print("  NAKİT AKIŞ YÖNETİM SİSTEMİ  v3.1 (Web Edition)")
    print("=" * 55)
    print(f"  Adres  : {url}")
    ...
    # Otomatik gece yedekleme zamanlayicisi
    if GITHUB_TOKEN:
        threading.Thread(target=yedek_zamanlayici, daemon=True).start()
        print("  Otomatik yedekleme aktif (her gece 03:00)")

    # Sadece local çalışmada tarayıcı aç
    if not IS_RENDER:
        def open_browser():

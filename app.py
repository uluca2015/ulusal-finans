...
def banka_kart_kullanim_guncelle():
    ...
    Manuel girilen kullanım değerleri (manuel_kullanim_*) korunur.
    Gider tablosunda eşleşme bulunan bankalar için otomatik hesaplama yapılır.
    """
    try:
        tur_map = {
        ...
        for kart in BANKA_KART_DATA:
            ...
            # Bu kart için gider tablosundaki toplam hesapla
            gider_toplam = {}  # alan -> tutar
            for rec in DATA:
            ...
            # - Gider tablosunda eşleşme varsa → gider toplamını kullan
            # - Eşleşme yoksa → manuel_kullanim_* varsa onu koru, yoksa mevcut kullanim_* değerini koru
            for alan in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
                if alan in gider_toplam:
                    # Gider tablosunda kayıt var → otomatik hesaplanan değeri kullan
                    kart[f'kullanim_{alan}'] = gider_toplam[alan]
                else:
                    # Gider tablosunda kayıt yok → manuel kaydedilen değeri koru
                    # Öncelik: manuel_kullanim_* > mevcut kullanim_*
                    manuel = kart.get(f'manuel_kullanim_{alan}')
                    if manuel is not None:
                        kart[f'kullanim_{alan}'] = float(manuel)
                    # Aksi halde mevcut değer olduğu gibi kalır (sıfırlanmaz)
...
def github_yedekten_yukle():
    ...
    global BANKA_KART_DATA, HATIRLATMALAR, DOCS_META, OTEL_DATA
    if not GITHUB_TOKEN: return False
    _RESTORING = True
...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX15")
    ...
    def _do_POST_inner(self):
        ...
        if self.path == '/api/banka_kart/kaydet':
            ...
            try:
                ...
                # Kullanım değerlerini manuel olarak da sakla (gider tablosu sıfırlamasına karşı koruma)
                for alan in ['taksitli','rotatif','kmh','kredi_karti','teminat','diger']:
        ...
        if self.path == '/api/ipotek/kaydet':
            ...
            try:
                ...
                # Eski kaydı bul — banka değişti mi?
                ...
                if banka_degisti:
                    # Eski bankadan bu ipotek ID'yi kaldır
                    if eski_banka:
                        for kart in BANKA_KART_DATA:
                    ...
                    # Yeni bankaya bu ipotek ID'yi ekle
        ...
        if self.path == '/api/ipotek/sil':
            ...
            try:
                ...
                # Silinecek ipotek hangi bankaya bağlı?
                ...
                # Banka kartlarından da bu ipotek ID'yi kaldır
                if silinen:
                    guncellendi = False
                    for kart in BANKA_KART_DATA:
                        mevcut_ipt = [str(x) for x in (kart.get('ipotekler') or [])]
  ...
  <div class="src-card" style="margin:0;background:rgba(79,156,249,.04);border-color:rgba(79,156,249,.2)">
    ...
    <div style="margin-bottom:14px">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        ...
        <span style="font-size:10px;color:var(--t3)">💡 Çoklu seçim: <kbd style="background:var(--s3);border:1px solid var(--b1);border-radius:3px;padding:1px 5px">Ctrl</kbd> / <kbd style="background:var(--s3);border:1px solid var(--b1);border-radius:3px;padding:1px 5px">⌘</kbd></span>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:10px">
          <label style="font-size:10px;color:var(--t3);font-weight:700;letter-spacing:.5px;display:block;margin-bottom:6px">📅 YIL SEÇİMİ (çoklu)</label>
          <select id="fn-yil" multiple size="4" style="width:100%;background:var(--s3);border:1px solid var(--b1);border-radius:6px;color:var(--t1);font-size:12px;padding:4px"></select>
          <div style="font-size:10px;color:var(--t3);margin-top:4px">Seçim yok = tüm dönem</div>
        </div>
        <div style="background:var(--s2);border:1px solid var(--b1);border-radius:var(--rad);padding:10px">
          <label style="font-size:10px;color:var(--t3);font-weight:700;letter-spacing:.5px;display:block;margin-bottom:6px">🗓 AY SEÇİMİ (çoklu)</label>
          <select id="fn-ay" multiple size="4" style="width:100%;background:var(--s3);border:1px solid var(--b1);border-radius:6px;color:var(--t1);font-size:12px;padding:4px"></select>
          <div style="font-size:10px;color:var(--t3);margin-top:4px">Seçim yok = tüm aylar</div>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:10px;align-items:flex-end">
        <div>
          <label style="font-size:10px;color:var(--t3);font-weight:700;letter-spacing:.5px;display:block;margin-bottom:5px">📆 BAŞLANGIÇ TARİHİ</label>
          <input class="fi" type="date" id="fn-tarih-bas" style="width:100%;font-size:13px">
        </div>
        <div>
          <label style="font-size:10px;color:var(--t3);font-weight:700;letter-spacing:.5px;display:block;margin-bottom:5px">📆 BİTİŞ TARİHİ</label>
          <input class="fi" type="date" id="fn-tarih-bit" style="width:100%;font-size:13px">
        </div>
        <div style="display:flex;gap:6px;padding-bottom:1px">
          <button class="btn bp" onclick="hesaplaOneri()" style="padding:8px 18px;font-size:13px">🔍 Analiz Et</button>
          <button class="btn bo" onclick="fnFiltreTemizle()" title="Filtreleri temizle" style="padding:8px 14px;font-size:13px">✕</button>
        </div>
      </div>

      <div style="font-size:10px;color:var(--t3);margin-top:8px;padding:6px 10px;background:var(--s2);border-radius:6px">
        ℹ️ Tarih aralığı girilirse yıl/ay seçimi devre dışı kalır. Hiçbir filtre seçilmezse tüm veriler analiz edilir.
      </div>
...
async function ipotekKaydet(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.ok){
      const mesaj = d.banka_sync ? 'Kaydedildi ✓ Banka kartı bağlantısı güncellendi' : 'Kaydedildi ✓';
      toast(mesaj,'s');
      ipotekKapat();
      renderIpotek();
      // Banka kartı verisini de güncelle (ipotek bağlantısı değişmiş olabilir)
      try{const rb=await api('/api/banka_kart',{credentials:'include'});const db=await rb.json();if(db.records)_bkData=db.records;}catch(ex){}
    } else toast('Hata: '+(d.msg||''),'e');
  }catch(e){toast('Hata: '+e.message,'e');}
...
async function ipotekSil(id){
  ...
  if(!confirm('Bu ipotek kaydı silinsin mi? Banka kartı bağlantısı da kaldırılacak.'))return;

  try{
    const r=await api('/api/ipotek/sil',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
    const d=await r.json();
    if(d.ok){
      toast('Silindi — banka kartı bağlantısı temizlendi','i');
      renderIpotek();
      // Banka kart verisini güncelle
      try{const rb=await api('/api/banka_kart',{credentials:'include'});const db=await rb.json();if(db.records)_bkData=db.records;}catch(ex){}
    } else toast('Hata','e');

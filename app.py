...
def tcmb_kur_alternatif(tarih_str=None):
    """Frankfurter.app üzerinden EUR bazlı kurlar (TCMB yedeği)"""
    import urllib.request, json as _json
    try:
        # Frankfurter.app - ücretsiz, ECB kurları
        if tarih_str:
            parts = tarih_str.split('.')
            if len(parts) == 3:
                gun, ay, yil = parts[0], parts[1], parts[2]
                tarih_iso = f"{yil}-{ay}-{gun}"
                url = f"https://api.frankfurter.app/{tarih_iso}?from=TRY"
            else:
        ...
        # TRY bazlı → ters çevir (1 birim = kaç TRY)
        kurlar = {}
        tarih_xml = data.get('date', tarih_str or '')
        isim_map = {'USD':'ABD Doları','EUR':'Euro','GBP':'İngiliz Sterlini',
                    'CHF':'İsviçre Frangı','JPY':'Japon Yeni','SAR':'Suudi Riyali',
        ...
        for kod, oran in data.get('rates', {}).items():
            if oran and oran > 0:
                try_kuru = round(1.0 / oran, 4)
                kurlar[kod] = {
                    'kod': kod, 'isim': isim_map.get(kod, kod),
                    'isim_en': kod, 'alis': try_kuru, 'satis': try_kuru,
                    'efektif_alis': try_kuru, 'efektif_satis': try_kuru, 'birim': 1,
                    'kaynak': 'ECB/Frankfurter'
                }
        return {'ok': True, 'tarih': tarih_xml, 'kurlar': kurlar, 
                'url': url, 'uyari': 'TCMB erişilemedi, ECB (Frankfurter.app) verileri gösteriliyor'}
    except Exception as e2:
        return {'ok': False, 'hata': f'TCMB ve alternatif kaynak başarısız: {e2}', 'tarih': tarih_str or ''}

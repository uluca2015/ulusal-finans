...
print("=" * 50)
print("  NAKIT AKIS v3 - BUILD: 2026-03-24-FIX20-KK5")
...
function kkOzetBar(){
  ...
  // Her kart için harcamalardan doğru hesapla
  let topLim=0, topKull=0, topKalan=0;
  _kkData.forEach(k=>{
    const limit=parseFloat(k.limit||0);
    const harcamalar=k.harcamalar||[];
    const harcama=harcamalar.filter(h=>!h.odendi&&h.tur!=='odeme').reduce((s,h)=>s+parseFloat(h.tutar||0),0);
    const kalan=Math.max(0,limit-harcama);
    topLim  +=limit;
    topKull +=harcama;
    topKalan+=kalan;
  });

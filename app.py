    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            try:
                ...
                headers = ['S.NO','TASINMAZ KOD','SAHİBİ','İLİ','İLÇE','ADRES',
                           'ADA','PARSEL','B.B','KAT','BLOK','NİTELİK',
                           'İPOTEK','TUTAR','İPOTEK TARİHİ',
                           'EKSPERTİZ DEĞERİ','EKSPERTİZ TARİHİ',
                           'TAŞINMAZ TİPİ','DASK POLİÇE NO','DASK VADE',
                           'KONUT POLİÇE NO','KONUT VADE']
                col_widths = [6,12,20,12,14,20,8,8,8,8,8,18,14,14,14,16,14,16,16,12,16,12]
                ...
                # Örnek satır
                ornek = ['1','IPT-001','Şahin Erkin','İstanbul','Şişli',
                         'Eskişehir Mah. No:5','1316','76','5','1','','Mesken',
                         'Akbank','5000000','01.01.2024',
                         '8000000','15.06.2023','Kat Mülkiyeti',
                         '76416712','20.10.2025','0001-0110-123456','20.10.2025']
                for col,val in enumerate(ornek,1):
                    cell = ws.cell(row=2,column=col,value=val)
                    cell.fill = PatternFill("solid",fgColor="E8F5E9")
                    cell.font = Font(italic=True,color="555555",size=9)
                ws.cell(row=3,column=1,value="* 2. satır örnektir, silebilirsiniz. Tarih: GG.AA.YYYY veya AA.GG.YYYY")
                ...
                self.send_file(out.getvalue(),'ipotek_sablon.xlsx',
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            except Exception as e:
                self.send_json({'ok':False,'msg':str(e)})
    ...
    def do_POST(self):
        ...
        if self.path == '/api/ipotek/excel_yukle':
            ...
            try:
                ...
                os.unlink(tmp.name)
                def nc(col):
                    return str(col).upper().strip().replace('İ','I').replace('Ş','S').replace('Ğ','G').replace('Ü','U').replace('Ö','O').replace('Ç','C').replace(' ','').replace('.','')
                col_map = {}
                for col in df.columns:
                    c = nc(col)
                    ...
                    elif 'SAHIB' in c or 'SAHIBI' in c:      col_map['sahip']=col
                    elif c=='ILI' or c=='IL':                 col_map['il']=col
                    ...
                    elif c in ('BB','B.B','BAGIMSIZ'):       col_map['bb']=col
                    ...
                    elif 'TASINMAZ' in c and 'TIP' in c:     col_map['tasınmaz_tip']=col
                ...
                for _, row in df.iterrows():
                    ...
                    kayitlar.append({
                        ...
                        'kod':gv('kod'),'sahip':gv('sahip'),
                        'il':il,'ilce':gv('ilce'),'adres':gv('adres'),
                        'ada':gv('ada'),'parsel':gv('parsel'),'bb':gv('bb'),
                        'kat':gv('kat'),'blok':gv('blok'),'nitelik':gv('nitelik'),
                        'banka':gv('banka'),'tutar':gn('tutar'),'ipotek_tarih':gv('ipotek_tarih'),
                        'exp_deger':gn('exp_deger'),'exp_tarih':gv('exp_tarih'),
                        'tasınmaz_tip':gv('tasınmaz_tip'),
                        'dask_no':gv('dask_no'),'dask_vade':gv('dask_vade'),
                        'konut_no':gv('konut_no'),'konut_vade':gv('konut_vade'),
                    })
                IPOTEK_DATA.extend(kayitlar)
                save_ipotek()
                self.send_json({'ok':True,'count':len(kayitlar),'msg':f'{len(kayitlar)} taşınmaz yüklendi'})
            except Exception as e:
                traceback.print_exc()
                self.send_json({'ok':False,'msg':str(e)})
            return
...
<div class="view" id="vw-ipotek">
  ...
  <div class="mets" style="margin-bottom:16px">
    ...
    <div class="met"><div class="met-stripe" style="background:#10b981"></div><div class="mlb">İpotek Durumu</div><div class="mvl" id="ipt-limit-fark" style="color:#10b981">₺0</div><div class="msb" id="ipt-limit-fark-lbl">ekspertiz - ipotek</div></div>
  </div>
  ...
  <div class="tbr" style="margin-bottom:12px;flex-wrap:wrap;gap:8px">
    ...
    <select class="fi" id="ipt-tip-f" onchange="ipotekFiltrele()" style="width:140px"><option value="">Tüm Tipler</option></select>
  ...
  <table style="width:100%;border-collapse:collapse;font-size:11px;min-width:1800px">
    <thead>
      <tr style="background:#1e3a5f;color:#fff">
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">S.No</th>
        <th style="padding:8px 6px;white-space:nowrap">Kod</th>
        <th style="padding:8px 6px;white-space:nowrap">Sahibi</th>
        <th style="padding:8px 6px;white-space:nowrap">İli</th>
        <th style="padding:8px 6px;white-space:nowrap">İlçe</th>
        <th style="padding:8px 6px;white-space:nowrap">Adres</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Ada</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Parsel</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">B.B</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Kat</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Blok</th>
        <th style="padding:8px 6px;white-space:nowrap">Nitelik</th>
        <th style="padding:8px 6px;white-space:nowrap">İpotek Bankası</th>
        <th style="padding:8px 6px;text-align:right;white-space:nowrap">Tutar (₺)</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">İpotek Tarihi</th>
        <th style="padding:8px 6px;text-align:right;white-space:nowrap">Ekspertiz (₺)</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Exp. Tarihi</th>
        <th style="padding:8px 6px;white-space:nowrap">Taşınmaz Tipi</th>
        <th style="padding:8px 6px;white-space:nowrap">DASK Poliçe</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">DASK Vade</th>
        <th style="padding:8px 6px;white-space:nowrap">Konut Poliçe</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">Konut Vade</th>
        <th style="padding:8px 6px;text-align:center;white-space:nowrap">İşlem</th>
      </tr>
    </thead>
    <tbody id="ipt-tbody">
      <tr><td colspan="23" style="text-align:center;padding:30px;color:var(--t3)">Kayıt yok — Şablon indirip Excel yükleyin veya + Yeni Ekle</td></tr>
    </tbody>
  </table>
  </div>
</div>
...
<div class="ov" id="ipt-mod" style="display:none">
  <div class="mod" style="max-width:760px;width:96%;max-height:90vh;overflow-y:auto">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;padding:16px">
      ...
      <div><label class="flb">B.B (Bağımsız Bölüm)</label><input class="fi2" id="ipt-bb" placeholder="5"></div>
      ...
      <div><label class="flb">Nitelik</label><input class="fi2" id="ipt-nitelik" placeholder="Mesken, Dükkan..."></div>
      ...
      <div><label class="flb">DASK Poliçe No</label><input class="fi2" id="ipt-dask-no" placeholder="123456"></div>
      <div><label class="flb">DASK Vade</label><input class="fi2" id="ipt-dask-vade" placeholder="01.01.2025"></div>
      <div></div>

      <div><label class="flb">Konut Poliçe No</label><input class="fi2" id="ipt-konut-no" placeholder="123456"></div>
      ...
      <div style="grid-column:1/-1;background:#f0fdf4;border-radius:8px;padding:10px;display:flex;gap:20px;align-items:center">
        <span style="font-size:12px;color:#6b7280">Ekspertiz - İpotek Farkı:</span>
...
function ipotekFiltreleriDoldur(){
  const fm = (id, vals) => {
    const el = document.getElementById(id);
    const cur = el.value;
    el.innerHTML = el.options[0].outerHTML + vals.map(v=>`<option ${v===cur?'selected':''}>${v}</option>`).join('');
  };
  ...
  fm('ipt-tip-f',   [...new Set(_ipotekData.map(r=>r.tasınmaz_tip).filter(Boolean))].sort());
}
...
function ipotekFiltrele(){
  ...
  const tip    = document.getElementById('ipt-tip-f').value;
  _ipotekFiltered = _ipotekData.filter(r=>{
    if(banka && r.banka !== banka) return false;
    if(il    && r.il    !== il)    return false;
    if(sahip && r.sahip !== sahip) return false;
    if(tip   && r.tasınmaz_tip !== tip) return false;
    if(ara){
...
function ipotekOzetGuncelle(){
  ...
  farkEl.textContent  = fm(Math.abs(fark));
  farkEl.style.color  = fark >= 0 ? '#10b981' : '#ef4444';
  lblEl.textContent   = fark >= 0 ? 'Ekspertiz Fazlası' : 'İpotek Fazlası';
}
...
function ipotekTabloRender(){
  ...
  if(!_ipotekFiltered.length){
    tbody.innerHTML = '<tr><td colspan="23" style="text-align:center;padding:30px;color:var(--t3)">Kayıt bulunamadı</td></tr>';
    return;
  }
  const rowStyle = 'border-bottom:1px solid var(--b2)';
  const tdStyle  = 'padding:6px 6px;vertical-align:middle';
  tbody.innerHTML = _ipotekFiltered.map((r,i)=>{
    const fark = (r.exp_deger||0) - (r.tutar||0);
    const bg   = i%2===0 ? 'background:var(--bg2)' : '';
    return `<tr style="${rowStyle};${bg}">
      <td style="${tdStyle};text-align:center;color:var(--t2)">${r.sno||i+1}</td>
      <td style="${tdStyle};font-size:10px;color:var(--t2)">${r.kod||'—'}</td>
      <td style="${tdStyle};white-space:nowrap"><strong>${r.sahip||'—'}</strong></td>
      <td style="${tdStyle}">${r.il||'—'}</td>
      <td style="${tdStyle}">${r.ilce||'—'}</td>
      <td style="${tdStyle};max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r.adres||''}">${r.adres||'—'}</td>
      <td style="${tdStyle};text-align:center">${r.ada||'—'}</td>
      <td style="${tdStyle};text-align:center">${r.parsel||'—'}</td>
      <td style="${tdStyle};text-align:center">${r.bb||'—'}</td>
      <td style="${tdStyle};text-align:center">${r.kat||'—'}</td>
      <td style="${tdStyle};text-align:center">${r.blok||'—'}</td>
      <td style="${tdStyle};max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r.nitelik||''}">${r.nitelik||'—'}</td>
      <td style="${tdStyle}"><strong style="color:#f59e0b">${r.banka||'—'}</strong></td>
      <td style="${tdStyle};text-align:right;font-family:var(--mono);color:#f59e0b">${fm(r.tutar)}</td>
      <td style="${tdStyle};text-align:center;font-size:10px">${r.ipotek_tarih||'—'}</td>
      <td style="${tdStyle};text-align:right;font-family:var(--mono);color:#0ea5e9"><strong>${fm(r.exp_deger)}</strong></td>
      <td style="${tdStyle};text-align:center;font-size:10px">${r.exp_tarih||'—'}</td>
      <td style="${tdStyle};font-size:10px">${r.tasınmaz_tip||'—'}</td>
      <td style="${tdStyle};font-size:10px">${r.dask_no||'—'}</td>
      <td style="${tdStyle};text-align:center;font-size:10px">${r.dask_vade||'—'}</td>
      <td style="${tdStyle};font-size:10px">${r.konut_no||'—'}</td>
      <td style="${tdStyle};text-align:center;font-size:10px">${r.konut_vade||'—'}</td>
...
function ipotekOtoCap(){
  ...
  farkEl.textContent  = (fark>=0?'+':'')+('₺'+Math.abs(fark).toLocaleString('tr-TR',{minimumFractionDigits:0}));
  farkEl.style.color  = fark>=0?'#10b981':'#ef4444';
  durumEl.textContent = fark>=0?'✅ Ekspertiz fazla':'⚠️ İpotek fazla';
...
function ipotekEkleAc(){
  ...
  ['ipt-sno','ipt-kod','ipt-sahip','ipt-il','ipt-ilce','ipt-adres','ipt-ada','ipt-parsel',
   'ipt-bb','ipt-kat','ipt-blok','ipt-nitelik','ipt-banka','ipt-tasınmaz-tip',
   'ipt-dask-no','ipt-dask-vade','ipt-konut-no','ipt-konut-vade'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.value='';
  });
...
function ipotekDuzenle(id){
  const r = _ipotekData.find(x=>String(x.id)===String(id));
  if(!r) return;
  _ipotekEditId = id;
  ...
  f('ipt-sno',r.sno); f('ipt-kod',r.kod); f('ipt-sahip',r.sahip);
  f('ipt-il',r.il); f('ipt-ilce',r.ilce); f('ipt-adres',r.adres);
  f('ipt-ada',r.ada); f('ipt-parsel',r.parsel); f('ipt-bb',r.bb);
  f('ipt-kat',r.kat); f('ipt-blok',r.blok); f('ipt-nitelik',r.nitelik);
  f('ipt-banka',r.banka); f('ipt-tutar',r.tutar); f('ipt-ipotek-tarih',r.ipotek_tarih);
  f('ipt-exp-deger',r.exp_deger); f('ipt-exp-tarih',r.exp_tarih);
  f('ipt-tasınmaz-tip',r.tasınmaz_tip);
  f('ipt-dask-no',r.dask_no); f('ipt-dask-vade',r.dask_vade);
  f('ipt-konut-no',r.konut_no); f('ipt-konut-vade',r.konut_vade);
  ipotekOtoCap();
  document.getElementById('ipt-mod').style.display='flex';
}
...
async function ipotekKaydet(){
  ...
  const kayit = {
    id: _ipotekEditId || String(Date.now()),
    sno:gv('ipt-sno'), kod:gv('ipt-kod'), sahip:gv('ipt-sahip'),
    il:gv('ipt-il'), ilce:gv('ipt-ilce'), adres:gv('ipt-adres'),
    ada:gv('ipt-ada'), parsel:gv('ipt-parsel'), bb:gv('ipt-bb'),
    kat:gv('ipt-kat'), blok:gv('ipt-blok'), nitelik:gv('ipt-nitelik'),
    banka:gv('ipt-banka'), tutar:gn('ipt-tutar'), ipotek_tarih:gv('ipt-ipotek-tarih'),
    exp_deger:gn('ipt-exp-deger'), exp_tarih:gv('ipt-exp-tarih'),
    tasınmaz_tip:gv('ipt-tasınmaz-tip'),
    dask_no:gv('ipt-dask-no'), dask_vade:gv('ipt-dask-vade'),
    konut_no:gv('ipt-konut-no'), konut_vade:gv('ipt-konut-vade'),
  };

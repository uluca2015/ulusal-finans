...
async function api(url,opts={}){
  ...
  // Token'ı header olarak da gönder
  const tok=(document.cookie.split(';').map(c=>c.trim()).find(c=>c.startsWith('na_token='))||'').split('=')[1];
  if(tok){if(!opts.headers)opts.headers={};opts.headers['X-Token']=tok;}
...
async function docsYukle(files){
  ...
  for(let i=0;i<files.length;i++){
    ...
    fd.append('not', '');
    try {
      const token = document.cookie.split(';').map(c=>c.trim()).find(c=>c.startsWith('na_token='));
      const authHeader = token ? {'Authorization': 'Bearer '+token.split('=')[1]} : {};
      const r = await fetch('/api/docs/yukle',{method:'POST',body:fd,headers:authHeader});
      const d = await r.json();
